...
| `ui.parity.invoices.update` | `ui_invoices_update_open` | `ui_invoices_update_{preview,execute}` | Fresh page. Exact **Enhedspris** fill `2,00`. Live FastMCP update proved on a second session | Reverse-delete six leftover drafts next. Honesty still red | `owner_proved_pending_fastmcp` |
| `ui.parity.invoices.delete` | `ui_invoices_delete_open` | `ui_invoices_delete_{preview,execute}` | Unique **Mere**, `A` **Slet**, dump, exact **Ja, slet faktura** | Owner `01FCBA13`: seven leftover triples still live. Product DELETE 2xx watch still missing (`A0633A17`). No clean-org | `owner_proved_pending_fastmcp` |
