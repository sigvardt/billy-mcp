...
desc: Residual honesty-16 UI write table from existing evidence. No actionable write remains.
...
Owner-scope UI write residual after `D68E402A`. Built from existing
coverage, inventories, and dumps only. No live Billy session. No new
helper. Shared ticket rules live in [[ui_write_ticket_protocol]].
...
already live-proved through FastMCP. Every remaining unproved row
lacks a unique normal mutation control or a proved cleanup path.
...
| `ui.parity.invoices.create` | `ui_invoices_create_open` | `ui_invoices_create_{preview,execute}` exist offline. Coverage still names `*_open` | **Gem som kladde** exists. Existing-customer bind does not (`proved_bind=none`) | Draft delete chrome exists. Create never persists | `interface_control_absent_UI_CHANGED` |
| `ui.parity.invoices.update` | `ui_invoices_update_open` | `ui_invoices_update_{preview,execute}` | Blocked on a tagged draft | Blocked on create | `interface_control_absent_UI_CHANGED` |
| `ui.parity.invoices.delete` | `ui_invoices_delete_open` | `ui_invoices_delete_{preview,execute}` | **Mere** then **Slet** on open chrome | Blocked on create | `interface_control_absent_UI_CHANGED` |
...
1. Invoice: accept `UI_CHANGED` for this org's missing existing-customer
   bind, or name one different normal control. `113F1E05` forbids another
   entry-CTA dump.
