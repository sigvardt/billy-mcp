...
| `ui.parity.invoices.update` | `ui_invoices_update_open` | `ui_invoices_update_{preview,execute}` | List-row open then **Opdater**. Header `PUT /v2/invoices/:id` is not persist. Line persist is `PUT /v2/invoiceLines/:id`. Fresh **Enhedspris** is still 1,00 (`DECEA79B`) | Fresh-session row open must show 2,00 before delete | `owner_proved_pending_fastmcp` |
