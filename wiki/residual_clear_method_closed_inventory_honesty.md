...
desc: Residual clear API write honesty. 22 toolless rows remain after ticketed cities create/update. Not completeness.
...
| --- | ---: | --- | --- |
| HTTP **405** `METHOD_NOT_ALLOWED` (historical research186 class) | 25 then, **18** remaining | `method_closed_offline` | `METHOD_NOT_ALLOWED_UNAUTH` |
...
### Method-closed (18 remaining)

Supports still lists these writes. Unauthenticated 405 is not the contract. Rows stay toolless here only when the official property table has no writable field map, or the slice has not landed yet. `accountNatures` create/update, `bankPayments.delete`, `invoiceReminderAssociations` create/update, and `cities` create/update are no longer in this set.
...
| Residual clear honesty (this freeze, still red) | 22 API |
| annual_reports org inaccessible | 1 UI discovery |

Generated snapshot after ticketed `cities` create/update: implemented **530**, contract **535**, live/vision **339**, complete **false**. Historical research186 freeze snapshot was implemented/contract **470**, live/vision **286**.
...
Residual **22** stay red and toolless (`tools_allowed=false`). Inventory `evidence_ref` remains ending at `research192_unauth_reconfirm`. Not greening; not product ACCEPT; complete stays false.
