...
class FamilyWrite:
    ...
    try_labeled_fills: tuple[tuple[str, str], ...] = ()
...
async def perform_family_write(
    ...
    *,
    ...
    try:
        slug = await require_matching_org_slug(page, organization_id, action.write_path)
        ...
        for label in action.pre_clicks:
            ...
            if missing is not None:
                return missing
        for label, value in action.try_labeled_fills:
            await _try_fill_labeled(page, label, value)
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            pass
        found = await _visible_text_count(page, text, visible_body=visible_body)
        if found < 1 and allow_search:
...
async def _visible_text_count(page: LoginPage, text: str, *, visible_body: bool) -> int:
    """Count exact text. Visible body text only when hidden Ember nodes must not count."""

    if visible_body:
        body = await page.locator("body").inner_text()
        return 1 if exact_name_in_text(body, text) else 0
    return await page.get_by_text(text, exact=True).count()


async def require_matching_org_slug(
    page: LoginPage, organization_id: str, path: str
) -> str | ToolError:
    """Return the live org slug only when it matches the ticket organisation."""
...
async def _try_fill_labeled(page: LoginPage, label: str, value: str) -> None:
    """Fill a visible labeled field. Missing labels are ignored."""
...
async def _click_named(page: LoginPage, label: str) -> ToolError | None:
    for _ in range(20):
        ...
        if await text.count() >= 1 and await text.first.is_visible():
            ...
            return None
        loose = page.locator(f"text={label}")
        if await loose.count() >= 1 and await loose.first.is_visible():
