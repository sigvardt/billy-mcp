...
from billy_mcp.ui_writes.organizations_phone import phone_action_from, preview_summary_for
from billy_mcp.ui_writes.organizations_submit import perform_company_phone_write
...
class OrganizationUpdatePreviewInput(BaseModel):
    ...
    @classmethod
    def phone_is_exact_empty_or_trimmed(cls, value: str) -> str:
        if value == "":
            return value
        stripped = value.strip()
        if not stripped:
            raise ValueError("phone must be exact empty or a non-blank value")
        return stripped
...
class BrowserOrganizationSubmitter:
    ...
    async def submit(self, prepared: UiWritePrepared) -> UiOrganizationUpdateResult | ToolError:
        phone = str(prepared.canonical_request.get("phone", ""))
        failed = await perform_company_phone_write(
            self._runtime,
...
def register_ui_organization_write_tools(
    ...
    *,
    ...
    def ui_organizations_update_preview(
        ...
        action = phone_action_from(parsed.phone)
        ...
        return protocol.preview(
            ...
            summary=preview_summary_for(action),
        )
