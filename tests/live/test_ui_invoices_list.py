"""Live dual-session qualification for read-only ui_invoices_list.
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiInvoicesListSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-invoices-", dir=str(_DATA_ROOT)))
...
async def _capture_invoices_list_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot the invoices list surface (not dashboard root) for vision review."""
...
@pytest.mark.asyncio
async def test_dual_profiles_open_invoices_list_shell() -> None:
    """Profile A and independent profile B both classify the invoices list shell."""
    ...
    record_path = _VISION_RECORD_DIR / "ui_invoices_list.json"
    ...
    try:
        ...
        list_a = await runtime_a.ui_invoices_list()
        if isinstance(list_a, ToolError):
            pytest.fail(f"session A ui_invoices_list failed: {list_a.code}: {list_a.message}")
        assert isinstance(list_a, UiInvoicesListSuccess)
        assert list_a.path_class == "/:org_slug/invoices"
        assert list_a.heading == "Fakturaer"
        assert list_a.create_action_visible is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_invoices_list.png"
        await _capture_invoices_list_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        list_b = await runtime_b.ui_invoices_list()
        if isinstance(list_b, ToolError):
            pytest.fail(f"session B ui_invoices_list failed: {list_b.code}: {list_b.message}")
        assert isinstance(list_b, UiInvoicesListSuccess)
        assert list_b.model_dump() == list_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_invoices_list.png"
        await _capture_invoices_list_frame(runtime_b, org_b, frame_b)
        await runtime_b.close()
        runtime_b = None

        # Frames are list-surface only (path + h1 + CTA asserted before capture).
        # DOM dual-session is the primary pass; vision co-signs the same shell.
        keep_frames = os.environ.get("BILLY_KEEP_VISION_FRAMES", "").strip() == "1"
        write_vision_record(
            record_path,
            workflow_ref="ui.invoices.list",
            assertion_refs=[
                "tests/live/test_ui_invoices_list.py::test_dual_profiles_open_invoices_list_shell",
                "session_a_ui_invoices_list",
                "session_b_ui_invoices_list",
                "session_a_invoices_list_frame",
                "session_b_invoices_list_frame",
                "list_surface_h1_faktureringer_cta",
            ],
