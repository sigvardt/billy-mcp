"""Live dual-session qualification for read-only ui_settings_invoicing_open.

Indstillinger Faktura (invoicing) panel at /:org_slug/settings via SPA seed
settings/invoicing (research128).
...
Never clicks Gem ændringer / Opret* / Tilføj* / Upload / Opret betalingsmetode
(research128 freeze).
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiSettingsInvoicingOpenSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-settings-invoicing-", dir=str(_DATA_ROOT)))
...
async def _capture_settings_invoicing_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot Indstillinger Faktura surface for vision (never write CTAs)."""
    ...
    try:
        await page.goto(
            f"https://mit.billy.dk/{slug}/settings/invoicing",
        ...
        assert "Faktura" in body
        assert "Produkter" in body
        assert "Betalingsmetoder" in body or "Standard fakturalogo" in body
        assert "Upsedasse" not in body
        # Company default markers should not both define the active panel.
        assert "Navn og adresse" not in body or "Kontaktinformation" not in body
        # Accounting panel trio should not all be active panel content.
        # Rail may still list Regnskab; require that Køb+Kontoplan are not both
        # present together with full accounting shell (research128).
        if "Køb" in body and "Kontoplan" in body:
            # still ok if Faktura panel markers dominate; accounting reject is
            # tool-side. For frame, just ensure Faktura section present.
            pass
...
@pytest.mark.asyncio
async def test_dual_profiles_open_settings_invoicing_shell() -> None:
    """Dual profiles classify the Indstillinger Faktura settings panel."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), (
        "live settings invoicing shell must not use API token"
    )
    ...
    record_path = _VISION_RECORD_DIR / "ui_settings_invoicing_open.json"
    ...
    try:
        ...
        open_a = await runtime_a.ui_settings_invoicing_open()
        if isinstance(open_a, ToolError):
            pytest.fail(
                f"session A ui_settings_invoicing_open failed: {open_a.code}: {open_a.message}"
            )
        assert isinstance(open_a, UiSettingsInvoicingOpenSuccess)
        assert open_a.path_class == "/:org_slug/settings"
        assert open_a.heading == "Indstillinger"
        assert open_a.shell_kind == "settings_invoicing"
        assert open_a.invoicing_panel_markers_present is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_settings_invoicing.png"
        await _capture_settings_invoicing_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        open_b = await runtime_b.ui_settings_invoicing_open()
        if isinstance(open_b, ToolError):
            pytest.fail(
                f"session B ui_settings_invoicing_open failed: {open_b.code}: {open_b.message}"
            )
        assert isinstance(open_b, UiSettingsInvoicingOpenSuccess)
        assert open_b.model_dump() == open_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_settings_invoicing.png"
        await _capture_settings_invoicing_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.discovery.settings_invoicing",
            assertion_refs=[
                "tests/live/test_ui_settings_invoicing_open.py::test_dual_profiles_open_settings_invoicing_shell",
                "session_a_ui_settings_invoicing_open",
                "session_b_ui_settings_invoicing_open",
                "session_a_settings_invoicing_frame",
                "session_b_settings_invoicing_frame",
                "settings_invoicing_path_settings",
            ],
