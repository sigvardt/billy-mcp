"""Live dual-session qualification for read-only ui_integrations_open.

Soft-empty shell at /:org_slug/integrations (research124).
...
Never navigates www.billy.dk; never clicks partner CTAs (research124 freeze).
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiIntegrationsOpenSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-integrations-", dir=str(_DATA_ROOT)))
...
async def _capture_integrations_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot soft-empty integrations chrome for vision review (never CTAs)."""
    ...
    try:
        await page.goto(
            f"https://mit.billy.dk/{slug}/integrations",
        ...
        assert host == "mit.billy.dk", "vision frame must stay on app host"
        path = urlsplit(page.url).path or ""
        assert path.rstrip("/").endswith("/integrations"), (
            "vision frame must be integrations soft path"
        )
        live_page = cast(Any, page)
        body = await live_page.locator("body").inner_text()
        assert "Upsedasse" not in body
        # Soft empty: no Fordele content hub on this path.
        h1_count = await live_page.locator("h1").count()
        if h1_count >= 1:
            h1_text = (await live_page.locator("h1").first.inner_text()).strip()
            assert h1_text != "Fordele"
            assert h1_text == ""
...
@pytest.mark.asyncio
async def test_dual_profiles_open_integrations_soft_empty_shell() -> None:
    """Dual profiles classify the integrations soft-empty shell."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), "live integrations shell must not use API token"
    ...
    record_path = _VISION_RECORD_DIR / "ui_integrations_open.json"
    ...
    try:
        ...
        open_a = await runtime_a.ui_integrations_open()
        if isinstance(open_a, ToolError):
            pytest.fail(f"session A ui_integrations_open failed: {open_a.code}: {open_a.message}")
        assert isinstance(open_a, UiIntegrationsOpenSuccess)
        assert open_a.path_class == "/:org_slug/integrations"
        assert open_a.shell_kind == "soft_empty"
        assert open_a.dedicated_shell is False
        assert open_a.same_shell_as_addons is False
        assert open_a.heading == ""
        assert open_a.shell_markers_present is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_integrations.png"
        await _capture_integrations_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        open_b = await runtime_b.ui_integrations_open()
        if isinstance(open_b, ToolError):
            pytest.fail(f"session B ui_integrations_open failed: {open_b.code}: {open_b.message}")
        assert isinstance(open_b, UiIntegrationsOpenSuccess)
        assert open_b.model_dump() == open_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_integrations.png"
        await _capture_integrations_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.discovery.integrations",
            assertion_refs=[
                "tests/live/test_ui_integrations_open.py::"
                "test_dual_profiles_open_integrations_soft_empty_shell",
                "session_a_ui_integrations_open",
                "session_b_ui_integrations_open",
                "session_a_integrations_frame",
                "session_b_integrations_frame",
                "soft_empty_path_integrations",
            ],
