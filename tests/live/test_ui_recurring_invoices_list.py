"""Live dual-session qualification for read-only ui_recurring_invoices_list.
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiRecurringInvoicesListSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-recurring-", dir=str(_DATA_ROOT)))
...
async def _capture_recurring_invoices_list_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot the recurring invoices list surface for vision review."""
    ...
    try:
        await page.goto(
            f"https://mit.billy.dk/{slug}/recurring_invoices", wait_until="domcontentloaded"
        )
        try:
        ...
        assert path.endswith("/recurring_invoices") or path.endswith("/recurring_invoices/empty"), (
            "vision frame must be the recurring invoices list path class"
        )
        live_page = cast(Any, page)
        heading = live_page.locator("h1")
        assert await heading.count() >= 1
        assert (await heading.first.inner_text()).strip() == "Abonnementer"
        create_action = live_page.locator("text=Opret abonnement")
...
@pytest.mark.asyncio
async def test_dual_profiles_open_recurring_invoices_list_shell() -> None:
    """Profile A and independent profile B both classify the recurring invoices list shell."""
    ...
    record_path = _VISION_RECORD_DIR / "ui_recurring_invoices_list.json"
    ...
    try:
        ...
        list_a = await runtime_a.ui_recurring_invoices_list()
        if isinstance(list_a, ToolError):
            pytest.fail(
                f"session A ui_recurring_invoices_list failed: {list_a.code}: {list_a.message}"
            )
        assert isinstance(list_a, UiRecurringInvoicesListSuccess)
        assert list_a.path_class == "/:org_slug/recurring_invoices"
        assert list_a.heading == "Abonnementer"
        assert list_a.create_action_visible is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_recurring_invoices_list.png"
        await _capture_recurring_invoices_list_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        list_b = await runtime_b.ui_recurring_invoices_list()
        if isinstance(list_b, ToolError):
            pytest.fail(
                f"session B ui_recurring_invoices_list failed: {list_b.code}: {list_b.message}"
            )
        assert isinstance(list_b, UiRecurringInvoicesListSuccess)
        assert list_b.model_dump() == list_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_recurring_invoices_list.png"
        await _capture_recurring_invoices_list_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.discovery.recurring_invoices",
            assertion_refs=[
                "tests/live/test_ui_recurring_invoices_list.py::test_dual_profiles_open_recurring_invoices_list_shell",
                "session_a_ui_recurring_invoices_list",
                "session_b_ui_recurring_invoices_list",
                "session_a_recurring_invoices_list_frame",
                "session_b_recurring_invoices_list_frame",
                "list_surface_h1_abonnementer_create_cta",
            ],
