"""Live dual-session qualification for read-only ui_products_list.
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiProductsListSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-products-", dir=str(_DATA_ROOT)))
...
async def _capture_products_list_frame(
    ...
    try:
        ...
        assert path.endswith("/products"), "vision frame must be the products list path class"
...
@pytest.mark.asyncio
async def test_dual_profiles_open_products_list_shell() -> None:
    """Profile A and independent profile B both classify the products list shell."""
    ...
    record_path = _VISION_RECORD_DIR / "ui_products_list.json"
    ...
    try:
        ...
        list_a = await runtime_a.ui_products_list()
        if isinstance(list_a, ToolError):
            pytest.fail(f"session A ui_products_list failed: {list_a.code}: {list_a.message}")
        ...
        list_b = await runtime_b.ui_products_list()
        if isinstance(list_b, ToolError):
            pytest.fail(f"session B ui_products_list failed: {list_b.code}: {list_b.message}")
        assert isinstance(list_b, UiProductsListSuccess)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.products.list",
            assertion_refs=[
                "tests/live/test_ui_products_list.py::test_dual_profiles_open_products_list_shell",
                "session_a_ui_products_list",
