"""Live dual-session qualification for read-only ui_bills_get_open.
...
Never submits Gem/Godkend/Opdater/Slet on the product get path. Live harness
seeds a disposable draft bill via SPA token when needed, then deletes it.
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiBillsGetOpenSuccess
...
_MARKER_C = "R18671-TMP-CLIENT-BILL-GET-DO-NOT-USE"
_MARKER_B = "R18671-TMP-BILL-GET-DO-NOT-USE"
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-bills-get-", dir=str(_DATA_ROOT)))
...
async def _capture_bills_get_frame(
    ...
    try:
        ...
        if not opened:
            ...
            for i in range(min(rn, 20)):
                ...
                try:
                    ...
                    continue
        assert opened, "vision capture could not open a bill detail row"
        try:
        ...
        # list click may land on /edit; product normalizes to read path.
        if re.search(r"/bills/[^/]+/edit", path):
            read = re.sub(r"/edit$", "", path)
            await page.goto(f"https://mit.billy.dk{read}", wait_until="domcontentloaded")
            await page.wait_for_timeout(1200)
            path = str(urlsplit(str(page.url)).path or "")
        assert re.search(r"/bills/[^/]+$", path) and "/new" not in path, (
            f"expected bill detail path, got {path}"
        )
...
@pytest.mark.asyncio
async def test_dual_profiles_open_bill_detail() -> None:
    """Profile A and independent profile B both classify a bill detail surface."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), "live bills get must not use API token"
    ...
    record_path = _VISION_RECORD_DIR / "ui_bills_get_open.json"
    ...
    try:
        ...
        assert ids.get("bill"), "could not seed disposable bill for get-open"

        detail_a = await runtime_a.ui_bills_get_open()
        if isinstance(detail_a, ToolError):
            pytest.fail(f"session A ui_bills_get_open failed: {detail_a.code}: {detail_a.message}")
        assert isinstance(detail_a, UiBillsGetOpenSuccess)
        assert detail_a.path_class == "/:org_slug/bills/:id"
        assert detail_a.shell_kind == "bills_get"
        assert detail_a.detail_open is True
        assert detail_a.kladde_or_state_chrome_present is True
        assert detail_a.supplier_chrome_present is True
        assert detail_a.amount_or_line_chrome_present is True

        frame_a = frame_dir / "session_a_bills_get.png"
        await _capture_bills_get_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        detail_b = await runtime_b.ui_bills_get_open()
        if isinstance(detail_b, ToolError):
            pytest.fail(f"session B ui_bills_get_open failed: {detail_b.code}: {detail_b.message}")
        assert isinstance(detail_b, UiBillsGetOpenSuccess)
        assert detail_b.model_dump() == detail_a.model_dump()

        frame_b = frame_dir / "session_b_bills_get.png"
        await _capture_bills_get_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.bills.get",
            assertion_refs=[
                "tests/live/test_ui_bills_get_open.py::test_dual_profiles_open_bill_detail",
                "session_a_ui_bills_get_open",
                "session_b_ui_bills_get_open",
                "session_a_bills_get_frame",
                "session_b_bills_get_frame",
                "detail_open_read_path_chrome",
            ],
