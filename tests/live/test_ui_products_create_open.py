"""Live dual-session qualification for read-only ui_products_create_open.
...
Never submits Gem/Opret/Save on the create form.
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiProductsCreateOpenSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-products-create-", dir=str(_DATA_ROOT)))
...
async def _capture_products_create_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot the products create form surface for vision review."""
    ...
    try:
        await page.goto(f"https://mit.billy.dk/{slug}/inventory", wait_until="domcontentloaded")
        try:
            await page.wait_for_load_state("networkidle", timeout=20000)
        except Exception:
            pass
        live_page = cast(Any, page)
        cta = live_page.locator("text=Opret produkt")
        ...
        assert path.rstrip("/").endswith("/inventory") and not path.rstrip("/").endswith(
            "/inventory/new"
        ), "vision frame must stay on inventory path class"
...
@pytest.mark.asyncio
async def test_dual_profiles_open_products_create_form() -> None:
    """Profile A and independent profile B both classify the products create form."""
    ...
    record_path = _VISION_RECORD_DIR / "ui_products_create_open.json"
    ...
    try:
        ...
        form_a = await runtime_a.ui_products_create_open()
        if isinstance(form_a, ToolError):
            pytest.fail(
                f"session A ui_products_create_open failed: {form_a.code}: {form_a.message}"
            )
        assert isinstance(form_a, UiProductsCreateOpenSuccess)
        assert form_a.path_class == "/:org_slug/inventory"
        assert form_a.heading == "Lagermodul"
        assert form_a.shell_kind == "products_create"
        assert form_a.create_form_open is True
        assert form_a.name_field_visible is True
        assert form_a.account_field_present is True
        assert form_a.sales_tax_ruleset_field_present is True
        assert form_a.unit_price_field_present is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_products_create.png"
        await _capture_products_create_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        form_b = await runtime_b.ui_products_create_open()
        if isinstance(form_b, ToolError):
            pytest.fail(
                f"session B ui_products_create_open failed: {form_b.code}: {form_b.message}"
            )
        assert isinstance(form_b, UiProductsCreateOpenSuccess)
        assert form_b.model_dump() == form_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_products_create.png"
        await _capture_products_create_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            ...
            assertion_refs=[
                "tests/live/test_ui_products_create_open.py::"
                "test_dual_profiles_open_products_create_form",
                "session_a_ui_products_create_open",
                "session_b_ui_products_create_open",
                "session_a_products_create_frame",
                "session_b_products_create_frame",
                "create_form_opret_produkt_name_account_salesTaxRuleset_unitPrice",
            ],
