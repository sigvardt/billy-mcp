"""Live dual-session qualification for read-only ui_reports_open.
...
Never clicks Eksport / download / generate (research119 freeze).
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiReportsOpenSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-reports-", dir=str(_DATA_ROOT)))
...
async def _capture_reports_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot the reports hub surface for vision review (never export)."""
    ...
    try:
        await page.goto(
            f"https://mit.billy.dk/{slug}/reports-all",
        ...
        assert "/reports-all" in path, "vision frame must be under reports-all"
        assert not path.rstrip("/").endswith("/reports"), "vision frame must not be bare reports"
        live_page = cast(Any, page)
        body = await live_page.locator("body").inner_text()
        assert "Rapporter" in body
        for marker in _TAB_MARKERS:
            assert marker in body, f"missing tab marker {marker!r}"
...
@pytest.mark.asyncio
async def test_dual_profiles_open_reports_hub_shell() -> None:
    """Profile A and independent profile B both classify the Rapporter hub shell."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), "live reports shell must not use API token"
    ...
    record_path = _VISION_RECORD_DIR / "ui_reports_open.json"
    ...
    try:
        ...
        open_a = await runtime_a.ui_reports_open()
        if isinstance(open_a, ToolError):
            pytest.fail(f"session A ui_reports_open failed: {open_a.code}: {open_a.message}")
        assert isinstance(open_a, UiReportsOpenSuccess)
        assert open_a.path_class == "/:org_slug/reports-all"
        assert open_a.heading == "Rapporter"
        assert open_a.export_action_visible is True
        assert open_a.shell_markers_present is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_reports.png"
        await _capture_reports_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        open_b = await runtime_b.ui_reports_open()
        if isinstance(open_b, ToolError):
            pytest.fail(f"session B ui_reports_open failed: {open_b.code}: {open_b.message}")
        assert isinstance(open_b, UiReportsOpenSuccess)
        assert open_b.model_dump() == open_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_reports.png"
        await _capture_reports_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.discovery.reports",
            assertion_refs=[
                "tests/live/test_ui_reports_open.py::test_dual_profiles_open_reports_hub_shell",
                "session_a_ui_reports_open",
                "session_b_ui_reports_open",
                "session_a_reports_frame",
                "session_b_reports_frame",
                "tab_markers_resultat_balance_saldo",
            ],
