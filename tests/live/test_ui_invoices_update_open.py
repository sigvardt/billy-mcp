"""Live dual-session qualification for read-only ui_invoices_update_open.
...
Never submits Gem/Gem som kladde/Godkend/Send/Slet on the product update form
path. Live harness seeds a disposable draft invoice via SPA token when the list
is empty, then deletes it.
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiInvoicesUpdateOpenSuccess
...
_MARKER_C = "R18674-TMP-CLIENT-INVOICE-UPDATE-DO-NOT-USE"
_MARKER_P = "R18674-TMP-PRODUCT-INVOICE-UPDATE-DO-NOT-USE"
_MARKER_I = "R18674-TMP-INVOICE-UPDATE-DO-NOT-USE"
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-invoices-update-", dir=str(_DATA_ROOT)))
...
async def _capture_invoices_update_frame(
...
@pytest.mark.asyncio
async def test_dual_profiles_open_invoice_edit_form() -> None:
    """Profile A and independent profile B both classify an invoice detail surface."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), "live invoices update must not use API token"
    ...
    record_path = _VISION_RECORD_DIR / "ui_invoices_update_open.json"
    ...
    try:
        ...
        assert ids.get("invoice"), "could not seed disposable invoice for update-open"

        detail_a = await runtime_a.ui_invoices_update_open()
        if isinstance(detail_a, ToolError):
            pytest.fail(
                f"session A ui_invoices_update_open failed: {detail_a.code}: {detail_a.message}"
            )
        assert isinstance(detail_a, UiInvoicesUpdateOpenSuccess)
        assert detail_a.path_class == "/:org_slug/invoices/:id/edit"
        assert detail_a.shell_kind == "invoices_update"
        assert detail_a.form_open is True
        assert detail_a.gem_kladde_or_save_chrome_present is True
        assert detail_a.date_or_payment_terms_chrome_present is True
        assert detail_a.contact_or_customer_chrome_present is True
        assert detail_a.inputs_present is True

        frame_a = frame_dir / "session_a_invoices_update.png"
        await _capture_invoices_update_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        detail_b = await runtime_b.ui_invoices_update_open()
        if isinstance(detail_b, ToolError):
            pytest.fail(
                f"session B ui_invoices_update_open failed: {detail_b.code}: {detail_b.message}"
            )
        assert isinstance(detail_b, UiInvoicesUpdateOpenSuccess)
        assert detail_b.model_dump() == detail_a.model_dump()

        frame_b = frame_dir / "session_b_invoices_update.png"
        await _capture_invoices_update_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.invoices.update",
            assertion_refs=[
                "tests/live/test_ui_invoices_update_open.py::test_dual_profiles_open_invoice_edit_form",
                "session_a_ui_invoices_update_open",
                "session_b_ui_invoices_update_open",
                "session_a_invoices_update_frame",
                "session_b_invoices_update_frame",
                "form_open_edit_path_gem_kladde_chrome",
            ],
