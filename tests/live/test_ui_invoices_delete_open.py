"""Live dual-session qualification for read-only ui_invoices_delete_open.
...
Opens Mere and classifies Slet text only; never confirms Slet / Gem /
Godkend / Send. Live harness seeds a disposable draft invoice via SPA token
when the list is empty, then deletes it.
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiInvoicesDeleteOpenSuccess
...
_MARKER_C = "R18675-TMP-CLIENT-INVOICE-DELETE-DO-NOT-USE"
_MARKER_P = "R18675-TMP-PRODUCT-INVOICE-DELETE-DO-NOT-USE"
_MARKER_I = "R18675-TMP-INVOICE-DELETE-DO-NOT-USE"
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-invoices-delete-", dir=str(_DATA_ROOT)))
...
async def _capture_invoices_delete_frame(
...
@pytest.mark.asyncio
async def test_dual_profiles_open_invoice_delete_chrome() -> None:
    """Profile A and independent profile B both classify an invoice detail surface."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), "live invoices delete must not use API token"
    ...
    record_path = _VISION_RECORD_DIR / "ui_invoices_delete_open.json"
    ...
    try:
        ...
        detail_a = await runtime_a.ui_invoices_delete_open()
        if isinstance(detail_a, ToolError):
            pytest.fail(
                f"session A ui_invoices_delete_open failed: {detail_a.code}: {detail_a.message}"
            )
        assert isinstance(detail_a, UiInvoicesDeleteOpenSuccess)
        assert detail_a.path_class == "/:org_slug/invoices/:id/edit"
        assert detail_a.shell_kind == "invoices_delete"
        assert detail_a.edit_open is True
        assert detail_a.mere_open is True
        assert detail_a.slet_text_visible is True
        assert detail_a.primary_slet_absent is True

        frame_a = frame_dir / "session_a_invoices_delete.png"
        await _capture_invoices_delete_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        detail_b = await runtime_b.ui_invoices_delete_open()
        if isinstance(detail_b, ToolError):
            pytest.fail(
                f"session B ui_invoices_delete_open failed: {detail_b.code}: {detail_b.message}"
            )
        assert isinstance(detail_b, UiInvoicesDeleteOpenSuccess)
        assert detail_b.model_dump() == detail_a.model_dump()

        frame_b = frame_dir / "session_b_invoices_delete.png"
        await _capture_invoices_delete_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.invoices.delete",
            assertion_refs=[
                "tests/live/test_ui_invoices_delete_open.py::test_dual_profiles_open_invoice_delete_chrome",
                "session_a_ui_invoices_delete_open",
                "session_b_ui_invoices_delete_open",
                "session_a_invoices_delete_frame",
                "session_b_invoices_delete_frame",
                "mere_slet_text_delete_chrome",
            ],
