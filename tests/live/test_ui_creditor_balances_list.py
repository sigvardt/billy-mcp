"""Live dual-session qualification for read-only ui_creditor_balances_list.
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiCreditorBalancesListSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-creditor-balances-", dir=str(_DATA_ROOT)))
...
async def _capture_creditor_balances_list_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot the creditor balances list surface for vision review."""
    ...
    try:
        await page.goto(
            f"https://mit.billy.dk/{slug}/creditorbalance", wait_until="domcontentloaded"
        )
        try:
        ...
        assert path.endswith("/creditorbalance"), (
            "vision frame must be the creditor balances list path class"
        )
        live_page = cast(Any, page)
        heading = live_page.locator("h1")
        assert await heading.count() >= 1
        assert (await heading.first.inner_text()).strip() == "Skyldige udgifter"
        create_action = live_page.locator("text=Opret køb")
...
@pytest.mark.asyncio
async def test_dual_profiles_open_creditor_balances_list_shell() -> None:
    """Profile A and independent profile B both classify the creditor balances list shell."""
    ...
    record_path = _VISION_RECORD_DIR / "ui_creditor_balances_list.json"
    ...
    try:
        ...
        list_a = await runtime_a.ui_creditor_balances_list()
        if isinstance(list_a, ToolError):
            pytest.fail(
                f"session A ui_creditor_balances_list failed: {list_a.code}: {list_a.message}"
            )
        assert isinstance(list_a, UiCreditorBalancesListSuccess)
        assert list_a.path_class == "/:org_slug/creditorbalance"
        assert list_a.heading == "Skyldige udgifter"
        assert list_a.create_action_visible is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_creditor_balances_list.png"
        await _capture_creditor_balances_list_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        list_b = await runtime_b.ui_creditor_balances_list()
        if isinstance(list_b, ToolError):
            pytest.fail(
                f"session B ui_creditor_balances_list failed: {list_b.code}: {list_b.message}"
            )
        assert isinstance(list_b, UiCreditorBalancesListSuccess)
        assert list_b.model_dump() == list_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_creditor_balances_list.png"
        await _capture_creditor_balances_list_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.discovery.creditor_balances",
            assertion_refs=[
                "tests/live/test_ui_creditor_balances_list.py::test_dual_profiles_open_creditor_balances_list_shell",
                "session_a_ui_creditor_balances_list",
                "session_b_ui_creditor_balances_list",
                "session_a_creditor_balances_list_frame",
                "session_b_creditor_balances_list_frame",
                "list_surface_h1_skyldige_udgifter_create_cta",
            ],
