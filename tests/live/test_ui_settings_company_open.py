...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiSettingsCompanyOpenSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-settings-company-", dir=str(_DATA_ROOT)))
...
async def _capture_settings_company_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot Indstillinger company surface for vision (never write CTAs)."""
...
@pytest.mark.asyncio
async def test_dual_profiles_open_settings_company_shell() -> None:
    """Dual profiles classify the Indstillinger company settings shell."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), (
        "live settings company shell must not use API token"
    )
    ...
    record_path = _VISION_RECORD_DIR / "ui_settings_company_open.json"
    ...
    try:
        ...
        open_a = await runtime_a.ui_settings_company_open()
        if isinstance(open_a, ToolError):
            pytest.fail(
                f"session A ui_settings_company_open failed: {open_a.code}: {open_a.message}"
            )
        assert isinstance(open_a, UiSettingsCompanyOpenSuccess)
        assert open_a.path_class == "/:org_slug/settings"
        assert open_a.heading == "Indstillinger"
        assert open_a.shell_kind == "settings_company"
        assert open_a.company_panel_markers_present is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_settings_company.png"
        await _capture_settings_company_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        open_b = await runtime_b.ui_settings_company_open()
        if isinstance(open_b, ToolError):
            pytest.fail(
                f"session B ui_settings_company_open failed: {open_b.code}: {open_b.message}"
            )
        assert isinstance(open_b, UiSettingsCompanyOpenSuccess)
        assert open_b.model_dump() == open_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_settings_company.png"
        await _capture_settings_company_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.discovery.settings_company",
            assertion_refs=[
                "tests/live/test_ui_settings_company_open.py::test_dual_profiles_open_settings_company_shell",
                "session_a_ui_settings_company_open",
                "session_b_ui_settings_company_open",
