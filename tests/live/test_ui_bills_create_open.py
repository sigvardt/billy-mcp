"""Live dual-session qualification for read-only ui_bills_create_open.
...
Never submits Godkend/Gem/Upload/Tilføj/Træk.
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiBillsCreateOpenSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-bills-create-", dir=str(_DATA_ROOT)))
...
async def _capture_bills_create_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot the bill create form surface for vision review."""
    ...
    try:
        await page.goto(f"https://mit.billy.dk/{slug}/bills/new", wait_until="domcontentloaded")
        try:
            await page.wait_for_load_state("networkidle", timeout=20000)
        except Exception:
            pass
        path = urlsplit(page.url).path or ""
        assert path.endswith("/bills/new"), "vision frame must be create form path class"
        live_page = cast(Any, page)
        heading = live_page.locator("h1")
        assert await heading.count() >= 1
        assert (await heading.first.inner_text()).strip() == "Opret køb"
...
@pytest.mark.asyncio
async def test_dual_profiles_open_bills_create_form() -> None:
    """Profile A and independent profile B both classify the bill create form."""
    ...
    record_path = _VISION_RECORD_DIR / "ui_bills_create_open.json"
    ...
    try:
        ...
        form_a = await runtime_a.ui_bills_create_open()
        if isinstance(form_a, ToolError):
            pytest.fail(f"session A ui_bills_create_open failed: {form_a.code}: {form_a.message}")
        assert isinstance(form_a, UiBillsCreateOpenSuccess)
        assert form_a.path_class == "/:org_slug/bills/new"
        assert form_a.heading == "Opret køb"
        assert form_a.shell_kind == "bills_create"
        ...
        frame_a = frame_dir / "session_a_bills_create.png"
        await _capture_bills_create_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        form_b = await runtime_b.ui_bills_create_open()
        if isinstance(form_b, ToolError):
            pytest.fail(f"session B ui_bills_create_open failed: {form_b.code}: {form_b.message}")
        assert isinstance(form_b, UiBillsCreateOpenSuccess)
        assert form_b.model_dump() == form_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_bills_create.png"
        await _capture_bills_create_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.bills.create",
            assertion_refs=[
                "tests/live/test_ui_bills_create_open.py::"
                "test_dual_profiles_open_bills_create_form",
                "session_a_ui_bills_create_open",
                "session_b_ui_bills_create_open",
                "session_a_bills_create_frame",
                "session_b_bills_create_frame",
                "create_form_h1_opret_kob_draft_chrome",
            ],
