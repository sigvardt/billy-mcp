"""Live dual-session qualification for read-only ui_daybooks_open.
...
Never clicks create/add-line/post daybook CTAs (research117 freeze).
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiDaybooksOpenSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-daybooks-", dir=str(_DATA_ROOT)))
...
async def _capture_daybooks_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot the daybook editor surface for vision review (never create)."""
    ...
    try:
        await page.goto(
            f"https://mit.billy.dk/{slug}/daybooks/new",
        ...
        assert path.endswith("/daybooks/new"), "vision frame must be the daybooks/new path class"
        assert not path.rstrip("/").endswith("/daybooks"), "vision frame must not be bare daybooks"
...
@pytest.mark.asyncio
async def test_dual_profiles_open_daybooks_editor_shell() -> None:
    """Profile A and independent profile B both classify the daybook editor shell."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), "live daybooks shell must not use API token"
    ...
    record_path = _VISION_RECORD_DIR / "ui_daybooks_open.json"
    ...
    try:
        ...
        open_a = await runtime_a.ui_daybooks_open()
        if isinstance(open_a, ToolError):
            pytest.fail(f"session A ui_daybooks_open failed: {open_a.code}: {open_a.message}")
        assert isinstance(open_a, UiDaybooksOpenSuccess)
        assert open_a.path_class == "/:org_slug/daybooks/new"
        assert open_a.editor_markers_present is True
        assert open_a.shell_markers_present is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_daybooks.png"
        await _capture_daybooks_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        open_b = await runtime_b.ui_daybooks_open()
        if isinstance(open_b, ToolError):
            pytest.fail(f"session B ui_daybooks_open failed: {open_b.code}: {open_b.message}")
        assert isinstance(open_b, UiDaybooksOpenSuccess)
        assert open_b.model_dump() == open_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_daybooks.png"
        await _capture_daybooks_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.discovery.daybooks",
            assertion_refs=[
                "tests/live/test_ui_daybooks_open.py::test_dual_profiles_open_daybooks_editor_shell",
                "session_a_ui_daybooks_open",
                "session_b_ui_daybooks_open",
                "session_a_daybooks_frame",
                "session_b_daybooks_frame",
