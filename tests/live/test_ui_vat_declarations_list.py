"""Live dual-session qualification for read-only ui_vat_declarations_list.
...
Never clicks declare/submit/export (research120 freeze).
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiVatDeclarationsListSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-vat-decl-", dir=str(_DATA_ROOT)))
...
async def _capture_vat_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot the VAT declarations list surface for vision review (never write)."""
    ...
    try:
        await page.goto(
            f"https://mit.billy.dk/{slug}/vat-declarations",
        ...
        assert path.rstrip("/").endswith("/vat-declarations"), (
            "vision frame must be vat-declarations"
        )
        live_page = cast(Any, page)
        body = await live_page.locator("body").inner_text()
        assert "Momsangivelser" in body
...
@pytest.mark.asyncio
async def test_dual_profiles_open_vat_declarations_list_shell() -> None:
    """Profile A and independent profile B both classify the Momsangivelser list shell."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), "live VAT shell must not use API token"
    ...
    record_path = _VISION_RECORD_DIR / "ui_vat_declarations_list.json"
    ...
    try:
        ...
        open_a = await runtime_a.ui_vat_declarations_list()
        if isinstance(open_a, ToolError):
            pytest.fail(
                f"session A ui_vat_declarations_list failed: {open_a.code}: {open_a.message}"
            )
        assert isinstance(open_a, UiVatDeclarationsListSuccess)
        assert open_a.path_class == "/:org_slug/vat-declarations"
        assert open_a.heading == "Momsangivelser"
        assert open_a.period_column_visible is True
        assert open_a.shell_markers_present is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_vat_declarations.png"
        await _capture_vat_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        open_b = await runtime_b.ui_vat_declarations_list()
        if isinstance(open_b, ToolError):
            pytest.fail(
                f"session B ui_vat_declarations_list failed: {open_b.code}: {open_b.message}"
            )
        assert isinstance(open_b, UiVatDeclarationsListSuccess)
        assert open_b.model_dump() == open_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_vat_declarations.png"
        await _capture_vat_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.discovery.vat_declarations",
            assertion_refs=[
                "tests/live/test_ui_vat_declarations_list.py::"
                "test_dual_profiles_open_vat_declarations_list_shell",
                "session_a_ui_vat_declarations_list",
                "session_b_ui_vat_declarations_list",
                "session_a_vat_declarations_frame",
                "session_b_vat_declarations_frame",
                "period_chrome",
            ],
