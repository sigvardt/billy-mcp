"""Live dual-session qualification for read-only ui_daybook_transactions_create_open.
...
Classifies Tilføj kassekladdelinje + Ingen postering valgt only; never clicks Tilføj.
Live harness seeds a disposable daybook via SPA token, then deletes it.
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiDaybookTransactionsCreateOpenSuccess
...
_MARKER_D = "R18681-TMP-DAYBOOK-TX-CREATE-DO-NOT-USE"
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-daybook-tx-create-", dir=str(_DATA_ROOT)))
...
async def _ensure_disposable_daybook(runtime: BrowserRuntime, org_identity: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Always seed a disposable daybook for create-chrome observation.
        dbr = await _spa(
            ctx,
            "POST",
...
async def _capture_daybook_tx_create_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
    *,
    preferred_daybook_id: str | None = None,
) -> None:
    """Screenshot daybook create chrome after tool success for vision review (outside git)."""
    ...
    try:
        ...
        listed = await _spa(
            ctx,
            "GET",
            f"https://api.billysbilling.com/v2/daybooks?organizationId={_ORG}&pageSize=50",
            headers,
        )
        candidates: list[str] = []
        if preferred_daybook_id:
            candidates.append(preferred_daybook_id)
        if listed["status"] == 200 and isinstance(listed.get("data"), dict):
            rows = cast(list[dict[str, Any]], listed["data"].get("daybooks") or [])
            for row in reversed(rows):
                rid = row.get("id")
                if rid and str(rid) not in candidates:
                    candidates.append(str(rid))
        assert candidates, "no daybook id for vision frame"

        captured = False
        for daybook_id in candidates[:12]:
            await live_page.goto(
            ...
            if not re.search(r"/daybooks/[^/]+$", path) or path.rstrip("/").endswith(
                "/daybooks/new"
            ):
                continue
            body = await live_page.locator("body").inner_text()
            if "Upsedasse" in body:
                continue
            if not all(marker in body for marker in _EDITOR_MARKERS):
                continue
            if "Tilføj kassekladdelinje" not in body or "Ingen postering valgt" not in body:
                continue
            await live_page.screenshot(path=str(destination), full_page=False)
            assert destination.is_file() and destination.stat().st_size > 0
            captured = True
            break
        assert captured, "no daybook exposed create chrome for vision frame"
    finally:
        await page.close()


@pytest.mark.asyncio
async def test_dual_profiles_open_daybook_transactions_create_chrome() -> None:
    """Profile A and independent profile B both classify daybookTransactions create chrome."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), "live daybook tx create must not use API token"
    ...
    record_path = _VISION_RECORD_DIR / "ui_daybook_transactions_create_open.json"
    ...
    try:
        ...
        open_a = await runtime_a.ui_daybook_transactions_create_open()
        if isinstance(open_a, ToolError):
            pytest.fail(f"session A create open failed: {open_a.code}: {open_a.message}")
        assert isinstance(open_a, UiDaybookTransactionsCreateOpenSuccess)
        assert open_a.path_class == "/:org_slug/daybooks/:id"
        assert open_a.shell_kind == "daybook_transactions_create"
        assert open_a.detail_open is True
        assert open_a.line_add_chrome_visible is True
        assert open_a.empty_postering_state is True
        assert open_a.shell_markers_present is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_daybook_tx_create.png"
        await _capture_daybook_tx_create_frame(
            runtime_a, org_a, frame_a, preferred_daybook_id=ids_a.get("daybook")
        )
        ...
        open_b = await runtime_b.ui_daybook_transactions_create_open()
        if isinstance(open_b, ToolError):
            pytest.fail(f"session B create open failed: {open_b.code}: {open_b.message}")
        assert isinstance(open_b, UiDaybookTransactionsCreateOpenSuccess)
        assert open_b.model_dump() == open_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_daybook_tx_create.png"
        await _capture_daybook_tx_create_frame(
            runtime_b, org_b, frame_b, preferred_daybook_id=ids_b.get("daybook")
        )
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.parity.daybookTransactions.create",
            assertion_refs=[
                "tests/live/test_ui_daybook_transactions_create_open.py::test_dual_profiles_open_daybook_transactions_create_chrome",
                "session_a_ui_daybook_transactions_create_open",
                "session_b_ui_daybook_transactions_create_open",
                "session_a_daybook_tx_create_frame",
                "session_b_daybook_tx_create_frame",
                "path_class_daybooks_id_create_chrome",
                "tilfoej_kassekladdelinje_empty_postering_chrome",
            ],
