"""Live dual-session qualification for read-only ui_uploads_list.
...
Never sets file inputs or clicks upload actions.
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiUploadsListSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-uploads-", dir=str(_DATA_ROOT)))
...
async def _capture_uploads_list_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot the uploads list surface for vision review (never set files)."""
    ...
    try:
        await page.goto(f"https://mit.billy.dk/{slug}/uploads", wait_until="domcontentloaded")
        try:
            await page.wait_for_load_state("networkidle", timeout=20000)
        except Exception:
            pass
        path = urlsplit(page.url).path or ""
        assert path.endswith("/uploads"), "vision frame must be the uploads list path class"
...
@pytest.mark.asyncio
async def test_dual_profiles_open_uploads_list_shell() -> None:
    """Profile A and independent profile B both classify the uploads list shell."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), "live uploads shell must not use API token"
    ...
    record_path = _VISION_RECORD_DIR / "ui_uploads_list.json"
    ...
    try:
        ...
        list_a = await runtime_a.ui_uploads_list()
        if isinstance(list_a, ToolError):
            pytest.fail(f"session A ui_uploads_list failed: {list_a.code}: {list_a.message}")
        assert isinstance(list_a, UiUploadsListSuccess)
        ...
        await _capture_uploads_list_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        list_b = await runtime_b.ui_uploads_list()
        if isinstance(list_b, ToolError):
            pytest.fail(f"session B ui_uploads_list failed: {list_b.code}: {list_b.message}")
        assert isinstance(list_b, UiUploadsListSuccess)
        assert list_b.model_dump() == list_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_uploads_list.png"
        await _capture_uploads_list_frame(runtime_b, org_b, frame_b)
