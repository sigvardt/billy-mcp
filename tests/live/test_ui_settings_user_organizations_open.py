"""Live dual-session qualification for read-only ui_settings_user_organizations_open.

Indstillinger Virksomheder (user organizations) multi-org panel at
/:org_slug/settings via hub open + observe-only Profil then Virksomheder
(research152).
...
Never clicks Opret organisation / Gem ændringer / Upload / password submit /
Opret* / Tilføj* (research152 freeze).
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiSettingsUserOrganizationsOpenSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-settings-user-orgs-", dir=str(_DATA_ROOT)))
...
async def _capture_settings_user_organizations_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot Indstillinger Virksomheder surface for vision (never write CTAs)."""
    ...
    try:
        ...
        async def _click_exact(label: str) -> bool:
            loc = live_page.get_by_text(label, exact=True)
        ...
        # Same contract as product: Profil group then Virksomheder (research152).
        await _click_exact("Profil")
        try:
            await page.wait_for_load_state("networkidle", timeout=10000)
        except Exception:
            pass
        clicked = await _click_exact("Virksomheder")
        assert clicked, "vision frame must click Virksomheder side-nav"
        try:
        ...
        assert "Virksomheder" in body
        assert "Alle organisationer" in body or "Opret organisation" in body
        assert "Upsedasse" not in body
        # Full Profil user-edit panel is not the active Virksomheder multi-org shell.
        assert not ("Billede" in body and "Sprog og tema" in body and "Skift adgangskode" in body)
...
@pytest.mark.asyncio
async def test_dual_profiles_open_settings_user_organizations_shell() -> None:
    """Dual profiles classify the Indstillinger Virksomheder settings panel."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), (
        "live settings user organizations shell must not use API token"
    )
    ...
    record_path = _VISION_RECORD_DIR / "ui_settings_user_organizations_open.json"
    ...
    try:
        ...
        open_a = await runtime_a.ui_settings_user_organizations_open()
        if isinstance(open_a, ToolError):
            pytest.fail(
                f"session A ui_settings_user_organizations_open failed: "
                f"{open_a.code}: {open_a.message}"
            )
        assert isinstance(open_a, UiSettingsUserOrganizationsOpenSuccess)
        assert open_a.path_class == "/:org_slug/settings"
        assert open_a.heading == "Indstillinger"
        assert open_a.shell_kind == "settings_user_organizations"
        assert open_a.user_organizations_panel_markers_present is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_settings_user_organizations.png"
        await _capture_settings_user_organizations_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        open_b = await runtime_b.ui_settings_user_organizations_open()
        if isinstance(open_b, ToolError):
            pytest.fail(
                f"session B ui_settings_user_organizations_open failed: "
                f"{open_b.code}: {open_b.message}"
            )
        assert isinstance(open_b, UiSettingsUserOrganizationsOpenSuccess)
        assert open_b.model_dump() == open_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_settings_user_organizations.png"
        await _capture_settings_user_organizations_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.discovery.settings_user_organizations",
            assertion_refs=[
                "tests/live/test_ui_settings_user_organizations_open.py::test_dual_profiles_open_settings_user_organizations_shell",
                "session_a_ui_settings_user_organizations_open",
                "session_b_ui_settings_user_organizations_open",
                "session_a_settings_user_organizations_frame",
                "session_b_settings_user_organizations_frame",
                "settings_user_organizations_path_settings",
            ],
