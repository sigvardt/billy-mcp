"""Live dual-session qualification for read-only ui_bank_reconciliation_open.
...
Never clicks Forbind til bank, Importer, or Match.
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiBankReconciliationOpenSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-bank-recon-", dir=str(_DATA_ROOT)))
...
async def _capture_bank_reconciliation_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot the Afstemning recon surface for vision review (no write clicks)."""
    ...
    try:
        await page.goto(f"https://mit.billy.dk/{slug}/bank-accounts", wait_until="domcontentloaded")
        try:
            await page.wait_for_load_state("networkidle", timeout=20000)
        except Exception:
            pass
        live_page = cast(Any, page)
        link = live_page.locator("a", has_text="Afstemning")
        assert await link.count() >= 1
        href = await link.first.get_attribute("href")
        assert href
        full = href if href.startswith("http") else f"https://mit.billy.dk{href}"
        ...
        assert "/bank_accounts/" in path and path.rstrip("/").endswith("/sync"), (
            "vision frame must be the bank_accounts/:id/sync path class"
        )
        # Empty shell is valid; do not require h1.
...
@pytest.mark.asyncio
async def test_dual_profiles_open_bank_reconciliation_shell() -> None:
    """Profile A and independent profile B both classify the Afstemning recon shell."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), "live bank recon shell must not use API token"
    ...
    record_path = _VISION_RECORD_DIR / "ui_bank_reconciliation_open.json"
    ...
    try:
        ...
        open_a = await runtime_a.ui_bank_reconciliation_open()
        if isinstance(open_a, ToolError):
            pytest.fail(
                f"session A ui_bank_reconciliation_open failed: {open_a.code}: {open_a.message}"
            )
        assert isinstance(open_a, UiBankReconciliationOpenSuccess)
        assert open_a.path_class == "/:org_slug/bank_accounts/:id/sync"
        assert open_a.empty_content_shell is True
        assert open_a.afstemning_nav_visible is True
        assert open_a.heading == ""
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_bank_reconciliation.png"
        await _capture_bank_reconciliation_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        open_b = await runtime_b.ui_bank_reconciliation_open()
        if isinstance(open_b, ToolError):
            pytest.fail(
                f"session B ui_bank_reconciliation_open failed: {open_b.code}: {open_b.message}"
            )
        assert isinstance(open_b, UiBankReconciliationOpenSuccess)
        assert open_b.model_dump() == open_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_bank_reconciliation.png"
        await _capture_bank_reconciliation_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.discovery.bank_reconciliation",
            assertion_refs=[
                "tests/live/test_ui_bank_reconciliation_open.py::"
                "test_dual_profiles_open_bank_reconciliation_shell",
                "session_a_ui_bank_reconciliation_open",
                "session_b_ui_bank_reconciliation_open",
                "session_a_bank_reconciliation_frame",
                "session_b_bank_reconciliation_frame",
                "recon_surface_bank_accounts_id_sync_empty_shell",
            ],
