"""Live dual-session qualification for read-only ui_transactions_list.
...
Never clicks Ny postering / create / post / void (research118 freeze).
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiTransactionsListSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-transactions-", dir=str(_DATA_ROOT)))
...
async def _capture_transactions_list_frame(
    ...
    """Screenshot the Posteringer list surface for vision review (never create)."""
...
@pytest.mark.asyncio
async def test_dual_profiles_open_transactions_list_shell() -> None:
    ...
    record_path = _VISION_RECORD_DIR / "ui_transactions_list.json"
    ...
    try:
        ...
        list_a = await runtime_a.ui_transactions_list()
        if isinstance(list_a, ToolError):
            pytest.fail(f"session A ui_transactions_list failed: {list_a.code}: {list_a.message}")
        assert isinstance(list_a, UiTransactionsListSuccess)
        assert list_a.path_class == "/:org_slug/transactions"
        assert list_a.heading == "Posteringer"
        ...
        list_b = await runtime_b.ui_transactions_list()
        if isinstance(list_b, ToolError):
            pytest.fail(f"session B ui_transactions_list failed: {list_b.code}: {list_b.message}")
        assert isinstance(list_b, UiTransactionsListSuccess)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.discovery.transactions",
            assertion_refs=[
                "tests/live/test_ui_transactions_list.py::"
                "test_dual_profiles_open_transactions_list_shell",
                "session_a_ui_transactions_list",
                "session_b_ui_transactions_list",
