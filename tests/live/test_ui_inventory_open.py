"""Live dual-session qualification for read-only ui_inventory_open.

Lagermodul shell at /:org_slug/inventory (research125).
...
Never clicks Opret primo / Opret produkt / Opret status (research125 freeze).
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiInventoryOpenSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-inventory-", dir=str(_DATA_ROOT)))
...
async def _capture_inventory_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot the Lagermodul surface for vision review (never create CTAs)."""
    ...
    try:
        await page.goto(
            f"https://mit.billy.dk/{slug}/inventory",
        ...
        assert path.rstrip("/").endswith("/inventory"), "vision frame must be inventory shell"
        live_page = cast(Any, page)
        body = await live_page.locator("body").inner_text()
        assert "Lagermodul" in body
        assert "Upsedasse" not in body
        # Observe create CTA text may exist; never click.
...
@pytest.mark.asyncio
async def test_dual_profiles_open_inventory_lagermodul_shell() -> None:
    """Dual profiles classify the Lagermodul inventory shell."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), "live inventory shell must not use API token"
    ...
    record_path = _VISION_RECORD_DIR / "ui_inventory_open.json"
    ...
    try:
        ...
        open_a = await runtime_a.ui_inventory_open()
        if isinstance(open_a, ToolError):
            pytest.fail(f"session A ui_inventory_open failed: {open_a.code}: {open_a.message}")
        assert isinstance(open_a, UiInventoryOpenSuccess)
        assert open_a.path_class == "/:org_slug/inventory"
        assert open_a.heading == "Lagermodul"
        assert open_a.shell_kind == "lagermodul"
        assert open_a.create_cta_markers_present is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_inventory.png"
        await _capture_inventory_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        open_b = await runtime_b.ui_inventory_open()
        if isinstance(open_b, ToolError):
            pytest.fail(f"session B ui_inventory_open failed: {open_b.code}: {open_b.message}")
        assert isinstance(open_b, UiInventoryOpenSuccess)
        assert open_b.model_dump() == open_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_inventory.png"
        await _capture_inventory_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.discovery.inventory",
            assertion_refs=[
                "tests/live/test_ui_inventory_open.py::test_dual_profiles_open_inventory_lagermodul_shell",
                "session_a_ui_inventory_open",
                "session_b_ui_inventory_open",
                "session_a_inventory_frame",
                "session_b_inventory_frame",
                "lagermodul_path_inventory",
            ],
