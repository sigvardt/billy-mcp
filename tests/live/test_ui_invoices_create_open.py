"""Live dual-session qualification for read-only ui_invoices_create_open.
...
Never submits Godkend/Send/Gem/Tilføj/Vedhæft.
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiInvoicesCreateOpenSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-invoices-create-", dir=str(_DATA_ROOT)))
...
async def _capture_invoices_create_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot the invoice create form surface for vision review."""
    ...
    try:
        await page.goto(f"https://mit.billy.dk/{slug}/invoices/new", wait_until="domcontentloaded")
        try:
            await page.wait_for_load_state("networkidle", timeout=20000)
        except Exception:
            pass
        path = urlsplit(page.url).path or ""
        assert path.endswith("/invoices/new"), "vision frame must be create form path class"
        live_page = cast(Any, page)
        heading = live_page.locator("h1")
        assert await heading.count() >= 1
        assert (await heading.first.inner_text()).strip() == "Opret faktura"
        draft = live_page.locator("text=Gem som kladde")
        assert await draft.count() >= 1 and await draft.first.is_visible()
...
@pytest.mark.asyncio
async def test_dual_profiles_open_invoices_create_form() -> None:
    """Profile A and independent profile B both classify the invoice create form."""
    ...
    record_path = _VISION_RECORD_DIR / "ui_invoices_create_open.json"
    ...
    try:
        ...
        form_a = await runtime_a.ui_invoices_create_open()
        if isinstance(form_a, ToolError):
            pytest.fail(
                f"session A ui_invoices_create_open failed: {form_a.code}: {form_a.message}"
            )
        assert isinstance(form_a, UiInvoicesCreateOpenSuccess)
        assert form_a.path_class == "/:org_slug/invoices/new"
        assert form_a.heading == "Opret faktura"
        assert form_a.shell_kind == "invoices_create"
        assert form_a.draft_save_chrome_visible is True
        assert form_a.line_chrome_visible is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_invoices_create.png"
        await _capture_invoices_create_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        form_b = await runtime_b.ui_invoices_create_open()
        if isinstance(form_b, ToolError):
            pytest.fail(
                f"session B ui_invoices_create_open failed: {form_b.code}: {form_b.message}"
            )
        assert isinstance(form_b, UiInvoicesCreateOpenSuccess)
        assert form_b.model_dump() == form_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_invoices_create.png"
        await _capture_invoices_create_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.invoices.create",
            assertion_refs=[
                "tests/live/test_ui_invoices_create_open.py::"
                "test_dual_profiles_open_invoices_create_form",
                "session_a_ui_invoices_create_open",
                "session_b_ui_invoices_create_open",
                "session_a_invoices_create_frame",
                "session_b_invoices_create_frame",
                "create_form_h1_opret_faktura_draft_chrome",
            ],
