"""Live dual-session qualification for read-only ui_addons_open.

Fordele hub at /:org_slug/add-ons (research123).
...
Never clicks partner CTAs (install/connect/access-token/loan) (research123 freeze).
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiAddonsOpenSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-addons-", dir=str(_DATA_ROOT)))
...
async def _capture_addons_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot the Fordele hub surface for vision review (never partner CTAs)."""
    ...
    try:
        await page.goto(
            f"https://mit.billy.dk/{slug}/add-ons",
        ...
        assert path.rstrip("/").endswith("/add-ons"), "vision frame must be add-ons hub"
        live_page = cast(Any, page)
        body = await live_page.locator("body").inner_text()
        assert "Fordele" in body
        assert "Upsedasse" not in body
        # Observe partner CTA text may exist; never click.
...
@pytest.mark.asyncio
async def test_dual_profiles_open_addons_fordele_shell() -> None:
    """Dual profiles classify the Fordele add-ons hub shell."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), "live addons shell must not use API token"
    ...
    record_path = _VISION_RECORD_DIR / "ui_addons_open.json"
    ...
    try:
        ...
        open_a = await runtime_a.ui_addons_open()
        if isinstance(open_a, ToolError):
            pytest.fail(f"session A ui_addons_open failed: {open_a.code}: {open_a.message}")
        assert isinstance(open_a, UiAddonsOpenSuccess)
        assert open_a.path_class == "/:org_slug/add-ons"
        assert open_a.heading == "Fordele"
        assert open_a.shell_markers_present is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_addons.png"
        await _capture_addons_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        open_b = await runtime_b.ui_addons_open()
        if isinstance(open_b, ToolError):
            pytest.fail(f"session B ui_addons_open failed: {open_b.code}: {open_b.message}")
        assert isinstance(open_b, UiAddonsOpenSuccess)
        assert open_b.model_dump() == open_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_addons.png"
        await _capture_addons_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.discovery.addons",
            assertion_refs=[
                "tests/live/test_ui_addons_open.py::test_dual_profiles_open_addons_fordele_shell",
                "session_a_ui_addons_open",
                "session_b_ui_addons_open",
                "session_a_addons_frame",
                "session_b_addons_frame",
                "fordele_hub_path_add_ons",
            ],
