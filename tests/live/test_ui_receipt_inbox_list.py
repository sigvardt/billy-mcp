"""Live dual-session qualification for read-only ui_receipt_inbox_list.
...
Never sets file inputs or clicks edit/upload actions.
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiReceiptInboxListSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-receipt-inbox-", dir=str(_DATA_ROOT)))
...
async def _capture_receipt_inbox_list_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot the receipt inbox list surface for vision review (never set files)."""
    ...
    try:
        await page.goto(f"https://mit.billy.dk/{slug}/vouchers", wait_until="domcontentloaded")
        try:
            await page.wait_for_load_state("networkidle", timeout=20000)
        except Exception:
            pass
        path = urlsplit(page.url).path or ""
        assert path.endswith("/vouchers"), "vision frame must be the vouchers path class"
        live_page = cast(Any, page)
        heading = live_page.locator("h1")
        assert await heading.count() >= 1
        assert (await heading.first.inner_text()).strip() == "Bilagsindbakke"
...
@pytest.mark.asyncio
async def test_dual_profiles_open_receipt_inbox_list_shell() -> None:
    """Profile A and independent profile B both classify the receipt inbox list shell."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), "live receipt inbox shell must not use API token"
    ...
    record_path = _VISION_RECORD_DIR / "ui_receipt_inbox_list.json"
    ...
    try:
        ...
        list_a = await runtime_a.ui_receipt_inbox_list()
        if isinstance(list_a, ToolError):
            pytest.fail(f"session A ui_receipt_inbox_list failed: {list_a.code}: {list_a.message}")
        assert isinstance(list_a, UiReceiptInboxListSuccess)
        assert list_a.path_class == "/:org_slug/vouchers"
        assert list_a.heading == "Bilagsindbakke"
        assert list_a.file_control_present is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_receipt_inbox_list.png"
        await _capture_receipt_inbox_list_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        list_b = await runtime_b.ui_receipt_inbox_list()
        if isinstance(list_b, ToolError):
            pytest.fail(f"session B ui_receipt_inbox_list failed: {list_b.code}: {list_b.message}")
        assert isinstance(list_b, UiReceiptInboxListSuccess)
        assert list_b.model_dump() == list_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_receipt_inbox_list.png"
        await _capture_receipt_inbox_list_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.discovery.receipt_inbox",
            assertion_refs=[
                "tests/live/test_ui_receipt_inbox_list.py::test_dual_profiles_open_receipt_inbox_list_shell",
                "session_a_ui_receipt_inbox_list",
                "session_b_ui_receipt_inbox_list",
                "session_a_receipt_inbox_list_frame",
                "session_b_receipt_inbox_list_frame",
                "list_surface_h1_bilagsindbakke_vouchers",
            ],
