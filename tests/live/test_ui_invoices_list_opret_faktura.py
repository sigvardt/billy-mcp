"""Live inspect: official invoice-list Opret faktura landing.

Creates one tagged customer through FastMCP, opens the invoice list, and
clicks exact Opret faktura once. Never Godkend, Send, Gem, or email.
"""
...
_BLOCKER_PATH = (
    ...
    / "main.billy_complete"
    / "tmp"
    / "live-invoices-list-opret-faktura-blocker.txt"
)
...
def _require_live_credentials() -> None:
    if _credentials_configured():
        return
    reason = (
        "Live list Opret faktura inspect blocked: BILLY_BROWSER_PRIMARY_REFERENCE and "
...
def _invoice_id_from(url: str) -> str | None:
    ...
    if len(parts) < 2 or parts[-2] != "invoices":
        return None
    token = parts[-1]
    if token in {"new", "empty"}:
        return None
    return token
...
def _assert_dump_clean(payload: dict[str, object], *, require_code: bool = False) -> None:
    ...
    assert set(REQUIRED_LIST_OPRET_FAKTURA_KEYS) <= set(payload)
...
@pytest.mark.asyncio
async def test_ui_invoices_list_opret_faktura(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Click official list Opret faktura once and record the landing. Do not Gem."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), "list CTA dump must not use API token"
    if list_opret_faktura_dump_is_delivered():
        written = json.loads(LIST_OPRET_FAKTURA_DUMP.read_text(encoding="utf-8"))
        _assert_dump_clean(written)
        text = LIST_OPRET_FAKTURA_DUMP.read_text(encoding="utf-8")
    ...
    try:
        ...
        opened = await _call(server, "ui_invoices_list")
        # Empty orgs land on /invoices/empty. The list tool may return
        # UI_CHANGED. That is not a customer bind. Stay on this path class.
        if opened.get("code") and opened.get("code") != StableErrorCode.UI_CHANGED:
            _record_blocker(f"ui_invoices_list failed: {opened}")
            pytest.fail(f"ui_invoices_list failed: {opened}")
        ...
        assert "/invoices/new" not in urlsplit(page_url).path
        result = await capture_list_opret_faktura(page, tag=tag)
        leftover_invoice = _invoice_id_from(page_url)
        _assert_dump_clean(result, require_code=True)
        assert LIST_OPRET_FAKTURA_DUMP.is_file()
        written = json.loads(LIST_OPRET_FAKTURA_DUMP.read_text(encoding="utf-8"))
        _assert_dump_clean(written)
