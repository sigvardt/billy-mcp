"""Live dual-session qualification for read-only ui_settings_accounting_open.

Indstillinger Regnskab (accounting) panel at /:org_slug/settings via SPA seed
settings/accounting (research127).
...
Never clicks Gem ændringer / Sæt låsedato / Opret* / Tilføj* (research127 freeze).
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiSettingsAccountingOpenSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-settings-accounting-", dir=str(_DATA_ROOT)))
...
async def _capture_settings_accounting_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot Indstillinger Regnskab surface for vision (never write CTAs)."""
    ...
    try:
        await page.goto(
            f"https://mit.billy.dk/{slug}/settings/accounting",
        ...
        assert "Regnskab" in body
        assert "Køb" in body
        assert "Kontoplan" in body
        assert "Upsedasse" not in body
        # Company default markers should not both define the active panel.
        # (Rail may still list Virksomhed; company h2s should be absent.)
...
@pytest.mark.asyncio
async def test_dual_profiles_open_settings_accounting_shell() -> None:
    """Dual profiles classify the Indstillinger Regnskab settings panel."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), (
        "live settings accounting shell must not use API token"
    )
    ...
    record_path = _VISION_RECORD_DIR / "ui_settings_accounting_open.json"
    ...
    try:
        ...
        open_a = await runtime_a.ui_settings_accounting_open()
        if isinstance(open_a, ToolError):
            pytest.fail(
                f"session A ui_settings_accounting_open failed: {open_a.code}: {open_a.message}"
            )
        assert isinstance(open_a, UiSettingsAccountingOpenSuccess)
        assert open_a.path_class == "/:org_slug/settings"
        assert open_a.heading == "Indstillinger"
        assert open_a.shell_kind == "settings_accounting"
        assert open_a.accounting_panel_markers_present is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_settings_accounting.png"
        await _capture_settings_accounting_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        open_b = await runtime_b.ui_settings_accounting_open()
        if isinstance(open_b, ToolError):
            pytest.fail(
                f"session B ui_settings_accounting_open failed: {open_b.code}: {open_b.message}"
            )
        assert isinstance(open_b, UiSettingsAccountingOpenSuccess)
        assert open_b.model_dump() == open_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_settings_accounting.png"
        await _capture_settings_accounting_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.discovery.settings_accounting",
            assertion_refs=[
                "tests/live/test_ui_settings_accounting_open.py::test_dual_profiles_open_settings_accounting_shell",
                "session_a_ui_settings_accounting_open",
                "session_b_ui_settings_accounting_open",
                "session_a_settings_accounting_frame",
                "session_b_settings_accounting_frame",
                "settings_accounting_path_settings",
            ],
