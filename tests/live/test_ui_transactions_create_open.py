"""Live dual-session qualification for read-only ui_transactions_create_open.
...
Classifies Posteringer + Ny postering create chrome only;
never clicks Ny postering submit/post/void (research182 freeze).
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiTransactionsCreateOpenSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-transactions-create-", dir=str(_DATA_ROOT)))
...
async def _capture_transactions_list_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot the Posteringer create chrome surface for vision review (never submit)."""
    ...
    try:
        await page.goto(
            f"https://mit.billy.dk/{slug}/transactions",
        ...
        assert path.endswith("/transactions"), (
            "vision frame must be the transactions list path class"
        )
        live_page = cast(Any, page)
        heading = live_page.locator("h1")
        assert await heading.count() >= 1
        assert (await heading.first.inner_text()).strip() == "Posteringer"
        create_action = live_page.locator("text=Ny postering")
...
@pytest.mark.asyncio
async def test_dual_profiles_open_transactions_create_chrome() -> None:
    """Profile A and independent profile B both classify the Posteringer list shell."""
    ...
    record_path = _VISION_RECORD_DIR / "ui_transactions_create_open.json"
    runtime_a: BrowserRuntime | None = None
    runtime_b: BrowserRuntime | None = None

    try:
        assert is_outside_repository(frame_dir, _REPO_ROOT)
        # Tool path is browser-only: never constructs BillyHttpClient / BILLY_API_TOKEN.

        runtime_a = _runtime(profile_a, org_a)
        await _login_until_ready(runtime_a)
        list_a = await runtime_a.ui_transactions_create_open()
        if isinstance(list_a, ToolError):
            pytest.fail(
                f"session A ui_transactions_create_open failed: {list_a.code}: {list_a.message}"
            )
        assert isinstance(list_a, UiTransactionsCreateOpenSuccess)
        assert list_a.path_class == "/:org_slug/transactions"
        assert list_a.shell_kind == "transactions_create"
        assert list_a.list_open is True
        assert list_a.create_cta_visible is True
        assert list_a.shell_markers_present is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_transactions_list.png"
        await _capture_transactions_list_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        list_b = await runtime_b.ui_transactions_create_open()
        if isinstance(list_b, ToolError):
            pytest.fail(
                f"session B ui_transactions_create_open failed: {list_b.code}: {list_b.message}"
            )
        assert isinstance(list_b, UiTransactionsCreateOpenSuccess)
        assert list_b.model_dump() == list_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_transactions_list.png"
        await _capture_transactions_list_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.parity.transactions.create",
            assertion_refs=[
                "tests/live/test_ui_transactions_create_open.py::"
                "test_dual_profiles_open_transactions_create_chrome",
                "session_a_ui_transactions_create_open",
                "session_b_ui_transactions_create_open",
                "session_a_transactions_list_frame",
                "session_b_transactions_list_frame",
                "list_surface_h1_posteringer_create_cta",
            ],
