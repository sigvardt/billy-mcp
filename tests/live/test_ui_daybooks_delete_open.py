"""Live dual-session qualification for read-only ui_daybooks_delete_open.
...
Opens Mere and classifies Slet menu text only; never confirms Slet. Live harness
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiDaybooksDeleteOpenSuccess
...
_MARKER_D = "R18680-TMP-DAYBOOK-DELETE-DO-NOT-USE"
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-daybooks-delete-", dir=str(_DATA_ROOT)))
...
async def _ensure_disposable_daybook(runtime: BrowserRuntime, org_identity: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Always seed a disposable daybook: system journals may omit Mere→Slet.
        dbr = await _spa(
            ctx,
            "POST",
...
async def _capture_daybooks_delete_frame(
    ...
    *,
    ...
    """Screenshot Mere delete chrome after tool success for vision review (outside git)."""
    ...
    try:
        ...
        for daybook_id in candidates[:12]:
            ...
            if not all(marker in body for marker in _EDITOR_MARKERS):
                continue
            mere = live_page.get_by_role("button", name="Mere", exact=True)
            if await mere.count() < 1:
                continue
            await mere.first.click(timeout=2500)
            await live_page.wait_for_timeout(900)
            body2 = await live_page.locator("body").inner_text()
            if "Slet" not in body2:
            ...
            captured = True
            break
        assert captured, "no daybook exposed Mere→Slet chrome for vision frame"
    finally:
        await page.close()


@pytest.mark.asyncio
async def test_dual_profiles_open_daybooks_delete_chrome() -> None:
    """Profile A and independent profile B both classify daybook Mere delete chrome."""
    ...
    record_path = _VISION_RECORD_DIR / "ui_daybooks_delete_open.json"
    ...
    try:
        ...
        open_a = await runtime_a.ui_daybooks_delete_open()
        if isinstance(open_a, ToolError):
            pytest.fail(
                f"session A ui_daybooks_delete_open failed: {open_a.code}: {open_a.message}"
            )
        assert isinstance(open_a, UiDaybooksDeleteOpenSuccess)
        assert open_a.path_class == "/:org_slug/daybooks/:id"
        assert open_a.shell_kind == "daybooks_delete"
        assert open_a.detail_open is True
        assert open_a.mere_open is True
        assert open_a.slet_text_visible is True
        assert open_a.export_menu_visible is True
        assert open_a.primary_slet_absent is True
        assert open_a.shell_markers_present is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_daybooks_delete.png"
        await _capture_daybooks_delete_frame(
        ...
        open_b = await runtime_b.ui_daybooks_delete_open()
        if isinstance(open_b, ToolError):
            pytest.fail(
                f"session B ui_daybooks_delete_open failed: {open_b.code}: {open_b.message}"
            )
        assert isinstance(open_b, UiDaybooksDeleteOpenSuccess)
        assert open_b.model_dump() == open_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_daybooks_delete.png"
        await _capture_daybooks_delete_frame(
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.parity.daybooks.delete",
            assertion_refs=[
                "tests/live/test_ui_daybooks_delete_open.py::test_dual_profiles_open_daybooks_delete_chrome",
                "session_a_ui_daybooks_delete_open",
                "session_b_ui_daybooks_delete_open",
                "session_a_daybooks_delete_frame",
                "session_b_daybooks_delete_frame",
                "path_class_daybooks_id_mere_slet",
                "mere_menu_slet_export_chrome",
            ],
