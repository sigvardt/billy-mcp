"""Live dual-session qualification for read-only ui_settings_beta_open.

Indstillinger Betas (Tidlig adgang) panel at /:org_slug/settings via hub open +
observe-only side-nav click Betas (research133).
...
Never clicks Invitér* / Overdrag* / Gem* / Opret* (research133 freeze).
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiSettingsBetaOpenSuccess
...
async def _capture_settings_beta_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot Indstillinger Betas surface for vision (never write CTAs)."""
    ...
    try:
        ...
        # Observe-only Betas nav click (same contract as product tool).
        loc = live_page.get_by_text("Betas", exact=True)
        ...
        for i in range(min(count, 8)):
            ...
            try:
                ...
                continue
        assert clicked, "vision frame must click Betas side-nav"
        try:
        ...
        assert "Tidlig adgang" in body
        assert "Upsedasse" not in body
        # Active panel must not be company default (research133).
...
@pytest.mark.asyncio
async def test_dual_profiles_open_settings_beta_shell() -> None:
    """Dual profiles classify the Indstillinger Betas settings panel."""
    ...
    record_path = _VISION_RECORD_DIR / "ui_settings_beta_open.json"
    ...
    try:
        ...
        open_a = await runtime_a.ui_settings_beta_open()
        if isinstance(open_a, ToolError):
            pytest.fail(f"session A ui_settings_beta_open failed: {open_a.code}: {open_a.message}")
        assert isinstance(open_a, UiSettingsBetaOpenSuccess)
        assert open_a.path_class == "/:org_slug/settings"
        assert open_a.heading == "Indstillinger"
        assert open_a.shell_kind == "settings_beta"
        assert open_a.beta_panel_markers_present is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_settings_beta.png"
        await _capture_settings_beta_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        open_b = await runtime_b.ui_settings_beta_open()
        if isinstance(open_b, ToolError):
            pytest.fail(f"session B ui_settings_beta_open failed: {open_b.code}: {open_b.message}")
        assert isinstance(open_b, UiSettingsBetaOpenSuccess)
        assert open_b.model_dump() == open_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_settings_beta.png"
        await _capture_settings_beta_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.discovery.settings_beta",
            assertion_refs=[
                "tests/live/test_ui_settings_beta_open.py::test_dual_profiles_open_settings_beta_shell",
                "session_a_ui_settings_beta_open",
                "session_b_ui_settings_beta_open",
                "session_a_settings_beta_frame",
                "session_b_settings_beta_frame",
                "settings_beta_path_settings",
            ],
