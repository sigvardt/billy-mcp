"""Live dual-session qualification for read-only ui_clients_create_open.
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiClientsCreateOpenSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-clients-create-", dir=str(_DATA_ROOT)))
...
async def _capture_clients_create_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot the clients create dialog surface for vision review."""
    ...
    try:
        ...
        assert path.rstrip("/").endswith("/clients") and not path.rstrip("/").endswith(
            "/clients/new"
        ), "vision frame must stay on clients list path class"
...
@pytest.mark.asyncio
async def test_dual_profiles_open_clients_create_form() -> None:
    """Profile A and independent profile B both classify the clients create form."""
    ...
    record_path = _VISION_RECORD_DIR / "ui_clients_create_open.json"
    ...
    try:
        ...
        form_a = await runtime_a.ui_clients_create_open()
        if isinstance(form_a, ToolError):
            pytest.fail(f"session A ui_clients_create_open failed: {form_a.code}: {form_a.message}")
        assert isinstance(form_a, UiClientsCreateOpenSuccess)
        assert form_a.path_class == "/:org_slug/clients"
        assert form_a.heading == "Kunder"
        assert form_a.shell_kind == "clients_create"
        ...
        frame_a = frame_dir / "session_a_clients_create.png"
        await _capture_clients_create_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        form_b = await runtime_b.ui_clients_create_open()
        if isinstance(form_b, ToolError):
            pytest.fail(f"session B ui_clients_create_open failed: {form_b.code}: {form_b.message}")
        assert isinstance(form_b, UiClientsCreateOpenSuccess)
        assert form_b.model_dump() == form_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_clients_create.png"
        await _capture_clients_create_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            ...
            assertion_refs=[
                "tests/live/test_ui_clients_create_open.py::"
                "test_dual_profiles_open_clients_create_form",
                "session_a_ui_clients_create_open",
                "session_b_ui_clients_create_open",
                "session_a_clients_create_frame",
                "session_b_clients_create_frame",
