"""Live inspect: official product dialog Gem produkt and Arkiveret.

Read-only. Never clicks Gem or Gem produkt. Never creates a product.
"""
...
_BLOCKER_PATH = (
    ...
    / "main.billy_complete"
    / "tmp"
    / "live-products-form-contract-blocker.txt"
)
...
def _require_live_credentials() -> None:
    if _credentials_configured():
        return
    reason = (
        "Live product form-contract blocked: BILLY_BROWSER_PRIMARY_REFERENCE and "
...
def _assert_dump_clean(payload: dict[str, object], *, require_code: bool = False) -> None:
    ...
    assert set(REQUIRED_PRODUCTS_FORM_CONTRACT_KEYS) <= set(payload)
    ...
    if require_code and (
        payload.get("proved_submit") == "none" or payload.get("unique_cleanup_path") == "none"
    ):
        assert payload.get("code") == "UI_CHANGED"


@pytest.mark.asyncio
async def test_ui_products_form_contract() -> None:
    """Inspect exact Gem produkt and Arkiveret on the create dialog. Do not save."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), "form-contract dump must not use API token"
    if products_form_contract_dump_is_delivered():
        written = json.loads(PRODUCTS_FORM_CONTRACT_DUMP.read_text(encoding="utf-8"))
        _assert_dump_clean(written)
        text = PRODUCTS_FORM_CONTRACT_DUMP.read_text(encoding="utf-8")
        assert "https://" not in text
        assert "MCP-TEST-PRODUCT-" not in text
        return

    _DATA_ROOT.mkdir(parents=True, exist_ok=True)
    profile = Path(tempfile.mkdtemp(prefix="billy-live-products-form-", dir=str(_DATA_ROOT)))
    ...
    try:
        ...
        inspected = await inspect_products_form_contract(page, open_create=True)
        try:
            await page.keyboard.press("Escape")
        except Exception:
            pass
        result = apply_products_form_contract_dump(inspected)
        _assert_dump_clean(result, require_code=True)
        assert PRODUCTS_FORM_CONTRACT_DUMP.is_file()
        written = json.loads(PRODUCTS_FORM_CONTRACT_DUMP.read_text(encoding="utf-8"))
        _assert_dump_clean(written)
        if result.get("proved_submit") != "none" and result.get("unique_cleanup_path") != "none":
            _record_blocker("official Gem produkt and Arkiveret present; persist stays fail-closed")
    finally:
        if page is not None:
            await page.close()
