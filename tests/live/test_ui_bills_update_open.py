"""Live dual-session qualification for read-only ui_bills_update_open.
...
Never submits Gem/Godkend/Opdater/Slet on the product update form path. Live harness
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiBillsUpdateOpenSuccess
...
_MARKER_C = "R18672-TMP-CLIENT-BILL-UPDATE-DO-NOT-USE"
_MARKER_B = "R18672-TMP-BILL-UPDATE-DO-NOT-USE"
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-bills-update-", dir=str(_DATA_ROOT)))
...
async def _capture_bills_update_frame(
    ...
    try:
        ...
        if not opened:
            ...
            for i in range(min(rn, 20)):
                ...
                try:
                    ...
                    continue
        assert opened, "vision capture could not open a bill edit form row"
        try:
        ...
        # list click may land on read detail; soft-navigate to edit for update frames.
        if re.search(r"/bills/[^/]+$", path) and not path.endswith("/edit") and "/new" not in path:
            ...
            await page.wait_for_timeout(1200)
            path = str(urlsplit(str(page.url)).path or "")
        assert re.search(r"/bills/[^/]+/edit$", path), f"expected bill edit form path, got {path}"
...
@pytest.mark.asyncio
async def test_dual_profiles_open_bill_edit_form() -> None:
    """Profile A and independent profile B both classify a bill edit form surface."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), "live bills update must not use API token"
    ...
    record_path = _VISION_RECORD_DIR / "ui_bills_update_open.json"
    ...
    try:
        ...
        assert ids.get("bill"), "could not seed disposable bill for update-open"

        detail_a = await runtime_a.ui_bills_update_open()
        if isinstance(detail_a, ToolError):
            pytest.fail(
                f"session A ui_bills_update_open failed: {detail_a.code}: {detail_a.message}"
            )
        assert isinstance(detail_a, UiBillsUpdateOpenSuccess)
        assert detail_a.path_class == "/:org_slug/bills/:id/edit"
        assert detail_a.shell_kind == "bills_update"
        assert detail_a.form_open is True
        assert detail_a.ret_regning_chrome_present is True
        assert detail_a.opdater_present is True
        assert detail_a.leverandor_or_dates_chrome_present is True
        assert detail_a.inputs_present is True

        frame_a = frame_dir / "session_a_bills_update.png"
        await _capture_bills_update_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        detail_b = await runtime_b.ui_bills_update_open()
        if isinstance(detail_b, ToolError):
            pytest.fail(
                f"session B ui_bills_update_open failed: {detail_b.code}: {detail_b.message}"
            )
        assert isinstance(detail_b, UiBillsUpdateOpenSuccess)
        assert detail_b.model_dump() == detail_a.model_dump()

        frame_b = frame_dir / "session_b_bills_update.png"
        await _capture_bills_update_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.bills.update",
            assertion_refs=[
                "tests/live/test_ui_bills_update_open.py::test_dual_profiles_open_bill_edit_form",
                "session_a_ui_bills_update_open",
                "session_b_ui_bills_update_open",
                "session_a_bills_update_frame",
                "session_b_bills_update_frame",
                "form_open_edit_path_chrome",
            ],
