"""Live dual-session qualification for read-only ui_exports_open.
...
Never clicks Eksport / Download / Eksportér som SAF-T (research121 freeze).
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiExportsOpenSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-exports-", dir=str(_DATA_ROOT)))
...
async def test_dual_profiles_open_exports_hub_shell() -> None:
    """Profile A and independent profile B both classify the Eksportér data hub shell."""
    ...
    record_path = _VISION_RECORD_DIR / "ui_exports_open.json"
    ...
    try:
        ...
        open_a = await runtime_a.ui_exports_open()
        if isinstance(open_a, ToolError):
            pytest.fail(f"session A ui_exports_open failed: {open_a.code}: {open_a.message}")
        assert isinstance(open_a, UiExportsOpenSuccess)
        ...
        frame_a = frame_dir / "session_a_exports.png"
        ...
        open_b = await runtime_b.ui_exports_open()
        if isinstance(open_b, ToolError):
            pytest.fail(f"session B ui_exports_open failed: {open_b.code}: {open_b.message}")
        assert isinstance(open_b, UiExportsOpenSuccess)
        assert open_b.model_dump() == open_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_exports.png"
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.discovery.exports",
            assertion_refs=[
                "tests/live/test_ui_exports_open.py::test_dual_profiles_open_exports_hub_shell",
                "session_a_ui_exports_open",
                "session_b_ui_exports_open",
                "session_a_exports_frame",
                "session_b_exports_frame",
                "export_hub_chrome",
            ],
