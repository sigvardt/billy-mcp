"""Live inspect: Gem som kladde validation-open on invoices/new.

Creates one tagged customer through FastMCP, opens create, clicks Gem som
kladde once, and records allowlisted keys only. Never Godkend, Send, or email.
"""
...
_BLOCKER_PATH = (
    ...
    / "main.billy_complete"
    / "tmp"
    / "live-invoices-draft-save-validation-blocker.txt"
)
...
def _require_live_credentials() -> None:
    if _credentials_configured():
        return
    reason = (
        "Live draft-save validation inspect blocked: BILLY_BROWSER_PRIMARY_REFERENCE and "
...
def _assert_dump_clean(payload: dict[str, object], *, require_code: bool = False) -> None:
    ...
    assert set(REQUIRED_DRAFT_SAVE_VALIDATION_KEYS) <= set(payload)
...
@pytest.mark.asyncio
async def test_ui_invoices_draft_save_validation(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Click Gem som kladde once on invoices/new. Do not Godkend."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), "draft-save dump must not use API token"
    if draft_save_validation_dump_is_delivered():
        written = json.loads(DRAFT_SAVE_VALIDATION_DUMP.read_text(encoding="utf-8"))
        _assert_dump_clean(written)
        text = DRAFT_SAVE_VALIDATION_DUMP.read_text(encoding="utf-8")
    ...
    try:
        ...
        try:
            ...
            pass
        result = await capture_draft_save_validation(page, tag=tag)
        leftover_invoice = _invoice_id_from(page.url)
        _assert_dump_clean(result, require_code=True)
        assert DRAFT_SAVE_VALIDATION_DUMP.is_file()
        written = json.loads(DRAFT_SAVE_VALIDATION_DUMP.read_text(encoding="utf-8"))
        _assert_dump_clean(written)
