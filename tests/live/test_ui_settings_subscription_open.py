"""Live dual-session qualification for read-only ui_settings_subscription_open.

Indstillinger Abonnement (Abonnement) panel at /:org_slug/settings via hub open +
observe-only side-nav click Abonnement (research134).
...
Never clicks Invitér* / Overdrag* / Gem* / Opret* (research134 freeze).
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiSettingsSubscriptionOpenSuccess
...
async def _capture_settings_subscription_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot Indstillinger Abonnement surface for vision (never write CTAs)."""
    ...
    try:
        ...
        # Observe-only Abonnement nav click (same contract as product tool).
        loc = live_page.get_by_text("Abonnement", exact=True)
        ...
        for i in range(min(count, 8)):
            ...
            try:
                ...
                continue
        assert clicked, "vision frame must click Abonnement side-nav"
        try:
        ...
        assert "Abonnement" in body
        assert "Upsedasse" not in body
        # Active panel must be empty (research134): not company, not beta.
        assert not ("Navn og adresse" in body and "Kontaktinformation" in body)
        assert "Tidlig adgang" not in body
        assert "Opret adgangsnøgle" not in body
        # Optional N1 (IR 186.35): product requires no nonempty h2; mirror in vision DOM.
        h2_count = await live_page.locator("h2").count()
        for index in range(h2_count):
            el = live_page.locator("h2").nth(index)
            try:
                if not await el.is_visible():
                    continue
            except Exception:
                continue
            h2_text = (await el.inner_text()).strip()
            assert h2_text == "", f"empty Abonnement panel must not have h2 text: {h2_text!r}"
...
@pytest.mark.asyncio
async def test_dual_profiles_open_settings_subscription_shell() -> None:
    """Dual profiles classify the Indstillinger Abonnement settings panel."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), (
        "live settings access token shell must not use API token"
    )
    ...
    record_path = _VISION_RECORD_DIR / "ui_settings_subscription_open.json"
    ...
    try:
        ...
        open_a = await runtime_a.ui_settings_subscription_open()
        if isinstance(open_a, ToolError):
            pytest.fail(
                f"session A ui_settings_subscription_open failed: {open_a.code}: {open_a.message}"
            )
        assert isinstance(open_a, UiSettingsSubscriptionOpenSuccess)
        assert open_a.path_class == "/:org_slug/settings"
        assert open_a.heading == "Indstillinger"
        assert open_a.shell_kind == "settings_subscription"
        assert open_a.empty_panel is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_settings_subscription.png"
        await _capture_settings_subscription_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        open_b = await runtime_b.ui_settings_subscription_open()
        if isinstance(open_b, ToolError):
            pytest.fail(
                f"session B ui_settings_subscription_open failed: {open_b.code}: {open_b.message}"
            )
        assert isinstance(open_b, UiSettingsSubscriptionOpenSuccess)
        assert open_b.model_dump() == open_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_settings_subscription.png"
        await _capture_settings_subscription_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.discovery.settings_subscription",
            assertion_refs=[
                "tests/live/test_ui_settings_subscription_open.py::test_dual_profiles_open_settings_subscription_shell",
                "session_a_ui_settings_subscription_open",
                "session_b_ui_settings_subscription_open",
                "session_a_settings_subscription_frame",
                "session_b_settings_subscription_frame",
                "settings_subscription_path_settings",
            ],
