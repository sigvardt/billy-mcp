"""Live dual-session qualification for read-only ui_clients_update_open (Ret edit form).
...
Never submits Gem/Slet on the product update path. Live harness may create a
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiClientsUpdateOpenSuccess
...
_MARKER = "R18667-TMP-CLIENT-UPDATE-DO-NOT-USE"
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-clients-update-", dir=str(_DATA_ROOT)))
...
async def _capture_clients_update_frame(
    ...
    try:
        ...
        # Activate Ret for edit form vision frame
        ret = live.get_by_role("button", name=re.compile(r"^(Ret|Edit)$", re.I))
        if await ret.count() >= 1 and await ret.first.is_visible():
            await ret.first.click()
        else:
            await live.locator("text=Ret").first.click()
        await page.wait_for_timeout(1500)
        name = live.locator("input[name='name']")
        assert await name.count() >= 1 and await name.first.is_visible()
        assert (await name.first.input_value()).strip()
...
@pytest.mark.asyncio
async def test_dual_profiles_open_clients_update_form() -> None:
    """Profile A and independent profile B both classify a client edit form after Ret."""
    ...
    record_path = _VISION_RECORD_DIR / "ui_clients_update_open.json"
    ...
    try:
        ...
        probe = await runtime_a.ui_clients_update_open()
        ...
        detail_a = await runtime_a.ui_clients_update_open()
        if isinstance(detail_a, ToolError):
            pytest.fail(
                f"session A ui_clients_update_open failed: {detail_a.code}: {detail_a.message}"
            )
        assert isinstance(detail_a, UiClientsUpdateOpenSuccess)
        assert detail_a.path_class == "/:org_slug/contacts/:id/customer"
        assert detail_a.shell_kind == "clients_update"
        assert detail_a.edit_form_open is True
        assert detail_a.name_field_visible is True
        assert detail_a.name_field_has_value is True
        assert detail_a.address_or_person_fields_present is True
        assert detail_a.country_field_present is True
        assert detail_a.shell_markers_present is True

        frame_a = frame_dir / "session_a_clients_update.png"
        await _capture_clients_update_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        detail_b = await runtime_b.ui_clients_update_open()
        if isinstance(detail_b, ToolError):
            pytest.fail(
                f"session B ui_clients_update_open failed: {detail_b.code}: {detail_b.message}"
            )
        assert isinstance(detail_b, UiClientsUpdateOpenSuccess)
        assert detail_b.model_dump() == detail_a.model_dump()

        frame_b = frame_dir / "session_b_clients_update.png"
        await _capture_clients_update_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.contacts.update",
            assertion_refs=[
                "tests/live/test_ui_clients_update_open.py::test_dual_profiles_open_clients_update_form",
                "session_a_ui_clients_update_open",
                "session_b_ui_clients_update_open",
                "session_a_clients_update_frame",
                "session_b_clients_update_frame",
                "edit_form_open_name_valued_after_ret",
            ],
