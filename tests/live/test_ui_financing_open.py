"""Live dual-session qualification for read-only ui_financing_open.
...
Never clicks apply/offer/consent/submit financing CTAs (design §14.4).
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiFinancingOpenSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-financing-", dir=str(_DATA_ROOT)))
...
async def _capture_financing_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot the financing landing surface for vision review (never apply)."""
    ...
    try:
        await page.goto(f"https://mit.billy.dk/{slug}/financing", wait_until="domcontentloaded")
        try:
            await page.wait_for_load_state("networkidle", timeout=20000)
        except Exception:
            pass
        path = urlsplit(page.url).path or ""
        assert path.endswith("/financing"), "vision frame must be the financing path class"
        assert "/financing/" not in path, "vision frame must not be a financing soft subpath"
        live_page = cast(Any, page)
        heading = live_page.locator("h1")
        assert await heading.count() >= 1
        assert (await heading.first.inner_text()).strip() == "Ansøg om erhvervslån"
...
@pytest.mark.asyncio
async def test_dual_profiles_open_financing_shell() -> None:
    """Profile A and independent profile B both classify the financing shell."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), "live financing shell must not use API token"
    ...
    record_path = _VISION_RECORD_DIR / "ui_financing_open.json"
    ...
    try:
        ...
        open_a = await runtime_a.ui_financing_open()
        if isinstance(open_a, ToolError):
            pytest.fail(f"session A ui_financing_open failed: {open_a.code}: {open_a.message}")
        assert isinstance(open_a, UiFinancingOpenSuccess)
        assert open_a.path_class == "/:org_slug/financing"
        assert open_a.heading == "Ansøg om erhvervslån"
        assert open_a.apply_cta_observed is True
        assert open_a.shell_markers_present is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_financing.png"
        await _capture_financing_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        open_b = await runtime_b.ui_financing_open()
        if isinstance(open_b, ToolError):
            pytest.fail(f"session B ui_financing_open failed: {open_b.code}: {open_b.message}")
        assert isinstance(open_b, UiFinancingOpenSuccess)
        assert open_b.model_dump() == open_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_financing.png"
        await _capture_financing_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.discovery.financing",
            assertion_refs=[
                "tests/live/test_ui_financing_open.py::test_dual_profiles_open_financing_shell",
                "session_a_ui_financing_open",
                "session_b_ui_financing_open",
                "session_a_financing_frame",
                "session_b_financing_frame",
                "landing_h1_ansog_om_erhvervslaan_financing",
            ],
