"""Live dual-session qualification for read-only ui_settings_vat_open.

Indstillinger Momssatser (VAT) panel at /:org_slug/settings via hub open +
observe-only side-nav click Momssatser (research130).
...
Never clicks Opret* / Gem* / Tilføj* / Upload (research130 freeze).
"""
...
from billy_mcp.models import AuthLoginWaitSuccess, ToolError, UiSettingsVatOpenSuccess
...
def _ephemeral_profile() -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix="billy-live-settings-vat-", dir=str(_DATA_ROOT)))
...
async def _capture_settings_vat_frame(
    runtime: BrowserRuntime,
    org_identity: Path,
    destination: Path,
) -> None:
    """Screenshot Indstillinger Momssatser surface for vision (never write CTAs)."""
    ...
    try:
        ...
        # Observe-only Momssatser nav click (same contract as product tool).
        loc = live_page.get_by_text("Momssatser", exact=True)
        ...
        for i in range(min(count, 8)):
            ...
            try:
                ...
                continue
        assert clicked, "vision frame must click Momssatser side-nav"
        try:
        ...
        assert "Regelsæt" in body
        assert "Satser for salg" in body
        assert "Satser for køb" in body
        assert "Upsedasse" not in body
        # Company default markers should not both define the active panel.
        assert not (
            "Navn og adresse" in body and "Kontaktinformation" in body and "Regelsæt" not in body
        )
...
@pytest.mark.asyncio
async def test_dual_profiles_open_settings_vat_shell() -> None:
    """Dual profiles classify the Indstillinger Momssatser settings panel."""

    _require_live_credentials()
    assert not os.environ.get("BILLY_API_TOKEN"), "live settings VAT shell must not use API token"
    ...
    record_path = _VISION_RECORD_DIR / "ui_settings_vat_open.json"
    ...
    try:
        ...
        open_a = await runtime_a.ui_settings_vat_open()
        if isinstance(open_a, ToolError):
            pytest.fail(f"session A ui_settings_vat_open failed: {open_a.code}: {open_a.message}")
        assert isinstance(open_a, UiSettingsVatOpenSuccess)
        assert open_a.path_class == "/:org_slug/settings"
        assert open_a.heading == "Indstillinger"
        assert open_a.shell_kind == "settings_vat"
        assert open_a.vat_panel_markers_present is True
        assert _org_slug_len_only(org_a) > 0

        frame_a = frame_dir / "session_a_settings_vat.png"
        await _capture_settings_vat_frame(runtime_a, org_a, frame_a)
        await runtime_a.close()
        runtime_a = None

        runtime_b = _runtime(profile_b, org_b)
        await _login_until_ready(runtime_b)
        open_b = await runtime_b.ui_settings_vat_open()
        if isinstance(open_b, ToolError):
            pytest.fail(f"session B ui_settings_vat_open failed: {open_b.code}: {open_b.message}")
        assert isinstance(open_b, UiSettingsVatOpenSuccess)
        assert open_b.model_dump() == open_a.model_dump()
        assert _org_slug_len_only(org_b) == _org_slug_len_only(org_a)

        frame_b = frame_dir / "session_b_settings_vat.png"
        await _capture_settings_vat_frame(runtime_b, org_b, frame_b)
        ...
        write_vision_record(
            record_path,
            workflow_ref="ui.discovery.settings_vat",
            assertion_refs=[
                "tests/live/test_ui_settings_vat_open.py::test_dual_profiles_open_settings_vat_shell",
                "session_a_ui_settings_vat_open",
                "session_b_ui_settings_vat_open",
                "session_a_settings_vat_frame",
                "session_b_settings_vat_frame",
                "settings_vat_path_settings",
            ],
