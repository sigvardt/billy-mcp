...
def test_api_source_arithmetic_and_documented_contracts_are_frozen() -> None:
    ...
    assert status["qualification"]["implemented_rows"] == len(offline_evidence) + 30
    assert status["qualification"]["contract_tested_rows"] == len(offline_evidence) + 30
    # API live remains 0 (out of scope); UI shell rows are live-qualified.
    assert status["qualification"]["live_tested_rows"] == 30
    assert status["qualification"]["vision_verified_rows"] == 30
