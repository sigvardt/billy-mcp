...
def test_browser_policy_real_manifest_allows_contacts_data_plane() -> None:
    ...
    assert policy.allows("https://api.billysbilling.com/v2/contacts/abc", "PUT")
    assert not policy.allows("https://api.billysbilling.com/v2/contacts", "PUT")
    assert not policy.allows("https://api.billysbilling.com/v2/contacts/abc", "PATCH")
    assert policy.allows("https://api.billysbilling.com/v2/organizations/abc", "GET")
    assert policy.allows("https://api.billysbilling.com/v2/organizations/abc", "PUT")
    assert not policy.allows("https://api.billysbilling.com/v2/organizations", "PUT")
    assert not policy.allows("https://api.billysbilling.com/v2/organizations/abc", "PATCH")
    ...
    assert policy.allows("https://api.billysbilling.com/v2/bills/abc", "PUT")
    assert not policy.allows("https://api.billysbilling.com/v2/bills", "PUT")
    assert not policy.allows("https://api.billysbilling.com/v2/bills/abc", "PATCH")
...
def test_ui_invoices_list_returns_success_for_exact_empty_url(tmp_path: Path) -> None:
    identity_path = tmp_path / "ui-org-identity.json"
    identity_path.write_text(
        json.dumps({"source": "ui_dashboard_path", "org_slug": "test-org-slug"}) + "\n",
        encoding="utf-8",
    ...
    controls = _invoices_shell_controls()
    controls["text=Ingen fakturaer"] = FakeLoginControl(text="Ingen fakturaer")
    page = FakeLoginPage(
        final_url="https://mit.billy.dk/test-org-slug/dashboard",
        controls=controls,
        follow_goto=True,
    ...
    async def goto_empty(url: str, *, wait_until: str) -> object:
        result = await original_goto(url, wait_until=wait_until)
        if url.rstrip("/").endswith("/invoices"):
            page.url = "https://mit.billy.dk/test-org-slug/invoices/empty"
        return result

    page.goto = goto_empty  # type: ignore[method-assign]
    context = FakeLoginContext(page)
...
def test_ui_invoices_list_rejects_empty_url_without_exact_empty_state(tmp_path: Path) -> None:
    identity_path = tmp_path / "ui-org-identity.json"
    identity_path.write_text(
        json.dumps({"source": "ui_dashboard_path", "org_slug": "test-org-slug"}) + "\n",
        encoding="utf-8",
    ...
    controls = _invoices_shell_controls()
    controls["text=Ingen fakturaer"] = FakeLoginControl(count=0, visible=False)
    page = FakeLoginPage(
        final_url="https://mit.billy.dk/test-org-slug/dashboard",
        controls=controls,
        follow_goto=True,
