...
INVOICE_REMINDER_ASSOCIATION_WRITE_API_TOOL_NAMES = frozenset(
    {
...
CITY_WRITE_API_TOOL_NAMES = frozenset(
    {
...
COUNTRY_WRITE_API_TOOL_NAMES = frozenset(
    {
...
CURRENCY_WRITE_API_TOOL_NAMES = frozenset(
    {
...
LOCALE_WRITE_API_TOOL_NAMES = frozenset(
    {
...
ZIPCODE_WRITE_API_TOOL_NAMES = frozenset(
    {
...
TRANSACTION_DELETE_API_TOOL_NAMES = frozenset(
    {
...
class FakeUiInvoicesUpdateOpenService:
    def __init__(self) -> None:
        self.calls = 0

    async def ui_invoices_update_open(self) -> UiInvoicesUpdateOpenSuccess:
        self.calls += 1
...
def test_server_registers_coverage_reads_ticketed_writes_and_auth_status(tmp_path: Path) -> None:
    ...
    assert len(TRANSACTION_DELETE_API_TOOL_NAMES) == 2
    assert len(api_tool_names) == 317
    ...
    assert tool_names == (
        ...
        | TRANSACTION_DELETE_API_TOOL_NAMES
        | auth_tool_names
        | ui_tool_names
    )
...
def test_ui_invoices_update_open_registration_has_empty_input_and_typed_output(
    tmp_path: Path,
) -> None:
    write_coverage_fixture(tmp_path)
    invoices_update = FakeUiInvoicesUpdateOpenService()
    server = create_server(tmp_path, ui_invoices_update_open_service=invoices_update)
    tool = {item.name: item for item in asyncio.run(server.list_tools())}["ui_invoices_update_open"]
    ...
    result = asyncio.run(server.call_tool("ui_invoices_update_open", {}))
    ...
    assert invoices_update.calls == 1
