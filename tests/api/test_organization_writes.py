"""Contract tests for ticketed singular Billy organization write tools."""
...
from billy_mcp.api.organization_writes import (
    OrganizationCreatePreviewInput,
    OrganizationUpdatePreviewInput,
    register_organization_write_tools,
)
...
def make_server(
    handler: MockHandler,
    *,
    token: str | None = "organization-write-test-token",
    ...
    server = FastMCP("organization-write-contract-test")
    register_organization_write_tools(
        server,
        client,
...
def test_registers_exactly_four_flat_typed_organization_write_tools() -> None:
    ...
    expected_properties = {
        "api_organizations_create_preview": {"organization"},
        "api_organizations_create_execute": {"confirmation_ticket"},
        "api_organizations_update_preview": {"id", "organization"},
        "api_organizations_update_execute": {"confirmation_ticket"},
    }
...
@pytest.mark.parametrize(
    ("input_model", "payload"),
    [
        (OrganizationCreatePreviewInput, {"organization": {}, "extra": True}),
        (
            OrganizationUpdatePreviewInput,
            {"id": "organization-1", "organization": {}, "extra": True},
        ),
        (OrganizationUpdatePreviewInput, {"id": "", "organization": {}}),
        (OrganizationUpdatePreviewInput, {"id": "organization-1", "organization": {"id": "other"}}),
        (WriteExecuteInput, {"confirmation_ticket": "ticket", "organization": {}}),
        (WriteExecuteInput, {"confirmation_ticket": ""}),
    ],
)
def test_outer_inputs_forbid_extras_empty_values_and_mismatched_body_ids(
    input_model: type[OrganizationCreatePreviewInput]
    | type[OrganizationUpdatePreviewInput]
...
def test_opaque_organization_preserves_unknown_fields_and_matching_inner_id() -> None:
    input = OrganizationUpdatePreviewInput.model_validate(
        {
            "id": "organization-1",
            "organization": {
                "id": "organization-1",
    ...
    assert input.organization["futureField"] == {"rank": 2}
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {"organization": {"unknown": {"rank": 2}}},
            {"organization": {"unknown": {"rank": 2}}},
            {"action": "create", "resource": "organization"},
        ),
        (
            "api_organizations_update_preview",
            {"id": "organization-1", "organization": {"futureField": {"rank": 2}}},
            {"organization": {"futureField": {"rank": 2}}},
            {"action": "update", "resource": "organization", "id": "organization-1"},
        ),
    ],
)
def test_previews_are_mutation_free_and_bind_exact_organization_requests(
    tool_name: str,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"id": "organization /?", "organization": {"futureField": {"rank": 2}}},
            "PUT",
            "/v2/organizations/organization%20%2F%3F",
            {"organization": {"futureField": {"rank": 2}}},
        ),
    ],
)
def test_execute_sends_exact_organization_write_once_and_maps_only_declared_root(
    preview_name: str,
    execute_name: str,
    arguments: dict[str, object],
    method: str,
    ...
    assert execution == {
        "changed_records": {"organizations": [{"id": "organization-1"}]},
        "deleted_records": None,
    }
    assert "users" not in cast(dict[str, object], execution["changed_records"])
...
def test_execute_rejects_missing_or_malformed_required_organizations_root(
...
def test_tamper_wrong_executor_expiry_and_replay_do_not_make_unexpected_writes() -> None:
    clock = Clock()
    server, requests = make_server(
        lambda request: httpx.Response(200, json={"organizations": [{"id": "organization-1"}]}),
    ...
    assert first["changed_records"] == {"organizations": [{"id": "organization-1"}]}
...
def test_failed_organization_write_is_not_retried() -> None:
    server, requests = make_server(
