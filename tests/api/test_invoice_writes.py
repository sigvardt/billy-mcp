"""Contract tests for ticketed singular Billy parent-invoice write tools."""
...
def make_server(
    handler: MockHandler,
    *,
    token: str | None = "invoice-write-test-token",
    ...
    server = FastMCP("invoice-write-contract-test")
    register_invoice_write_tools(
        server,
        client,
...
def test_registers_exactly_six_flat_typed_invoice_write_tools() -> None:
    ...
    expected_properties = {
        "api_invoices_create_preview": {"invoice"},
        "api_invoices_create_execute": {"confirmation_ticket"},
        "api_invoices_update_preview": {"id", "invoice"},
        "api_invoices_update_execute": {"confirmation_ticket"},
        "api_invoices_delete_preview": {"id"},
        "api_invoices_delete_execute": {"confirmation_ticket"},
    }
    ...
    for name, field in (
        ("api_invoices_update_preview", "id"),
        ("api_invoices_delete_preview", "id"),
        ("api_invoices_create_execute", "confirmation_ticket"),
        ("api_invoices_update_execute", "confirmation_ticket"),
        ("api_invoices_delete_execute", "confirmation_ticket"),
...
@pytest.mark.parametrize(
    ("input_model", "payload"),
    [
        (InvoiceCreatePreviewInput, {"invoice": {}, "extra": True}),
        (InvoiceUpdatePreviewInput, {"id": "invoice-1", "invoice": {}, "extra": True}),
        (InvoiceDeletePreviewInput, {"id": "invoice-1", "extra": True}),
        (InvoiceUpdatePreviewInput, {"id": "", "invoice": {}}),
        (InvoiceDeletePreviewInput, {"id": ""}),
        (InvoiceUpdatePreviewInput, {"id": "invoice-1", "invoice": {"id": "other"}}),
        (WriteExecuteInput, {"confirmation_ticket": "ticket", "invoice": {}}),
    ],
)
...
def test_update_allows_an_opaque_invoice_body_id_when_it_matches_the_route() -> None:
    input = InvoiceUpdatePreviewInput.model_validate(
        {"id": "invoice-1", "invoice": {"id": "invoice-1", "unknown": {"rank": 2}}}
    )

    assert input.invoice["unknown"] == {"rank": 2}
...
@pytest.mark.parametrize(
    ...
    [
        (
            "api_invoices_create_preview",
            {"invoice": {"unresolvedField": {"rank": 2}}},
            {"invoice": {"unresolvedField": {"rank": 2}}},
        ),
        (
            "api_invoices_update_preview",
            {"id": "invoice-1", "invoice": {"customField": {"rank": 2}}},
            {"invoice": {"customField": {"rank": 2}}},
        ),
        (
            "api_invoices_delete_preview",
            {"id": "invoice-1"},
...
def test_previews_are_mutation_free_and_bind_the_exact_canonical_request(
    ...
    assert preview["expected_effect_state"] == {
        "action": tool_name.removeprefix("api_invoices_").removesuffix("_preview"),
        "resource": "invoice",
...
@pytest.mark.parametrize(
    ...
    [
        (
            "api_invoices_create_preview",
            "api_invoices_create_execute",
            {"invoice": {"entryDate": "2026-07-29", "lines": [{"description": "Sales"}]}},
        ...
        (
            "api_invoices_update_preview",
            "api_invoices_update_execute",
            {"id": "invoice /?", "invoice": {"description": "Corrected"}},
            {"invoices": [{"id": "invoice /?"}]},
            "PUT",
            "/v2/invoices/invoice%20%2F%3F",
            {"invoice": {"description": "Corrected"}},
        ),
        (
            "api_invoices_delete_preview",
            "api_invoices_delete_execute",
            {"id": "invoice /?"},
            {"meta": {"deletedRecords": {"invoices": ["invoice /?"]}}},
            "DELETE",
            "/v2/invoices/invoice%20%2F%3F",
            None,
        ),
...
def test_execute_sends_exact_singular_invoice_cud_shapes_once(
    preview_name: str,
    execute_name: str,
    arguments: dict[str, object],
    ...
    if body is None:
        ...
        assert execution == {
            "changed_records": {},
            "deleted_records": {"invoices": ["invoice /?"]},
        }
    else:
...
def test_tampered_expired_and_replayed_tickets_fail_without_extra_writes() -> None:
    clock = Clock()
    server, requests = make_server(
        lambda request: httpx.Response(200, json={"invoices": [{"id": "invoice-1"}]}),
    ...
    assert first["changed_records"] == {"invoices": [{"id": "invoice-1"}]}
    assert replayed["code"] == StableErrorCode.CONFIRMATION_CONSUMED
    assert len(requests) == 1


def test_invoice_ticket_rejects_a_different_executor_in_the_same_module_before_http() -> None:
    server, requests = make_server(
        lambda request: httpx.Response(200, json={"invoices": [{"id": "invoice-1"}]}),
    )
    ...
    assert correct_executor["changed_records"] == {"invoices": [{"id": "invoice-1"}]}
    assert len(requests) == 1
...
@pytest.mark.parametrize(
    "changes",
    [
        {"execute_tool_name": "api_invoices_other_execute"},
        {"organization_id": "other-organisation"},
        {"resource_id": "other-invoice"},
        {"payload": {"description": "Changed"}},
        {"expected_effect_state": {"action": "other"}},
    ],
)
def test_invoice_ticket_binding_rejects_every_mutated_operation_field(
    ...
    specification = WriteOperationSpec(
        ...
        expected_effect_state={"action": "update", "resource": "invoice", "id": "invoice-1"},
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "api_invoices_update_preview",
            {"id": "missing-invoice", "invoice": {"description": "Sales"}},
            "api_invoices_update_execute",
...
def test_absent_optional_deleted_records_are_not_fabricated() -> None:
    ...
    preview = call_tool(server, "api_invoices_delete_preview", {"id": "invoice-1"})
    execution = call_tool(
        server,
        "api_invoices_delete_execute",
        {"confirmation_ticket": preview["confirmation_ticket"]},
