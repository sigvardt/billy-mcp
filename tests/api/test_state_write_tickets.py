"""Ticket tamper, expiry, replay, and auth tests for state writes."""
...
from tests.api.test_state_writes import PARTIAL_STATE, call_tool, make_server
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {"state": PARTIAL_STATE},
            {"states": [{"id": "state-1", **PARTIAL_STATE}]},
            "POST",
            "/v2/states",
            {"state": PARTIAL_STATE},
        ),
        (
            ...
            {"id": "dk /?", "state": PARTIAL_STATE},
            {"states": [{"id": "dk /?", **PARTIAL_STATE}]},
            "PUT",
            "/v2/states/dk%20%2F%3F",
            {"state": PARTIAL_STATE},
        ),
    ],
)
def test_execute_sends_exact_singular_state_write_shapes_once(
    preview_name: str,
    execute_name: str,
...
def test_execute_maps_deleted_records_without_inventing_singular_write_root() -> None:
    ...
    assert execution["changed_records"] == {"states": [{"id": "state-1", **PARTIAL_STATE}]}
    assert execution["deleted_records"] == {"states": []}
    assert "state" not in execution
...
def test_tamper_wrong_executor_expiry_and_replay_do_not_make_unexpected_writes() -> None:
    ...
    server, requests = make_server(
        lambda request: httpx.Response(200, json={"states": [{"id": "state-1"}]}),
        confirmations=ConfirmationStore(clock),
    )
    preview = call_tool(server, "api_states_create_preview", {"state": PARTIAL_STATE})
    ticket = cast(str, preview["confirmation_ticket"])

    tampered = call_tool(server, "api_states_create_execute", {"confirmation_ticket": f"{ticket}x"})
    wrong_executor = call_tool(server, "api_states_update_execute", {"confirmation_ticket": ticket})
    ...
    first = call_tool(server, "api_states_create_execute", {"confirmation_ticket": ticket})
    replay = call_tool(server, "api_states_create_execute", {"confirmation_ticket": ticket})
    assert first["changed_records"] == {"states": [{"id": "state-1"}]}
    assert replay["code"] == StableErrorCode.CONFIRMATION_CONSUMED
    assert len(requests) == 1

    expired_preview = call_tool(server, "api_states_create_preview", {"state": PARTIAL_STATE})
...
def test_ticket_binding_rejects_a_changed_state_request() -> None:
    ...
    specification = WriteOperationSpec(
        ...
        singular_root="state",
        plural_root="states",
        payload={"name": "California"},
        resource_id="state-1",
        organization_id=None,
        summary="Update one Billy state.",
        expected_effect_state={"action": "update", "resource": "state", "id": "state-1"},
    )
...
def test_failed_state_write_is_not_retried() -> None:
    server, requests = make_server(
