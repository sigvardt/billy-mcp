"""Ticket tamper, expiry, replay, and auth tests for association writes."""
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {"invoiceReminderAssociations": [{"id": "assoc-1", **VALID_ASSOCIATION}]},
            "POST",
        ...
        (
            ...
            {"id": "assoc /?", "invoiceReminderAssociation": VALID_ASSOCIATION},
            {"invoiceReminderAssociations": [{"id": "assoc /?", **VALID_ASSOCIATION}]},
            "PUT",
...
def test_execute_sends_exact_singular_association_write_shapes_once(
    preview_name: str,
    execute_name: str,
...
def test_execute_maps_deleted_records_without_inventing_singular_write_root() -> None:
    ...
    response: dict[str, object] = {
        "invoiceReminderAssociations": [{"id": "assoc-1", **VALID_ASSOCIATION}],
        "meta": {"deletedRecords": {"invoiceReminderAssociations": deleted_ids}},
    }
    ...
    assert execution["deleted_records"] == {"invoiceReminderAssociations": []}
    assert "invoiceReminderAssociation" not in execution
...
def test_tamper_wrong_executor_expiry_and_replay_do_not_make_unexpected_writes() -> None:
    ...
    server, requests = make_server(
        lambda request: httpx.Response(
            200, json={"invoiceReminderAssociations": [{"id": "assoc-1"}]}
        ),
    ...
    assert first["changed_records"] == {"invoiceReminderAssociations": [{"id": "assoc-1"}]}
...
def test_failed_association_write_is_not_retried() -> None:
    server, requests = make_server(
