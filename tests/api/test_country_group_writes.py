"""Contract tests for ticketed singular Billy country-group write tools."""
...
from billy_mcp.api.country_group_writes import (
    CountryGroupCreatePreviewInput,
    CountryGroupUpdatePreviewInput,
    register_country_group_write_tools,
)
...
DOCUMENTED_FIELDS = {"name", "icon", "memberCountryIds"}
PARTIAL_COUNTRY_GROUP = {"name": "Nordics"}


def make_server(
    handler: MockHandler,
    *,
    token: str | None = "country-group-write-test-token",
    ...
    server = FastMCP("country-group-write-contract-test")
    register_country_group_write_tools(
        server,
        client,
...
def test_registers_exactly_four_flat_typed_country_group_write_tools() -> None:
    ...
    expected_properties = {
        ...
        "api_country_groups_create_execute": {"confirmation_ticket"},
        "api_country_groups_update_preview": {"id", "countryGroup"},
        "api_country_groups_update_execute": {"confirmation_ticket"},
    }
    ...
    for tool_name in (
        ...
        nested_schema = cast(
            dict[str, object], by_name[tool_name].parameters["properties"]["countryGroup"]
        )
...
@pytest.mark.parametrize(
    ("input_model", "payload"),
    [
        (CountryGroupCreatePreviewInput, {"countryGroup": {}, "extra": True}),
        (
            CountryGroupUpdatePreviewInput,
            {"id": "group-1", "countryGroup": {}, "extra": True},
        ),
        (CountryGroupUpdatePreviewInput, {"id": "", "countryGroup": {}}),
        (WriteExecuteInput, {"confirmation_ticket": "ticket", "countryGroup": {}}),
        (CountryGroupCreatePreviewInput, {"countryGroup": {"memberCountries": "DK"}}),
        (
            CountryGroupCreatePreviewInput,
            {"countryGroup": {"memberCountryIds": ["DK"]}},
        ),
        (CountryGroupCreatePreviewInput, {"countryGroup": {"name": {"id": "Nordics"}}}),
        (
            CountryGroupUpdatePreviewInput,
            {"id": "group-1", "countryGroup": {"name": "X", "undocumented": True}},
        ),
    ],
)
def test_outer_inputs_forbid_extra_fields_aliases_arrays_and_empty_ids(
    input_model: type[CountryGroupCreatePreviewInput]
    | type[CountryGroupUpdatePreviewInput]
...
@pytest.mark.parametrize(
    ...
    [
        (
            "api_country_groups_create_preview",
            {"countryGroup": {"name": "X", "memberCountries": "DK"}},
        ),
        (
            "api_country_groups_create_preview",
            {"countryGroup": {"memberCountryIds": ["DK"]}},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "api_country_groups_update_preview",
            {"id": "group-1", "countryGroup": PARTIAL_COUNTRY_GROUP},
            {"countryGroup": PARTIAL_COUNTRY_GROUP},
        ),
        ("api_country_groups_create_preview", {"countryGroup": {}}, {"countryGroup": {}}),
    ],
)
def test_previews_are_mutation_free_and_bind_the_exact_canonical_request(
    ...
    assert preview["expected_effect_state"] == {
        "action": tool_name.removeprefix("api_country_groups_").removesuffix("_preview"),
