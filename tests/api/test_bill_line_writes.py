"""Contract tests for ticketed singular Billy bill-line write tools."""
...
from billy_mcp.api.bill_line_writes import (
    BillLineCreatePreviewInput,
    BillLineDeletePreviewInput,
    BillLineUpdatePreviewInput,
    register_bill_line_write_tools,
)
...
def bill_line_payload() -> dict[str, object]:
    """Return a representative documented bill-line payload without guessing more fields."""
...
def make_server(
    handler: MockHandler,
    *,
    token: str | None = "bill-line-write-test-token",
    ...
    server = FastMCP("bill-line-write-contract-test")
    register_bill_line_write_tools(
        server,
        client,
...
def test_registers_exactly_six_flat_typed_bill_line_write_tools() -> None:
    ...
    expected_properties = {
        "api_bill_lines_create_preview": {"billLine"},
        "api_bill_lines_create_execute": {"confirmation_ticket"},
        "api_bill_lines_update_preview": {"billLine", "id"},
        "api_bill_lines_update_execute": {"confirmation_ticket"},
        "api_bill_lines_delete_preview": {"id"},
        "api_bill_lines_delete_execute": {"confirmation_ticket"},
    }
    ...
    for name, field in (
        ("api_bill_lines_update_preview", "id"),
        ("api_bill_lines_delete_preview", "id"),
        ("api_bill_lines_create_execute", "confirmation_ticket"),
        ("api_bill_lines_update_execute", "confirmation_ticket"),
        ("api_bill_lines_delete_execute", "confirmation_ticket"),
...
@pytest.mark.parametrize(
    ("input_model", "payload"),
    [
        (BillLineCreatePreviewInput, {"billLine": bill_line_payload(), "extra": True}),
        (
            BillLineUpdatePreviewInput,
            {"id": "line-1", "billLine": bill_line_payload(), "extra": True},
        ),
        (BillLineDeletePreviewInput, {"id": "line-1", "extra": True}),
        (BillLineUpdatePreviewInput, {"id": "", "billLine": bill_line_payload()}),
        (BillLineDeletePreviewInput, {"id": ""}),
        (
            BillLineUpdatePreviewInput,
            {"id": "line-1", "billLine": {**bill_line_payload(), "id": "other-line"}},
        ),
        (WriteExecuteInput, {"confirmation_ticket": "ticket", "billLine": bill_line_payload()}),
    ],
)
def test_outer_inputs_forbid_extra_fields_and_empty_or_mismatched_ids(
    input_model: type[BillLineCreatePreviewInput]
    | type[BillLineUpdatePreviewInput]
    | type[BillLineDeletePreviewInput]
...
def test_inner_bill_line_payload_remains_opaque() -> None:
    payload = {**bill_line_payload(), "description": {"opaque": {"rank": 2}}}

    input = BillLineCreatePreviewInput.model_validate({"billLine": payload})

    assert input.billLine == payload
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {
                "id": "line-1",
                "billLine": {"id": "line-1", "description": "Adjusted", "amount": 125},
            },
            {"billLine": {"id": "line-1", "description": "Adjusted", "amount": 125}},
        ),
        ("api_bill_lines_delete_preview", {"id": "line-1"}, {"id": "line-1"}),
    ],
)
def test_previews_are_mutation_free_and_bind_the_exact_canonical_request(
    ...
    assert preview["expected_effect_state"] == {
        "action": tool_name.removeprefix("api_bill_lines_").removesuffix("_preview"),
        "resource": "billLine",
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {"billLine": bill_line_payload()},
            {
                "billLines": [{"id": "line-1", **bill_line_payload()}],
                "bills": [{"id": "bill-1", "state": "draft"}],
            },
            "POST",
            "/v2/billLines",
            {"billLine": bill_line_payload()},
        ...
        (
            "api_bill_lines_update_preview",
            "api_bill_lines_update_execute",
            {"id": "line /?", "billLine": {"description": "Adjusted", "amount": 125}},
            {"billLines": [{"id": "line /?", "description": "Adjusted", "amount": 125}]},
            "PUT",
            "/v2/billLines/line%20%2F%3F",
            {"billLine": {"description": "Adjusted", "amount": 125}},
            {"billLines": [{"id": "line /?", "description": "Adjusted", "amount": 125}]},
            None,
        ),
        (
            ...
            "api_bill_lines_delete_execute",
            {"id": "line /?"},
            {"meta": {"deletedRecords": {"billLines": ["line /?"]}}},
            "DELETE",
            "/v2/billLines/line%20%2F%3F",
            None,
            {},
...
def test_execute_sends_exact_singular_bill_line_cud_shapes_once(
    preview_name: str,
    execute_name: str,
    arguments: dict[str, object],
...
def test_primary_only_response_does_not_fabricate_optional_parent_bill() -> None:
    server, _ = make_server(
        lambda request: httpx.Response(
            200, json={"billLines": [{"id": "line-1", **bill_line_payload()}]}
        )
    )
    ...
    assert execution == {
        "changed_records": {"billLines": [{"id": "line-1", **bill_line_payload()}]},
...
def test_tampered_expired_and_replayed_tickets_fail_without_extra_writes() -> None:
    ...
    assert first["changed_records"] == {"billLines": [{"id": "line-1", **bill_line_payload()}]}
...
def test_ticket_rejects_a_different_executor_in_the_same_module_before_http() -> None:
    ...
    assert correct_executor["changed_records"] == {
        "billLines": [{"id": "line-1", **bill_line_payload()}]
    }
    assert len(requests) == 1
...
@pytest.mark.parametrize(
    "changes",
    [
        {"execute_tool_name": "api_bill_lines_other_execute"},
        {"organization_id": "other-organisation"},
        {"resource_id": "other-line"},
        {"payload": {"description": "Changed", "amount": 200}},
...
def test_ticket_binding_rejects_every_mutated_operation_field(changes: dict[str, object]) -> None:
    specification = WriteOperationSpec(
        ...
        payload={"description": "Adjusted", "amount": 100},
        resource_id="line-1",
        organization_id=None,
        summary="Update one Billy bill line.",
        expected_effect_state={"action": "update", "resource": "billLine", "id": "line-1"},
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "api_bill_lines_update_preview",
            {"id": "missing-line", "billLine": {"description": "Adjusted", "amount": 100}},
            "api_bill_lines_update_execute",
...
def test_absent_optional_deleted_records_are_not_fabricated() -> None:
    ...
    preview = call_tool(server, "api_bill_lines_delete_preview", {"id": "line-1"})
    execution = call_tool(
        server,
        "api_bill_lines_delete_execute",
        {"confirmation_ticket": preview["confirmation_ticket"]},
