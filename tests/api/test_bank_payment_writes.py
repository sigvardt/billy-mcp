"""Contract tests for ticketed singular Billy bank-payment write tools."""
...
def make_server(
    handler: MockHandler,
    *,
    token: str | None = "bank-payment-write-test-token",
    ...
    server = FastMCP("bank-payment-write-contract-test")
    register_bank_payment_write_tools(
        server,
        client,
...
def test_registers_exactly_six_flat_typed_bank_payment_write_tools() -> None:
    ...
    expected_properties = {
        "api_bank_payments_create_preview": {"bankPayment"},
...
@pytest.mark.parametrize(
    ("input_model", "payload"),
    [
        (BankPaymentCreatePreviewInput, {"bankPayment": {}, "extra": True}),
...
def test_opaque_bank_payment_preserves_unknown_fields_and_matching_inner_id() -> None:
    input = BankPaymentUpdatePreviewInput.model_validate(
        {
            "id": "payment-1",
            "bankPayment": {
                "id": "payment-1",
    ...
    assert input.bankPayment["futureField"] == {"rank": 2}
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {"bankPayment": {"unknown": {"rank": 2}}},
            {"action": "create", "resource": "bankPayment"},
        ),
        (
            "api_bank_payments_update_preview",
            {"id": "payment-1", "bankPayment": {"futureField": {"rank": 2}}},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"id": "payment /?", "bankPayment": {"futureField": {"rank": 2}}},
            "PUT",
...
def test_execute_sends_exact_bank_payment_write_once_and_maps_only_declared_root(
    preview_name: str,
    execute_name: str,
    arguments: dict[str, object],
    method: str,
...
def test_execute_rejects_missing_or_malformed_required_bank_payments_root(
    response: dict[str, object],
) -> None:
...
def test_tamper_wrong_executor_expiry_and_replay_do_not_make_unexpected_writes() -> None:
    clock = Clock()
    server, requests = make_server(
        lambda request: httpx.Response(200, json={"bankPayments": [{"id": "payment-1"}]}),
    ...
    assert first["changed_records"] == {"bankPayments": [{"id": "payment-1"}]}
...
def test_failed_bank_payment_write_is_not_retried() -> None:
    server, requests = make_server(
