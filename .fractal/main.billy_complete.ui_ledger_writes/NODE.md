...
Use only Grok. Do not spawn further children unless a create vs update/delete split is required.

Own only these paths (plus this node seed):
- src/billy_mcp/ui_writes/ledger.py
- tests/unit/test_ui_ledger_writes.py
- tests/live/test_ui_ledger_writes.py
- wiki/ui_ledger_writes.md

Do not edit generate_coverage_report.py, scripts/check_coverage.py, src/billy_mcp/server.py, or src/billy_mcp/browser.py. Root already wired register_ui_ledger_write_tools(server, ui_write_protocol). Fill that function.
...
Qualify through FastMCP call_tool, never BrowserRuntime as the pass proof.

Offline tests must cover preview no-submit, execute once, replay, expiry, and wrong-tool mismatch.

Live submit only after root radio go-ahead (contacts family is first). Unique tagged names. Independent second UI session read-back. Four-state capture. Purge raw frames. Store only a non-sensitive vision record.

Do not green coverage rows. Do not invent ui_annual_* tools. Do not run live API tests. Do not send invoices/emails, make payments, submit VAT/filings, or change users/access/tokens/subscription.

Implement daybook create/delete first if reversible. Transactions create only if cleanup is proven.
Do not post irreversible ledger entries. If a submit cannot be deleted, fail closed and report the blocker.

Tools to register: ui_daybooks_create_preview/execute, ui_daybooks_delete_preview/execute, ui_daybook_transactions_create_preview/execute, ui_transactions_create_preview/execute
Coverage rows: ui.parity.daybooks.create, ui.parity.daybooks.delete, ui.parity.daybookTransactions.create, ui.parity.transactions.create
Read wiki/ui_write_ticket_protocol.md and the parent plan 190.1.


## Completion Requirements

1. register_ui_ledger_write_tools registers the family preview and execute tools from src/billy_mcp/ui_writes/ledger.py.
2. Offline unit tests in tests/unit/test_ui_ledger_writes.py pass (preview, execute-once, replay, expiry, wrong tool).
3. Live MCP test in tests/live/test_ui_ledger_writes.py passes, or the node recorded a concrete live blocker that is not an unsafe skipped submit.
4. wiki/ui_ledger_writes.md describes the tools, fail-closed rules, and cleanup. No secrets.
5. bash $NODE_DIR/scripts/test.sh passes for the owned offline tests.
6. Branch is committed. Do not claim parent complete:true.
