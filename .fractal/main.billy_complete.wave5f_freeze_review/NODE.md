...
Independently audit the Wave-5f offline contract at the current branch tip.
You are the mandatory Grok freeze gate for six Billy API v2 singular tax-write
operations. Your sole project deliverable is
`wiki/wave_fivef_freeze_independent_review.md`; generated `wiki/_index.md`
changes are permitted. Do not modify production code, tests, coverage manifests,
coverage reports, the frozen contract, configuration, or any other project file.

The root baseline must be the committed `d412e8c` tip containing
`wiki/wave_fivef_ticketed_writes_contract.md`. First confirm the checked-out
revision and re-fetch the current official Billy API documentation. Cite the
official docs URL and report observed fingerprint evidence, or explicitly state
any source drift. The cited freeze contract and the design at
`docs/superpowers/specs/2026-07-28-billy-mcp-complete-design.md` are the local
contract surfaces to audit.

Audit each exact operation: POST, PUT, and bodyless DELETE for `/taxRates` with
`taxRate`/`taxRates`, and POST, PUT, and bodyless DELETE for
`/taxRateDeductionComponents` with
`taxRateDeductionComponent`/`taxRateDeductionComponents`. Check stable preview
and execute names, exact server-owned ticket/executor binding, locked API base,
opaque strict outer typing, partial PUT/id matching, no write retry, and
fail-closed response mapping. Specifically decide whether parent writes may
optionally map `taxRateDeductionComponents`, and whether any evidence permits a
child write to infer `taxRates`; the latter must remain absent without evidence.

Verify the documentation-bound field and risk limits: immutable/read-only tax
rate fields, `isActive` as an offline mutable candidate, immutable deduction
component fields, undocumented `source` enum values, and `priority` as a
candidate. Confirm unauthenticated 401 POST/PUT plus empty 200 DELETE are not
live or cleanup proof. Confirm all tax bulk rows, UI, vision, live,
files/specials, webhooks, and unrelated resources remain excluded and red.

Write a durable non-sensitive review record with a clear **ACCEPT** or
**REJECT** verdict for the contract only. An ACCEPT must remain offline-only and
must not authorise implementation or green any coverage row. A REJECT must name
the exact incompatible contract statement and the corrective evidence. Never
use a Codex fallback as a substitute for this Grok review. Do not inspect or use
credentials, create Billy records, start a browser UI flow, preserve browser
evidence, or claim live/UI qualification.

Run the relevant non-live verification and `wiki lint --path=wiki` before your
commit. If the contract is accurately frozen, run `fractal node finish` in the
same iteration after committing the review page; the parent will merge and act
on the documented verdict.

## Completion Requirements

1. `wiki/wave_fivef_freeze_independent_review.md` exists, is indexed and lint
   clean, and records a cited Grok **ACCEPT** or **REJECT** for the exact
   committed Wave-5f contract baseline.
2. The review covers all six routes, names, roots, optional parent response
   mapping, child non-inference, ticket/executor binding, API base lock,
   documentation fingerprint or drift, and the documented field boundaries.
3. The review explicitly preserves fail-closed status for bulk, live, UI,
   vision, specials, and overall completeness. It contains no credentials,
   customer data, screenshots, frames, HARs, traces, or raw probe payloads.
4. No project code, test, coverage, frozen-contract, or status artifact changes
   are committed by this node.
5. `bash $NODE_DIR/scripts/test.sh` and `bash $NODE_DIR/scripts/lint.sh` pass
   from the child worktree, and the child branch is clean after its review
   commit.
