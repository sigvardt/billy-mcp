...
- tests/unit/test_ui_files_writes.py
- tests/live/test_ui_files_writes.py
- wiki/ui_files_writes.md

Do not edit generate_coverage_report.py, scripts/check_coverage.py, src/billy_mcp/server.py, or src/billy_mcp/browser.py. Root already wired register_ui_file_write_tools(server, ui_write_protocol). Fill that function.
...
Bind the exact resolved local path and file digest on preview.
Ticket must reject changed path or digest. Use a disposable local fixture file outside git. Delete the uploaded record after.

Tools to register: ui_files_create_preview/execute
Coverage rows: ui.parity.files.create
Read wiki/ui_write_ticket_protocol.md and the parent plan 190.1.


## Completion Requirements

1. register_ui_file_write_tools registers the family preview and execute tools from src/billy_mcp/ui_writes/files.py.
2. Offline unit tests in tests/unit/test_ui_files_writes.py pass (preview, execute-once, replay, expiry, wrong tool).
3. Live MCP test in tests/live/test_ui_files_writes.py passes, or the node recorded a concrete live blocker that is not an unsafe skipped submit.
4. wiki/ui_files_writes.md describes the tools, fail-closed rules, and cleanup. No secrets.
