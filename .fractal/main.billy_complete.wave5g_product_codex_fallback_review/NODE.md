...
Perform the routing-policy fallback audit for the merged Wave-5g sales-tax
ticketed-write cohort at root merge `55faa02` (product candidate `e6f96e7`).
The designated Grok reviewer could not authenticate before making any edits.
This is a full Codex Power retry of the static audit, but it is supplemental:
it cannot replace the mandatory independent Grok acceptance gate.

You own only `wiki/wave_fiveg_product_codex_fallback_review.md`; source code,
coverage files, configuration, and tests are read-only. Use the accepted cited
contract `wiki/wave_fiveg_ticketed_writes_contract.md`, the existing official
documentation evidence in `wiki/wave_fiveg_product_ready_research_independent_review.md`,
and current official Billy documentation only if needed. Inspect committed
bytes from a clean `git archive`, run non-live read-only checks, and keep raw
work in ignored scratch. Do no Billy API write, browser/UI action, credential
access, or live qualification.

Independently verify exactly twelve typed FastMCP preview/execute tools for
sales-tax ruleset/rule singular create, update, and delete; POST/PUT/DELETE
paths and singular roots; partial update id equality; bodyless delete; no write
retry; strict outer and opaque inner payloads; typed errors; the sole root
confirmation store and executor mismatch rejection before ticket consumption or
HTTP; optional ruleset `salesTaxRules` mapping without fabrication; child rule
responses without fabricated parent roots; and coverage changes confined to the
six frozen rows with 202 `api_*`, 148 offline rows, zero live/vision rows, 92
ambiguous bulk rows, unchanged UI/special rows, and `complete: false`.

Record an evidence-backed PASS or FAIL for this supplemental audit, never an
authoritative ACCEPT. Explicitly state that the Grok authentication failure
occurred before edits and that mandatory Grok review remains pending. Report
precise defects if any, make no code changes, commit the review page, and post
a concise handoff to the parent.

## Completion Requirements

1. `wiki/wave_fiveg_product_codex_fallback_review.md` is committed and includes
   scope, commits reviewed, cited contract/source evidence, concrete static and
   test checks, PASS or FAIL, and explicit non-authoritative status.
2. The fallback independently reviews a clean archive and runs focused
   sales-tax tests plus the generated-coverage checker without editing product
   source or coverage state.
3. The page states that no live/UI/vision/bulk/completeness claim is made and
   that the required Grok verdict is still pending because authentication
   failed before the Grok node could edit.
4. No secret or raw browser/research artifact is committed; the child reports
   its result to the parent and calls `fractal node finish` after the review
   page is committed.
