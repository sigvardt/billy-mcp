...
Perform an independent Grok product audit of the merged Wave-5g sales-tax
ticketed-write cohort at root merge commit `55faa02` (candidate product commit
`e6f96e7`). You own only the durable review record
`wiki/wave_fiveg_product_independent_review.md`; source code, coverage files,
configuration, and tests are read-only.

Re-fetch and inspect only current official Billy documentation at
`https://www.billy.dk/api/`. Cite the source URL, ETag, MD5, body size, and
access time in the review. Compare the merged product diff against the accepted
contract in `wiki/wave_fiveg_ticketed_writes_contract.md` and the approved
design. Inspect committed bytes from a clean `git archive`, not uncommitted
working-tree state. You may run read-only non-live checks, but perform no Billy
API writes, use no credentials, do no browser/UI work, and keep all raw
research output in ignored scratch only.

Verify all of the following independently:

- exactly twelve typed FastMCP tools: preview/execute pairs for singular
  `salesTaxRulesets` and `salesTaxRules` create, update, and delete;
- exact POST/PUT/DELETE paths and singular payload roots, partial update id
  equality, bodyless delete, escaped ids, and no write retry;
- strict outer tool inputs with opaque inner payloads; typed authentication and
  not-found error mapping; no generic HTTP or browser control;
- use of root's sole `ConfirmationStore` and `WriteProtocolService`, exact
  executor binding before ticket consumption/HTTP, and tests for tamper, expiry,
  replay, wrong organisation, target, payload, and same/cross-resource
  executor mismatch;
- ruleset results preserve an optional present `salesTaxRules` root and never
  fabricate it; rule results never fabricate `salesTaxRulesets`;
- coverage changes are exactly the six approved CUD rows and leave the offline
  state at 202 `api_*` tools, 148 implemented and contract-tested API rows,
  zero live/vision rows, 92 ambiguous bulk rows, unchanged UI/special rows, and
  `complete: false`.

The review must distinguish an offline product-quality ACCEPT/REJECT from the
overall completion gate. It must never claim live qualification, UI parity,
vision verification, bulk support, attachment support, or completeness.

If a finding fails the frozen contract, issue REJECT with precise file and line
evidence and leave all code untouched. If the cohort meets the contract, issue
ACCEPT only for this offline product slice. In either case, finish with the
durable review page committed and a concise outbox report to the parent.

## Completion Requirements

1. `wiki/wave_fiveg_product_independent_review.md` is the sole project
   deliverable and contains a clear ACCEPT or REJECT, reviewed commit IDs,
   official-documents citation/fingerprint, audit scope, concrete checks, test
   evidence, and explicit non-claims for live/UI/vision/bulk/completeness.
2. The audit independently checks the committed product diff and clean archive,
   then runs at least the focused sales-tax tests and the generated-coverage
   checker without modifying project code or coverage state.
3. The verdict verifies or rejects every contract invariant listed in the
   Instructions, including exact executor binding and fail-closed coverage
   arithmetic.
4. No credentials, tokens, browser artifacts, screenshots, frames, HAR files,
   traces, customer data, or raw research material are committed.
5. The review page is committed on this child branch, repository checks needed
   for that page pass, the parent receives an outbox summary, and the node calls
   `fractal node finish` with an evidence-based reason.
