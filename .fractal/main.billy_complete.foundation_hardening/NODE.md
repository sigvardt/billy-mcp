...
Own only `src/billy_mcp/` and `tests/unit/`. Implement the three confirmed
foundation findings in Grok's current review; do not edit coverage manifests,
scripts, package metadata, wiki, or other node seeds.

Contract evidence: the approved design and the parent-provided independent
review findings C1, C2, and S3. This is an offline safety-hardening slice; do
not make Billy API calls, launch a browser, add a domain tool, or claim any
coverage row green.

1. Reject a relative API path beginning `/v2/` before constructing a duplicate
   `/v2/v2/...` URL. Preserve the fixed host, the existing documented paging
   rules, and the no-generic-HTTP surface.
2. Extend structured redaction for the high-sensitivity organisation
   subscription-card/payment fields identified in the official organisation
   schema. Add focused regression fixtures; never reveal their values in errors
   or logs.
3. Add a typed, fail-closed way to construct browser host policy from the
   reviewed `coverage/browser_egress.yaml` structure: accept only exact entries
   whose browser action is allow; reject missing/malformed/unknown policies.
   Keep `BrowserRuntime` headless-only and do not broaden its constructor into a
   generic browser control. Test this loader entirely with fixtures.

Add strict unit tests for every change. Run Ruff format/check, Pyright, owned
pytest, and repository policy. Report exact files, checks, and any contract
question by radio. Escalate rather than changing an inventory contract.

## Completion Requirements

1. `/v2/...` inputs are rejected deterministically and no duplicate API prefix
   can be produced; existing host and paging tests still pass.
2. Subscription-card/payment values are recursively redacted in structured
   upstream payloads and direct redaction tests.
3. The browser policy loader is manifest-backed, deny-by-default, typed, and
   tested for allowed, denied, missing, and malformed entries without launching
   Chromium.
4. No `api_*`, `ui_*`, generic HTTP, generic browser, auth, or write tool is
   added, and no coverage state changes.
5. Owned format, lint, Pyright, pytest, and policy checks pass from committed
   bytes; the child posts a concise delivery report and completes.
