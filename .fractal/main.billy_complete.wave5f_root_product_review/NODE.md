...
Perform an independent Grok product review of the merged Wave-5f offline tax
write slice. You own only
`wiki/wave_fivef_product_independent_review.md`; do not change production
code, tests, manifests, generated status, plans, or any other project wiki
page.

The exact product to assess is root merge commit
`353449d` (child implementation `8c1cdf6`) against the accepted contract at
`wiki/wave_fivef_ticketed_writes_contract.md`, the freeze ACCEPT at
`wiki/wave_fivef_freeze_independent_review.md`, the approved design, and the
current official Billy docs at https://www.billy.dk/api/.

Work independently of the implementer:

1. Re-fetch the official docs and record the URL, date, ETag/body fingerprint
   where available. Treat source drift as a concrete finding; do not silently
   substitute a different contract.
2. Inspect the root merge's exact tax-write diff, not only the child report.
   Verify each of the six singular operations, the twelve stable tool names,
   client-relative POST/PUT/bodyless-DELETE paths and roots, strict outer
   Pydantic input validation, update-id equality, optional parent
   `taxRateDeductionComponents` mapping, child non-fabrication, and locked
   `https://api.billysbilling.com/v2` traffic.
3. Independently run relevant offline checks from this clean review worktree:
   Ruff format/check, Pyright, `tests/api/test_tax_writes.py`, shared write
   protocol/confirmation tests when present, registry/coverage tests, the
   coverage generator/checker, and the commit-mode non-live suite. Report the
   exact commands and outcomes. Do not run live tests, create records, or use
   credentials.
4. Verify ticket protocol claims: literal server-owned executor binding before
   consumption/HTTP, short-lived single-use tickets, tamper/expiry/replay and
   organisation/target/payload mismatches, one-write execution, and no retry.
5. Verify coverage honesty: exactly the six tax CUD rows changed to
   implemented/contract-tested with genuine test references; 190 `api_*`, 142
   offline API rows, zero live/vision, bulk still red, and `complete: false`.

Your wiki page must give an explicit **ACCEPT** or **REJECT** of this bounded
offline product, cite the documentation evidence and commit under review, list
verification commands, and state that live, UI, vision, bulk, special routes,
and overall completion are not accepted. An ACCEPT is a product gate only; it
must never claim completion. A REJECT must name precise remediations and leave
the product unaccepted. Do not open a browser window or retain screenshots,
frames, HAR files, traces, tokens, credentials, or customer data.

If Grok authentication, quota, or a rate limit prevents any review edit, report
the failure via radio before stopping so the parent can retry the full review
with Codex Power as a non-gating fallback and later re-request Grok.

## Completion Requirements

1. `wiki/wave_fivef_product_independent_review.md` is the only project
   deliverable and contains a clear, evidence-backed ACCEPT or REJECT for root
   commit `353449d`.
2. The page records official-doc revalidation, exact operation/tool/path/root
   coverage, ticket binding and response-mapping findings, test commands and
   results, coverage counts, and fail-closed live/UI/vision/bulk status.
3. No production, inventory, coverage, test, credential, browser-evidence, or
   unrelated wiki change is committed.
4. `wiki lint --path=$WIKI_DIR` passes and the committed review page is the
   basis for the parent decision. Signal completion only after that commit.
