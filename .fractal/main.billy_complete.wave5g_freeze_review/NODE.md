...
Perform an independent Grok freeze review of exact root commit
`5612aa8` and `wiki/wave_fiveg_ticketed_writes_contract.md`. This is a
clean-context gate for the Wave-5g offline contract only, covering the six
singular CUD operations for `salesTaxRulesets` and `salesTaxRules`.

Re-fetch and inspect the current official Billy documentation at
`https://www.billy.dk/api/`; compare its fingerprint and relevant operation,
field, Supports, response-root, error, and bulk evidence against the frozen
page, `coverage/api_v2_manifest.yaml`, `coverage/status.json`, the approved
design, and `.fractal/main.billy_complete/tmp/grok-research.md`. Treat the
scratch brief as a lead: cite independently verified official evidence in the
durable review record.

Your only project deliverable is
`wiki/wave_fiveg_freeze_independent_review.md` (with the generated
`wiki/_index.md` entry). It must issue a decisive ACCEPT or REJECT for the
exact root commit. Check all six preview/execute names, client-relative
method/path/root mapping, parent optional `salesTaxRules` response handling,
child no-parent inference, field immutability and opaque-payload boundaries,
locked API base, ticket/executor binding, probe interpretation, and
fail-closed coverage state. An ACCEPT opens only later offline product work;
it must explicitly reject any implied live, UI, vision, bulk, special-route,
or completeness claim.

Do not edit production code, tests, inventories, coverage status, the frozen
contract, or any other wiki page. Do not create tools or green rows. Do not
use credentials, make mutations, launch a browser, retain screenshots/frames/
HAR/traces, or write raw probes into the repository. If review finds a defect,
write a precise REJECT record with the cited correction required; do not repair
the frozen contract yourself.

If Grok cannot perform the independent review before editing because of
authentication, quota, or rate limiting, do not write a verdict. Post the
specific blocker to the parent through radio and stop; the parent will carry
out the policy-required fallback attempt without treating it as the Grok gate.

## Completion Requirements

- `wiki/wave_fiveg_freeze_independent_review.md` gives a cited ACCEPT or REJECT
  for exact root commit `5612aa8`, with no broader acceptance claim.
- The record covers official-doc freshness, the exact six operation/tool map,
  field and multi-root boundaries, write-ticket safety, probe limitations, and
  coverage fail-closed state.
- `wiki update --path=wiki` and `wiki lint --path=wiki` pass, and the node's
  project changes are limited to the review page and its generated index entry.
- The review independently runs the applicable non-live verification and
  confirms it did not mutate Billy, use credentials, or persist sensitive
  browser evidence.
- After committing the review record and reporting its verdict through radio,
  run `fractal node finish` in the same iteration. The decision is self-
  contained; it does not require a parent merge or acknowledgement to finish.
