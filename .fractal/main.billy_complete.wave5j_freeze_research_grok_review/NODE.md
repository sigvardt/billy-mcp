...
Perform an independent, defect-first Grok review of the Wave-5j freeze-ready
research before any implementation begins. Re-fetch Billy's current official
API documentation and use cited primary evidence; the parent's scratch research
is only a lead, not contract evidence.

Review the nine proposed clear bank-line CUD operations across
`bankLineMatches`, `bankLines`, and `bankLineSubjectAssociations`: create,
update, and delete for each. Check their exact paths, methods, documented
payload/response semantics, authentication behaviour, and whether the proposed
offline 401 and missing-id DELETE 200 probes support the stated interpretation.
Explicitly validate that `bankPayments` DELETE is 405 and therefore excluded.
Check that the 92 ambiguous bulk entries remain unresolved, that unverified
enum/owner/subject relationships stay opaque, and that optional response roots
are proposed only for match resources.

Read the root manifest, status, and offline-probe rules as implementation
context. Do not write tools, tests, coverage state, or a freeze page. Do not
claim live, UI, bulk, or product coverage. Do not use browser automation or
persist raw evidence.

Your only project deliverable is
`wiki/wave_fivej_freeze_ready_research_independent_review.md`. It must contain
a cited ACCEPT or REJECT verdict, the nine-operation evidence table, any exact
defects, and an explicit statement of what remains red. If Grok authentication,
quota, or rate limits prevent the review before any edit, report that exact
blocker to the parent without changing findings.

## Completion Requirements

- A durable, non-sensitive cited review is present at
  `wiki/wave_fivej_freeze_ready_research_independent_review.md`, with an
  unambiguous ACCEPT or REJECT verdict and the evidence needed for a later
  Codex implementation decision.
- The review confirms or rejects every proposed operation and the explicit
  `bankPayments` exclusion without treating a probe as a live qualification.
- No product, tool, test, coverage, or freeze-page change has been made.
- `wiki lint --path="$WIKI_DIR"` passes after the review; commit and push the
  review with `fractal commit`, then announce the outcome in the outbox.
