...
Implement the Wave-5d offline ticketed-write leaf for **parent invoices** only.

This child has exclusive file ownership of exactly:

- `src/billy_mcp/api/invoice_writes.py`
...
opens this product gate. Use `daybook_transaction_writes.py` and its tests as
the local pattern; do not reimplement `ConfirmationStore` or
`WriteProtocolService`.

Implement exactly these six flat FastMCP tools with typed Pydantic success and
error paths:

- `api_invoices_create_preview` / `api_invoices_create_execute`
- `api_invoices_update_preview` / `api_invoices_update_execute`
- `api_invoices_delete_preview` / `api_invoices_delete_execute`

Use client-relative `POST /invoices`, `PUT /invoices/:id`, and bodyless
`DELETE /invoices/:id`; `invoice` is the singular request root and `invoices`
the primary response root. Create and update take an opaque inner `invoice`
dictionary inside strict outer models. Delete binds only a nonempty `id`.
Executors accept only `confirmation_ticket` and every preview/execute binds its
exact server-owned execute name through the shared protocol. No write retry is
allowed. Preserve unknown inner invoice fields rather than inventing live field
validation; do not make bulk, special, live, UI, or coverage claims.
...
2. All six invoice tools are registered by the module using the exact names and
   typed Pydantic input/success/error models.
3. Focused tests prove preview non-mutation; exact method, route, singular-root
   ...
   and not-found propagation; and no write retry.
4. `uv run pytest -q tests/api/test_invoice_writes.py`, the node lint script,
   and the node test script pass from the child worktree.
