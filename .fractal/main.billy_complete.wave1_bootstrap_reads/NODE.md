...
Implement the first authenticated API read module only, using the frozen
official contract in `wiki/wave_one_reads_contract.md` and
...
Own exactly these product files: `src/billy_mcp/api/bootstrap_reads.py` and
`tests/api/test_bootstrap_reads.py`. You may create the test directory. Do not
...
support for `api_user_get`, `api_user_list_organizations`,
`api_organizations_get`, and `api_organizations_list`. Use only
`BillyHttpClient` and documented relative paths. Lists may expose only
page/pageSize/include/sortProperty/sortDirection; do not infer filters. Map
singular/list roots and optional paging without fabricating unknown fields.
...
- The four named tools have real typed registration support, Pydantic input and
