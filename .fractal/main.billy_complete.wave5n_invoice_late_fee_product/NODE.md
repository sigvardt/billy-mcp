...
Implement one bounded, offline-only Billy API product slice: singular
`invoiceLateFees` create and update. You are a `codex-power` leaf: own the
repository implementation, focused tests, generated coverage, and commit; do
not delegate or perform independent current-document research.

### Authoritative contract and gate

- Current cited Grok brief:
  `.fractal/main.billy_complete/tmp/grok-research.md` (Research62).
- Frozen contract:
  `wiki/wave_fiven_ticketed_writes_contract.md`.
- Independent Grok freeze acceptance:
  `wiki/wave_fiven_freeze_independent_review.md`.
- Approved architecture:
  `docs/superpowers/specs/2026-07-28-billy-mcp-complete-design.md`.
- Closest implementation and test template:
  `src/billy_mcp/api/contact_balance_payment_writes.py` and
  `tests/api/test_contact_balance_payment_writes.py`.

Research62 authorises exactly these four FastMCP tools:

| Preview | Execute | Exact request |
| --- | --- | --- |
| `api_invoice_late_fees_create_preview` | `api_invoice_late_fees_create_execute` | `POST /invoiceLateFees`, root `invoiceLateFee`, required response root `invoiceLateFees` |
| `api_invoice_late_fees_update_preview` | `api_invoice_late_fees_update_execute` | `PUT /invoiceLateFees/:id`, root `invoiceLateFee`, required response root `invoiceLateFees` |

Use client-relative paths (never add `/v2` in tool code). Strict outer preview
inputs are `{invoiceLateFee: map}` and `{id: non-empty string, invoiceLateFee:
map}`; execute inputs are the shared ticket-only input. Keep the inner map
opaque JSON and reject an optional mismatched inner `id` on update. Use the
existing `ConfirmationStore` and `WriteProtocolService`; preview never makes an
HTTP request and execute performs exactly one non-retried write.

### File ownership

You exclusively own these project files for this delivery:

- create `src/billy_mcp/api/invoice_late_fee_writes.py`;
- create `tests/api/test_invoice_late_fee_writes.py`;
- narrowly modify `src/billy_mcp/server.py` for registration;
- update only the `api.invoiceLateFees.create` and `.update` rows in
  `coverage/api_v2_manifest.yaml`;
- update the coverage generator/status and narrow coverage/registry tests only
  when their generated evidence/count assertions require it:
  `src/billy_mcp/coverage_generator.py`, `coverage/status.json`,
  `tests/coverage/test_coverage_inventory.py`, and
  `tests/unit/test_coverage_server.py`.

Do not change unrelated product files, wiki pages, contracts, UI inventory,
authentication, browser runtime, or test infrastructure. Escalate contract
ambiguity to the parent by radio; do not resolve it by inference.

### Coverage and safety boundary

After passing real offline tests, set only create and update
`implemented: true` and `contract_tested: true`. Change create cleanup exactly
to `live non-production cleanup strategy unqualified; singular DELETE is unsupported`.
The resulting honest counts are 254 `api_*` tools and 174 implemented plus
contract-tested rows; live and vision counts remain zero and
`coverage/status.json` remains `complete: false`.

Never add singular DELETE, bulk save/delete, a webhook, generic HTTP/browser
controls, a typed invoice-late-fee field schema, an invented relation/date/
number encoding, a cleanup mutation, credentials, live tests, UI tests, or
vision evidence. The official method boundary is POST/PUT 401 with JSON body;
singular and bulk DELETE are 405 and remain excluded. Do not alter either bulk
row, which must retain an empty tool name and red state.

### Required verification

Before committing, run the focused API tests and all affected coverage/registry
tests, then the repository formatting, lint, Pyright, coverage-policy, and
non-live test suite. Tests must cover strict outer schemas, opaque values,
inner-ID equality, mutation-free previews, exact canonical binding, path
escaping, one exact request, plural-root-only mapping, malformed/missing root
rejection, tamper/wrong executor/expiry/replay, typed HTTP errors, absent token,
and no retry. Record exact commands and results in your final outbox report.

Commit a clean, self-contained product result and report it to the parent. Do
not claim independent product acceptance or overall completeness: a separate
Grok review will run only after the parent merges your commit.

## Completion Requirements

1. The four named invoice-late-fee preview/execute tools are real typed FastMCP
   tools, registered by `create_server`, and use only the frozen method, path,
   request root, and required response root.
2. The exact strict/opaque/ticket-only behaviour above has focused passing
   tests, including all required safety and error cases.
3. Only the two authorised inventory rows are green offline; both bulk rows,
   all live/UI/vision fields, and `complete: false` retain their fail-closed
   values. The generated count truth is 254 tools and 174 offline rows.
4. Required formatting, lint, Pyright, coverage-policy, focused, and non-live
   suite commands pass; no live credentials, browser sessions, records, raw
   evidence, or secrets were used or retained.
5. The worktree is clean, the product commit is pushed by `fractal commit`, and
   your outbox report names changed files, verification, count deltas, and the
   retained red boundaries. Finish the node in the same run only after these
   conditions are true.
