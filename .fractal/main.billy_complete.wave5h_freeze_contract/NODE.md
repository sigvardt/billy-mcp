...
Act as the Grok research and contract-authoring leaf for the next Billy MCP
write cohort. Your exclusive project-file ownership is exactly
`wiki/wave_fiveh_ticketed_writes_contract.md`. Do not modify any other project
file, including coverage, source, tests, existing wiki pages, or the parent
plan. Do not spawn children.

Write a cited, offline-only contract for precisely these attachment JSON CUD
operations:

- `api.attachments.create`: `POST /v2/attachments`, singular `attachment`,
  primary response root `attachments`.
- `api.attachments.update`: `PUT /v2/attachments/:id`, singular `attachment`,
  primary response root `attachments`.
- `api.attachments.delete`: `DELETE /v2/attachments/:id`, no request body,
  canonical ticket target `{"id": id}`, optional `meta.deletedRecords.attachments`.

Before writing, re-fetch the current official Billy API documentation from
`https://www.billy.dk/api/` and record a source citation, response fingerprint,
and the exact public evidence for methods, roots, fields, errors, and Supports
flags. The parent research handoff is available only as a lead at
`/Volumes/ssd_1/Repositories/billy-mcp/.worktrees/main.billy_complete/.fractal/main.billy_complete/tmp/grok-research.md`;
do not treat it as a substitute for independent source checking. The locked
runtime base remains `https://api.billysbilling.com/v2` and client paths omit
the repeated prefix.

Freeze an opaque inner attachment payload: `priority` is the only clear mutable
field. Do not invent serialization for the required `owner`
belongs-to-reference, do not use a bank-payment reference format by analogy,
and distinguish JSON attachment records from the high-sensitivity binary file
upload special. Include the shared preview/execute requirements: strict outer
inputs, no-write preview, ticket-only execute, one root confirmation store and
write protocol, server-owned exact execute-tool binding, no write retry,
partial PUT with route/body ID match, and no fabricated additional response
roots.

Keep `api.attachments.bulk_save`, `api.attachments.bulk_delete`,
`api.files.create`, `api.special.files_upload`, live testing, UI parity,
vision, and all unrelated resources out of scope and red. An unauthenticated
DELETE 200 is never cleanup evidence. Do not access credentials, run a browser,
create Billy records, or claim any coverage state is green. Do not include raw
frames, HAR, traces, tokens, cookies, source snapshots, or probe payloads in
the repository.

This page is a freeze candidate, not an independent acceptance. After the
parent merges it, a separate Grok reviewer will determine ACCEPT or REJECT.
Report source drift or any contradiction to the parent by radio immediately.

## Completion Requirements

1. `wiki/wave_fiveh_ticketed_writes_contract.md` is the only project deliverable
   and has valid frontmatter, a durable description, and cited current official
   documentation evidence.
2. It freezes exactly the three attachment JSON CUD inventory rows and their
   six stable preview/execute names, paths, roots, response mapping, documented
   errors, sensitivity, cleanup constraints, and test obligations.
3. It records the opaque-payload and file-upload boundary without inventing
   owner serialization, binary upload behaviour, bulk contracts, webhooks, or
   unobserved response roots.
4. It explicitly keeps all coverage, live, UI, vision, bulk, and completeness
   status fail-closed and makes no acceptance verdict.
5. `git diff --check`, `wiki lint --path="$WIKI_DIR"`, the inherited test
   script, and inherited lint script pass. Commit the deliverable and run
   `fractal node finish` in the same run after reporting the contract to the
   parent.
