...
Independently review the committed Wave-5s-B files-upload research handoff at
root baseline `5b8719b`. You are a Grok research-review gate, not a product
implementer. Recheck current official Billy documentation and the cited
unauthenticated evidence, then write a cited, non-sensitive verdict page at
exactly `wiki/wave_fivesb_files_upload_research_independent_review.md`.
...
Audit `wiki/wave_fivesb_files_upload_research.md` and the cited scratch brief
against current official documentation. Establish whether the research correctly
freezes a binary-only, ticketed `POST /v2/files` contract with the locked
`https://api.billysbilling.com/v2` base, path+digest binding, a narrow
allowlisted-header client extension, required `files[]` and optional
`attachments[]`, and the two-row alias boundary:

- `api.special.files_upload` owns `api_files_upload_preview` plus execute;
- `api.files.create` is an empty-name alias only;
- no `api_files_create*`, JSON create body, generic header map, webhook, bulk,
  UI, live, or headed-browser claim is authorised.

Confirm current docs/unauth method boundaries as reproducibly as permitted;
cite URL, access time, and document fingerprint. Never send credentials or
mutate data. Do not implement source, create tools, change inventory status, or
green coverage. Record an ACCEPT-as-research only if every bounded claim is
supported; otherwise give a precise REJECT and retain fail-closed boundaries.

## Completion Requirements

- The sole review page is committed and contains a current-doc citation,
  reviewed root hash, evidence commands/results, explicit ACCEPT-as-research or
  REJECT verdict, and all non-claims.
- The page confirms or rejects the binary request shape, host lock, ticket
  binding requirements, dual inventory alias rule, method boundaries, and
  no-generic-tool boundary without implementing any product work.
- No project source, tests, inventory, generated coverage, root status,
  `_index.md`, credentials, browser evidence, or raw artefact was changed or
  added.
