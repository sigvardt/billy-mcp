...
You are the Grok owner of current official-document research for the next
candidate Billy API v2 write contracts. This is a wiki-only research leaf, not
an implementation, coverage, UI, browser, live-qualification, or review leaf.
Do not delegate: this leaf has no child capacity.

### Scope and ownership

You may change only `wiki/wave_fivep_candidate_write_research.md`, the
generated entry for that page in `wiki/_index.md`, and your own `.fractal/`
seed. Do not change source, tests, coverage manifests, status/report files,
accepted Wave-5o documents, or any other wiki page. Keep raw responses,
headers, and probe bodies only in owner-only scratch and purge them before
finish; the committed page must contain only non-sensitive findings.

### Research question

Produce a cited current brief for exactly these already-inventoried candidates:

- `api.organizations.create` and `api.organizations.update`
- `api.users.update`
- `api.salesTaxReturns.update`
- `api.invoiceReminderAssociations.delete`

Start from the current official API documentation at
https://www.billy.dk/api/ and the locked API base
https://api.billysbilling.com/v2. Re-fetch the official page and record a
non-sensitive access fingerprint. Parse the relevant Supports and property
tables. You may perform only unauthenticated, bounded, non-mutating method
probes with valid object bodies when needed to distinguish a 401 auth gate from
a 405 method closure. Never use credentials, create data, invoke a browser, or
make any production or test-organisation mutation.

Existing research is a lead, not authority: earlier object-body probes found
organizations POST/PUT 401 and DELETE 405; users PUT 401 and POST/DELETE 405;
salesTaxReturns PUT 401 and POST/DELETE 405; and
invoiceReminderAssociations DELETE 200 meta-only while POST/PUT are 405.
Reconfirm or correct every conclusion from primary evidence. A 200 meta-only
association delete is not cleanup proof and must not be inferred into a green
or implemented contract. Keep all ambiguous bulk operations red and out of
scope.

### Required durable brief

Write `wiki/wave_fivep_candidate_write_research.md` with a clear cited
recommend or block verdict for each candidate. For every verdict, include the
inventory id, official endpoint/method, documented request and response fields,
filters/pagination where applicable, errors, sensitivity, known method-gate
result, cleanup limitations, the exact future tool shape if and only if a
freeze is defensible, and all exclusions. Clearly distinguish official-doc
facts, unauthenticated gate evidence, and unproven live/UI behaviour.

The page must state that research is not a freeze acceptance, implementation,
contract test, live test, UI/vision verification, or completeness claim. Do
not invent body schemas, relation encodings, webhooks, bulk operations, or
cleanup success. If evidence is insufficient, record an explicit blocker and
leave the candidate unavailable to Codex Power implementation.

Run `wiki update --path="$WIKI_DIR"` and `wiki lint --path="$WIKI_DIR"`,
inspect `git diff --check`, commit the researched page and seed with `fractal
commit`, and send the parent a concise radio report naming the commit, citations,
and each recommended or blocked candidate. Do not wait for a reply.

## Completion Requirements

- `wiki/wave_fivep_candidate_write_research.md` contains current cited primary
  evidence and a non-sensitive official-access fingerprint for all five named
  inventory operations.
- Each candidate has an explicit recommend or block conclusion with documented
  field boundaries, method-gate interpretation, and no unproven tool, cleanup,
  live, UI, vision, bulk, or completeness claim.
- Only the owned wiki page/index and node seed changed; no credentials, raw
  frames, HARs, traces, probe bodies, source, tests, or coverage claims entered
  the repository.
- Project wiki update/lint and `git diff --check` pass; the page and seed are
  committed and reported through radio. Do not await a separate review to
  finish this research leaf.
