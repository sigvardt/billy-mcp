...
Produce the first current, cited, non-sensitive Billy interface-discovery brief
for the parent. You own only `wiki/`; do not edit source, tests, coverage
manifests, scripts, package metadata, or another node's seed.

Use these contracts:

- `docs/superpowers/specs/2026-07-28-billy-mcp-complete-design.md`
- the parent research brief at
  `.fractal/main.billy_complete/tmp/grok-research.md`
- current official API documentation at `https://www.billy.dk/api/` for API
  parity context only.

Run any browser work through the merged `BrowserRuntime` or an equivalently
bounded Playwright setup: persistent MCP-owned profile, Chromium headless only,
no desktop controls, no generic browser helper, and deny-by-default egress.
For this initial read-only discovery, permit only `mit.billy.dk` in the browser
lane. Never make API requests through the browser or use an arbitrary URL.

Attempt only read-only, bounded UI/auth discovery. Reuse a persisted session
when configured; use stored credentials/TOTP only when already configured
outside git. If credentials are absent or a CAPTCHA, passkey, push approval, or
unsupported MFA appears, stop that workflow and record the precise blocker.
Do not open a visible browser, create records, change settings, or exercise
writes. Do not infer a UI route or mark any API parity row `not_applicable`.

For each observed route or screen, record DOM assertion references and an
independent read-back possibility. If a rendered frame is needed for Grok
vision, keep it owner-only under node scratch, inspect it, then purge it; never
commit it. No discovery or vision finding turns a coverage row green without a
dedicated non-production-org workflow, independent read-back, and durable
non-sensitive review record.

Write a concise cited durable result in `wiki/` with the observed host, session
state, routes/screens actually reached, safe selectors or DOM assertions,
read-back route, plan/MFA blocks, no-write confirmation, and evidence-purge
result. If access is blocked before authenticated navigation, publish that
evidenced blocker rather than a speculative interface inventory. Report the
wiki path and verdict via radio.

## Completion Requirements

1. A committed `wiki/` page contains the cited, non-sensitive discovery brief
   and distinguishes observed facts from blocked or unverified items.
2. All browser use, if any, is demonstrated headless with the MCP-owned
   profile and exact `mit.billy.dk` allowlist; no visible window, generic
   browsing, API-browser traffic, test record, or write occurs.
3. Any raw browser evidence is stored only outside the repository, purged after
   vision inspection, and the durable brief records the purge result without
   retaining frame paths or sensitive content.
4. No coverage row, API/UI tool, or `not_applicable` assertion is made green or
   complete from this discovery pass.
5. The owned wiki passes `wiki update` and `wiki lint`, the node's lint/test
   hooks pass, and the parent receives a radio delivery message with the result
   and blockers.
