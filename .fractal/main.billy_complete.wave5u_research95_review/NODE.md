...
You are the independent `grok` review leaf for research95. Own only the new
project record `wiki/wave_fives_research95_independent_review.md`; do not edit
source, tests, coverage, existing wiki pages, or the auth-product review
child's work.

Independently review the current research95 promotion on the parent baseline:
...
Use Billy's current official API documentation as the primary source. Produce a
concise cited independent-review record that states a clear ACCEPT or REJECT
verdict for research95 *as research only*. Check the primary document's locked
base URL and no-webhook conclusion; the exact `ids[]` candidate form; the
empty-form `INVALID_DELETE_ID_ARRAY` boundary; and why a synthetic non-empty
id's unauthenticated metadata-only 200 cannot prove live safety. Confirm the
current coverage remains 184 implemented/contract-tested with no live or vision
claim and `complete: false`, or record exact discrepancies.

The record must explicitly preserve the hard boundary: no residual/bulk real
method, FastMCP tool, coverage greening, or UI/live claim is authorised. State
whether the bounded Codex Power harness encode is safe to begin, but do not
review or sign off an implementation diff that does not yet exist. Do not rely
on an uncited claim, screenshots, browser frames, credentials, or raw sensitive
evidence. Do not create business data or submit credentials; browser use, if
needed, remains headless.

Run wiki lint and the node lint/test scripts. Report your verdict and any
material discrepancy through the outbox. Keep private memory current before
finishing.

## Completion Requirements

1. Exactly one project deliverable is added:
   `wiki/wave_fives_research95_independent_review.md`; all other project files
   remain unchanged by this leaf.
2. The record gives a cited ACCEPT or REJECT research-only verdict, checks the
   official-doc source against the promoted form-matrix claims, and records any
   discrepancy concretely.
3. The record explicitly leaves residual 29, bulk 92, live, vision, and UI
   rows red; it authorises no residual/bulk tools or live network method.
4. `wiki lint --path=$WIKI_DIR`, `bash $NODE_DIR/scripts/lint.sh`, and
   `bash $NODE_DIR/scripts/test.sh` pass.
5. The branch contains the review-record commit and an outbox report with the
   verdict and verification evidence.
