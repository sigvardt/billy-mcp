...
You are the Codex Power shared-foundation leaf. Own `src/billy_mcp/` except
the root-owned `models.py`, and `tests/unit/` only. Do not edit `models.py`,
package metadata, coverage manifests, coverage scripts, root tests, CI,
README, wiki prose, or node scripts. Escalate cross-cutting contract needs to
the parent instead of drifting around them.

Implement the approved-design Phase 0 foundation using Python 3.12 and locked
FastMCP 3. The source contract is the approved design and the cited parent
brief at `/Volumes/ssd_1/Repositories/billy-mcp/.worktrees/main.billy_complete/.fractal/main.billy_complete/tmp/grok-research.md`.
Do not add domain `api_*` or `ui_*` stubs, generic HTTP/browser controls,
telemetry, shell execution, dynamic code execution, secret logging, or headed
browser behaviour.

Deliver real, typed modules for:

- Pydantic configuration: fixed API base `https://api.billysbilling.com/v2`,
  `BILLY_API_TOKEN` then keyring resolution, optional selected organisation,
  configurable external persistent browser profile, allowed upload roots, and
  never-returned secrets.
- Stable error translation to root-owned `ToolError`; both
  `AUTHENTICATION_REQUIRED` and `OAUTH_INVALID_ACCESS_TOKEN` become
  `AUTH_REQUIRED`, preserving sanitised upstream metadata.
- Recursive structured redaction for tokens, passwords, TOTP values, cookies,
  tickets, and Authorization values.
- A thread-safe in-memory confirmation store with at least 256 bits of entropy,
  maximum five-minute expiry, single use, canonical JSON bindings for tool,
  organisation, target, request, expected effect/state, file digest/path, and
  destination URL. Reject tamper, replay, expiry, and binding mismatches.
- An HTTP client hard-locked to the official host with injectable httpx
  transport, `X-Access-Token` only, page/pageSize validation, retry-safe error
  handling, and JSON parsing that tolerates the documented non-JSON logger
  prefix. It must not allow arbitrary URLs.
- A bounded Playwright persistent-context runtime that launches exclusively
  headless, enforces caller-independent egress allowlisting, refuses unknown
  hosts, contains no desktop manipulation, and persists no trace/frame/HAR.
- Coverage loading/report data models plus a FastMCP stdio shell exposing only
  real typed `coverage_status` and `coverage_report` tools. It must work once
  the coverage branch is merged; absence of manifests produces a typed error,
  never an invented green report.

Write focused unit tests for every security-sensitive invariant above and for
FastMCP tool registration. Tests must not contact Billy or launch a headed
browser. Keep the implementation independently mergeable with the manifest
branch.

## Completion Requirements

- All owned source modules and focused unit tests are committed on this child
  branch; root-owned `models.py` is untouched.
- Tests prove host lock, known 401 translation, recursive redaction, ticket
  entropy/expiry/replay/tampering/binding rules, page/pageSize-only request
  construction, headless-only browser configuration, egress denial, and
  typed coverage tool registration without API/UI stubs.
- `uv run ruff format --check src/billy_mcp tests/unit`,
  `uv run ruff check src/billy_mcp tests/unit`, `uv run pyright`, focused
  pytest, and `uv run python scripts/check_repository_policy.py` pass.
- Send a concise parent-radio summary listing modules, test results, and any
  unresolved coverage integration point, then run `fractal node finish` with a
  reason limited to this owned foundation delivery.
