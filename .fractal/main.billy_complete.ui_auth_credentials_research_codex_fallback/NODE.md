...
Produce a cited Codex Power fallback research brief for the next secure
UI-authentication implementation slice. Grok could not run because its runner
authentication was unavailable before task edits; this fallback must not be
represented as a Grok finding. This is research-only: you own
`wiki/ui_auth_credentials_login_organization_research_codex_fallback.md` and
may not modify source, tests, coverage inventories, or generated status.

Research current official Billy documentation first, then inspect committed
configuration, browser runtime, inventory, and design records. Separate
official facts from bounded anonymous observation. Only the current observed
login signature is in bounds: root redirects to
`https://mit.billy.dk/login`, with visible email/password/remember and
`button[data-cy=login-button]`; labels observed are `Log in` and `Log ind`.
No credentials, submitted login, organisation selection, or post-login state
has been observed.

Give an implementation-ready but explicitly bounded plan for credential-store
references, browser credential/TOTP resolution, redaction/non-persistence,
typed headless login pre-submit checks, supported failures, expiry/
reauthentication, only-observed challenge classification,
organisation discovery/selection, test seams, dedicated non-production
organisation cleanup/read-back, evidence purge, and later mandatory Grok vision
review. Cite direct official URLs near every official claim. Mark unknown
post-login behaviour as a discovery blocker.

Do not use credentials, submit login, create records, make arbitrary
browser/API calls, retain raw evidence, infer coverage, or make a live/vision/
parity/completeness claim. State that all UI coverage remains red and that the
mandatory Grok research/review gate remains outstanding. Post a concise parent
handoff on completion.

## Completion Requirements

- `wiki/ui_auth_credentials_login_organization_research_codex_fallback.md`
  contains cited findings, bounded next-slice design, explicit unknowns, and
  the Grok-fallback limitation.
- It covers credentials/TOTP, challenges, organisation selection, tests,
  cleanup, evidence purge, and red coverage without sensitive values or login.
- The wiki lints cleanly; only the research record is committed; a parent
  handoff is posted; and the leaf finishes.
