...
Produce the cited Wave-5u safe method-observation contract required before any
implementation. You are the Codex Power fallback because the designated Grok
research leaf failed before making any edit due to host authentication. This
fallback does not satisfy the mandatory Grok research/review gate; report that
limitation explicitly. Your only project deliverable is
`wiki/wave5u_method_probe_contract.md`; do not modify source, tests, coverage
manifests/status, scripts, dependencies, or other wiki pages.

Read these sources first:

- `.fractal/main.billy_complete/tmp/grok-research.md` (research90)
...
- `src/billy_mcp/live_probe.py` and its focused tests
- the current official Billy API documentation at https://www.billy.dk/api/

Research the exact static, non-persistent request construction for each frozen
candidate kind. The contract must separate facts from inference and state, for
every kind, the fixed method, route replacement, query, body/content type, and
why it cannot produce a durable Billy change. If any candidate lacks official
or reproducible non-production evidence for a safe real-method probe, say so
and require the future runner to block that candidate before network traffic.
Never turn an unauthenticated 401, 405, empty OPTIONS 204, or meta-only 200
response into a supported-operation or live-qualified claim.

Preserve these hard boundaries:

- API base is exactly `https://api.billysbilling.com/v2`.
- A token plus an explicitly matched dedicated non-production organisation is
  required before a future live observation, but those secrets must never be
  read into the wiki or committed.
- Evidence may retain only candidate id, method, static route, status, and a
  top-level `errorCode`; never retain headers, response bodies, tokens,
  customer values, screenshots, HAR files, or traces.
- The deliverable is research/infrastructure only. Residual 29, ambiguous bulk
  92, and all UI rows remain red; coverage remains 184/184/0/0 and
  `complete: false`.
- Do not add generic HTTP/browser control, an MCP tool, a write path, or a
  webhook.

Use current official-document citations in the new page and explicitly cite
research90 for its reproducible unauthenticated observations. Headless-only
rules apply to any browser use.

## Completion Requirements

1. `wiki/wave5u_method_probe_contract.md` exists with valid wiki frontmatter,
   a direct official Billy API documentation citation, research90 references,
   a clearly stated non-qualification boundary, and an explicit statement that
   this Codex Power fallback does not replace a Grok review.
2. The page records the static request strategy and the safety decision for
   residual, bulk-save, and bulk-delete candidates; unsafe or unproven cases
   are explicitly blocked before network activity rather than inferred safe.
3. It requires exact base-URL locking, token + matching dedicated
   non-production-organisation gates, owner-only evidence outside the
   repository, status/errorCode-only sanitisation, and zero coverage updates.
4. It explicitly rejects OPTIONS-only qualification and meta-only 200 as a
   contract or mutation-success signal.
5. The three-dot project diff contains only the declared wiki page (plus
   generated `wiki/_index.md` if required); it has no secret, raw evidence,
   source, test, coverage, or child-seed product artifact.
6. `wiki lint --path=$WIKI_DIR` and `bash $NODE_DIR/scripts/test.sh` pass. The
   node reports its cited conclusion and runs `fractal node finish` with a
   research-only fallback reason.
