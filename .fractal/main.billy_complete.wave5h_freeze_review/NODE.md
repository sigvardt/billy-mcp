...
Act as the independent **Grok** reviewer for the merged Wave-5h attachment
ticketed-write freeze candidate at root commit `c14266e`.

Your only project deliverable is
`wiki/wave_fiveh_freeze_independent_review.md` plus its generated `wiki/_index.md`
entry. Do not change the candidate contract, production code, tests, coverage,
configuration, browser workflows, or the attachment inventory state.

Independently re-fetch the current official Billy API documentation and compare
its fingerprint and attachment create/update/delete contract with:

- `wiki/wave_fiveh_ticketed_writes_contract.md`;
- `wiki/wave_fiveh_freeze_ready_research_independent_review.md`;
- `wiki/offline_write_probe_rules.md`;
- `coverage/api_v2_manifest.yaml` and `coverage/status.json`; and
- `docs/superpowers/specs/2026-07-28-billy-mcp-complete-design.md`.

The review record must state an explicit **ACCEPT** or **REJECT**. It must cite
the official docs URL and observed document fingerprint, identify the exact
candidate commit, check each of the three singular attachment JSON CUD routes,
strict outer/opaque inner schema boundary, PUT/body-id rule, bodyless delete,
exact ticket binding and shared root protocol, response-root discipline,
authentication/error mapping, binary-files exclusion, bulk ambiguity, and
coverage honesty. It must state clearly that a Codex fallback candidate is not
its own gate and that the verdict is offline only.

No browser, credential, live test, UI, vision, screenshot, HAR, trace, or
customer data is needed or allowed for this static contract review. A failed
documentation fetch or evidence contradiction requires a REJECT or BLOCKED
verdict; never fill gaps from inference. Do not make a product, live, UI,
vision, coverage-green, or completeness claim.

Run `wiki update --path=wiki`, `wiki lint --path=wiki`, the inherited lint
script, and the inherited non-live test script. Post a concise radio result to
your parent. Do not spawn children: this is a bounded independent review.

## Completion Requirements

1. A durable, non-sensitive review page exists at
   `wiki/wave_fiveh_freeze_independent_review.md` and the generated wiki index
   is current; no other project file is changed.
2. The page contains a cited, independently derived ACCEPT, REJECT, or BLOCKED
   verdict against the merged candidate at `c14266e`, with every contract
   boundary named in the Instructions addressed.
3. The page preserves fail-closed status: attachment product code, three
   coverage rows, live tests, UI/vision, bulk rows, binary file upload, and
   overall completeness remain unclaimed.
4. Wiki validation, inherited lint, and inherited non-live tests pass and are
   reported with the verdict.
5. The review is committed, pushed by the node's normal commit flow, reported
   on radio, and the node calls `fractal node finish` in its own completion
   step. It must not wait for a parent merge or approval.
