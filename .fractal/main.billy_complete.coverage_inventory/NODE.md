...
You are the Codex Power coverage-inventory leaf. Own only `coverage/`,
`tests/coverage/`, and the two exact scripts `scripts/generate_coverage_report.py`
and `scripts/check_coverage.py`. Do not edit package code, project metadata,
CI, root tests, README, or wiki prose except to report an unavoidable contract
conflict to the parent.

Use the approved design and the cited parent brief at
`/Volumes/ssd_1/Repositories/billy-mcp/.worktrees/main.billy_complete/.fractal/main.billy_complete/tmp/grok-research.md`
as the contract evidence. The live official-doc fingerprint is etag
`hsisik4g9p3603`, MD5 `c2efda0ee4cf9cf200e14910c5fc6996`. Do not perform
additional external research and do not infer contracts from historical gists.

Create all four Phase 0 coverage deliverables:

1. `coverage/api_v2_manifest.yaml`: at least the 207 clear official resource
   operations, exactly one red `ambiguous_bulk` row for every one of 92 bulk
   mentions, and the six documented special routes. Every row has the design
   fields (including request/response data, documented filters and pagination,
   known errors, sensitivity/side effects, cleanup, planned tool name, test
   references, and evidence). Preserve `page`/`pageSize` only, full
   invoice/bill/daybookTransaction filters, and both 401 error codes. Do not
   create `/documents`, webhook, bulk, or singular-organisation tools.
2. `coverage/ui_workflows_manifest.yaml`: red discovery rows for the documented
   interface route families plus an explicit red parity mapping for every API
   business capability. `not_applicable` requires real interface evidence and
   therefore is not available in this slice. Every UI row has
   `vision_verified: false` and no raw frame data.
3. `coverage/browser_egress.yaml`: deny by default; permit `mit.billy.dk` for
   UI/auth and `download.billy.dk` only for future typed download handling;
   mark API host exclusive to the API client and `api.billy.dk` denied. Do not
   add partner hosts without a verified workflow.
4. A deterministic report generator/checker and generated `coverage/status.json`
   plus report. `complete` must be `false`; no row may claim implemented,
   contract-tested, live-tested, or vision-verified.

Make the data self-validating: fail for missing required fields, registered
tool names without coverage rows, false completeness, a green ambiguous row,
or raw browser-evidence paths. Tests must assert source arithmetic and status
rules. Use no domain MCP stubs and no mock/live qualification result.

Keep all live/API/UI rows red because `BILLY_API_TOKEN` is unavailable. The
output must be safe to merge independently of the foundation branch.

## Completion Requirements

- All four owned coverage deliverables and focused tests are committed on this
  child branch.
- API manifest contains the researched 305-operation snapshot (207 clear,
  92 ambiguous bulk, six specials) with no dropped supported route and no
  invented endpoint.
- UI/parity inventory and browser egress manifest are structurally complete but
  visibly red; generated `status.json` has `complete: false`.
- `uv run ruff format --check coverage tests/coverage scripts`,
  `uv run ruff check coverage tests/coverage scripts`, focused pytest, and
  `uv run python scripts/check_repository_policy.py` pass. Report exact commands
  and resulting counts in the outbox.
- Send the parent one concise radio update naming all deliverables and any
  blocker, then run `fractal node finish` with a reason limited to this owned
  delivery.
