...
Implement only the Wave-5a contact ticketed-write module. You are a childless
`codex-power` leaf and may edit exactly these product files:

- `src/billy_mcp/api/contact_writes.py`
...
`c2efda0ee4cf9cf200e14910c5fc6996`. Do not perform fresh web research or
change the contract.

Deliver exactly six real typed FastMCP tools: `api_contacts_create_preview`,
`api_contacts_create_execute`, `api_contacts_update_preview`,
`api_contacts_update_execute`, `api_contacts_delete_preview`, and
`api_contacts_delete_execute`. Register them only through a
`register_contact_write_tools(server, client, write_protocol)` helper. Each
preview uses a typed `extra="forbid"` outer input and a `WriteOperationSpec`;
each execute accepts only the existing `WriteExecuteInput`. Create and update
bind exact `{ "contact": payload }` JSON to the `/contacts` relative path;
delete binds the ID but sends no body. Let `WriteProtocolService` own ticket
issue/consume, escaping, locked-client calls, no-retry behaviour, and response
mapping.

The nested contact payload may remain an opaque `dict[str, JsonValue]`. Do not
invent strict validation for unresolved values, especially `paymentTermsDays`,
contact type, country wire shape, locale, or embedded contact-person semantics.
Keep all request and response outer models typed and forbid extra fields. Never
log emails, phone numbers, registration values, tokens, or tickets.

Tests must use only local mocks/doubles and prove preview has zero writes,
execute has the exact method/path/body, delete has no body, ticket-only input,
...
confirmations, auth, UI, browser, bulk, specials, wiki, or any other leaf's
...
1. Only the owned contact module and focused test file contain product changes;
   no coverage, server, client, auth, UI, browser, bulk, special, or inventory
   edit is present.
2. All six named contact preview/execute tools are real, typed, registered by
   the module helper, and execute accepts only the confirmation-ticket model.
...
4. `bash $NODE_DIR/scripts/lint.sh` and `bash $NODE_DIR/scripts/test.sh` pass,
   with focused `tests/api/test_contact_writes.py` evidence reported.
