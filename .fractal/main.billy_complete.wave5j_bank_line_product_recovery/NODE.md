...
Implement the accepted **Wave-5j offline bank-line product** only. You are the
Codex Power implementation leaf: do not perform external research, browse,
write a new freeze, conduct the independent review, or implement Wave-5k
bankPayments. Current official-document research is already accepted for this
scope.
...
Implement exactly these nine inventory rows as preview/execute twins (eighteen
typed `api_*` tools):

- `bankLineMatches`: create, update, delete
- `bankLines`: create, update, delete
- `bankLineSubjectAssociations`: create, update, delete

Use the root's existing `ConfirmationStore` and `WriteProtocolService`. Every
write must be preview then exact-ticket execution; binds a non-empty,
server-owned `execute_tool_name`; accepts no replacement payload at execution;
is single-use and no-retry. Keep the API base lock, redaction, typed success and
error paths, strict flat outer Pydantic inputs, opaque inner `JsonValue` maps,
partial-update route/body id equality, and bodyless deletes. Match operations
may map `bankLines` and `bankLineSubjectAssociations` only when returned. Line
and association operations must not fabricate extra roots or deleted metadata.

Own only these tracked project files:
...
Keep all bulk rows, live/UI/vision rows, `bankPayments`, `balanceModifiers`,
browser work, credentials, screenshots, and the overall `complete` flag out of
scope. Do not mutate the freeze/review wiki pages. Do not green coverage unless
real offline tests for the implemented row pass. Before your commit, run the
focused tests and the repository formatter, Ruff, Pyright, coverage checker,
and applicable non-live safety checks. Report a blocker to the parent by radio
with exact evidence; do not guess unsupported wire shapes or enum values.

## Completion Requirements

1. The nine frozen Wave-5j CUD rows have real preview and execute tools, with
   the exact paths, roots, HTTP methods, ticket binding, and response-root
   rules from the accepted contract.
2. The owned offline tests cover request construction, response mapping,
   preview/execute, expiry, replay, tampering, wrong organisation/target/payload,
...
5. The branch contains only the owned implementation, test, and coverage files
   plus normal node bookkeeping. Post a concise outbox handoff for the parent;
   do not claim independent product acceptance or overall completion.
