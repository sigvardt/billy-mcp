...
Perform the full offline product review of the integrated Wave-5d invoice and
invoice-line ticketed-write slice at root tip `1108e2f`. This is a **Codex
Power fallback** because the mandated Grok reviewer failed before any review
edit with documented CLI authentication failure. It must be independent of the
implementing leaves, but it must never be represented as a Grok acceptance or
open the Wave-5e gate.

You own only `wiki/wave_fived_product_codex_fallback_review.md`; do not modify
source code, tests, coverage artifacts, shared protocols, inventories, or the
approved design. Use the cited current official-document brief at
`.fractal/main.billy_complete/tmp/grok-research.md`, the Wave-5d frozen
contract, and freeze review as the contract evidence; do not perform new
external research.

Audit the committed root for exact invoice/invoice-line POST, PUT, and bodyless
DELETE routes; singular request roots; typed strict outer inputs; update-ID
binding; ticket-only executors; one shared `WriteProtocolService` and
`ConfirmationStore`; exact executor-name binding; no retries; optional
invoice-line parent-root mapping; server registration of 166 `api_*` tools;
and generated coverage evidence for exactly six new rows at 130 offline
implemented/contract-tested rows. Reproduce focused and root validation as
appropriate. Check that the new cross-module test rejects both executor
mismatches before ticket consumption or HTTP and preserves each original
ticket for its rightful executor.

Do not accept unauthenticated DELETE 200, missing credentials, inaccessible UI,
or prior fallback work as live/UI/Grok evidence. Keep all live/UI/vision/bulk/
special/completeness rows fail-closed and preserve the Wave-5e product gate.

## Completion Requirements

1. `wiki/wave_fived_product_codex_fallback_review.md` records a non-sensitive
   PASS or FAIL result with the reviewed tip, cited frozen evidence, exact test
   references, coverage posture, and an explicit statement that it is a Codex
   fallback and **not** the mandatory Grok product acceptance.
2. Wiki update and lint pass; no raw browser evidence, secrets, or source edits
   are committed.
3. Concrete defects produce a FAIL with file locations and reproduction detail;
   do not repair outside the review scope.
4. Send the parent an outbox handoff with verdict and durable page, then finish.
