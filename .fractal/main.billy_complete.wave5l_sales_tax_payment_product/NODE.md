...
Implement the bounded Wave-5l API product slice only. You are the Codex Power
implementation leaf; do not spawn children and do not perform independent
review. The root has already accepted the cited contract, so do not re-open
research or modify the contract.

Authoritative contract and gates:

- `wiki/wave_fivel_ticketed_writes_contract.md` is the frozen operation
  contract.
- `wiki/wave_fivel_freeze_independent_review.md` is the authoritative freeze
  ACCEPT that opens this leaf.
- The root's cited Grok product-ready brief established current docs ETag
  `hsisik4g9p3603`, MD5 `c2efda0ee4cf9cf200e14910c5fc6996`, POST/PUT 401,
  and singular DELETE 405. Use those sources; implement no additional API
  capability.

Implement exactly four typed FastMCP tools on the one shared
`WriteProtocolService`:

1. `api_sales_tax_payments_create_preview` and
   `api_sales_tax_payments_create_execute`
2. `api_sales_tax_payments_update_preview` and
   `api_sales_tax_payments_update_execute`

Create `src/billy_mcp/api/sales_tax_payment_writes.py` and
`tests/api/test_sales_tax_payment_writes.py`, closely following the established
bank-payment mirror. Register the module on the shared protocol in
`src/billy_mcp/server.py`. Use client-relative `/salesTaxPayments`, singular
root `salesTaxPayment`, required result root `salesTaxPayments`, and
`additional_plural_roots=()`. Inputs must be flat and strict: create accepts
only `salesTaxPayment`; update accepts only non-empty `id` plus
`salesTaxPayment`; execute accepts only non-empty `confirmation_ticket`. Keep
the inner object opaque and reject an inner `id` that differs from the update
route id.

Your exclusive product-file ownership is:
...
Do not change any other product file or the shared wiki. Do not edit
`sales_tax_writes.py`. Root will exclude this node's `.fractal` seed state from
the product merge; it is not part of your deliverable.

Coverage may become green only after real offline tests: map exactly
`api.salesTaxPayments.create` and `.update` to your test module plus the server
registry reference. The expected post-product values are 246 `api_*` tools and
170 implemented/contract-tested API rows. Preserve `live_tested: false`, all
UI/vision fields red, 92 ambiguous bulk rows red, `complete: false`, and the
singular DELETE exclusion. Do not add delete or bulk tools, PATCH, webhooks,
generic HTTP/browser controls, live tests, credentials, or cleanup claims.

The focused test suite must prove: flat schemas; outer extra-field rejection;
empty/mismatched IDs; opaque maps; mutation-free preview; exact canonical
request/effect; one HTTP write with encoded update path and singular-root body;
only the declared success root; malformed/missing required root errors; typed
401/404 propagation; empty-token no-I/O; ticket tamper, wrong-executor,
replay, and expiry handling; and one request only on a failed write. Run
formatting, Ruff, Pyright, coverage/policy checks, your focused tests, and the
full non-live commit suite before committing. Full qualification must remain
fail-closed because the project is incomplete.

Use a bare lowercase `fractal commit` summary for the product tip. Post its
commit hash, changed product files, tests, and exact coverage arithmetic to
your outbox. If blocked before edits by authentication, quota, or rate limit,
report the full-step retry condition; otherwise finish the scoped work yourself.

## Completion Requirements

1. A committed product tip contains exactly the bounded Wave-5l product files
   and no leaked generated evidence, credentials, screenshots, HARs, traces,
   or unrelated product changes.
2. The four frozen tools are real, registered, flat typed FastMCP tools that
   use the shared confirmation protocol and exact create/update paths and
   roots.
3. Focused tests cover every safety and response-mapping case specified above
   and pass; formatting, lint, Pyright, coverage/policy checks, and the full
   non-live suite pass from the committed worktree.
4. Generated coverage marks only `api.salesTaxPayments.create` and `.update`
   implemented and contract-tested, leaves live/UI/vision/delete/bulk red, and
   reports 246 API tools, 170 offline-green API rows, and `complete: false`.
5. A concise outbox report names the committed tip and verification results.
   Independent Grok product review is explicitly left to root after merge.
