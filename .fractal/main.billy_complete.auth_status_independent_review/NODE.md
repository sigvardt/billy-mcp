...
Perform an independent, defect-first Grok review of the committed typed
`auth_status` slice at parent commit `cf38b5c`. This is a review-only leaf:
you own `wiki/auth_status_independent_review.md` and may not change source,
tests, coverage inventories, or generated coverage status.

Read the approved design, `wiki/wave5t_ui_auth_discovery.md`, the relevant
official Billy documentation, and the implementation in
`src/billy_mcp/browser.py`, `models.py`, `server.py`, and their focused
tests. Cite official sources for every contract-dependent finding. Review the
following exact contract:

- fixed, headless navigation starts at `https://mit.billy.dk/` and accepts
  only an HTTPS `https://mit.billy.dk/login` final route with no credentials,
  query, or fragment;
- the login DOM requires exactly one visible email, password, and remember
  control and exactly one visible `button[data-cy=login-button]` whose trimmed
  label is exactly `Log in` or `Log ind`;
- do not accept a `button[type=submit]`, generic text selector, form
  inference, arbitrary URL, or generic browser control;
- expected anonymous state is typed `AUTH_REQUIRED`; signature/route drift is
  `UI_CHANGED`; policy loading failure is `EGRESS_DENIED`; unknown runtime
  failures are redacted `BILLY_ERROR`;
- the capability remains read-only and all UI coverage stays red. It has no
  live qualification, interface E2E, read-back, or vision verdict.

Run the focused tests and static checks appropriate for review. Inspect the
committed bytes from a clean archive before trusting executable results. Do not
use credentials, submit the login form, retain browser evidence, or make a
coverage/completeness claim. The review record must state reviewer identity,
commit, sources, commands/results, examined invariants, defects or explicit
no-defect verdict, and explicit non-qualification boundary.

If you find a defect, document exact file/line, consequence, and minimal
repair in the wiki record; notify the parent over radio at high priority. Do
not repair it yourself.

## Completion Requirements

- `wiki/auth_status_independent_review.md` is a precise, non-sensitive,
  cited Grok review record satisfying the instructions.
- The record explicitly keeps every UI coverage row red and makes no live,
  vision, UI-parity, or completeness claim.
- Focused tests and review static checks are recorded with their result.
- The wiki lints cleanly, the leaf commits only its review deliverable, posts a
  concise parent handoff, and finishes.
