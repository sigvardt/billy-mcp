...
Perform the policy-required Codex Power fallback inspection after the root Grok
reviewer failed before any review artifact because it was not authenticated.
This is a repository review only: it does not replace the mandatory cited Grok
independent-review gate, and it may not approve a candidate merge.
...
- `.fractal/main.billy_complete/tmp/grok-research.md` (the cited current
  official Billy API contract),
...
Inspect the exact three-dot diffs from root `5ab3efa` to these committed tips,
ignoring uncommitted child seed state:
...
checks. Verify focused tests and inspect typed tool schemas and response
mapping. Check the same fail-closed boundaries: preview is non-mutating,
execute accepts ticket-only input, tickets bind exact operations and are
single-use, writes are never retried, paths omit duplicate `/v2`, and no
sensitive values are logged. Specifically inspect the contact flat parameter
surface, contact-person update `id` minimum length, and declared multi-root
response/deleted-record mapping. Confirm no candidate mutates inventory,
coverage status, or live/UI/bulk qualification.

Publish the sole deliverable at
`wiki/wave5a_repaired_candidate_fallback_review.md`. It must give a
per-candidate findings/verdict table, exact commits and files reviewed,
reproducible check evidence, direct official-doc citations from the cited brief,
and an explicit statement that this fallback does not satisfy the required Grok
review and therefore cannot authorize a merge. Do not put screenshots, browser
frames, HAR files, credentials, or raw sensitive evidence in the repository.

## Completion Requirements

- `wiki/wave5a_repaired_candidate_fallback_review.md` records the exact
  candidate commits, reviewed files, reproducible checks, and any findings.
- The record explicitly preserves the mandatory Grok review gate and all
