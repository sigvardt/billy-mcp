...
Perform a fresh, independent **Grok** product review of the committed
Wave-5b P1/P2 repair at `a2a0996`. The review target is the committed root
source only; do not review parent uncommitted files, child seed files, or prior
fallback reports as product evidence.

You own exactly these project deliverables:

- `wiki/wave_fiveb_repair_independent_review.md`
- the generated row for that page in `wiki/_index.md`

Do not change production code, tests, coverage artifacts, inventories, status,
or any other wiki page. Do not green rows, claim live/UI/vision qualification,
or start Wave-5c. A REJECT result is a complete and useful review deliverable;
an ACCEPT is only an offline product-quality acceptance and never a completion
claim for the overall project.

## Review scope and contract

- Compare `a2a0996` to the previously integrated Wave-5b product `920ceab`.
- Use `docs/superpowers/specs/2026-07-28-billy-mcp-complete-design.md`,
  `wiki/wave_fiveb_ticketed_writes_contract.md`,
  `wiki/wave_fiveb_product_independent_review.md`, and current official Billy
  documentation at `https://www.billy.dk/api/` as the contract.
- Re-check and cite the official-doc fingerprint. Research only with the
  official documentation; never use credentials, production data, a visible
  browser, or non-Billy hosts.
- Work from a `git archive a2a0996` clean export for all executable evidence.
  Raw scratch evidence belongs in the owner-only node `tmp/` directory and must
  be purged before completion; the durable wiki page contains only commands,
  result counts, hashes, and non-sensitive conclusions.

## Required independent checks

1. Inspect the exact repair diff and all 24 registered write execute handlers.
   Confirm every preview binds a literal server-owned `execute_tool_name` and
   every execute path passes the matching literal to `WriteProtocolService`.
2. Independently reproduce or directly test a cross-module misuse: an
   account-create ticket supplied to a different execute handler must return
   `CONFIRMATION_MISMATCH` before HTTP and without consuming the correct ticket.
   Also inspect a same-module wrong-executor path.
3. Verify P2 lifecycle behavior: terminal prepared requests are discarded,
   expired pending bindings and replay markers are bounded, while direct expiry
   and replay errors remain typed and observable.
4. Run proportionate clean-export verification: focused P1/P2 tests, root tool
   registration/coverage checks, Ruff, Pyright, and the complete non-live
   suite. Record exact commands and result counts. Full qualification is
   expected to remain fail-closed without credentials and must not be treated
   as a failed offline review.
5. Confirm no sensitive evidence, credentials, raw browser material, or
   unintended product changes were introduced by this review.

## Required report

Write a single self-contained report at
`wiki/wave_fiveb_repair_independent_review.md` with frontmatter, a clear title,
reviewed commits, official-document citation/fingerprint, checks and outcomes,
an explicit **ACCEPT** or **REJECT** verdict, and precise follow-up gate.

- **ACCEPT** only if P1/P2 are closed on `a2a0996` and the product remains
  fail-closed for live, UI, vision, bulk, specials, and overall completeness.
  State that only then may the parent promote the already-researched Wave-5c
  brief to a shared freeze.
- **REJECT** must include actionable file/line or test-path findings and state
  that Wave-5c remains blocked.

Run `wiki update --path="$WIKI_DIR"` and `wiki lint --path="$WIKI_DIR"` after
writing the report. Post the verdict and commit hash to your outbox. If Grok
authentication, quota, or rate limit prevents the review before editing, post
that exact blocker to the parent and stop without a product verdict; do not
substitute a Codex review yourself.

## Completion Requirements

1. The sole report and its generated index row are committed, have clean wiki
   lint, and contain no sensitive material.
2. The report names `a2a0996`, compares it with `920ceab`, cites current
   official Billy documentation, and gives an explicit ACCEPT or REJECT.
3. It records independent evidence for every P1/P2 check above and states that
   all live/UI/vision/bulk/completeness categories remain unqualified.
4. The child has posted its verdict to the parent, purged raw review scratch,
   and left a clean worktree.
