...
Codex Power implementation.  You are a Grok research leaf.  Your only project
deliverable is `wiki/wave5u_method_probe_contract.md`; do not modify source,
tests, coverage manifests/status, scripts, dependencies, or other wiki pages.
...
candidate kind.  The contract must separate facts from inference and state,
for every kind, the fixed method, route replacement, query, body/content type,
and why it cannot produce a durable Billy change.  If any candidate lacks
official or reproducible non-production evidence for a safe real-method probe,
say so and require the future runner to block that candidate before network
traffic.  Never turn an unauthenticated 401, 405, empty OPTIONS 204, or
meta-only 200 response into a supported-operation or live-qualified claim.
...
- The deliverable is research/infrastructure only.  Residual 29, ambiguous
  bulk 92, and all UI rows remain red; coverage remains 184/184/0/0 and
  `complete: false`.
...
rules apply to any browser use.  If Grok authentication, quota, or a rate limit
prevents research before any project edit, stop without edits and report the
failure to the parent; the parent will retry the exact mission with Codex Power
per routing policy.
...
1. `wiki/wave5u_method_probe_contract.md` exists with valid wiki frontmatter,
   ...
   and a clearly stated non-qualification boundary.
