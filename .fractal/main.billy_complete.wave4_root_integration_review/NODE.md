...
Perform an independent Grok review of the committed Wave-4 root integration at
`main.billy_complete` commit `3f56399`. This is a read-only source and contract
audit: do not implement or modify product code, coverage state, tests, or
generated artifacts.

Treat the current official Billy API documentation and the frozen cited sources
as the contract: `https://www.billy.dk/api/`,
`wiki/wave_four_remaining_clear_reads_contract.md`, and
`.fractal/main.billy_complete/tmp/grok-research.md`. Recheck the official docs
fingerprint and record direct citations. Inspect the committed archive, not
uncommitted worktrees.

Audit all of the following:

1. The 50 new clear get/list tools correspond exactly to documented resources,
   routes, singular/plural response roots, paging, and the only approved extra
   filter (`countryId` for cities, states, and zipcodes).
2. `server.py` exposes exactly 94 `api_*` tools plus `coverage_status` and
   `coverage_report`; no unimplemented write, bulk, generic HTTP, or generic
   browser tool is exposed.
3. Generated inventory evidence marks only the 50 documented offline reads as
   implemented and contract-tested; live/UI/vision fields remain red and
   `coverage/status.json` remains `complete: false`.
4. Sensitive read diagnostic keys for bank account material, user email/phone,
   and reminder/file `downloadUrl` tokens are redacted; API host locking and
   typed error mapping remain intact.
5. The reported 487 non-live test result, lint/type/coverage/policy checks,
   and expected full-mode completeness-gate failure are reproducible from the
   archive. Never call live API endpoints, run a browser, use credentials, or
   create Billy records.

Write the non-sensitive, durable verdict to
`wiki/wave_four_root_integration_independent_review.md`. It must state PASS or
FAIL for the offline integration, separately state product completeness status,
list every concrete discrepancy with file/line evidence, state exact test
commands/results, preserve citations, and never claim live/UI/vision/bulk/write
qualification. If a discrepancy exists, leave the review verdict FAIL and
describe the minimal repair; do not fix it yourself.

Report the verdict to the parent through radio. Keep this review leaf limited
to the one wiki record and its normal node-close artifacts.

## Completion Requirements

- `wiki/wave_four_root_integration_independent_review.md` exists, contains a
  cited PASS or FAIL verdict for the committed archive, and explicitly records
  product completeness as incomplete unless every parent completion condition
  is independently proven.
- The record covers all five Wave-4 modules, root registration, generated
  evidence/status, redaction/error boundaries, and the offline/full-mode test
  posture without inventing live or UI evidence.
- The child has posted its verdict and concrete findings to the parent via
  radio, run wiki update/lint for its review page, and committed only its
  scoped wiki delivery before running `fractal node finish`.
