...
This is the routing-policy fallback after the planned Grok reviewer failed
before editing because its research runner was not authenticated. Perform the
full Wave-5i freeze-contract review with Codex Power, but label the resulting
record as a **non-authoritative fallback**. It cannot satisfy the mandatory
independent Grok acceptance or authorise product work.

Review the committed candidate `wiki/wave_fivei_ticketed_writes_contract.md`.
Your exclusive project-file ownership is
`wiki/wave_fivei_freeze_codex_fallback_review.md` plus the generated link row
in `wiki/_index.md`; do not edit production code, tests, coverage artifacts,
memory, the candidate contract, or any other wiki page.

Independently verify current official documentation at
`https://www.billy.dk/api/` where available, then reconcile the candidate with
`coverage/api_v2_manifest.yaml`, `coverage/status.json`,
`wiki/offline_write_probe_rules.md`, the accepted Wave-5i freeze-ready handoff,
and the Wave-5g contract pattern. Check the locked API base, the exact six
singular methods/paths/request roots/tool names, property-table boundaries,
documented authentication errors, empty additional plural roots, bulk
ambiguity, and the 401 POST/PUT plus missing-id DELETE-200 probe interpretation.

Issue an explicit fallback PASS or REJECT for the freeze text. A PASS must say
that it is not Grok authority and does not open the product gate; a REJECT must
identify every actionable discrepancy with source and remediation. Never
invent unpublished enum values, relationship serialisations, bulk bodies, or
live evidence. Preserve the candidate's fail-closed boundary for coverage,
product, live, UI, vision, binary files, webhooks, and generic controls.

Do not use production data, credentials, a browser session, or live API writes.
Do not retain frames, HAR, traces, tokens, or raw evidence. Use only the
official Billy documentation and committed local material. If current official
documentation cannot be rechecked, record that limitation and reject rather
than fabricating a pass.

Use `wiki update --path=$WIKI_DIR` and `wiki lint --path=$WIKI_DIR` after
writing the review page. Run the repository lint gate and proportionate static
checks. Post the fallback verdict to the parent through radio immediately.

## Completion Requirements

1. `wiki/wave_fivei_freeze_codex_fallback_review.md` is a durable,
   non-sensitive fallback review record with citations, exact scope, explicit
   PASS or REJECT, checks performed, and clear non-claims.
2. The record verifies the six singular sales-tax account/meta-field CUD rows,
   locked base and client paths, ticket boundary, bodyless delete, strict
   outer/opaque inner model rule, primary-root mapping, and bulk ambiguity.
3. The record clearly says it is a Codex Power fallback after a pre-edit Grok
   authentication failure; it neither replaces the Grok review nor opens product
   implementation or coverage green.
4. `wiki update`, `wiki lint`, `git diff --check`, and repository lint pass.
   No code, coverage, credentials, production data, raw browser evidence, or
   unrelated project change is introduced.
5. The child commits and pushes its review record, posts the verdict to the
   parent, and calls `fractal node finish` only after these requirements are
   satisfied. A REJECT is a valid completed fallback review.
