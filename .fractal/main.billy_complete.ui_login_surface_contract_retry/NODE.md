...
You are the authorized Codex Power retry of a Grok research leaf that failed
before task edits because the local Grok CLI was not authenticated. Complete
the same research-brief deliverable from the existing cited Grok evidence; do
not substitute uncited recollection for contract evidence. Your only project
deliverable is `wiki/ui_login_surface_contract.md`; do not modify application
source, tests, coverage inventories/status, package metadata, or non-wiki
documentation. Child seed files are allowed only to configure and run this
node.

Read the approved design, current source around `auth_status`,
`auth_login_start`, and `auth_login_wait`, existing UI/auth wiki pages, and the
current cited relay at
`/Volumes/ssd_1/Repositories/billy-mcp/.worktrees/main.billy_complete/.fractal/main.billy_complete/tmp/grok-research.md`.
Use its official Billy citations (canonical URL, retrieval timestamp, and
evidence note) in the deliverable. It is the required Grok-produced cited
brief for this contract-dependent work. Do not make a new UI exploration task,
do not submit credentials, and do not make application writes.

The brief must choose exactly one of these evidence-bounded outcomes without
inventing a generic browser wrapper:

1. An interface-only, stable `ui_*` read workflow genuinely distinct from
   existing shared `auth_*` tools; or
2. A shared auth/session requirement (such as organisation selection or session
   restoration) when a new `ui_*` tool would merely duplicate `auth_status`.

For the selected outcome specify its stable name, Pydantic input and typed
success/error output shape, scope, non-generic bounded workflow, exact DOM
selectors and assertions where applicable, headless test scenarios, independent
read-back, and the inventory row/status that must remain red until live and
vision evidence exists. State how a later Codex Power implementation can test
it offline without asserting an unobserved authenticated surface.

Verified relay evidence to preserve rather than expand: the unauthenticated
login page is `https://mit.billy.dk/login`; observed controls are
`input[type=email][name=email]`, `input[type=password][name=password]`,
`input[type=checkbox][name=remember]`, and `button[data-cy=login-button]`;
the visible label can be `Log in` or `Log ind`; there are no form elements and
the login button is not a `button[type=submit]` contract. Danish rendering
requires a Playwright `da-DK` locale plus `Accept-Language: da-DK,da`; a query
parameter alone was insufficient. The browser allowlist permits `mit.billy.dk`;
API transport stays at `api.billysbilling.com` and is never a browser target.

Explicitly document that post-login selectors, organisation selection, and
session restoration cannot be asserted without dedicated non-production
credentials. Give exact blockers, keep all unobserved live/vision flags red,
and never convert inaccessible or plan-gated work into complete coverage.
Do not persist screenshots, frames, HARs, traces, cookies, tokens, or customer
data. Do no residual/bulk API research and add no FastMCP tools or coverage
greens.

## Completion Requirements

1. `wiki/ui_login_surface_contract.md` exists with `desc:` frontmatter and a
   cited official-document/observed-surface brief containing the selected
   outcome and all contract details above.
2. The brief contains an explicit evidence-bounded decision between a distinct
   `ui_*` workflow and shared auth/session work; it does not recommend a generic
   browser action or duplicate existing `auth_status` without saying so.
3. The brief records precise unauthenticated selectors/assertions only when
   observed, exact blockers for authenticated state, and red live/vision
   inventory status for everything not evidenced.
4. No credentials, sensitive evidence, screenshots, frames, HARs, traces,
   cookies, source code, tests, coverage files, or non-wiki project artifacts
   are changed.
5. `wiki update --path=$WIKI_DIR`, `wiki lint --path=$WIKI_DIR`, and
   `bash $NODE_DIR/scripts/test.sh` pass. The committed project diff is limited
   to the wiki brief (aside from this node's own seed metadata).
6. Commit the work, publish a concise radio outbox finding, and run
   `fractal node finish --reason="published cited UI/auth login contract brief"`
   in the iteration that meets these conditions; do not wait for parent review.
