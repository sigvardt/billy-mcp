...
Perform the full repository-side fallback review for the Wave-5k bankPayments
product-ready handoff. Grok could not start because its environment is not
authenticated; this review must be explicitly labelled as a **Codex Power
fallback** and must never be represented as the mandatory Grok gate.

Own exactly one project file:
`wiki/wave_fivek_product_ready_research_codex_fallback_review.md`. Do not
modify the freeze contract, production code, tests, manifests, status, existing
wiki pages or indexes, or browser evidence. Node seed, plans, memory, and
owner-only scratch are operational files only. Do not spawn children.

The current official-document research is already cited and maintained by Grok
in the parent handoff and accepted pages; do not conduct fresh web research in
this Codex Power fallback. Inspect the repository and these sources:
...
- `coverage/api_v2_manifest.yaml`, `coverage/status.json`, and the parent
  research49 plan at
  `.fractal/main.billy_complete/plans/2026-07-30T03-46-00Z-2.49-wave5k_product_ready_research.md`.

Record an ACCEPT or REJECT of the handoff's offline implementation readiness.
Verify that it is bounded to `api.bankPayments.create` and `.update`, with four
ticketed tools, client-relative `POST`/`PUT /bankPayments`, singular
`bankPayment`, primary `bankPayments`, opaque payload maps, partial update
inner/route id equality, exact server-owned ticket executor binding, and no
write retry. Confirm that singular DELETE is excluded by the documented 405
finding and bulk stays ambiguous/red. Check the 238-to-242 registry and
166-to-168 offline coverage arithmetic and that live/UI/vision/bulk/completion
stay fail-closed.

State prominently that this result is an offline Codex fallback, not an
independent Grok research review, not freeze-page acceptance, and not product
implementation approval. No coverage may change. Run wiki lint and report the
verdict through outbox.

## Completion Requirements

- `wiki/wave_fivek_product_ready_research_codex_fallback_review.md` exists
  with valid frontmatter/`desc`, cited repository evidence, a clear ACCEPT or
  REJECT, and an explicit non-Grok/non-authoritative label.
- The review verifies or disputes the exact two-row/four-tool boundary, 405
  delete exclusion, opaque payload and ticket protocol constraints, arithmetic,
  and fail-closed state without overclaiming any live or UI qualification.
- The project diff outside this node seed contains only the owned review page;
  ...
  report the verdict via outbox before finish.
