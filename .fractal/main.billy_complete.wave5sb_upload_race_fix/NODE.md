...
Repair only the exact-byte binding gap in the merged Wave-5s-B binary upload
implementation at root baseline `98484f3`. You own exactly these production
paths: `src/billy_mcp/api/file_upload_writes.py` and
`tests/api/test_file_upload_writes.py`. Do not edit server registration, the
HTTP client, coverage inventory/status/generator, wiki, or unrelated tests.

The approved design §§8.3–8.4 and
`wiki/wave_fivesb_files_upload_product_ready_research.md` require an upload
ticket to bind the exact resolved local path and SHA-256 file digest. Current
execution validates an identity then calls `read_bytes()`, leaving a replacement
window. Fix it so the exact in-memory bytes passed to
`BillyHttpClient.post_file` are SHA-256 checked against the prepared ticket
digest immediately before transport. If contents changed after preview or
before that verified read, return `FILE_CHANGED` and do not make an HTTP call.
Bytes changed only after a verified read cannot alter the in-memory transport
content. Never persist or log file bytes.

Preserve all existing ticket safety semantics: preview has no HTTP; execute is
ticket-only and single-use; path containment and symlink escape remain
fail-closed; no retries are added for an upload; redaction remains intact.
Add deterministic focused regressions that prove a changed/replaced file cannot
reach the client, that matching bytes still reach it, and that existing behavior
remains intact. This is an offline safety repair only: do not change coverage
claims, live/UI/vision status, or invent API behavior. No external research,
live API use, browser use, arbitrary local access, or credentials are needed.

## Completion Requirements

1. The two owned files contain a minimal, reviewed exact-byte/digest binding
   repair with deterministic race-boundary regression coverage.
2. A mismatch causes `FILE_CHANGED` before `post_file`; matching bytes preserve
   the documented raw upload behavior and all existing ticket protections.
3. Run Ruff format/check, Pyright, the focused upload tests, coverage generation
   verification, and repository policy checks; report command results in the
   plan or outbox without sensitive values.
4. No file outside the assigned ownership is changed.
5. Commit the repair, post the commit and verification summary to the outbox,
   and call `fractal node finish --reason="..."` in the same iteration.
