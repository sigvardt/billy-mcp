...
Implement the bounded Wave-5s-C API product slice on this branch. You are the
Codex Power implementation owner; do not perform current official-document
research, interface exploration, vision review, or independent product review.
The parent has already committed the cited Grok handoff:
`wiki/wave_fivesc_research86_product_implementation_handoff.md`. Treat it,
the approved design, and the linked official `https://www.billy.dk/api/`
documentation as the contract. The handoff pins the current public-doc body to
MD5 `8b94b0135c91fd15fe54ea33e088a4be` and opens this offline-only product gate.

Implement exactly these typed FastMCP tools:

1. `api_invoices_send_email_preview` and
   `api_invoices_send_email_execute`
2. `api_invoice_deliveries_create_preview` and
   `api_invoice_deliveries_create_execute`

Exclusive production/test ownership:

- `src/billy_mcp/api/invoice_email_delivery_writes.py` (new)
- `tests/api/test_invoice_email_delivery_writes.py` (new)
- the import and registration of that module in `src/billy_mcp/server.py`
- rows `api.special.invoice_email` and `api.special.invoice_delivery` in
  `coverage/api_v2_manifest.yaml`, plus only generator-produced artifacts
  needed to keep coverage internally consistent.

Do not edit `file_upload_writes.py`, generic `write_protocol.py`,
`invoice_log_reads.py`, HTTP client/base-url policy, UI inventory, bulk rows,
or residual clear rows. Escalate any necessary expansion to the parent before
editing it.

Use a custom `ConfirmationStore` service patterned after the upload special;
do not try to force these operations through `WriteProtocolService`. Every
preview must do no HTTP and create one short-lived, single-use binding for the
matching execute tool. Its binding must contain the exact selected organisation,
invoice target, canonical request, and expected effect. Each execute function
accepts only the opaque ticket, validates and consumes the exact binding before
one POST, and does not automatically retry an external send.

Email contract:

- POST only to `/v2/invoices/{invoiceId}/emails` with `{"email": {...}}`.
- Strictly require `contactPersonId`, `emailBody`, and `emailSubject`; omit
  optional `copyToUserId` when absent.
- Do not accept or invent raw email destinations, multi-recipient input,
  destination URLs, webhooks, or a boolean approval argument.

Delivery contract:

- POST only to `/v2/invoiceDeliveries` with `{"invoiceDelivery": {...}}`.
- Strictly require invoice and organisation IDs, `receiverIdType`, and
  `senderUserId`; allow only `gln` or `cvr`.
- For `gln`, require a 13-digit `receiverGln`; never invent `receiverCvr`.
- Do not productise delivery get/list/update/delete or an invoice-nested route.

Use Pydantic models with `extra="forbid"` and strict meaningful string
validation for every public input and success output. Response roots are
provisional offline: map only the frozen root shape and return a typed stable
error for unknown/malformed envelopes. Ensure subjects and bodies are redacted
in errors and logged metadata. Keep both rows live-untested and all UI/vision
claims red.

Add a focused offline contract suite that verifies real FastMCP schemas and
tool names; preview performs no HTTP; exact paths, methods, and JSON bodies;
ticket expiry/tampering/replay/tool-org-target-payload mismatch; validation
(including GLN); typed invalid-envelope failures; base URL lock; no automatic
retry; and redaction of email subject/body. Update the two manifest rows only
after those tests are real and passing, with test references to the focused
suite. Do not mark product or overall coverage complete; expected offline
counts after this slice are 184 implemented/contract-tested and 0 live/vision.

Run the focused tests, all non-live project tests, format, Ruff, Pyright,
coverage consistency, repository policy, credential safety, and both wiki lints
before committing. Use `fractal commit` from this worktree so the commit is
pushed. Report completion and exact verification evidence in your outbox. A
fresh Grok product review is mandatory after the parent merges this branch;
your completion is not that review.

## Completion Requirements

1. The four requested API tools are real typed FastMCP registrations with
   strict Pydantic input and typed success/error outputs; there are no generic
   HTTP/browser controls, boolean approvals, raw destinations, webhooks, or
   out-of-scope delivery tools.
2. Preview/execute semantics bind the exact execute tool, organisation,
   invoice target, canonical request, expected effect, expiry, and one-use
   ticket. Execute accepts only the ticket, performs exactly one documented
   POST on success, and never retries the external send.
3. The focused contract suite covers request construction, schemas, ticket
   rejection paths, GLN and body validation, redaction, malformed responses,
   URL lock, and no-retry behaviour. It has no live credentials or test data.
4. The two manifest rows truthfully show `implemented: true` and
   `contract_tested: true`, reference the focused tests, retain
   `live_tested: false`, and preserve fail-closed overall coverage.
5. Focused tests, `BILLY_TEST_MODE=commit bash "$NODE_DIR/scripts/test.sh"`,
   `bash "$NODE_DIR/scripts/lint.sh"`, coverage consistency, repository
   policy, and both wiki lints pass from committed bytes. The worktree is clean
   and the source commit is pushed.
6. Memory states the delivered offline slice and its remaining independent
   Grok-review/live/UI limitations. The outbox reports the commit, tests, and
   exact remaining gate. Then run `fractal node finish` with an accurate reason.
