...
Perform the required fallback retry of the Wave-5f product review after Grok
failed to authenticate before any edits. You are Codex Power, not Grok: this is
a static, non-gating audit and may never replace the mandatory future Grok
verdict.

Own only `wiki/wave_fivef_product_codex_fallback_review.md`. Do not change
production code, tests, inventories, generated coverage, status, plans, or
other project wiki pages. Assess root merge `353449d` (child product `8c1cdf6`)
against the already cited official-doc research at
`.fractal/main.billy_complete/tmp/grok-research.md`,
`wiki/wave_fivef_ticketed_writes_contract.md`,
`wiki/wave_fivef_freeze_independent_review.md`, and the approved design.

Independently inspect and test the merged product:

1. Validate all six tax CUD operations, twelve literal preview/execute names,
   POST/PUT/bodyless DELETE paths and roots, strict outer inputs, update-id
   equality, and the locked Billy base URL.
2. Validate shared ticket semantics and failure order: server-owned executor
   name binding, single-use/expiry/tamper/replay/organisation/target/payload
   failures before extra HTTP, no retry, optional parent child-root mapping,
   and child non-fabrication.
3. Run Ruff, Pyright, tax-write tests, shared write-protocol and confirmation
   tests, registry/coverage tests, coverage generator/checker, and the
   commit-mode non-live suite from this clean worktree. Record commands and
   outcomes.
4. Confirm coverage honesty: exactly six tax rows advance offline, 190
   `api_*`, 142 implemented/contract-tested, zero live/vision, bulk red, and
   `complete: false`.

Write a durable page with an explicit static audit verdict and concrete
findings. It must say plainly that Grok could not authenticate, the audit is
non-gating, and Wave-5f remains pending a Grok product review even if all
offline checks pass. Do not use a browser, credentials, live tests, or retain
screenshots, frames, HAR files, traces, tokens, or customer data.

## Completion Requirements

1. The only project deliverable is
   `wiki/wave_fivef_product_codex_fallback_review.md`, with commit `353449d`,
   evidence sources, static verdict, commands/results, coverage-honesty result,
   and the explicit non-gating limitation.
2. No production or unrelated project wiki file is committed.
3. `wiki lint --path=$WIKI_DIR` passes and the page is committed before
