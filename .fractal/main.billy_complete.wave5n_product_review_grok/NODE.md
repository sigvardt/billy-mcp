...
Perform the independent Grok product review for the root-integrated Wave-5n
invoice-late-fee ticketed-write implementation. This is a read-only review of
committed root bytes plus current official Billy documentation; do not modify
production source, tests, coverage rows, status values, or prior contract pages.

The reviewed baseline is root commit `a3ad538`, which contains merge
`8c21d03` and the four product tools added by `8fab311`. The only project
deliverable you may author is
`wiki/wave_fiven_product_independent_review.md`. It must be a durable,
non-sensitive review record with frontmatter, a precise ACCEPT or REJECT, cited
official Billy evidence, reviewed commit, checked assertions, commands/results,
and fail-closed coverage boundaries. Use `wiki update` and `wiki lint` after
authoring it.

Read and verify the current contract sources before reaching a verdict:

- `docs/superpowers/specs/2026-07-28-billy-mcp-complete-design.md`
- `wiki/wave_fiven_ticketed_writes_contract.md`
- `wiki/wave_fiven_freeze_independent_review.md`
- `wiki/offline_write_probe_rules.md`
- `coverage/api_v2_manifest.yaml` and `coverage/status.json`
- `src/billy_mcp/api/invoice_late_fee_writes.py`, its server registration, and
  `tests/api/test_invoice_late_fee_writes.py`

Fetch current official docs from https://www.billy.dk/api/ and state a direct
citation plus the observed contract fingerprint or an evidence-backed contract
comparison. Do not use any non-Billy network endpoint, browser UI, credentials,
or headed browser. The only API base permitted in inspected code is
https://api.billysbilling.com/v2. Unauthenticated historical probes are evidence
only, not live qualification.

Independently verify all of the following:

1. Exactly four named tools exist: create/update preview and execute; no
   invoiceLateFees delete, bulk, webhook, generic HTTP, or browser tool exists.
2. POST `/invoiceLateFees` maps `invoiceLateFee` to `invoiceLateFees`; PUT uses
   the escaped `/:id` path with the same roots and accepts only an optional
   matching inner id.
3. Preview inputs reject unknown outer fields and preserve opaque JSON inner
   maps; executes are ticket-only and reject replacement business fields.
4. Shared confirmation tickets bind exact executor, payload, target, expected
   effect, organisation, and expiry; execution is single-use and makes one
   non-retried write.
5. Response mapping accepts only the required plural root and returns typed
   errors for malformed/missing roots and HTTP/auth failures.
6. The product tests cover the frozen schema, no-mutation preview, canonical
   binding, path escaping, tamper/wrong-executor/expiry/replay, and no retry.
7. Both inventory rows are honestly `implemented` and `contract_tested`; create
   cleanup says `live non-production cleanup strategy unqualified; singular
   DELETE is unsupported`; update remains restoration-only.
8. Root count is 254 `api_*` tools and 174 implemented/contract-tested rows;
   live and vision remain zero and `complete` remains false.
9. invoiceLateFees bulk save/delete remain empty-tool, ambiguous, and red.
10. Current docs still support create/update and do not justify a singular
    delete; existing JSON-body 401 and DELETE 405 evidence is accurately
    represented without claiming it as live testing.
11. No sensitive material, raw browser evidence, live mutation, or UI/vision
    claim is introduced.
12. Root focused invoice-late-fee/coverage/registry checks and the non-live
    suite remain passing.

An ACCEPT may accept only this offline product slice. It must explicitly retain
red live API, all UI/vision, all ambiguous bulk, singular delete, and overall
completeness. A REJECT must identify exact file/line or reproducible evidence;
do not soften coverage to create an acceptance.

If Grok authentication, quota, or rate limits prevent documentation review
before you edit the review page, report that blocker by outbox and stop without
edits so the parent can run the required Codex Power fallback. Do not switch
reviewer route yourself. Do not spawn children.

## Completion Requirements

1. `wiki/wave_fiven_product_independent_review.md` is committed and lint-clean,
   contains a cited ACCEPT or REJECT, and identifies root `a3ad538` as the
   reviewed product baseline.
2. The review covers every assertion in the Instructions and distinguishes
   current official-doc evidence, historical unauthenticated probe evidence,
   offline contract tests, and unqualified live/UI/vision work.
3. The review leaves all code and coverage files untouched and contains no
   credentials, raw frames, traces, HAR files, or sensitive customer data.
4. Focused invoice-late-fee, coverage, and registry tests plus the root
   non-live test command pass before an ACCEPT; a REJECT includes a reproducible
   failed assertion instead.
5. An ACCEPT does not claim live testing, interface parity, vision verification,
   singular delete, bulk qualification, webhook support, or product completeness.
6. Report the verdict and exact evidence through the outbox, then run
   `fractal node finish --reason="Wave-5n product review <verdict>"` only after
   the above deliverable and checks are complete.
