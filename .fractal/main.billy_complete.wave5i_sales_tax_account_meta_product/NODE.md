...
Implement the accepted offline Wave-5i product: twelve ticketed typed FastMCP
tools for the six singular `salesTaxAccounts` and `salesTaxMetaFields` CUD
operations. You are the Codex Power implementation lane. Do not conduct fresh
external research; the parent supplied the cited official-doc handoff and
accepted freeze contract.

Contract authority, in order:

1. `docs/superpowers/specs/2026-07-28-billy-mcp-complete-design.md`;
2. `wiki/wave_fivei_ticketed_writes_contract.md`;
3. `wiki/wave_fivei_freeze_independent_review.md`;
4. parent research summary at `.fractal/main.billy_complete/plans/2026-07-30T00-33-00Z-2.39-wave5i_product_ready_research.md`;
5. existing accepted patterns in `sales_tax_writes.py`, `attachment_writes.py`,
   their tests, and the shared write protocol.

Exclusive product ownership:

- create `src/billy_mcp/api/sales_tax_account_meta_writes.py`;
- adjust only the registration import/call in `src/billy_mcp/server.py`;
- create `tests/api/test_sales_tax_account_meta_writes.py` and
  `tests/api/test_sales_tax_account_meta_cross_executor.py`;
- update `tests/unit/test_coverage_server.py`,
  `scripts/generate_coverage_report.py`, and generated coverage artifacts only
  for these six rows.

Do not alter `sales_tax_writes.py`, `attachment_writes.py`, the shared write
protocol, bulk rows, UI/auth/browser modules, or unrelated coverage rows unless
you prove a directly blocking defect and ask the parent first.

Tool surface (preview + execute for each):

- `api_sales_tax_accounts_{create,update,delete}_{preview,execute}`;
- `api_sales_tax_meta_fields_{create,update,delete}_{preview,execute}`.

Use strict outer Pydantic models with flat FastMCP schemas and camelCase Billy
parameter aliases. Keep the inner `salesTaxAccount` and `salesTaxMetaField`
maps opaque `JsonValue` dictionaries: do not invent enum values, relation wire
forms, ownership encodings, or required fields beyond the singular root. For
updates, reject an inner `id` that differs from route `id`. Deletes are id-only
previews and bodyless executions.

Each server-known `WriteOperationSpec` must bind its exact non-empty execute
tool name, use the root's shared `ConfirmationStore` and
`WriteProtocolService`, client-relative `/salesTaxAccounts` or
`/salesTaxMetaFields` paths, the stated singular/plural roots, and
`additional_plural_roots=()`. Execute accepts only a confirmation ticket;
there is no boolean approval, retry, bulk, generic HTTP, webhook, or live call.
Executor mismatch must fail before ticket consumption and HTTP.

The offline test suite must prove flat schema strictness, preview non-mutation,
exact POST/PUT/bodyless-DELETE request construction and path encoding, primary
root mapping, optional deleted metadata, no fabricated sibling roots, typed
auth/not-found mapping, no retry, and ticket tamper/expiry/replay/organisation/
target/payload/effect/executor protections. Add same-resource and cross-resource
executor tests through the root single-store server path.

Only after real offline tests pass may the generator set exactly these six rows
to `implemented: true` and `contract_tested: true`; all six remain
`live_tested: false`. Expected post-change state is 220 registered `api_*`
tools, 157 offline implemented/contract-tested API rows, 52 clear red CUD
rows, zero live/vision rows, 92 ambiguous bulk rows, and `complete: false`.

Run the focused tests and repository format/lint/type/coverage-policy/full
non-live suite. Keep the public tree free of credentials, browser evidence,
HAR/traces, or customer data. Report the tested commit, changed files, and
remaining fail-closed boundary to the parent outbox. A later parent-owned Grok
review determines product acceptance; do not claim it yourself.

## Completion Requirements

1. The new module registers exactly the twelve specified preview/execute tools
   with real typed input and typed success/error outputs, bound to the shared
   ticket protocol and exact client-relative Billy methods/paths/roots.
2. Offline suites cover all six operations and every stated protocol, request,
   response, error, and executor-binding boundary using a locked mock transport;
   they are neither stubs nor live-result mocks.
3. The root server registers 220 `api_*` tools and no tool name is duplicated;
   the existing 208-tool surface remains present.
4. The coverage generator and artifacts advance exactly the six Wave-5i CUD
   rows to implemented plus contract-tested, preserve live/UI/vision/bulk
   fail-closed state, and report 157 offline API rows with `complete: false`.
5. Formatting, Ruff, Pyright, coverage/policy checks, focused tests, and the
   full non-live repository suite pass from this child worktree.
6. No credentials, production writes, live qualification claims, browser
   evidence, or unrelated product changes are introduced.
7. The child commits and pushes its clean product branch, posts a concise
   handoff with verification evidence, updates its private memory to terminal
   state, and runs `fractal node finish` for this bounded implementation
   deliverable. It must not wait for or claim the parent-owned Grok review.
