...
Perform an independent Grok review of the Wave-3 documented read-only API
integration on this branch. You own only `wiki/wave_three_independent_review.md`.

Read the approved design, the current official Billy API documentation, the
cited Wave-3 contract at `wiki/wave_three_unfiltered_reads_contract.md`, the
five Wave-3 source modules and their tests, `src/billy_mcp/server.py`, and the
generated API coverage inventory. Independently verify all 22 rows for
`invoiceLines`, `billLines`, `daybookTransactionLines`, `contactPersons`,
`daybooks`, `daybookBalanceAccounts`, `accounts`, `accountGroups`,
`accountNatures`, `files`, and `attachments`.

For each reviewed resource, assess official route/method, get/list inputs,
pagination and sorting, roots, typed output/error handling, server registry,
coverage evidence, and sensitive-value redaction. Cite current official
documentation URLs and distinguish verified facts from inferences. Do not
modify production code, tests, generated coverage, credentials, or raw browser
evidence. Do not mark live or UI coverage complete: credentials and interface
qualification are outside this review.

Write a concise, non-sensitive wiki report with frontmatter `desc:`. State the
branch revision reviewed, source URLs, review method, the 22-row result, and
actionable findings classified as required, blocker, or advisory. Send every
required discrepancy to the parent by radio immediately; do not wait for a
reply. If the review finds no required discrepancy, explicitly say so in the
report and outbox.

## Completion Requirements

1. `wiki/wave_three_independent_review.md` exists, has a description, contains
   cited current official-document evidence, and records results for all 22
   Wave-3 rows.
2. The report has no credentials, personal data, screenshots, frames, HAR
   files, or traces, and it does not overstate live or UI qualification.
3. Any required discrepancy has been sent to the parent radio inbox with exact
   affected row and evidence; otherwise the outbox states there are no required
   discrepancies.
4. The node commits only its wiki report, runs its non-live test and lint
   scripts successfully, then finishes with a reason naming the report.
