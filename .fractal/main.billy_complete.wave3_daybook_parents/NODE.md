...
Implement only the Wave-3 daybook-parent read cluster from
`wiki/wave_three_unfiltered_reads_contract.md` and the cited Grok brief.

Own exactly `src/billy_mcp/api/daybook_reads.py` and
`tests/api/test_daybook_reads.py`. Deliver real typed get/list tools for
daybooks and daybookBalanceAccounts (four tools total) with a module-local
registration function. Root alone owns `server.py`, shared redaction, coverage
evidence/manifests, and wiki updates.

Use only documented paths `/daybooks`, `/daybooks/{id}`,
`/daybookBalanceAccounts`, and `/daybookBalanceAccounts/{id}`; their matching
singular/plural roots; URL-encoded IDs; and optional paging. Lists accept only
page, pageSize, include, free sortProperty, and ASC/DESC sortDirection.
Reject offset, daybookId, organizationId, and every unknown input. Reuse the
existing typed 401 behavior and opaque payload pattern.

Do not write any other files or implement writes, bulk, UI, live calls, or
generic controls. Commit the scoped delivery and report it to parent radio.

## Completion Requirements

- The owned module has four real typed tools and focused tests cover paths,
  query encoding, URL encoding, response roots/paging, rejected invented
  filters, and typed 401 behavior.
- Focused pytest, Ruff format/check, and Pyright pass from committed bytes.
- Scoped work is committed and reported to parent without any coverage/live/
  completeness claim.
