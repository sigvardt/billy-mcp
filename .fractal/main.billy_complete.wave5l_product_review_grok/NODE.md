...
- `src/billy_mcp/api/sales_tax_payment_writes.py`
...
- generated `coverage/api_v2_manifest.yaml`, `coverage/status.json`, and
  `coverage/report.md` when changed by the generator.

The frozen contract is `wiki/wave_fivel_ticketed_writes_contract.md`, whose
independent freeze acceptance is
`wiki/wave_fivel_freeze_independent_review.md`. Check current official Billy
documentation yourself and cite the exact page in the review. Existing cited
evidence records the unchanged ETag `hsisik4g9p3603`, MD5
`c2efda0ee4cf9cf200e14910c5fc6996`, and 147934-byte body; unauthenticated
salesTaxPayments POST/PUT are 401 while singular DELETE is 405.

Required review checks:

1. Confirm exactly four registered `api_sales_tax_payments_*` create/update
   preview/execute tools, client-relative POST/PUT paths, strict flat outer
   schemas, opaque inner map, optional update-body/id equality, correct
   plural response root, one shared confirmation store/protocol, exact ticket
   binding, one write/no retry, typed errors, and no generic HTTP/browser
   controls.
2. Confirm delete and bulk tools are absent, singular DELETE remains excluded,
   live/UI/vision/bulk stay red, live counts remain zero, and
   `complete: false`. The salesTaxPayments-create cleanup field must explicitly
   say that singular DELETE is unsupported and the live cleanup strategy is
   unqualified.
...
3. The review changed no production code, coverage row state, or unrelated
   wiki content; it contains no credentials, raw frames, HARs, traces, or
   sensitive data.
