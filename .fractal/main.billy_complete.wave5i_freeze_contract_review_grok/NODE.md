...
Perform an independent Grok review of the committed Wave-5i freeze contract at
`wiki/wave_fivei_ticketed_writes_contract.md`. This is an external-document and
contract review only. Your exclusive project-file ownership is
`wiki/wave_fivei_freeze_independent_review.md` plus the generated link row in
`wiki/_index.md`; do not edit production code, tests, coverage artifacts,
memory, the candidate contract, or any other wiki page.

Start from the committed parent baseline containing the candidate contract. Use
current primary official documentation at `https://www.billy.dk/api/`, not
memory or earlier reviews alone. Produce cited findings that independently
verify the locked API base, exact six singular methods/paths/request roots and
tool names, property-table boundaries, documented authentication errors,
empty additional plural roots, bulk ambiguity, and the 401 POST/PUT plus
missing-id DELETE-200 probe interpretation. Reconcile the candidate with
...
and the Wave-5g contract pattern.

Issue an explicit **ACCEPT** or **REJECT** for the freeze page only. ACCEPT is
allowed only if the candidate is current-doc accurate, preserves ticket and
response-mapping safety, and makes no premature coverage, product, live, UI,
vision, bulk, file-upload, or webhook claim. REJECT must identify each
actionable discrepancy with a precise source and remediation; do not repair the
candidate yourself. In either verdict, state that product work remains blocked
until this freeze gate is accepted and a separate product-ready research handoff
exists.

Never use production data, credentials, a browser session, or a visible window.
Do not run live API writes. Do not retain raw web captures, frames, HAR, traces,
or credentials. Review only official Billy documentation and committed local
files; no generic HTTP/browser tools or unrelated network destinations.
...
checks. Report the result and any authentication/quota/rate-limit failure to
the parent through radio immediately. If external research cannot run before
any edit, do not fabricate a verdict or modify project files.

## Completion Requirements

1. `wiki/wave_fivei_freeze_independent_review.md` is a durable, non-sensitive
   Grok review record with source citations, the exact reviewed scope, explicit
   ACCEPT or REJECT verdict, checks performed, and clear non-claims.
2. The review verifies all six singular sales-tax account/meta-field CUD rows
   against current official documentation and the committed inventory; it does
   not infer a bulk request contract or unpublished enum/relationship semantics.
3. The review explicitly confirms the two-step ticket requirements, locked API
   base, no duplicated `/v2`, bodyless delete, strict outer/opaque inner model
   rule, and no fabricated sibling response roots.
4. `wiki update`, `wiki lint`, `git diff --check`, and the repository lint gate
   pass for the committed review record. No secret, browser evidence, API token,
   raw capture, production data, product code, coverage mutation, or unrelated
   change is introduced.
5. The child commits and pushes its review record, posts its verdict to the
   parent, and calls `fractal node finish` only after the requirements above are
   satisfied. A documented REJECT is a valid completed review; an unavailable
   Grok research route is not.
