...
Implement only the Wave-3 chart-of-accounts read cluster from the frozen
contract at `wiki/wave_three_unfiltered_reads_contract.md` and cited Grok
brief.

Own exactly `src/billy_mcp/api/account_reads.py` and
`tests/api/test_account_reads.py`. Deliver real typed get/list tools for
accounts, accountGroups, and accountNatures (six total), with a module-local
registration function. Root owns `server.py`, all shared redaction work,
coverage evidence/manifests, and wiki files.

Use documented paths and exact singular/plural roots, URL-encoded IDs,
opaque payload models, optional paging, and existing typed 401 behavior. List
inputs admit only page, pageSize (1-1000), include, free sortProperty, and
ASC/DESC sortDirection. Reject offset and every resource-specific parent or
organization filter. Account bank fields must stay opaque and must never be
printed in tests or logs; root handles generic redaction coverage.

Do not implement writes, bulk, UI, live calls, generic controls, or edits
outside the two scoped files. Commit and report delivery through parent radio.

## Completion Requirements

- Owned files provide six real typed tools and tests covering each resource's
  paths, query construction, ID encoding, roots/paging, rejected unknown
  inputs, and typed 401 behavior.
- Focused pytest, Ruff format/check, and Pyright pass on committed bytes.
- Scoped work is committed and reported to parent; no coverage, live, or
