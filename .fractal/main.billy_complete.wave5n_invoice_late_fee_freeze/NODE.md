...
Author the single frozen Wave-5n contract page
`wiki/wave_fiven_ticketed_writes_contract.md` from the cited research60 package
at `.fractal/main.billy_complete/tmp/grok-research.md`. This is offline
contract authoring only. Do not browse, write source code or tests, register
tools, alter coverage/inventory/status, or make live, UI, vision, bulk, or
overall-completeness claims.

The page freezes exactly `api.invoiceLateFees.create` and
`api.invoiceLateFees.update`: later preview/execute tools are
`api_invoice_late_fees_create_preview` / `_create_execute` and
`api_invoice_late_fees_update_preview` / `_update_execute`; client-relative
paths are `POST /invoiceLateFees` and `PUT /invoiceLateFees/:id`; singular root
is `invoiceLateFee`; required success root is `invoiceLateFees`.

It must cite the official-doc access fingerprint `juf598rs793603` /
`f3925615c452b34694abb7f2856e845a` / 147934 bytes, explain that the stripped
plain contract matches the earlier inventory lock, and cite the accepted
Wave-5m product review on root. State the strict outer create/update inputs,
opaque inner map, optional body-id equality, execute-ticket-only input, one
shared ConfirmationStore plus WriteProtocolService, one write/no retry, and
required-root-only response mapping.

Document the official field boundaries: invoice, entryDate, flatFee, and
percentageFee are immutable/required; createdTime and amount are readonly;
isVoided is an unproven update candidate. Do not invent a relation wire form,
payload schema, date/number encoding, accepted update semantics, cleanup, or
additional response roots.

The unauthenticated POST and PUT probes were 401; singular DELETE and probed
bulk DELETE were 405. Exclude singular delete and both bulk operations; retain
the two bulk inventory rows empty-tool red. Preserve 250 api tools, 172 offline
implemented/contract-tested rows, zero live/vision rows, and complete false.
No webhooks are documented.

Only own the named page and generated wiki index entries. Do not touch the
Wave-5m fallback-review page owned by its active reviewer. Report completion or
any contract ambiguity to the parent by radio.

## Completion Requirements

- A committed `wiki/wave_fiven_ticketed_writes_contract.md` precisely records
  the two authorised rows, four later tool names, request/response roots,
  ticket protocol, field limits, 401/405 evidence, and all exclusions.
- The page explicitly says it authorises no product implementation or coverage
  greening and that a separate independent Grok freeze review is required before
  any product child can start.
- `wiki lint --path=$WIKI_DIR` passes, the child worktree is clean, and the
  parent has received a concise completion report.
