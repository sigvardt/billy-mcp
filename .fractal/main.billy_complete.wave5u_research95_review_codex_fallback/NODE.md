...
You are the `codex-power` fallback for the failed Grok research95 review. The
Grok client failed before any review edit because it was not authenticated, so
repeat the review task fully but label the result accurately as a **non-
authoritative Codex fallback review**, never a Grok independent review or a
gate substitute.

Own only the new project record
`wiki/wave_fives_research95_codex_fallback_review.md`; do not edit source,
tests, coverage, existing wiki pages, or any other child’s files.

Review the committed research95 promotion:

- `wiki/wave5u_method_probe_contract.md`
- `wiki/wave_fives_residual_specials_research.md`
- `.fractal/main.billy_complete/plans/2026-07-30T23:12:00Z-research95-bulk-delete-form-matrix.md`
- `coverage/status.json`, `coverage/api_v2_manifest.yaml`
- `src/billy_mcp/live_probe.py`, `tests/unit/test_live_probe.py`

Use only cited primary official Billy documentation and reproducible repository
evidence. Check the locked API base, no-webhook conclusion, exact `ids[]`
candidate query key, empty-form `INVALID_DELETE_ID_ARRAY` boundary, and why an
unauthenticated synthetic-id metadata-only 200 cannot establish live safety.
Confirm coverage still reports 184 implemented/contract-tested, zero live and
vision, and `complete: false`, or record concrete discrepancies.

The fallback record must preserve the hard boundary: no residual/bulk real
method, FastMCP tool, coverage greening, or UI/live claim is authorised. It may
assess whether the limited harness encode is consistent with the cited research,
but may not approve an implementation diff or replace the required Grok review
once Grok authentication is available. Do not create business data, submit
credentials, use a visible browser, retain sensitive evidence, or change
coverage.

Run wiki lint and the node lint/test scripts. Report the fallback verdict,
verification evidence, and the authentication-limited provenance through the
outbox. Keep private memory current before finishing.

## Completion Requirements

1. Exactly one project deliverable is added:
   `wiki/wave_fives_research95_codex_fallback_review.md`; all other project
   files remain unchanged by this leaf.
2. It states an explicit ACCEPT or REJECT **non-authoritative Codex fallback**
verdict; it neither claims Grok authorship nor replaces a Grok audit.
3. It verifies the cited research boundaries and explicitly keeps residual 29,
bulk 92, live, vision, and UI rows red, authorising no real method or tool.
4. `wiki lint --path=$WIKI_DIR`, `bash $NODE_DIR/scripts/lint.sh`, and
   `bash $NODE_DIR/scripts/test.sh` pass.
5. The branch contains the fallback-record commit and an outbox report that
   documents the Grok authentication failure and no-coverage-change result.
