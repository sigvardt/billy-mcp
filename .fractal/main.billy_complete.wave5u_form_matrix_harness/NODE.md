...
Implement the bounded Wave-5u bulk-delete form-matrix harness slice. You are a
`codex-power` implementation leaf. Your exclusive project-file ownership is:

- `src/billy_mcp/live_probe.py`
- `tests/unit/test_live_probe.py`

The parent committed research95 and its harness plan before this node was
created. Read `wiki/wave5u_method_probe_contract.md`,
`wiki/wave_fives_residual_specials_research.md`, and the current implementation
and tests before editing. Treat the cited official-doc research in those pages
as the contract.

Research95 facts to encode as fail-closed infrastructure only:

- The frozen bulk-delete query name is exactly `ids[]`.
- Empty `ids[]`, bare ids, and empty JSON/plural-array variants return
  `INVALID_DELETE_ID_ARRAY` unauthenticated. They are validation errors, never
  a safe no-op probe.
- A synthetic non-empty id can yield unauthenticated metadata-only 200, but
  that does not prove authenticated non-persistence, safe cleanup, or a live
  wire contract.
- Every residual and bulk real method remains block-before-network without the
  separately reviewed non-persistence, dual-organisation, owner-only-evidence,
  cleanup, and independent read-back gates. OPTIONS remains unqualified
  infrastructure, never proof of method support.

Prefer a minimal typed, testable representation or explicit invariant that
makes these rules hard to regress. Preserve the fixed API base and redaction
rules. Keep candidate construction deterministic and leave the current
options-only observation behaviour unchanged unless a fail-closed correction is
strictly required.

Hard exclusions: no `api_*` residual/bulk tool, no preview/execute tool, no
coverage manifest/status edit, no browser/auth edit, no webhook, no real API
call, no credentials, no live-test flag, and no broad refactor. Do not edit any
other project file; ask the parent by radio if a necessary change appears
outside your two owned files.

Run the focused tests and the node test/lint scripts. Report the exact files,
tests, and confirmation that the red coverage posture is untouched through your
outbox. Fold any durable implementation boundary into memory before finishing.

## Completion Requirements

1. Only the two owned project files are changed by this leaf; no coverage,
   FastMCP registration, browser/auth, or credential-bearing file changes.
2. Focused unit tests prove the exact `ids[]` candidate invariant, empty-form
   error/non-no-op semantics, and that unauthenticated metadata-only success
   cannot qualify or open a residual/bulk real network method.
3. Existing options-only observations remain unqualified and redacted; default
   offline test execution introduces no network traffic.
4. `bash $NODE_DIR/scripts/lint.sh` and `bash $NODE_DIR/scripts/test.sh` pass,
   and the focused live-probe tests pass.
5. The branch contains a clear implementation commit and an outbox report with
   the test evidence and no-coverage-change confirmation.
