...
Implement only the Wave-3 file-and-attachment read cluster from the frozen
contract at `wiki/wave_three_unfiltered_reads_contract.md` and cited Grok
brief.

Own exactly `src/billy_mcp/api/file_attachment_reads.py` and
`tests/api/test_file_attachment_reads.py`. Deliver real typed
`api_files_get/list` and `api_attachments_get/list` tools using the established
read-only service pattern with a module-local registration function. Root owns
server registration, redaction, coverage evidence/manifests, and wiki files.

Use `/files` and `/attachments` collection/item paths, matching singular and
plural roots, URL-encoded IDs, opaque payloads, optional paging, and existing
typed 401 mapping. List inputs permit only page, pageSize (1-1000), include,
free sortProperty, and ASC/DESC sortDirection. Reject offset and invented
filters. Never fetch `downloadUrl`, reveal URL tokens, implement upload, or
add a generic file/local-path capability.

Do not implement writes, preview/execute, bulk, UI, live calls, or scope
outside the two owned files. Commit the scoped delivery and report it to parent.

## Completion Requirements

- The module exposes four real typed tools. Focused tests cover paths, IDs,
  query surface, response roots/paging, rejected unknown inputs, and typed 401
  behavior without recording token-bearing URLs.
- Focused pytest, Ruff format/check, and Pyright pass from committed bytes.
- Scoped files are committed and reported to parent; no root-registration,
  coverage-green, live, upload, or completeness claim is made.
