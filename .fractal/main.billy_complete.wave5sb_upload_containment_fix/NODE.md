...
Repair the Wave-5s-B file-upload execute-time containment TOCTOU on the
committed parent baseline. The parent has a deterministic, local
`httpx.MockTransport` reproduction: after execute's identity validation, a
same-digest file path can be replaced with a symlink to an outside-root file;
the current `Path.read_bytes()` follows it and permits one POST. The transient
buffer SHA-256 check is necessary for byte integrity but does not prove
configured-root containment.

This is repository-local security implementation, so work only as
`codex-power`; do not perform current-document research or live/UI work. The
official Billy wire contract, tool names, inventory, and coverage surfaces are
already frozen. Do not add or rename tools, change request/response mappings,
touch the API base/headers, update coverage rows, or make completeness claims.
The required Grok product audit is a parent-level gate and must remain open.

Exclusive production ownership:

- `src/billy_mcp/api/file_upload_writes.py`

Exclusive focused-test ownership:

- `tests/api/test_file_upload_writes.py`

Do not modify `client.py`, `server.py`, `config.py`, generic write protocols,
confirmation-store semantics, inventory/status files, docs contract pages,
email/delivery code, UI code, or any other test module. Escalate any need to
touch them to the parent through radio.

Implement a configured-root-aware, no-follow descriptor traversal for execute:

1. Traverse from an allowed canonical upload root using descriptor-relative
   opens. Every directory component must be directory/no-follow validated and
   the terminal file must be opened with no-follow semantics; never reopen a
   user-controlled path after validating it.
2. Obtain regular-file metadata from the acquired descriptor and compare it
   against the preview ticket's exact identity. Read the transport buffer from
   that same descriptor, SHA-256 it, compare it to the ticket digest, and send
   only that verified buffer.
3. A post-identity symlink escape, including one whose bytes equal the ticket
   digest, must return `FILE_NOT_ALLOWED` and issue no HTTP request. A changed
   permitted file must still return `FILE_CHANGED` and issue no request.
4. Preserve the ticket lifecycle: consume/discard before file work as today;
   no retries or file bytes retained/logged; maintain existing raw binary,
   allowlisted-header, redaction, and strict FastMCP input behavior.

Add deterministic local regressions for the same-digest outside-root symlink
replacement in the identity-to-open window (zero requests), and for any
intermediate component race your traversal must reject. Keep and pass the
existing changed-byte replacement, pre-execute containment, normal raw-body,
ticket, replay, expiry, and redaction tests. Store any raw local evidence only
under `tmp/`; do not use credentials, browsers, or network API traffic.

Report an exact source/test summary and test outcomes in the outbox. If a
review step cannot use Grok because of pre-edit authentication, perform the
full scoped fallback review with Codex Power and label it non-authoritative;
do not claim it closes the mandatory parent-level Grok product audit.

## Completion Requirements

1. The exact same-digest outside-root symlink replacement is rejected before
   HTTP through descriptor-level configured-root containment, with a focused
   deterministic test proving zero requests.
2. The implementation reads and hashes the exact descriptor-acquired buffer;
   it preserves changed-file, non-regular-file, ticket, raw-binary,
   allowlisted-header, no-retry, and redaction behavior.
3. The diff is limited to the two owned source/test paths plus this node's
   private plan/memory configuration. No tool, coverage, live, UI, vision,
   bulk, or product-acceptance state is changed.
4. `bash $NODE_DIR/scripts/lint.sh`, the focused upload tests, and
   `bash $NODE_DIR/scripts/test.sh` pass. Run the focused tests from a clean
   `git archive` checkout before claiming the leaf ready.
5. Publish a non-sensitive outbox report and finish the leaf only after its
   code/test work is committed. The report must state that the parent still
   needs an authoritative Grok product audit before Wave-5s-C can start.
