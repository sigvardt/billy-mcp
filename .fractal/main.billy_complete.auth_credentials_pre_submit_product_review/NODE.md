...
Perform an independent Grok product review of the merged offline authentication
pre-submit slice at parent baseline `a9c7a9b`. You own a single durable,
non-sensitive review record in `wiki/` and no product, test, coverage, or
configuration code. This is a product-quality review of an offline,
credential-absent implementation; it is not a live API, UI workflow, vision,
or completeness review.

Treat the following as the contract, in this order:

1. `docs/superpowers/specs/2026-07-28-billy-mcp-complete-design.md` §§6–7,
   10–11, and 15;
2. current official Billy documentation at `https://www.billy.dk/api/`, which
   you must re-fetch and cite before a contract claim;
3. `wiki/auth_credentials_pre_submit_research.md` (research92/93 freeze),
   `wiki/wave_fives_research93_independent_review.md` (the earlier rejection),
   and the parent scratch research94 brief;
4. the exact parent merge `06fb108` and committed parent baseline `a9c7a9b`.

Independently inspect a clean `git archive a9c7a9b`; do not rely on a mutable
worktree. Confirm the reviewed diff has only opaque credential-store references
and purpose-built `auth_status`, `auth_login_start`, and `auth_login_wait`
tools; no generic browser, selector, navigation, HTTP, shell, or dynamic-code
tool; no secret-bearing model, fixture, output, exception, log, coverage row,
or raw browser artifact. Verify the browser is forcibly headless and
allowlisted, the exact URL and `Log in`/`Log ind` signature are checked before
credential resolution and again before fill/click, remember-me remains
unchecked, unknown state fails closed, errors are redacted, and pages close.
Legacy label `Login` must return `UI_CHANGED` before resolver, fill, or click.

Run a focused non-live evidence check from the archive or parent baseline,
including the auth unit/config/model tests and the documented lint/safety
checks. Do not submit credentials, open a visible browser, create Billy data,
or run residual/bulk methods. Do not treat CAPTCHA bootstrap configuration as
an observed challenge. Do not green API or UI coverage.

Write `wiki/auth_credentials_pre_submit_product_independent_review.md` with a
cited **ACCEPT** or **REJECT** verdict, exact commit reviewed, checks run,
bounded scope, unresolved blockers, coverage status, and evidence that no raw
browser artifacts were retained. Record an ACCEPT only for this offline product
slice; explicitly state that live, UI, vision, organisation, TOTP,
reauthentication, residual/bulk, and completeness gates remain open. A REJECT
must name concrete reproducible defects for a focused codex-power repair.

Post the concise verdict and wiki path to your outbox. Do not wait for a parent
reply and do not spawn children.

## Completion Requirements

- Current official documentation has been re-fetched and cited in the review
  record, including its date and stable fingerprint where available.
- The review record is the only project deliverable, has a clear ACCEPT/REJECT
  verdict for the bounded offline product slice, and names `a9c7a9b` plus the
  reviewed merge/diff.
- The record contains independent clean-archive inspection and focused
  non-live test/lint/policy evidence, and it identifies no retained secret,
  browser-profile, screenshot, frame, HAR, trace, or customer-data artifact.
- It explicitly preserves `coverage/status.json` as `complete: false`; no
  live, UI, vision, residual, or bulk status may be accepted.
- The branch contains no edits outside `wiki/` other than its required node
  seed, private memory, and plan metadata; it is clean and committed, and the
  outbox reports the verdict.
