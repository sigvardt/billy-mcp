...
Perform an independent Grok review of the merged Wave-5p organizations
create/update ticketed-write freeze page at parent baseline
`afbe5188b029e89e7e865a6b9ee8b9a5881f51f4`. The product is blocked unless
your independently evidenced review reaches an explicit ACCEPT.

Your sole tracked project deliverable is
`wiki/wave_fivep_freeze_independent_review.md` and its generated index entry.
You own that page, the index row generated by `wiki update`, and this node's
seed only. Do not modify source, tests, coverage, status, prior contracts or
reviews, or any other project wiki page.

Review `wiki/wave_fivep_ticketed_writes_contract.md` (MD5
`2742eda7bafecd619aba5fa8ad0694c5`) against current primary Billy API
documentation, cited candidate research
`wiki/wave_fivep_candidate_write_research.md`, and the locked API base
`https://api.billysbilling.com/v2`. Current public evidence reports a docs body
ETag `wcw4x9hqvu3603`, 147934 bytes, MD5
`8b94b0135c91fd15fe54ea33e088a4be`; JSON-object unauth probes give
organizations POST/PUT 401 `AUTHENTICATION_REQUIRED` and singular DELETE 405
`METHOD_NOT_ALLOWED`. Recheck enough primary evidence to support your own
citations; never treat an unauth gate as live qualification.

Independently verify the exact two inventory rows/four future tool names;
strict outer Pydantic inputs with opaque `organization` map; client-relative
POST `/organizations` and PUT `/organizations/:id`; required
`organizations` response root; ticket-only execution, five-minute single-use
exact operation bindings, and no retries; update body-id equality; the DELETE
405 cleanup limit; sensitive subscription-card redaction; and the exclusion of
delete, all bulk rows, generic transport, webhooks, live, UI, vision, and
completeness claims. Confirm coverage remains 175 offline green, zero live and
vision greens, 92 ambiguous bulk rows red, and `complete: false` at review
baseline.

Write a self-contained cited review that records either `ACCEPT` or concrete
actionable defects. An ACCEPT authorizes only a subsequent, separate Codex
Power offline product implementation leaf; it is not a product, live, UI,
vision, cleanup, or completeness acceptance. Do not retain credentials, raw
browser evidence, or sensitive values. Never open a visible browser.

## Completion Requirements

1. `wiki/wave_fivep_freeze_independent_review.md` exists, has valid metadata,
   cites primary/repository evidence, and records a clear ACCEPT or defect
   verdict for the fixed parent baseline.
2. The review covers every listed contract, method-gate, ticket, safety,
   cleanup, and coverage-honesty assertion without modifying product code,
   tests, coverage, status, or existing contract evidence.
3. `wiki update` and `wiki lint` pass for the project wiki; the node's
   prescribed test and lint scripts pass; and `git diff --check` is clean.
4. The committed project diff outside this node seed is limited to the review
   page and its generated index entry. Report the verdict and commit SHA to the
   parent through radio, then finish this node.
