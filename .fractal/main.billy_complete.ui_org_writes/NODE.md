...
- src/billy_mcp/ui_writes/organizations.py
- tests/unit/test_ui_organizations_writes.py
- tests/live/test_ui_organizations_writes.py
- wiki/ui_organizations_writes.md

Do not edit generate_coverage_report.py, scripts/check_coverage.py, src/billy_mcp/server.py, or src/billy_mcp/browser.py. Root already wired register_ui_organization_write_tools(server, ui_write_protocol). Fill that function.
...
Only reversible company fields. Restore the original values.
Fail closed on users, access tokens, and subscription. Never leave company settings changed.

Tools to register: ui_organizations_update_preview/execute
Coverage rows: ui.parity.organizations.update
Read wiki/ui_write_ticket_protocol.md and the parent plan 190.1.


## Completion Requirements

1. register_ui_organization_write_tools registers the family preview and execute tools from src/billy_mcp/ui_writes/organizations.py.
2. Offline unit tests in tests/unit/test_ui_organizations_writes.py pass (preview, execute-once, replay, expiry, wrong tool).
3. Live MCP test in tests/live/test_ui_organizations_writes.py passes, or the node recorded a concrete live blocker that is not an unsafe skipped submit.
4. wiki/ui_organizations_writes.md describes the tools, fail-closed rules, and cleanup. No secrets.
