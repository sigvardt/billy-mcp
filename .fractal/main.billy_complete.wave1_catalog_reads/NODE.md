...
Implement the catalogue API read module only, using the frozen official contract
in `wiki/wave_one_reads_contract.md` and
...
Own exactly these product files: `src/billy_mcp/api/catalog_reads.py` and
`tests/api/test_catalog_reads.py`. You may create the test directory. Do not
...
support for `api_products_get`, `api_products_list`, `api_product_prices_get`,
and `api_product_prices_list`. Use only `BillyHttpClient` and documented
relative paths. Lists may expose only page/pageSize/include/sortProperty/
sortDirection; do not infer filters. Map singular/list roots and optional paging
