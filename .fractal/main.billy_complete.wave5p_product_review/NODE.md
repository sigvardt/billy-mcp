...
Independently review the integrated Wave-5p singular organizations ticketed
write product on this branch. You are the Grok reviewer; use current official
Billy documentation and the locked API base only for read-only research. Write
a cited, durable verdict to exactly
`wiki/wave_fivep_product_independent_review.md`.

Your exclusive project-file ownership is that review page. You may change your
own node seed, plans, and memory. Do not modify source, tests, coverage
manifests/status/report, browser configuration, credentials, or any other wiki
page. Do not run a browser or live request with credentials. Do not create or
retain screenshots, traces, HAR files, frames, tokens, or customer data.

Review the root integration rather than the old child worktree. Establish an
independent current-doc citation and verify: `/v2/organizations` supports
singular create/update; JSON-object POST and PUT gate at 401 without
credentials; singular DELETE is 405 and is therefore excluded. Inspect the
exact four exposed tools: `api_organizations_create_preview`,
`api_organizations_create_execute`, `api_organizations_update_preview`, and
`api_organizations_update_execute`. Confirm strict outer models, opaque inner
maps, update path/body id matching, ticket-only execution, exact ticket
binding/single use/expiry/tamper/replay handling, one write with no retry,
required `organizations` response root, and sensitive subscription-card
redaction.

Independently inspect the registry and generated inventories. The only API rows
that may be green offline are organizations create/update; their create cleanup
must say singular DELETE is unsupported, the create response field must be
`organizations[]`, live/vision remain zero, all UI rows remain red, the 92
bulk rows remain ambiguous/red, and `complete` remains false. Verify the root
has 260 `api_*` tools and 177 implemented/contract-tested API rows. Run the
focused test command supplied by this node, and separately inspect the
non-live test and policy evidence available in the branch.

Your review must clearly state ACCEPT or FAIL for this offline product only.
An ACCEPT is not live, UI, vision, bulk, delete-cleanup, or overall-completeness
acceptance. A FAIL must identify actionable source locations and retain the
fail-closed coverage state. Keep cited source facts distinct from inferences.

## Completion Requirements

- `wiki/wave_fivep_product_independent_review.md` exists with valid
  frontmatter, direct official-doc citations, an explicit ACCEPT or FAIL, the
  tested root commit, scope, evidence, and exclusions.
- The verdict independently verifies the method gates, exact four-tool surface,
  ticket protocol, response-root mapping, redaction, coverage/status counts,
  and UI/bulk/live/vision fail-closed boundaries described above.
- `bash $NODE_DIR/scripts/test.sh`, `wiki update --path=$WIKI_DIR`, and
  `wiki lint --path=$WIKI_DIR` pass. Any raw evidence has been purged from
  owner-only temporary storage and only non-sensitive references remain.
- The project diff is limited to the assigned review page; no product code,
  test, inventory, status, credential, browser, or unrelated wiki modification
  is made.
- Before finishing, post the concise verdict and evidence location to the
  outbox, fold current durable facts into memory, commit the review, and run
