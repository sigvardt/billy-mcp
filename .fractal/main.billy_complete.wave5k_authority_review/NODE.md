...
Perform an independent, read-only Grok review of the proposed Wave-5k freeze
authoring authority. This is a research/review task, not a product
implementation task.

Review the current official Billy API documentation at `https://www.billy.dk/api/`
and the committed root evidence:
...
- the current root registration and Wave-5j bank-line product bytes.

Independently determine whether the following narrow conclusion is justified:
the existing Wave-5k research ACCEPT chain plus a merged Wave-5j offline
product ACCEPT opens authoring of exactly
`wiki/wave_fivek_ticketed_writes_contract.md` for `bankPayments.create` and
`bankPayments.update`, despite the Wave-5j review correctly saying it does not
itself authorize later Wave-5k product work.

The verdict must distinguish a product review's scoped non-overclaim from a
separate Wave-5k freeze-authoring gate. Do not treat non-overclaim wording by
itself as a rejection. Conversely, reject with a concrete, evidenced missing
prerequisite if the record does not open authoring.

Create exactly one durable, non-sensitive project-wiki page:
`wiki/wave_fivek_freeze_authoring_authority_independent_review.md`.
It must cite the official-doc fingerprint or refreshed evidence, identify the
precise two-row / four-tool future scope, retain the bankPayments singular
DELETE 405 exclusion and ambiguous-bulk exclusion, and say clearly whether
freeze-page authoring is OPEN or BLOCKED. An OPEN verdict may authorize only
the freeze wiki page; it must explicitly exclude product code, registry changes,
coverage greens, live/UI/vision testing, browser work, and any Wave-5l scope.

No production code, tests, inventory flags, status files, browser state, or
credentials may be changed. Never open a visible browser. Do not create child
nodes. Keep any raw documentation or probe output only in owner-only scratch
and purge it before completion. Commit and push the review-page deliverable.

## Completion Requirements

1. The wiki page named in the instructions exists, has valid frontmatter, and
   records an independent ACCEPT/REJECT verdict with cited official evidence.
2. It explicitly establishes OPEN or BLOCKED for Wave-5k freeze-page authoring
   and records the exact consequence; it does not overclaim product, coverage,
   live, UI, vision, or completeness work.
3. It retains the documented `bankPayments` singular DELETE 405 and leaves bulk
   rows excluded and red.
4. `wiki update --path=$WIKI_DIR` and `wiki lint --path=$WIKI_DIR` pass.
5. The diff is limited to the review page plus mechanically generated wiki
   index metadata; no production, coverage, credential, or raw-evidence files
   are changed. The branch is committed and pushed.
