...
Perform the independent Grok review of the Wave-5k `bankPayments`
product-ready handoff. You own exactly one project deliverable:
`wiki/wave_fivek_product_ready_research_independent_review.md`.

File ownership is strict: modify that one page only. Do not author the freeze
contract, production code, tests, inventory/status, existing wiki pages or
indexes, browser evidence, or persistent records. Your node seed, plan,
private memory, and owner-only scratch evidence are normal node-owned
operational files. Do not spawn children.

As Grok, independently re-fetch the official API documentation and, without
credentials, re-probe only the locked base
`https://api.billysbilling.com/v2` as necessary. Keep evidence in owner-only
scratch; never use a headed browser, create data, expose a token, or claim
live/UI/vision qualification. Cite the official page directly in the durable
review. Compare against:
...
- the parent handoff facts: ETag `hsisik4g9p3603`, MD5
  `c2efda0ee4cf9cf200e14910c5fc6996`, 147934 bytes; unauthenticated
  bankPayments POST/PUT 401 and singular DELETE 405.

Record a durable cited ACCEPT or REJECT of the product-ready handoff only.
Verify the precise proposed boundary: `api.bankPayments.create` and `.update`
only, yielding four ticketed tools with POST/PUT client-relative
`/bankPayments` paths, singular `bankPayment` root, primary `bankPayments`
response root, opaque payload maps, partial update id equality, exact
server-owned ticket executor binding, and no write retry. Verify that DELETE is
excluded by the exact 405 finding even though Supports lists it, and that bulk
rows stay empty-tool ambiguous. Check the stated product arithmetic (238 to
242 tools; 166 to 168 offline rows only after real implementation/tests) and
that all live, UI, vision, bulk, and completeness state remains fail-closed.

An ACCEPT must explicitly say it is not acceptance of the freeze contract page
or product implementation. A REJECT must identify exact repair conditions. In
either outcome, do not alter coverage and do not start implementation. Run wiki
lint and report the verdict/path through your outbox.

If Grok authentication, quota, or rate limiting prevents research before you
edit the review page, make no product-page edit, report the blocker with the
exact failure class via outbox, and stop. The parent will retry the full step
with the authorized fallback route; do not switch agents yourself.

## Completion Requirements

- A single durable page at
  `wiki/wave_fivek_product_ready_research_independent_review.md` contains a
  cited, clearly scoped ACCEPT or REJECT and valid frontmatter with `desc`.
- The review independently verifies or explicitly disputes the current docs
  fingerprint, method gates, two-row/four-tool boundary, 405 delete exclusion,
  opaque-payload constraints, and fail-closed coverage arithmetic.
- The review does not overclaim freeze acceptance, implementation, live/UI/
  vision tests, bulk support, cleanup, or overall completeness.
- The project diff outside this node seed contains only the owned review page;
  `wiki lint --path="$WIKI_DIR"` and `bash "$NODE_DIR/scripts/test.sh"` pass;
  publish the verdict through outbox before finishing.
