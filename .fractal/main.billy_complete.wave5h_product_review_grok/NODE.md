...
Perform an independent Grok review of the merged Wave-5h attachment JSON
ticketed-write product at root commit `29cecbe`. You own review judgement, not
implementation. Current official Billy documentation is authoritative; perform
a fresh read-only official-docs check and cite it. The accepted implementation
contract is `wiki/wave_fiveh_ticketed_writes_contract.md`; the prior research
handoff acceptance is
`wiki/wave_fiveh_product_implementation_research_independent_review.md`.

Review exactly the six registered API tools for the three singular attachment
JSON CUD inventory rows:
...
Independently inspect `src/billy_mcp/api/attachment_writes.py`, its server
registration, manifest rows, generated coverage, and focused test suites.
Independently run the focused attachment tests plus registry/coverage tests and
the root non-live test script. Verify strict outer inputs, opaque attachment
payload preservation, partial PUT with route/body id match, bodyless DELETE,
client-relative `/attachments` paths, single shared confirmation store/protocol,
exact execute-tool binding before ticket consumption, ticket tamper/replay/
expiry/wrong-org/target/payload/effect coverage, response-root discipline, and
no write retries.

Verify the root registry is 208 `api_*` tools and generated coverage records
151 implemented/contract-tested rows. Check that only the three attachment CUD
rows transition offline; live and vision remain zero and `complete` remains
false. Explicitly reject any implied acceptance of binary `/files`, ambiguous
attachment bulk operations, live qualification, UI parity, vision evidence, or
overall product completion.

Your only project deliverable is
`wiki/wave_fiveh_product_independent_review.md`; do not modify source, tests,
coverage files, other wiki pages, or the implementation. A reject is a valid
outcome. Make the review durable and non-sensitive: include commit reviewed,
official source citation/fingerprint, commands/assertions run, findings,
verdict, exact scope, and explicit non-claims. Keep raw evidence only in
owner-only node scratch and purge it before finishing. You may alter only your
own `.fractal/main.billy_complete.wave5h_product_review_grok/` seed, memory,
plans, and scratch in addition to that one review page.

Post a concise status to your outbox. Do not wait for root approval; finish when
your own independently observable review record and checks are complete.

## Completion Requirements

- `wiki/wave_fiveh_product_independent_review.md` exists, is indexed, and
  records an unambiguous ACCEPT or REJECT for the offline Wave-5h attachment
  product at `29cecbe`, including its source citations and non-claims.
- The review independently verifies the official attachment CUD contract and
  implementation/registry/coverage/test evidence described above; it does not
  rely solely on the producer's report.
- The focused attachment, registry/coverage, and root non-live test commands
  pass for an ACCEPT. If a required assertion fails or cannot be independently
  established, the page records REJECT with the precise blocker.
- No project files change outside the one owned review page; no secrets, raw
  frames, HARs, traces, credentials, or customer data enter git.
- `bash $NODE_DIR/scripts/lint.sh` passes, the project wiki is structurally
  valid, raw scratch evidence is purged, memory is folded to terminal facts,
  and the node posts its result before `fractal node finish`.
