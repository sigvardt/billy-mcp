...
Perform an independent Grok freeze review of the committed root contract
`wiki/wave_fiveq_ticketed_writes_contract.md`. You are an evidence and review
leaf: do not implement product code, register tools, change tests, coverage
manifests, status reports, or the contract page itself.

Your exclusive project deliverable is
`wiki/wave_fiveq_freeze_independent_review.md`; you may also maintain only your
own node memory, plans, and seed. The parent will merge the review page.

After PREPARE merges the parent, independently validate the freeze against the
current official Billy documentation at https://www.billy.dk/api/, locked API
base https://api.billysbilling.com/v2, and the cited research pages:

- `wiki/wave_fiveq_users_freeze_ready_research.md`
- `wiki/wave_fiveq_users_product_ready_research.md`
- `.fractal/main.billy_complete/tmp/grok-research.md` when present
- `wiki/offline_write_probe_rules.md`
- `coverage/api_v2_manifest.yaml` and `coverage/status.json`
- the approved design and the root peer review/contract pattern.

Verify the exact update-only contract: `api.users.update`, preview
`api_users_update_preview`, execute `api_users_update_execute`, PUT
`/users/:id`, outer id plus opaque user map, matching optional inner id,
required `users` response root, ticket-only execute, one write with no retry,
and no generic controls. Independently check the documented method gates (PUT
401 with JSON object; POST and singular DELETE 405), sensitivity/redaction
boundary for PII and privilege flags, and all exclusions (create, delete, bulk,
webhooks, live, UI, vision, cleanup, and completeness).

Produce a cited, durable, non-sensitive review page with an unambiguous
**ACCEPT** or **REJECT** verdict. An ACCEPT may open only a future offline
Codex Power users-update implementation leaf. It must explicitly state that it
does not authorise coverage greening by itself, live/UI/vision work, bulk,
create/delete tools, or completion. A REJECT must name a concrete correction
and leave product work blocked. Keep all browser activity headless; do not use
credentials, create records, or retain raw evidence in git.

Report progress and the final verdict via radio; do not wait for a parent reply.

## Completion Requirements

1. `wiki/wave_fiveq_freeze_independent_review.md` exists with valid frontmatter,
   a cited official-evidence section, an explicit ACCEPT or REJECT, and the
   exact scope/boundary checks above.
2. The verdict does not overclaim product, coverage, live, UI, vision, bulk,
   cleanup, delete, or completeness authority.
3. No project files outside the exclusive review page and the node's own seed
   are changed; no secrets or raw browser evidence are tracked.
4. `wiki update` and `wiki lint` pass for the project wiki, and the node test
   and lint scripts pass.
5. The final radio update names the committed review page, commit, verdict, and
   any remaining gate. Then run `fractal node finish` in the same iteration.
