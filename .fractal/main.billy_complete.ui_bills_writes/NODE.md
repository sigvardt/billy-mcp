...
Do not edit generate_coverage_report.py, scripts/check_coverage.py, src/billy_mcp/server.py, or src/billy_mcp/browser.py. Root already wired register_ui_bill_write_tools(server, ui_write_protocol). Fill that function.
...
Implement draft create first. Never pay a bill.
Draft-only bills. Unique tags. Delete or restore. No payments.

Tools to register: ui_bills_create_preview/execute, ui_bills_update_preview/execute, ui_bills_delete_preview/execute
Coverage rows: ui.parity.bills.create, ui.parity.bills.update, ui.parity.bills.delete
Read wiki/ui_write_ticket_protocol.md and the parent plan 190.1.


## Completion Requirements

1. register_ui_bill_write_tools registers the family preview and execute tools from src/billy_mcp/ui_writes/bills.py.
2. Offline unit tests in tests/unit/test_ui_bills_writes.py pass (preview, execute-once, replay, expiry, wrong tool).
3. Live MCP test in tests/live/test_ui_bills_writes.py passes, or the node recorded a concrete live blocker that is not an unsafe skipped submit.
4. wiki/ui_bills_writes.md describes the tools, fail-closed rules, and cleanup. No secrets.
