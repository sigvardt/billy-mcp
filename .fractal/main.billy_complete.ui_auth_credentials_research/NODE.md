...
Produce a cited Grok research brief for the next secure UI-authentication
implementation slice. This is a research-only leaf: you own
`wiki/ui_auth_credentials_login_organization_research.md` and may not modify
source, tests, coverage inventories, or generated status.

Research current official Billy documentation first, then inspect the committed
configuration, browser runtime, and relevant inventory/design records. Separate
officially documented facts from bounded anonymous observations. The only
existing browser observation that may be relied on is the read-only login
signature: root redirects to `https://mit.billy.dk/login`, with visible
`email`, `password`, `remember`, and
`button[data-cy=login-button]` controls; labels observed are `Log in` and
`Log ind`. No credentials, login submit, account selection, or post-login
interface state has been observed.

The brief must give an implementable, security-first plan for:

- configuration and secret resolution for stored browser credentials and TOTP,
  including an operating-system credential-store reference and exact
  non-logging/non-persistence boundaries;
- a typed, bounded headless login transition without generic browser controls,
  with pre-submit DOM assertions and exact supported failure classes;
- post-login organisation selection/discovery, including what must be
  discovered live before any behaviour can be implemented;
- expiry, reauthentication, CAPTCHAs, passkeys, push approval, and unsupported
  MFA. Only an observed non-automatable challenge may map to
  `AUTH_INTERACTION_REQUIRED`;
- planned test seams and prerequisites for a dedicated non-production
  organisation, cleanup/read-back, browser evidence purge, and later Grok
  vision review.

Do not use credentials, submit any login form, create records, make arbitrary
browser/API calls, or retain screenshots/frames/HAR/traces. Do not turn an
inaccessible screen or an inference into a green inventory row. Cite direct
official URLs near every official claim and label every remaining uncertainty
as a blocker/discovery prerequisite. The record must explicitly state that all
UI coverage remains red.

Post a concise parent handoff once the cited brief is committed. If official
sources are unavailable or conflict, record the exact source issue and keep
the plan blocked rather than filling gaps from inference.

## Completion Requirements

- `wiki/ui_auth_credentials_login_organization_research.md` contains cited
  current-source findings plus an implementation-ready but explicitly bounded
  next-slice plan.
- The brief distinguishes official facts, bounded anonymous observations, and
  blocked/unknown post-login behaviour.
- It includes secret handling, TOTP, challenge, organisation, test, cleanup,
  evidence-purge, and coverage-boundary decisions without exposing sensitive
  values or performing a login.
- The wiki lints cleanly, the leaf commits only its research deliverable, posts
  a concise parent handoff, and finishes.
