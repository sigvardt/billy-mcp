...
Implement the offline-only Wave-5j product for the accepted singular bank-line
write contract. You are the sole Codex Power implementation leaf: do not spawn
children, do not browse for a replacement contract, and do not touch files
outside the explicit ownership list without reporting the conflict to the
parent.

## Authority

- Frozen contract: `wiki/wave_fivej_ticketed_writes_contract.md`.
- Product gate: `wiki/wave_fivej_freeze_independent_review.md` is ACCEPT.
- Cited current handoff: parent scratch
  `.fractal/main.billy_complete/tmp/grok-research.md` (research43), based on
  official `https://www.billy.dk/api/`, ETag `hsisik4g9p3603`, MD5
  `c2efda0ee4cf9cf200e14910c5fc6996`. It reconfirms POST/PUT 401 and a
  non-qualifying missing-id DELETE 200 for all three resources.

The brief authorises only these nine rows: `bankLineMatches`, `bankLines`, and
`bankLineSubjectAssociations` create/update/delete. Implement exactly eighteen
typed tools: each row has a `*_preview` and a `*_execute` tool, with these exact
names and frozen path/root map:

- `api_bank_line_matches_{create,update,delete}_{preview,execute}`:
  `/bankLineMatches`, `bankLineMatch`, `bankLineMatches`; match responses may
  map optional `bankLines` and `bankLineSubjectAssociations` roots only when
  actually present.
- `api_bank_lines_{create,update,delete}_{preview,execute}`: `/bankLines`,
  `bankLine`, `bankLines`; do not invent a parent root.
- `api_bank_line_subject_associations_{create,update,delete}_{preview,execute}`:
  `/bankLineSubjectAssociations`, `bankLineSubjectAssociation`,
  `bankLineSubjectAssociations`; do not invent a parent root.

Use the root's shared `ConfirmationStore` and `WriteProtocolService`. Previews
are non-mutating and issue exact short-lived single-use tickets. Execute accepts
only `confirmation_ticket`; use the server-owned execute name. Create uses
POST and one singular root; update uses partial PUT and any payload `id` must
match the route id; delete is bodyless and binds only `{"id": id}`. Keep outer
Pydantic models `extra="forbid"`, but keep every inner resource map opaque as
`dict[str, JsonValue]`.
...
Clone the established Wave-5i patterns from
`sales_tax_account_meta_writes.py` and its two test modules, but do not edit
`bank_reads.py`. Preserve every uncertainty as opaque: no enum values for
`side`/`differenceType`, no asserted `subject` wire form, no claimed embedded
replacement behaviour, and no inferred parent echo on child writes. Do not
implement bulk, bankPayments, balanceModifiers, generic HTTP, webhooks, UI,
browser, live, or vision work. Never mark live coverage true or claim overall
completion.

The required local tests must prove strict schemas; mutation-free previews;
canonical request/effect binding; exact escaped paths and request bodies;
bodyless delete; required primary root and optional match-only child roots;
deleted metadata only when returned; auth error mapping; ticket tampering,
replay, expiry, wrong organisation/target/payload and executor mismatch before
HTTP; one write/no retry; and root shared-store cross-executor rejection without
ticket consumption. Update the root registry assertion to 238 tools. Only after
these tests are real and passing, set the nine manifest rows `implemented` and
`contract_tested` true, cite the tests, leave `live_tested` false, leave the six
related bulk rows red/tool-less, and regenerate status to 166 offline rows and
`complete: false`.

## Completion Requirements

1. The seven owned paths contain the complete Wave-5j product described above;
   no out-of-scope source, UI, live, bulk, credential, or browser change is
   made.
2. Exactly 18 new flat typed tools are registered in the root server and the
   root `api_*` registry is 238; all use the shared confirmation service.
3. Focused API and root cross-executor suites pass, proving all frozen ticket,
   request, response, and no-retry safety properties. Format, lint, type, and
   relevant coverage-generation/registry tests pass.
4. Exactly the nine singular coverage rows are offline-greened as implemented
   and contract-tested with real test references; live/vision remain false,
   all six related bulk rows remain red, and status reports 166 offline rows
   with `complete: false`.
5. The child posts an outbox summary containing its committed SHA, changed
   paths, commands/results, and any remaining evidence boundary; it leaves its
   branch clean and runs `fractal node finish` only after its own checks pass.
