...
Implement the frozen offline-only Wave-5c line-resource contract from
`wiki/wave_fivec_ticketed_writes_contract.md`. You own only these product files:

- `src/billy_mcp/api/daybook_transaction_line_writes.py`
- `tests/api/test_daybook_transaction_line_writes.py`

Create exactly six typed FastMCP tools: preview/execute pairs for documented
create, update, and delete operations on `/daybookTransactionLines`, with
singular root `daybookTransactionLine` and plural response root
`daybookTransactionLines`. Follow `daybook_writes.py` and the repaired
Wave-5b modules exactly: Pydantic outer inputs forbid extras; payload remains
`dict[str, JsonValue]`; update and delete ids have `min_length=1`; executes
accept only `confirmation_ticket`.

Every preview must use a `WriteOperationSpec` with its exact literal
`execute_tool_name`. Every execute handler must pass that exact literal to the
existing `WriteProtocolService.execute`. Do not modify the shared protocol or
confirmation store. Use `/daybookTransactionLines` with no duplicate `/v2`,
one singular-root POST/PUT body, and a bodyless DELETE. Set no additional
plural roots without live evidence. Do not add retries, live calls, bulk, UI,
special routes, or any coverage/manifest/server/wiki changes.

Build a focused mock-transport contract suite proving tool schemas and names,
preview zero mutation, exact POST/PUT/DELETE requests, typed upstream errors,
tamper/replay/expiry/wrong organisation/target/payload handling, forbidden
outer extras, bodyless deletion, and same-module wrong-executor rejection
before HTTP while the correctly bound executor remains usable. The root will
own server registration, coverage evidence, and the cross-module mismatch test.

Use no credential, raw browser, or external live evidence. Run the focused
suite plus Ruff and Pyright for owned files, commit the result, and report the
commit and test outcome through the outbox. Do not wait for parent review.

## Completion Requirements

1. The two owned files contain exactly the six documented line-resource tools
   and their passing focused contract tests.
2. All previews are non-mutating; every execute is ticket-only and binds its
   literal server-owned executor name into and out of the shared protocol.
3. Focused pytest, Ruff, Pyright, and `git diff --check` pass from committed
   bytes, with no secret, live, bulk, UI, protocol, coverage, server, or wiki
   change.
4. The worktree is committed and clean, a concise outbox report names the
   commit and checks, and `fractal node finish` is called with this completed
   leaf deliverable.
