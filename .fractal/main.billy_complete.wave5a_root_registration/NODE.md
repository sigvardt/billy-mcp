...
Implement the approved Wave-5a root-registration slice as a focused Codex Power
leaf. The parent has already obtained the current cited Grok brief; treat these
committed documents as the contract:
...
- `.fractal/main.billy_complete/plans/2026-07-29T15:39:32.341Z-2.16-wave5a_root_registration.md`

The official Billy API documentation fingerprint is ETag `hsisik4g9p3603`, MD5
`c2efda0ee4cf9cf200e14910c5fc6996`. Do not invent any API contract beyond
those sources. This is offline registration only: never claim live, UI, vision,
bulk, specials, auth-browser, or product completeness progress.

You exclusively own these product paths for this slice:

- `src/billy_mcp/server.py`
- `scripts/generate_coverage_report.py`
- `tests/unit/test_coverage_server.py`
- generated `coverage/` artifacts required by the generator
- a new narrowly scoped server-registration test only when needed

Do not edit the existing write leaf modules or their focused contract tests:
`write_protocol.py`, `contact_writes.py`, `contact_person_writes.py`,
`catalog_writes.py`, and `daybook_writes.py` are already reviewed and merged.
Escalate any apparent need to change them to the parent rather than expanding
scope.

Implement exactly this runtime wiring in `create_server`:

1. Construct one `ConfirmationStore` and one `WriteProtocolService(client,
   confirmations)` per server instance.
2. Register contact, contact-person, catalog, and daybook write tools with the
   same shared protocol instance.
3. Retain all existing read and coverage tools, use no generic HTTP/browser
   surface, and update server instructions so they no longer falsely call the
   API surface read-only.
4. Prove the runtime registry has exactly 124 `api_*` tools and the two
   coverage tools. The 30 write names are preview/execute twins; their 15
   inventory entries are the preview names only.

Extend `OFFLINE_API_IMPLEMENTATION_EVIDENCE` only for these fifteen frozen
operations, each with its focused write suite plus
`tests/unit/test_coverage_server.py`:

- products: create, update, delete
- productPrices: create, update, delete
- contacts: create, update, delete
- contactPersons: create, update, delete
- daybooks: create, update, delete

Run the generator instead of hand-editing generated coverage artifacts. The
only intended green transition is `implemented` and `contract_tested` for those
15 rows. Expected report state is 109 implemented, 109 contract-tested, zero
live-tested, 92 empty-tool bulk rows, all UI rows red, and `complete: false`.

Run formatting, Ruff, Pyright, focused write/coverage tests, coverage policy,
repository policy, and the non-live/non-vision suite. Never use credentials or
perform a live write. Preserve the repository's secret and evidence rules.

If Codex Power fails before editing due to authentication, quota, or a rate
limit, report it immediately so the parent can retry the complete task through
Grok. Do not change agents after any edit.

## Completion Requirements

1. `create_server` exposes the prior 94 reads plus all 30 ticketed write
   preview/execute tools, for exactly 124 `api_*` names and two coverage tools.
   The shared confirmation store and protocol are instantiated once per server.
2. Exactly the fifteen frozen Wave-5a CUD rows have generated offline evidence
   and show `implemented=true` and `contract_tested=true`; their
   `live_tested` values remain false. No other qualification category is
   greened by this work.
3. Generated coverage reports validate with 109 implemented/contract-tested,
   zero live-tested, 92 bulk rows still missing tool names, all UI rows red, and
   `complete=false`.
4. Runtime server-registration assertions and all focused write suites pass,
   along with format, Ruff, Pyright, coverage/repository policy checks, and the
   project's non-live/non-vision test suite.
5. The product diff is limited to the owned paths, includes no secrets or raw
   evidence, and is committed and pushed through `fractal commit`. Report the
   commit and verification results to the parent via your outbox. Do not claim
   independent-review approval or overall completion.
