...
Perform the full, read-only Codex Power fallback review of the proposed
Wave-5k freeze-authoring authority. This retry is authorized because the Grok
reviewer failed before edits with an authentication error. It is not a Grok
review and must never be described as one.

Review the current official Billy API documentation at `https://www.billy.dk/api/`
and these committed root artifacts:
...
- `coverage/api_v2_manifest.yaml`, `coverage/status.json`, and merged Wave-5j
  bank-line product bytes.

Independently test the narrow reasoning in research48: the Wave-5k research
ACCEPT chain plus the existing Wave-5j offline product ACCEPT supports authoring
of exactly `wiki/wave_fivek_ticketed_writes_contract.md` for
`bankPayments.create` and `bankPayments.update`. Distinguish the product
review's scope-limited non-overclaim from a separate freeze-authoring gate.

Create exactly one durable, non-sensitive project-wiki page:
`wiki/wave_fivek_freeze_authoring_authority_fallback_review.md`.
State a fallback ACCEPT or REJECT with citations. An ACCEPT may say the
documented contract reasoning is internally consistent and ready for a future
Grok authority review, but it must not claim to replace that authority or open
Wave-5k by itself. A REJECT must name a concrete, evidenced missing condition.
Retain the bankPayments singular DELETE 405 exclusion and ambiguous-bulk
exclusion.

Do not modify production code, tests, coverage/status files, inventory flags,
browser state, or credentials. Never open a visible browser. Do not create
child nodes. Keep raw documentation/probe output only in owner-only scratch and
purge it before completion. Commit and push the review-page deliverable.

## Completion Requirements

1. The fallback review page exists, has valid frontmatter, cites current
   evidence, and clearly declares ACCEPT or REJECT.
2. It labels itself Codex Power fallback review, records that it does not
   replace Grok authority, and makes no product, coverage, live, UI, vision, or
   completeness claim.
3. It retains the documented bankPayments singular DELETE 405 and excludes
   both ambiguous bulk rows.
4. `wiki update --path=$WIKI_DIR` and `wiki lint --path=$WIKI_DIR` pass.
5. The committed diff is limited to the review page plus mechanically generated
   wiki index metadata; no production, coverage, credential, or raw-evidence
   files are changed.
