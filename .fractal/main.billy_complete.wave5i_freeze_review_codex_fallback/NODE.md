...
Retry the full Wave-5i freeze-ready research review after the assigned Grok
reviewer failed authentication before making edits. You are Codex Power fallback
review, so this is supplemental and cannot satisfy the mandatory authoritative
Grok research review. Do not draft the freeze contract and do not implement
write tools.

Independently examine the parent research lead at
`/Volumes/ssd_1/Repositories/billy-mcp/.worktrees/main.billy_complete/.fractal/main.billy_complete/tmp/grok-research.md`,
the API manifest, `coverage/status.json`,
`wiki/offline_write_probe_rules.md`, the approved design, and existing
sales-tax ticketed-write patterns. Re-check the cited official documentation
where available, but label any source limitation honestly; never call this a
replacement for a fresh authenticated Grok review.

Assess only the later freeze-drafting gate for six singular JSON CUD operations:
sales-tax accounts create/update/delete and sales-tax meta-fields
create/update/delete. Verify the cited POST/PUT/DELETE routes, singular roots,
partial PUT, bodyless delete/id ticket binding, opaque inner payload policy,
empty additional response-root expectations, documented auth errors, and the
unauthenticated 401/DELETE-200-trap interpretation. Ensure no enum values,
belongs-to wire encodings, field mutability, bulk bodies, binary-file behavior,
UI/vision/live claims, implementation, coverage green, or completion is
invented.

Your sole project deliverable is
`wiki/wave_fivei_freeze_codex_fallback_review.md`. It must cite the evidence,
state a supplemental PASS or FAIL verdict, name precise blockers where present,
and explicitly say that it cannot open the implementation gate without a later
authenticated Grok review. Do not modify source, tests, coverage, other wiki
pages, or the freeze/product contract. You may alter only your own
`.fractal/main.billy_complete.wave5i_freeze_review_codex_fallback/` seed,
memory, plans, and scratch in addition to the one review page. Keep raw
evidence in scratch and purge it before finish; post result to outbox.

## Completion Requirements

- `wiki/wave_fivei_freeze_codex_fallback_review.md` exists, is indexed, and
  contains a cited supplemental PASS or FAIL for the Wave-5i freeze-ready
  research with exact scope and non-authoritative status.
- The review verifies cited routes, roots, write protocol boundaries, manifest
  mapping, and probe interpretation; it preserves all bulk/binary/live/UI/
  vision/product/coverage exclusions.
- The page expressly states that a fresh Grok review remains required before
  freeze drafting can be accepted.
