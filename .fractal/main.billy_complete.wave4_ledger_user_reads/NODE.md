...
Implement the frozen, read-only Wave-4 ledger and users API slice as a Codex
Power leaf. The root's cited Grok brief at
`.fractal/main.billy_complete/tmp/grok-research.md` and the shared freeze at
`wiki/wave_four_remaining_clear_reads_contract.md` are the contract; do not
research a replacement or widen its claims.
...
Implement exactly six real typed read tools and their service functions:

- `api_transactions_get/list` → `/transactions`, roots `transaction` /
  `transactions`
- `api_postings_get/list` → `/postings`, roots `posting` / `postings`
- `api_users_get/list` → `/users`, roots `user` / `users`

`/users` is a resource collection and is distinct from the existing green
`/user` and `/user/organizations` specials. Do not change or reimplement those
specials.

Follow the established Wave-3 read-module pattern. Inputs must be flat
Pydantic models with `extra="forbid"`; get requires non-empty `id` plus optional
non-empty `include`; list permits only `page`, `pageSize` (1–1000), optional
non-empty `include`, optional free-form `sortProperty`, and `sortDirection`
`ASC|DESC`. Reject `offset`, nested `request`, invented filters, and all
unknown keys. URL-encode identifiers. Map only the documented singular/plural
roots and optional `meta.paging` envelope. Payload records remain opaque
allow-extra objects. Map both documented 401 authentication codes to typed
`AUTH_REQUIRED`; malformed response roots are typed errors. Register direct
stable FastMCP names with flat schemas.

This leaf is reads only. Do not add writes, bulk, webhooks, browser controls,
generic HTTP, preview/execute, server registration, coverage, inventory,
redaction, generated reports, or wiki changes. Do not set coverage flags,
`live_tested`, or `complete`. User email and phone are possible opaque payload
fields; root owns logging-redaction integration. Keep raw evidence and
credentials outside the repository.

Add focused synthetic contract tests covering every path/root, encoded IDs, the
exact query allowlist and defaults, paging inside `meta`, absent `meta`,
unknown-key rejection, bounds, typed 401 responses, flat FastMCP schemas, and
structured tool output. No live call is authorised.

Before committing, run focused Ruff format/check, Pyright on owned files, the
focused pytest module, and node lint/test scripts. Report only actual results.
Keep the product commit limited to the two owned product files; do not commit
injected `.system` skills, runtime logs, screenshots, downloaded docs, or other
generated seed noise. Use radio to report progress, questions, review findings,
and the final commit hash to the parent.

## Completion Requirements

- The two owned product files contain the six real typed read tools and their
  focused contract tests, with no out-of-scope product edit.
- The implementation exactly follows the cited frozen paths, roots, flat input
  rules, paging envelope, and typed authentication errors; it does not conflate
  `/users` with `/user` specials or invent filters/fields.
- Focused Ruff format/check, Pyright, and pytest pass, as do node lint and
...
- A self-contained scoped product commit is present on the branch; its diff has
  no credentials, raw evidence, runtime logs, `.system` skill copies, or
  unrelated generated node-seed changes.
