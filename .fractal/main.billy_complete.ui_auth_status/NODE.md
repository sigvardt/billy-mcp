...
Implement exactly one narrow, read-only FastMCP authentication workflow:
`auth_status`. This is a Codex Power repository implementation task. Its
authoritative, pre-implementation discovery brief is
`wiki/wave5t_ui_auth_discovery.md`; read it with the approved design at
`docs/superpowers/specs/2026-07-28-billy-mcp-complete-design.md` before edits.

Own only these project files, adding closely related new tests only where
needed:

- `src/billy_mcp/browser.py`, `src/billy_mcp/models.py`, and
  `src/billy_mcp/server.py`;
- the focused tests for those files under `tests/`.

Do not edit `coverage/`, the API inventory, UI inventory, wiki pages, API
tools, or unrelated write logic. All UI coverage rows must remain red: this
offline implementation is not discovery, contract qualification, live testing,
or vision verification.

Use the existing `BrowserRuntime` and its allowed, MCP-owned persistent
headless context only. Never add generic navigation, browser controls,
selectors, clicks, typing, DOM dumps, screenshots, traces, egress exceptions,
or a visible-browser path. `auth_status` must navigate only through the
existing allowlisted Billy root/login workflow and validate a conservative,
strict login DOM signature. It returns typed `AUTH_REQUIRED` only for the
known `/login` route with the observed email, password, remember-me, and
localized submit controls. A route/signature mismatch must fail closed as typed
`UI_CHANGED`; it must not guess an authenticated state or report
`AUTH_INTERACTION_REQUIRED` absent an observed unautomatable challenge.

Every tool boundary must use Pydantic input and output models, with no generic
tool parameters. Register the stable `auth_status` tool in the FastMCP server;
keep errors redacted and structured. Preserve the API base lock and browser
allowlist. Use dependency seams that make deterministic unit, contract, and
headless-runtime tests possible without browser credentials, production data,
or external Billy business actions.

Run formatting, Ruff, Pyright, focused tests, and the non-live test script.
Report the exact test evidence and the remaining mandatory post-merge Grok
review in an outbox status. Do not claim this initial status slice makes any UI
workflow qualified.

## Completion Requirements

1. `auth_status` is a real registered, typed, read-only FastMCP tool backed by
   `BrowserRuntime`; it exposes no generic browser capability.
2. It recognizes only the recorded login route and strict control signature as
   `AUTH_REQUIRED` and returns a typed fail-closed `UI_CHANGED` result for all
   other routes or signatures.
3. Pydantic models cover its input, success, and error/status response
   boundary; sensitive browser/session values never reach responses or logs.
4. Focused unit, contract/registration, and headless-runtime tests cover the
   known login state, changed route, changed signature, registration, headless
   mode, browser-host allowlist, and redaction boundaries. They make no live
   login attempt and use no credentials.
5. Formatting, Ruff, Pyright, and `BILLY_TEST_MODE=commit bash
   "$NODE_DIR/scripts/test.sh"` pass. The child branch is committed, pushed,
   and clean. UI coverage state is unchanged and honestly red.
6. The child outbox records the implementation and explicitly states that a
   Grok independent review remains mandatory after merge; completion does not
   depend on a parent-only review action.
