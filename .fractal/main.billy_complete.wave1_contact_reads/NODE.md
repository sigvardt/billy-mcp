...
Implement the contacts API read module only, using the frozen official contract
...
Own exactly these product files: `src/billy_mcp/api/contact_reads.py` and
`tests/api/test_contact_reads.py`. You may create the test directory. Do not
...
support for `api_contacts_get` and `api_contacts_list`. Use only
`BillyHttpClient` and documented relative paths. Lists may expose only
page/pageSize/include/sortProperty/sortDirection; do not infer `q`, customer,
supplier, or other contact filters. Map singular/list roots and optional paging
...
- The two named tools have real typed registration support, Pydantic input and
