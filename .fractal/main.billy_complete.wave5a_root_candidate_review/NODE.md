...
Perform an independent Grok review of the repaired Wave-5a candidate tips.
You own only the shared project wiki; do not edit product code, tests,
coverage/inventory, status files, child seeds, or configuration.

Read first:

- `.fractal/main.billy_complete/tmp/grok-research.md` (cited current official
  Billy API contract; ETag `hsisik4g9p3603`, MD5
  `c2efda0ee4cf9cf200e14910c5fc6996`),
- `wiki/wave_five_ticketed_writes_contract.md`,
- `wiki/wave_fivea_candidate_independent_review.md`, and
- `docs/superpowers/specs/2026-07-28-billy-mcp-complete-design.md` §§6 and 8.

Independently inspect the exact three-dot diffs from root `5ab3efa` to these
candidate tips, ignoring any uncommitted child seed state:

1. `main.billy_complete.wave5_protocol_response_mapping` @ `4b2c12c`:
   `src/billy_mcp/api/write_protocol.py` and
   `tests/unit/test_write_protocol.py`.
2. `main.billy_complete.wave5_contact_writes` @ `a86afe9`:
   `src/billy_mcp/api/contact_writes.py` and
   `tests/api/test_contact_writes.py`.
3. `main.billy_complete.wave5_contact_person_writes` @ `341d0b1`:
   `src/billy_mcp/api/contact_person_writes.py` and
   `tests/api/test_contact_person_writes.py`.

Use clean `git archive` exports in owner-only temporary storage for executable
checks. Verify the claimed focused tests and inspect typed tool schemas and
response mapping. Review the exact contract boundaries: preview is non-mutating;
execute accepts only a ticket; tickets are exact-operation, short-lived, and
single-use; writes are never retried; paths omit a duplicate `/v2`; no secrets
or sensitive values are logged. Also verify the repaired contact flat parameter
surface, contact-person update `id` minimum length, and declared multi-root
response/deleted-record mapping. Check that no candidate changes inventory or
coverage status or converts live/UI/bulk evidence into green.

Publish the sole deliverable at
`wiki/wave5a_repaired_candidate_independent_review.md`. It must include a
per-candidate ACCEPT/REJECT verdict, exact commit and files reviewed, concise
test/check evidence, required findings if any, direct official-doc citations,
and an explicit fail-closed statement covering inventory, live, browser/UI,
vision, and bulk state. Do not count a failed leaf Grok authentication attempt
as evidence. No screenshots, browser frames, HAR files, credentials, or raw
sensitive evidence may enter the repository.

If Grok authentication or official-doc access is unavailable before producing
the review artifact, report the exact blocker to the parent over radio and make
no product claims or edits.

## Completion Requirements

- `wiki/wave5a_repaired_candidate_independent_review.md` records an independent,
  cited ACCEPT/REJECT decision for each exact candidate tip.
- The record names each inspected product/test file and the reproducible
  command-level verification performed from committed bytes.
- The record contains no secrets or sensitive evidence and preserves all
  unqualified coverage categories as red/fail-closed.
- `wiki update --path=$WIKI_DIR` and `wiki lint --path=$WIKI_DIR` pass after
  the record is written.
- No product, test, inventory, status, or seed file is modified by this node.
