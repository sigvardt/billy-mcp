...
Own the Grok research and interface exploration prerequisite for the Wave-5t
headless UI/authentication lane. Produce a cited, non-sensitive discovery
brief before any Codex Power UI implementation. This is research only: do not
write product code or advance coverage.

Read the approved design, `wiki/billy_ui_discovery_brief.md`,
`wiki/wave_fives_residual_specials_research.md`,
the API manifest/status, and the current `src/billy_mcp/browser.py` and auth
registration before exploring. The frozen API research is context only; this
leaf's authority is the Billy interface and existing headless runtime posture.
...
interaction prerequisite for the next Codex Power UI/auth leaf. It must not
