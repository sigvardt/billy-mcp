...
Implement exactly the cited Wave-5s-A offline API product on the inherited
parent baseline. You are a Codex Power implementation leaf: do not research the
web, use a browser, call live Billy endpoints, or create other child nodes.

The complete contract is already frozen in
`wiki/wave_fivesa_invoice_logs_list_research.md` and the parent plan
`.fractal/main.billy_complete/plans/2026-07-30T16:31:16.559Z-142.9-wave5sa_invoice_logs_product.md`.
Read those files before changing code. Treat their cited official-document
evidence as the contract.

### Scope and ownership

Own only these project files and their generated coverage artifacts:

- `src/billy_mcp/api/invoice_log_reads.py` (new)
- `src/billy_mcp/server.py`
- `tests/api/test_invoice_log_reads.py` (new)
- `tests/unit/test_coverage_server.py`
- `scripts/generate_coverage_report.py`
- generated files under `coverage/`

Do not edit wiki research, API/UI inventory definitions, another API module,
auth/browser code, test scripts, or any other residual special. Escalate a
scope conflict to the parent over radio instead of widening the patch.

### Exact product contract

1. Register exactly one typed FastMCP read tool named
   `api_invoice_logs_list`. It is list-only and calls `GET /invoiceLogs` only
   through `BillyHttpClient` on the locked API base.
2. Its strict Pydantic request model requires nonempty `invoiceId` and
   `organizationId`; has fixed/default documented values
   `sortProperty=eventTime` and `sortDirection=DESC`; and rejects paging,
   arbitrary sorting, and undeclared fields.
3. Its typed success model maps the exact `invoiceLogs` array root to opaque,
   extra-preserving entry models. Preserve shared typed auth errors and return
   typed `BILLY_ERROR` for a missing/non-array root or non-object element.
4. Do not expose get-by-id, create, update, delete, preview/execute, generic
   HTTP/browser controls, page/pageSize, UI, live, vision, or bulk surfaces.
5. Add transport-only contract tests for the complete query, sample success
   fields plus unknown extras, strict validation, malformed root/elements,
   typed auth errors, exact FastMCP registration/structured result, and root
   server registry count. No network call, credential, browser, or test record
   is permitted.
6. Add only `api.special.invoice_logs` to
   `OFFLINE_API_IMPLEMENTATION_EVIDENCE`, point it at the new API test and
   server registry test, regenerate coverage, and leave every live/UI/vision/
   bulk row red. The only expected change is 264 to 265 API tools and
   179/179 to 180/180 offline implemented/contract-tested API rows, with
   `complete: false`.

Use the repository's established typed read patterns, format and type-check
strictly, and never hand-edit generated coverage artifacts if their generator
is the source of truth.

## Completion Requirements

1. The exact single tool, source module, real-server registration, contract
   tests, registry assertion, evidence mapping, and regenerated coverage
   artifacts are committed on this branch.
2. The tool has typed strict input, typed success and error output, makes the
   documented request shape, and has no additional operation or query surface.
3. `api.special.invoice_logs` alone becomes offline implemented and
   contract-tested. API totals are 265 registered tools and 180/180 offline;
   live and vision totals remain zero and `coverage/status.json` remains
   `complete: false`.
4. Run and report successful results for: Ruff format check, Ruff lint,
   Pyright, the new API test module, `tests/unit/test_coverage_server.py`, the
   coverage generator/checker, and the configured non-live/non-vision suite.
5. `git status --short` is empty after the child commits. Report its commit,
   verification results, exact coverage values, and non-claims to the parent
   through radio. Do not claim independent acceptance: a separate Grok review
   runs after the parent merges your product.
