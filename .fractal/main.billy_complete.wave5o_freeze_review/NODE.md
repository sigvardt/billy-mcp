...
Perform an independent Grok review of the committed Wave-5o invoice-reminder
freeze contract. This is a read-only review of product code and coverage; the
only project deliverable you may create is
`wiki/wave_fiveo_freeze_independent_review.md`.

Review the root baseline at commit `a6a56cf` and inspect these durable inputs:
...
Independently use current official Billy documentation at
`https://www.billy.dk/api/` as the primary source. Record cited current-source
findings and distinguish them from the inherited research brief. Do not use a
headed browser, production credentials, live mutations, UI discovery, generic
HTTP/browser tooling, or raw evidence in the repository.

The review must prove or reject all of the following:

1. The frozen page authorises exactly `api.invoiceReminders.create` and exactly
   `api_invoice_reminders_create_preview` plus
   `api_invoice_reminders_create_execute`.
2. It freezes only client-relative `POST /invoiceReminders`, outer
   `{invoiceReminder: map}`, ticket-only execute, the shared confirmation/write
   services, exactly one non-retried write, and required-root-only
   `invoiceReminders` response mapping.
3. It keeps the inner map opaque and preserves all documented field boundaries;
   it makes no relation wire-form, nested association, date/number, email, or
   download semantics claim.
4. Its evidence correctly treats `POST {}` 401 as the offline create gate and
   PUT, singular DELETE, and bulk DELETE 405 as exclusions. Update, delete,
   both bulk rows, all association writes, webhooks, live/UI/vision work,
   cleanup qualification, coverage changes, product implementation, and
   completeness must remain excluded.
5. The root inventory remains at 174 implemented/contract-tested API rows,
   zero live/vision rows, 92 ambiguous bulk rows, and `complete: false`; the
   invoice-reminders create row is still red.

Your review page must state an authoritative **ACCEPT** or **REJECT**, cite the
official documentation and the inspected root paths, enumerate checked
criteria, name any exact discrepancy, and explicitly state that a verdict is
not a product, live, UI, bulk, or completeness acceptance. A REJECT must be
actionable and must not fix the frozen page yourself. An ACCEPT only opens a
later parent-managed Codex Power product slice; do not create or configure that
product child.

Do not edit `src/**`, `tests/**`, `coverage/**`, `pyproject.toml`, server
registration, the frozen contract page, existing review records, or project
wiki index files. Do not spawn children. If current official access fails
before this page is edited because of authentication, quota, or rate limiting,
report the exact pre-edit failure to the parent and stop without a review page;
the parent will restart the full review on Codex Power as the permitted
fallback. Never hand off after editing the review page.

## Completion Requirements

- `wiki/wave_fiveo_freeze_independent_review.md` is the only project-wiki
  content change and has valid frontmatter, a clear ACCEPT or REJECT, current
  official-doc citations, root-path citations, criterion-by-criterion findings,
  and explicit non-claims.
- The review has checked the root contract against the cited official API
  documentation and the stated 401/405 method gates; it does not merely repeat
  the prior research.
- The root freeze page, source, tests, coverage manifests/status, and all
  unrelated wiki records remain unchanged.
- `wiki lint --path=$WIKI_DIR`, `bash $NODE_DIR/scripts/lint.sh`, and
  `bash $NODE_DIR/scripts/test.sh` pass before the review is committed. Report
  the result to the parent via radio and run `fractal node finish` in the same
  iteration after all requirements are met.
