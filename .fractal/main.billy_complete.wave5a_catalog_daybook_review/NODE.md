...
Perform the required independent Grok review of the committed Wave-5a catalog
and daybook ticketed-write candidates. This is a review/research-only leaf:
never edit product source, tests, server registration, inventories, status,
coverage, auth, browser, or CI files.

Review only the two candidate three-dot diffs from root baseline `909d054`:

- `main.billy_complete.wave5_catalog_writes` tip `03f5720` — expected product
  files: `src/billy_mcp/api/catalog_writes.py` and
  `tests/api/test_catalog_writes.py`.
- `main.billy_complete.wave5_daybook_writes` tip `0d4b0e5` — expected product
  files: `src/billy_mcp/api/daybook_writes.py` and
  `tests/api/test_daybook_writes.py`.

Read `.fractal/main.billy_complete/tmp/grok-research.md` first. It is the cited
contract brief: the official Billy documentation fingerprint is
`hsisik4g9p3603` / `c2efda0ee4cf9cf200e14910c5fc6996`. Verify current official
documentation yourself with citations before making an ACCEPT claim.

For each candidate, inspect its clean `git archive` export rather than any
working tree. Run focused tests, Ruff, Pyright, diff checks, and direct FastMCP
schema inspection on committed bytes. Verify exact method/path/body behavior,
preview non-mutation, execute-only confirmation-ticket input, ticket binding
and replay behavior, error mapping, no `/v2` path duplication, and no sensitive
logging. Catalog must use `productPrice` in the MCP schema and body, declare
`productPrices` only for product operations, and preserve that additional root
when returned. Daybooks must use `daybook`, `/daybooks`, and no additional root.

Write one durable, non-sensitive review record at
`wiki/wave5a_catalog_daybook_independent_review.md`. It must state the exact
tips/baseline, sources/citations, tests run, product-file boundary, findings,
and an explicit ACCEPT or REJECT for each candidate. It must also state that
acceptance authorizes only selective product/test merging and does not green
inventory or qualify live, bulk, UI, vision, or completion state. Update the
wiki index and run wiki lint.

If Grok authentication, quota, or rate limiting prevents review before any
review edit, do not create a false artifact. Report the exact blocker via radio
and stop; the parent will retry the whole review with Codex Power as the policy
fallback. Do not launch child nodes.

## Completion Requirements

1. `wiki/wave5a_catalog_daybook_independent_review.md` is committed with cited,
   reproducible review evidence and one explicit verdict for each pinned tip.
2. The review changed only its allowed wiki record/index plus node-private
   state; no product, inventory, registration, qualification, or secret-bearing
   artifact was changed.
3. The outbox reports the review verdict and any finding the parent must act on.
4. `wiki lint --path=$WIKI_DIR` and the node test/lint hooks pass. Call
   `fractal node finish` after these requirements are satisfied.
