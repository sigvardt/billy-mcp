...
Perform an independent Grok review of the root-integrated Wave-5k freeze
contract. This is a documentation and safety-gate review only; it is not a
product implementation task.

Review the committed `wiki/wave_fivek_ticketed_writes_contract.md` against the
current official Billy API documentation at `https://www.billy.dk/api/` and the
approved design at `docs/superpowers/specs/2026-07-28-billy-mcp-complete-design.md`.
Independently verify the locked base `https://api.billysbilling.com/v2` and, if
needed, make only unauthenticated, non-mutating method probes against that base.
Never use credentials, production data, browser UI, a visible window, raw
screenshots, traces, HAR files, arbitrary URLs, or arbitrary local files.

Your only shared-project deliverable is
`wiki/wave_fivek_freeze_independent_review.md`. It must be a cited Grok review
with a clear **ACCEPT** or **REJECT** verdict, direct official-source links, the
reviewed root commit, and the following findings:

- `api.bankPayments.create` is exactly a preview/execute pair for
  `POST /bankPayments` with singular `bankPayment` request root and required
  `bankPayments` success root;
- `api.bankPayments.update` is exactly a preview/execute pair for partial
  `PUT /bankPayments/:id`, with strict outer input, opaque inner map, and inner
  `id` equality when supplied;
- confirmations are short-lived, single-use, exact-operation tickets bound to
  the server-owned executor, organisation, target, canonical request, and
  expected effect; execution receives only the ticket and sends one write with
  no retry;
- unauthenticated POST and PUT are authentication-gated, while singular DELETE
  is 405 `METHOD_NOT_ALLOWED`; DELETE, both bulk rows, generic HTTP/browser
  tools, webhooks, and invented cleanup are excluded;
- the contract does not green coverage or authorise product code, live tests,
  UI parity, vision claims, or completeness. The current fail-closed baseline
  remains 238 `api_*` tools, 166 implemented/contract-tested offline API rows,
  zero live/vision rows, 92 ambiguous bulk rows, and `complete: false`.

An ACCEPT opens only a later, separately planned Codex Power leaf for the four
bank-payment tools. A REJECT is equally valid: state the exact correction and
leave product work blocked. Do not modify source, tests, inventories, coverage
status, other review pages, or any product-ready fallback page. Update only the
new review page and generated `wiki/_index.md`; private node records are
allowed. Run wiki update/lint and the configured non-live checks before commit.

## Completion Requirements

- `wiki/wave_fivek_freeze_independent_review.md` exists, is independently
  authored, has current official Billy documentation citations, identifies the
  reviewed commit, and issues an evidence-backed ACCEPT or REJECT.
- The verdict explicitly preserves the create/update-only four-tool boundary,
  singular DELETE 405 exclusion, bulk ambiguity, and fail-closed live/UI/vision
  coverage status.
- No production source, test, manifest, coverage, credential, browser-evidence,
  or unrelated wiki changes are made.
- `wiki update --path="$WIKI_DIR"` and `wiki lint --path="$WIKI_DIR"` pass;
  the configured lint and non-live test scripts pass; the branch is committed
  and pushed.
- Report the verdict and committed deliverable through the outbox, then run
  `fractal node finish` in the same iteration. Do not wait for parent merge or
  approval after a valid ACCEPT or REJECT page is committed.
