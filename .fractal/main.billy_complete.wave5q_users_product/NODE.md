...
Implement exactly the offline Wave-5q Billy API users-update product slice. You
are the Codex Power implementation leaf. Do not perform current web research or
independent review; the contract is already frozen and cited by the parent.
...
- `src/billy_mcp/api/organization_writes.py` and
  `tests/api/test_organization_writes.py` (update path only)

Implement and own only the following project files:

| Path | Required work |
| --- | --- |
| `src/billy_mcp/api/user_writes.py` | Create update-only `api_users_update_preview` and `api_users_update_execute`. |
| `tests/api/test_user_writes.py` | Create contract tests for the two named tools. |
| `src/billy_mcp/server.py` | Import and register `register_user_write_tools` once. |
| `scripts/generate_coverage_report.py` | Add the single offline evidence mapping for `api.users.update`. |
| `coverage/api_v2_manifest.yaml`, `coverage/status.json`, `coverage/report.md` | Regenerate after the evidence mapping changes. |
| `tests/unit/test_coverage_server.py` | Assert the two tool names and API-tool count 262. |

The product is exactly this frozen API operation:

- `PUT /users/:id`, with strict outer Pydantic input `{id, user}`; outer
  undeclared fields are forbidden and `id` is non-empty.
- `user` is an opaque `dict[str, JsonValue]`; if `user.id` is supplied it must
  equal path `id`. Do not create a field allowlist or invent password, invite,
  MFA, or special auth routes.
- Preview is non-mutating and must use the shared `WriteProtocolService`.
  Execute accepts only `{confirmation_ticket}`, consumes the exact short-lived
  single-use ticket, performs exactly one no-retry PUT, and maps only required
  response root `users`.
- Treat user PII and privilege flags as high sensitivity: do not log tickets or
  raw payloads and do not claim privilege fields are client-writable.

Hard exclusions: user create/delete/bulk tools; generic HTTP or browser tools;
webhooks; UI, live, vision, auth, salesTaxReturns, specials, all other write
areas, and any completeness claim. Singular DELETE is documented 405; live
cleanup remains restore-via-PUT and unqualified here.

Use generated coverage output rather than manually asserting completion. After
this leaf, only `api.users.update` may change to implemented and
contract-tested: expected counts are 178 offline rows and 262 `api_*` tools;
live, vision, UI, bulk, and `complete` must remain red/false.
...
1. The two required typed FastMCP tools exist with exactly the frozen names and
   no additional users write tools.
2. Their inputs, ticket-only execute boundary, one-write/no-retry behaviour,
   opaque-map preservation, optional id-match validation, error mapping, and
   redaction-safe behaviour have passing local contract tests.
3. Root server registration exposes both tools exactly once and the registry
   test asserts 262 `api_*` tools.
4. Generated coverage artifacts make only `api.users.update` offline green:
   178 implemented and contract-tested API rows; live false; UI/vision/bulk
   unchanged and `complete: false`.
5. `bash "$NODE_DIR/scripts/lint.sh"` and
   `BILLY_TEST_MODE=commit bash "$NODE_DIR/scripts/test.sh"` pass after the
   generated artifacts are current.
6. No credentials, sensitive evidence, screenshots, live records, UI claims,
   bulk claims, or product-independent-review claim is committed.
7. The child branch is clean, committed, and pushed by `fractal commit` before
