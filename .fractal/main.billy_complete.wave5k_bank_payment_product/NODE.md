...
Implement the accepted Wave-5k offline API product slice, using Codex Power
only. The authoritative contract is
`wiki/wave_fivek_freeze_independent_review.md`; supporting cited findings are
in `wiki/wave_fivek_ticketed_writes_contract.md` and
`.fractal/main.billy_complete/tmp/grok-research.md`.

Own only these product paths, plus directly generated coverage artifacts needed
to keep their claims true:

- `src/billy_mcp/api/bank_payment_writes.py` (new)
- `src/billy_mcp/server.py` (one import and one registration)
- `tests/api/test_bank_payment_writes.py` (new)
- `tests/unit/test_coverage_server.py` only if the registry assertion requires
  its documented 238 -> 242 update
- `coverage/api_v2_manifest.yaml`, `coverage/status.json`, and generated
  coverage report files only for `api.bankPayments.create` and
  `api.bankPayments.update`

Implement exactly four typed FastMCP tools:

- `api_bank_payments_create_preview` / `api_bank_payments_create_execute`
- `api_bank_payments_update_preview` / `api_bank_payments_update_execute`

Use the existing ticketed-write modules, especially `attachment_writes.py`, as
the implementation shape. Inputs have strict outer models: create accepts only
`{bankPayment}`, update only `{id, bankPayment}` with nonempty id; the opaque
inner map must be preserved, and a supplied `bankPayment.id` must equal route
id. Use the one shared `WriteProtocolService`; execute accepts only
`confirmation_ticket`, performs exactly one POST or PUT with relative paths
`/bankPayments` or `/bankPayments/{id}`, singular root `bankPayment`, and
required success root `bankPayments`.

Do not implement or register delete, bulk, generic HTTP/browser, webhook,
live, UI, or vision functionality. Singular DELETE is 405 and bulk is
ambiguous. Leave those inventory rows red. Do not claim live testing: after
real implementation and passing offline contract tests, only create and update
may become implemented and contract-tested, leaving `live_tested: false` and
overall `complete: false`.

Tests must cover registration count, strict outer validation, update id
equality, non-mutating preview, exact one-write POST/PUT request, required
plural success root, no fabricated related roots, ticket tamper/replay/expiry,
wrong executor before consume/HTTP, and typed authentication errors. Keep the
existing shared ticket tests authoritative; add focused coverage when the
bank-payment surface needs it.

Do not alter wiki review records, node seed files other than your own lifecycle
state, or unrelated product modules. Report the committed tip and exact test
results to the parent via radio. The parent owns integration and independent
product review.

## Completion Requirements

1. Exactly the four authorised bank-payment create/update preview/execute tools
   are registered; no delete or bulk bank-payment tool exists.
2. The tools use typed Pydantic input/output models and the shared
   confirmation-ticket protocol with all required bindings and one-write
   execution semantics.
3. Focused bank-payment tests and the configured lint and non-live test scripts
   pass. Coverage verifies 242 `api_*` tools and 168 implemented plus
   contract-tested offline API rows, while live/UI/vision/bulk/completeness
   remain fail-closed.
4. Only the owned product and generated coverage paths changed. No credential,
   screenshot, HAR, trace, browser session, or test customer data is committed.
5. Commit and push the leaf, post a concise radio handoff including the commit
   and test results, then run `fractal node finish` with a reason that does not
   overclaim product review or overall completeness.
