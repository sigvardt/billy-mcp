...
Own the Codex Power fallback for the Wave-5t UI/authentication discovery task.
The designated Grok leaf could not begin because of its own authentication
failure; therefore repeat the full bounded discovery task independently and
produce a cited, non-sensitive brief before any UI implementation. This is
research only: do not write product code or advance coverage.

Read the approved design, `wiki/billy_ui_discovery_brief.md`,
`wiki/wave_fives_residual_specials_research.md`, the API manifest/status, and
the current `src/billy_mcp/browser.py` and auth registration before exploring.
The frozen API research is context only; this leaf's authority is the Billy
interface and existing headless runtime posture.

Your project-file ownership is exclusively one new page
`wiki/wave5t_ui_auth_discovery.md`; you may update the generated wiki index.
Keep private observations in node memory or owner-only scratch. Do not edit
Python, tests, coverage manifests/status, existing wiki pages, node seed, or
any artifact outside that ownership.

Perform only bounded, headless Billy discovery:

- Never open, raise, focus, or manipulate a desktop window. Use Playwright
  only through the project's bounded browser capability and the approved Billy
  host allowlist. Do not use generic browser or arbitrary URL controls.
- Inspect the MCP-owned persistent profile only enough to establish whether a
  session can be reused; do not read, copy, log, or commit cookies, passwords,
  storage, TOTP material, downloads, or profile contents.
- Attempt the real bounded login/session-state workflow if it can run
  headlessly. Record precise DOM-observable state, redirect/login host, and
  recovery requirement. Return or recommend `AUTH_INTERACTION_REQUIRED` only
  for an observed CAPTCHA, passkey, push approval, or unsupported MFA; missing
  credentials or no session alone must be described accurately rather than
  relabelled as that error.
- Inventory the reachable authentication/organisation state and the first
  navigation/parity entry points. Map each observation to a specific future
  typed `auth_*` or `ui_*` workflow or an evidenced blocker; do not expose
  generic controls and do not claim all 339 UI rows are discovered.
- Do not perform UI business writes, create test data, send email/e-invoices,
  change settings, or test a production organisation. No screenshot, frame,
  HAR, trace, token, credential, or customer data may enter the repository.
  If a frame is essential to inspect, place it in owner-only temporary storage,
  record only the non-sensitive verdict, and purge it before completion.

The shared brief must cite Billy's current interface/official source material
where available and distinguish observed facts, existing-code findings, and
inferences. It must state the exact credential, dedicated-organisation, or
interaction prerequisite for the next UI/auth implementation leaf. It must not
claim UI implementation, DOM/read-back completion, vision verification, live
qualification, or coverage greening.

Post concise progress and the resulting evidence location to your outbox. Do
not wait for parent input.

## Completion Requirements

- `wiki/wave5t_ui_auth_discovery.md` is a cited, non-sensitive, lint-clean
  discovery record with a precise headless session/auth result, reachable
  bounded entry points, and a concrete next implementation or blocker path.
- The page names no secret, cookie, TOTP, browser-profile content, raw frame,
  HAR, trace, customer data, or production organisation. Any raw temporary
  evidence has been purged and that fact is recorded without sensitive detail.
- Every UI row remains red; no coverage manifest/status or product source is
  changed, and no generic browser control is introduced.
- The project wiki lints, the scoped diff contains only the owned page plus
  generated index state, and the node's lint/test checks pass as applicable.
- Commit the research leaf, report the commit and verified blocker/next path
  in the outbox, then run `fractal node finish` with a research-only reason.
  Independent product review is deliberately not a child completion gate.
