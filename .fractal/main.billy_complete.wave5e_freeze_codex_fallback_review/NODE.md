...
Perform the policy-required Codex Power fallback audit for the frozen Wave-5e
offline bill and bill-line ticketed-write contract at root tip `fc8118e`.
The planned Grok reviewer failed before any review edit because its CLI lacked
credentials. This is a read-only fallback and cannot replace the mandatory
independent Grok freeze gate.

You own exactly one project deliverable:
`wiki/wave_fivee_freeze_codex_fallback_review.md`. Do not modify production
code, tests, coverage artifacts, the frozen contract page, or any node seed
outside your own node directory. Do not claim current external research or a
Grok verdict; inspect only the cited evidence already frozen in the repository,
especially `wiki/wave_fivee_ticketed_writes_contract.md`,
`.fractal/main.billy_complete/tmp/grok-research.md`, the approved design,
inventory/status, server baseline, and accepted Wave-5d review pattern.

Audit all six exact contract rows: bills and billLines create/update/delete,
their preview/execute names, `/bills` and `/billLines` paths, singular roots,
PUT partial update rule, bodyless delete, strict outer / opaque inner input
boundary, required execute-name ticket binding, zero write retry, and optional
only `bills` parent mapping for bill-line responses. Verify the fail-closed
baseline is still 166 `api_*` tools, 130 offline rows, zero live/vision rows,
92 bulk rows red, and `complete: false`. Confirm no invoice-specific embedded
line/state rule, bulk body, webhook, live cleanup, browser/UI, or coverage
claim is inferred.

Write a concise non-sensitive fallback page with valid wiki frontmatter,
cited-repository sources, exact tip, explicit PASS/REJECT observations, and an
unambiguous statement that it is not an independent Grok acceptance and cannot
open the product gate. Run `wiki update --path=wiki`, `wiki lint --path=wiki`,
and applicable offline checks. Post a concise outbox handoff and self-finish
only after its committed page and checks are clean. Do not use credentials,
live mutations, headed browser automation, screenshots, HARs, traces, or raw
customer data.

## Completion Requirements

- `wiki/wave_fivee_freeze_codex_fallback_review.md` exists with valid wiki
  frontmatter, an exact reviewed root tip, and a PASS or REJECT result.
- The page covers every frozen bill and bill-line CUD row and all ticket,
  response-root, and fail-closed boundaries listed above.
- The page explicitly states that it is non-gating and not a Grok acceptance.
- Project wiki update/lint and applicable offline checks pass; the fallback
  branch is committed and clean.
- An outbox handoff reports the result and non-gating status.
