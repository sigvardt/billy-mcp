...
Retry the full Wave-5h attachment product review at root commit `29cecbe`
after the assigned Grok reviewer failed authentication before making edits. You
are Codex Power fallback review, so your review is explicitly supplemental and
cannot substitute for the mandatory authoritative Grok product review. Do not
implement or modify product code.

Independently inspect the accepted contract
`wiki/wave_fiveh_ticketed_writes_contract.md`, the cited product handoff,
`src/billy_mcp/api/attachment_writes.py`, server registration, manifest,
generated coverage, and attachment/registry/coverage tests. Repeat the
test-based review: run focused attachment plus registry/coverage tests and the
root non-live test script. Verify six attachment JSON CUD tools; strict outer
inputs; opaque inner payload; partial PUT with route/body id equality; bodyless
delete; client-relative `/attachments` paths; one shared confirmation protocol;
exact executor binding before ticket consume; ticket rejection boundaries;
response-root discipline; and no write retry.

Record whether the merged root independently supports registry 208 and 151
implemented/contract-tested coverage rows with live/vision zero and
`complete: false`. Explicitly preserve red boundaries for binary `/files`,
ambiguous bulk, live qualification, UI, vision, and overall completion. Do not
make current-official-doc claims beyond the cited frozen evidence: the Grok
authentication failure leaves the authoritative live-document/independent gate
open.

Your only project deliverable is
`wiki/wave_fiveh_product_codex_fallback_review.md`. It must name the reviewed
commit, commands/assertions run, findings, a PASS or FAIL supplemental verdict,
and the exact statement that it is non-authoritative and cannot satisfy the
Grok review requirement. Do not modify source, tests, coverage files, other
wiki pages, or the implementation. You may alter only your own
`.fractal/main.billy_complete.wave5h_product_review_codex_fallback/` seed,
memory, plans, and scratch in addition to the one review page. Keep raw
evidence in scratch and purge it before finish; post the result to outbox.

## Completion Requirements

- `wiki/wave_fiveh_product_codex_fallback_review.md` exists, is indexed, and
  records an explicit supplemental PASS or FAIL for the offline product at
  `29cecbe`, including test evidence and clear non-authoritative status.
- Focused attachment, registry/coverage, and root non-live test commands are
  independently run for a PASS; failures produce a precise FAIL record.
- The page preserves all red live/UI/vision/bulk/binary boundaries and states
  that a later authenticated Grok review is still required.
- No project files change outside the one owned review page; no sensitive raw
  evidence enters git; lint/wiki checks pass and scratch is purged before
  `fractal node finish`.
