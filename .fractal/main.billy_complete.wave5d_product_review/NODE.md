...
Wave-5d invoice and invoice-line ticketed-write slice at parent tip `1108e2f`.
You own only `wiki/wave_fived_product_independent_review.md`; do not edit
source code, tests, coverage artifacts, shared protocols, or the approved
design.

Use current official Billy documentation as primary evidence and cite the
direct source URL plus its observed fingerprint. Compare the committed root to
`wiki/wave_fived_ticketed_writes_contract.md` and the freeze review. Verify:

1. Exactly six documented CUD inventory rows map to the twelve `api_invoices_*`
   and `api_invoice_lines_*` preview/execute tools with POST/PUT/bodyless
   DELETE, singular roots, and locked `/invoices` and `/invoiceLines` client
   paths.
2. Inputs are strict around opaque inner payloads; update payload IDs cannot
   differ from route IDs; execute inputs accept only confirmation tickets.
3. The existing single confirmation store and `WriteProtocolService` remain
   shared; every handler binds its exact non-empty executor name; mismatch,
   replay, expiry, tamper, wrong organisation/target/payload, and zero-retry
   protections remain tested. Check the new cross-module invoice test proves a
   mismatch before ticket consumption or HTTP.
4. Invoice-line responses map optional parent `invoices` only when present;
   no fabricated parent root is accepted. Validate the documented
   `additional_plural_roots=("invoices",)` contract.
5. Server registration is 166 `api_*` tools, generated coverage contains the
   six new offline evidence rows, and status is 130 implemented/contract-tested
   with live/UI/vision/bulk/special rows still fail-closed and `complete: false`.
6. The root non-live suite passed 753 tests, with formatting, Ruff, Pyright,
   inventory, and repository-policy checks passing. Reproduce focused checks
   appropriate to the review from the committed tree.

Do not treat missing credentials, unauthenticated DELETE 200, Codex fallback,
or an inaccessible UI as positive evidence. No browser screenshots, traces,
HAR files, tokens, or customer data may be committed. Wave-5e remains closed
unless this review reaches an explicit offline product ACCEPT.

## Completion Requirements

1. `wiki/wave_fived_product_independent_review.md` contains a concise,
   non-sensitive ACCEPT or FAIL verdict, current official-source citation and
   fingerprint, reviewed parent tip, exact checks and test references, explicit
   coverage posture, and remaining live/UI/bulk/completeness limitations.
2. The review record is indexed and wiki lint passes.
3. If any concrete defect is found, the verdict is FAIL with actionable
   location and reproduction evidence; do not repair source outside this
   review scope.
4. Send the parent an outbox handoff naming the verdict and durable wiki path,
   then finish the node. A Codex fallback review is not an independent Grok
   acceptance and must say so explicitly.
