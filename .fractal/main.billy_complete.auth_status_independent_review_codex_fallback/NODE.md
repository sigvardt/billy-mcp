...
Perform an independent, defect-first Codex Power fallback review of the
committed typed `auth_status` slice at parent commit `cf38b5c`. Grok could
not run because its runner authentication was unavailable before task edits;
this fallback does not satisfy the mandatory Grok review gate. This is a
review-only leaf: you own `wiki/auth_status_independent_review_codex_fallback.md`
and may not change source, tests, coverage inventories, or generated coverage
status.
...
tests. Cite sources for every contract-dependent finding. Review the exact
fixed-root/headless route and DOM contract: only
`https://mit.billy.dk/login` with no query/fragment; exactly one visible
email/password/remember control; exactly one visible
`button[data-cy=login-button]` with trimmed `Log in` or `Log ind`.
Reject generic controls, `button[type=submit]`, generic text matching, or
form inference. Check typed and redacted outcomes: `AUTH_REQUIRED`,
`UI_CHANGED`, `EGRESS_DENIED`, `BILLY_ERROR`.

Inspect committed executable bytes from a clean archive and run focused tests
and static checks. Do not use credentials, submit login, retain browser
evidence, or modify/green coverage. State explicitly that no live,
interface-E2E, read-back, vision, UI-parity, completeness, or Grok-review claim
is being made. Record exact defects (file/line, impact, repair), or explicit
no-defect verdict, reviewer identity, sources, commands/results, and the
fallback boundary in the wiki. Escalate a defect to the parent via radio.

## Completion Requirements

- `wiki/auth_status_independent_review_codex_fallback.md` is a
  non-sensitive, cited, defect-first fallback review record.
- It records source/test evidence, makes the Grok-gate limitation explicit,
  and keeps every UI coverage row red.
- The wiki lints cleanly; only the review record is committed; a parent handoff
  is posted; and the leaf finishes.
