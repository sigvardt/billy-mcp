---
name: todo
desc: Open product work for the bounded offline closeout.
tags: [todo]
sources: []
...
- Binding `21A3D94F`: owner-skip encode landed, IR PASS, FIX-VERIFY recorded. COMMIT then finish. No live UI. No live API. Do not claim fresh production MCP qualification.
- Residual five UI writes plus annual reports stay `out_of_scope_by_user` (`FE6FA4B1` / `DC3B8E96`). Do not remake dumps. Do not arm `_ledger_write`.
- Typed `api_bank_lines_list` filters already landed. `accountId` required. Shared `BankListRequest` unchanged.
