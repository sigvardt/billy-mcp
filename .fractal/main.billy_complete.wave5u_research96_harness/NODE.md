...
Implement the bounded Wave-5u Research96 **offline harness fixture** slice.
You own only these project files:

- `src/billy_mcp/live_probe.py`
- `tests/unit/test_live_probe.py`

Do not modify any other project file. Node-seed files, plans, and private
memory are allowed only for this node's lifecycle; do not add project wiki,
coverage, documentation, tool-registration, browser, API-client, or CI changes.
Do not spawn children.

Contract authority is the committed Research96 relay in
`wiki/wave_fives_residual_specials_research.md`, the Wave-5u contract in
`wiki/wave5u_method_probe_contract.md`, and the current official Billy API
documentation cited there. The parent already obtained the Grok research brief
before this Codex Power implementation. Treat the following as exact
research-only facts, not product or live authority:

1. Cover every frozen residual candidate id from `research88_candidates()`
   exactly once with an immutable unauthenticated outcome fixture. The only
   permitted outcomes are: permanent `405` for the false-friend creates and
   updates plus `api.bankPayments.delete`; auth-gated `401` for transactions
   create/update; and meta-only unqualified `200` for transactions delete and
   invoiceReminderAssociations singular delete. A fixture must not allow a
   real-method request, a tool, a cleanup claim, or a coverage state change.
2. Add an immutable canonical bulk-delete form fixture that encodes the
   server-stated query-only shape: `DELETE /{resource}?ids[]=123&ids[]=456`.
   The exact repeated query name is `ids[]`; it has no request body and must
   not accept a caller-supplied route, body, or query parameter name. Empty or
   synthetic ids remain unqualified. JSON and form-body variants remain
   rejected/non-product forms and must be explicit in the fixture/tests.
3. Add a static, non-network organisation-path ambiguity fixture: no-token
   `/user/organizations` is `404 UNKNOWN_RESOURCE`; `/organizations` is
   `401 AUTHENTICATION_REQUIRED`; invalid-token `401` is auth-first and
   path-agnostic. It must preserve the documented `/user/organizations` tool
   route pending a valid dedicated-non-production-token observation. Do not
   change that tool.
4. Preserve every existing fail-closed invariant, particularly
   `RealMethodSafetyGate.permits_real_method_network is False`. This work must
   not introduce a non-OPTIONS request path, call any live service, use
   credentials, register a FastMCP tool, touch inventory/coverage, infer an
   alternate organisation route, or add a webhook.

Use strict frozen Pydantic models and static factories/fixtures consistent with
the existing module. Test all exact membership and adversarial rejection cases:
wrong status/classification, missing/duplicate residual ids, body-form
misclassification, invalid query name/body, and any attempt to authorise
networking. The default offline test path must still make no HTTP request when
there is no token or a gate is missing.

Before committing, run focused pytest, Ruff format/check, and Pyright for the
two owned files. Also run the parent project's commit-mode suite if practical.
Post concise status to your outbox. Escalate an evidence or scope conflict to
the parent instead of broadening the implementation.

## Completion Requirements

1. The only project-source changes are the two owned files.
2. Tests prove exact 29-row residual coverage, query-only repeated `ids[]`
   canonical form, explicit JSON/form-body non-acceptance, organisation-path
   ambiguity, and unchanged pre-network denial.
3. `uv run pytest tests/unit/test_live_probe.py`, `uv run ruff format --check
   src/billy_mcp/live_probe.py tests/unit/test_live_probe.py`, `uv run ruff
   check src/billy_mcp/live_probe.py tests/unit/test_live_probe.py`, and `uv
   run pyright src/billy_mcp/live_probe.py tests/unit/test_live_probe.py` pass.
4. Coverage is not edited or greened; no new API/UI tool or non-OPTIONS
   residual/bulk transport exists.
5. The worktree is committed and clean, durable child state is folded, and an
   outbox handoff identifies the changed files and verification results. Then
   run `fractal node finish --reason="Research96 fail-closed harness fixtures
   delivered"`.
