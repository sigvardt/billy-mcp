...
Author the wiki-only offline freeze contract for the already cited Research73
salesTaxReturns update slice. You are the Codex Power implementation-authoring
leaf. Do not conduct fresh web research: the parent-provided cited authority is
`wiki/wave_fiver_sales_tax_returns_freeze_ready_research.md` and its scratch
brief. Treat the approved design and that research page as the complete
contract.

Your exclusive product files are:

- `wiki/wave_fiver_ticketed_writes_contract.md`
- the generated entry for that page in `wiki/_index.md`

Do not edit source, tests, coverage artifacts, generator mappings, manifests,
root memory, plans outside your node, or any other wiki page. Do not create
tools, register tools, green inventory, use credentials, call live Billy APIs,
run a browser, or claim independent review, product authority, live/UI/vision
qualification, cleanup qualification, or completeness.

Create a cited contract that freezes exactly one inventory operation:

| Inventory id | Future preview tool | Future execute tool | Client-relative request | Required root |
| --- | --- | --- | --- | --- |
...
The contract must make the outer model strict (`extra="forbid"`), leave the
inner `salesTaxReturn` a `dict[str, JsonValue]`, accept an omitted inner id but
require a supplied inner id to match the route id, and require only the stated
response root. Freeze the shared preview/execute protocol: preview never
mutates; ticket is opaque, single-use, five minutes or less, and bound to exact
execute tool, organisation, target, canonical request, and expected effect;
execute accepts only a non-empty ticket, makes one write with no retry, and
fails closed on invalid/expired/consumed/mismatched tickets.

Record only the evidence-supported method gates: POST 405, PUT 401, DELETE
405, and empty-body PUT 400. Explain Supports/property boundaries: readonly
fields are not client writable; blank-notes `periodText`, `reportDeadline`,
and `isSettled` remain opaque and are not a writable or reversible live claim.
State clearly that singular DELETE is unsupported and restore-via-PUT remains
unproven; settlement might be one-way.

Explicitly exclude create, singular delete, all bulk operations, webhooks,
generic HTTP/browser controls, field-level payload samples or validation,
source implementation, registration, contract tests, coverage/status changes,
inventory greening, live work, UI/vision work, and completeness claims.

Run `wiki update --path="$WIKI_DIR"`, `wiki lint --path="$WIKI_DIR"`, the
node lint script, and `git diff --check`. Commit the page and index with the
node workflow, post a concise outbox handoff, update private memory only if it
contains durable current state, and finish when the requirements below are met.

## Completion Requirements

1. `wiki/wave_fiver_ticketed_writes_contract.md` is a complete, cited,
   non-sensitive wiki-only contract with the exact two-tool surface above.
2. The contract preserves the strict/opaque/ticket/no-retry, method-gate,
   response-root, property-boundary, cleanup-honesty, and exclusion limits in
   the instructions.
3. The only product-file changes are the contract page and its generated root
   wiki index entry; no code, tests, coverage, browser, credentials, or live
   data are touched.
4. Wiki update/lint, node lint, and `git diff --check` pass; the leaf commits
   its work and reports the result through its outbox.
