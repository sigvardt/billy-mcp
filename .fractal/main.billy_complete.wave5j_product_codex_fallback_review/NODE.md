...
Perform the required independent product review for the merged Wave-5j
bank-line ticketed-write implementation. Grok was unavailable before it made
any edit because its CLI lacked credentials, so this is the routing-policy
fallback. You are a reviewer, not an implementer: do not change production
code, tests, manifests, coverage flags, or the API/UI tool surface. Your only
project deliverable is the new shared-wiki page
`wiki/wave_fivej_product_independent_review.md`; your private memory and plan
files are also in scope for your operating state.

Review the committed parent baseline whose relevant integration is `d86844f`
and whose review-gate handoff is `5a42c0f`. Treat the following as authority:
...
- current official Billy API documentation at `https://www.billy.dk/api/`;
- the cited, owner-only research handoff at
  `.fractal/main.billy_complete/tmp/grok-research.md`.

Independently inspect the merged root bytes and run the relevant offline
checks. Audit all eighteen frozen preview/execute tools for exact handler
names, client-relative paths, methods, singular request roots, primary and
optional response-root mapping, bodyless deletes, opaque payload boundaries,
partial-update id equality, typed error mapping, and no write retry. Verify
ticket binding and single-use behavior across same-resource and cross-resource
executors before HTTP. Check the inventory changes, the 238 API-tool registry,
the 166 offline implemented/contract-tested rows, and that live, UI, vision,
bulk, bankPayments, and completeness surfaces remain fail-closed.

The page must contain `desc:` frontmatter, direct sources, exact commands and
their observed results, concise evidence for each review category, and an
unambiguous `ACCEPT` or `REJECT` verdict. A REJECT must name every actionable
finding with its exact file/location and correction needed; do not implement
those corrections. An ACCEPT must state the offline boundary explicitly and
must not equate tests or unauthenticated probes with live qualification.

Keep all raw frames, traces, credentials, tokens, downloads, and sensitive
material outside the repository. Do not launch a browser UI workflow or use
production data. Run `wiki update` and `wiki lint` after creating the page.
Report your verdict through radio and finish your node in the same iteration
when your own completion requirements hold.

## Completion Requirements

1. `wiki/wave_fivej_product_independent_review.md` is committed and contains a
   cited, non-sensitive ACCEPT or REJECT of the merged Wave-5j product.
2. The review evidences independently executed relevant offline tests plus the
   contract, inventory, registry, ticket-binding, error, response-mapping, and
   scope-boundary checks described above.
3. The page makes no unsupported live, UI, vision, bulk, cleanup, or overall
   completeness claim. It neither changes implementation/coverage state nor
   authorizes Wave-5k work by itself.
4. `wiki update --path=$WIKI_DIR` and `wiki lint --path=$WIKI_DIR` pass, the
   review worktree is clean, and the node has posted its verdict to the parent.
