...
Implement the Wave-5a response-mapping hardening only. You are a childless
`codex-power` leaf and may edit exactly these product files:

- `src/billy_mcp/api/write_protocol.py`
- `tests/unit/test_write_protocol.py`

The contract is frozen in `wiki/wave_five_ticketed_writes_contract.md`; the
current cited Grok brief is `.fractal/main.billy_complete/tmp/grok-research.md`
(official docs ETag `hsisik4g9p3603`, MD5
`c2efda0ee4cf9cf200e14910c5fc6996`). Read both plus the existing protocol and
tests before editing. Do not perform fresh web research or change the frozen
contract.

The existing generic protocol preserves only one response plural root. Extend
the operation specification and mapper so a resource can declare every
documented changed plural root and `WriteExecutionResult.changed_records`
retains every present declared root. Preserve current primary-root behaviour,
typed invalid-success errors, delete behaviour, optional
`meta.deletedRecords`, canonical request binding, ticket single use, escaped
IDs, locked relative paths, and the no-retry rule. Do not fabricate missing
response roots. Add focused fixtures that prove a product-with-prices-style
multi-root response survives while existing single-root and delete cases still
pass.

Never edit server registration, coverage, client, confirmations, auth, UI,
browser, inventory, wiki, or another resource module. Do not register tools,
change coverage status, claim live testing, or log tickets or sensitive values.
Use only local/offline test doubles; no Billy mutations. Keep raw evidence out
of the repository.

Run the focused tests, the node lint script, and the non-live node test script.
Commit the scoped product delivery, report the exact commit and checks through
your outbox, and finish only after the completion requirements below hold.

## Completion Requirements

1. The only product-code changes are the owned protocol and unit-test files;
   no server, coverage, client, auth, UI, browser, bulk, special-route, or
   inventory change is present.
2. A resource can declare multiple documented changed plural roots, and mapped
   success retains all present declared roots without fabricating absent roots.
3. Existing ticket, delete, error, and single-root semantics remain covered and
   passing; the new multi-root fixture passes from the committed bytes.
4. `bash $NODE_DIR/scripts/lint.sh` and `bash $NODE_DIR/scripts/test.sh` pass,
   with focused `tests/unit/test_write_protocol.py` evidence reported.
5. The delivery is committed, the working tree is clean, and a concise outbox
   report states the commit, owned files, and checks. Parent integration and
   independent review are explicitly out of scope for this leaf.
