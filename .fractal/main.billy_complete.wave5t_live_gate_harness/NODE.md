...
Implement the bounded Wave-5t live residual/bulk gate harness from the frozen
Research88 evidence on your starting commit. This is Codex Power repository
work, not fresh contract research and not an API product slice.

Read `wiki/wave_fives_residual_specials_research.md`,
`wiki/offline_write_probe_rules.md`,
`.fractal/main.billy_complete/tmp/grok-research.md`, and the approved design
before editing. The only API host is `https://api.billysbilling.com/v2`.

Exclusive source ownership: `src/billy_mcp/live_probe.py`,
`scripts/run_live_probe.py`, `tests/live/test_live_probe.py`, and
`tests/unit/test_live_probe.py`. Create only those files if absent. Do not
edit existing API tools, browser code, coverage manifests or generated status,
shared node seed, or project wiki. If a necessary contract boundary lies
outside this ownership, report it to the parent instead of widening scope.

The harness must:

- Resolve `BILLY_API_TOKEN` internally without logging, serialising, or
  returning it.
- Require an explicit dedicated non-production organisation identifier and
  fail closed before any request if it is missing or does not match the
  configured test-only guard.
- In the no-token path, make no network request, create no evidence file, exit
  successfully, and emit a structured non-sensitive `skipped_no_token` result.
- Bound every request to the fixed Billy API base. It may observe only the
  Research88 residual and bulk candidate matrix. It must not infer a wire
  contract from unauthenticated meta-success, Supports text, or a 401 alone.
- Create no persistent data until an authenticated observation has established
  a safe disposable operation. Any later data must have a unique test tag,
  be tracked, removed in reverse dependency order, independently read back,
  and fail visibly if cleanup cannot be proven.
- Write any live evidence only to an owner-only directory outside the
  repository, and record no raw token, customer data, screenshot, HAR, trace,
  or browser artefact.
- Never change coverage status or claim `implemented`, `contract_tested`, or
  `live_tested` from harness scaffolding or a skipped run. Do not implement
  residual/bulk FastMCP tools, webhooks, external email, or e-invoice sends.

Use strict typed Python models and deterministic unit tests for every safety
gate, including missing token, missing/wrong test organisation, fixed-base
lock, secret redaction, no-network skip, exact candidate construction, and
evidence path permissions. Mark real live tests with pytest's `live` marker;
they must skip safely without credentials and must never target production.
Keep worktree tests bounded and headless-free.

Post concise progress or blockers to your outbox. Do not wait for a reply.

## Completion Requirements

- The owned files contain a real, typed, fail-closed live gate harness and its
  focused unit/live tests; no stub, generic HTTP control, or arbitrary shell
  capability is introduced.
- Focused tests and `BILLY_TEST_MODE=commit bash "$NODE_DIR/scripts/test.sh"`
  pass. Run the node lint script and repository policy check before committing.
- The no-token execution path has been exercised and demonstrated to make no
  network request or repository evidence artifact.
- All current coverage rows remain red as before: no manifest/status mutation,
  no live/vision claim, and no completeness claim.
- `git diff` is restricted to the exclusive ownership paths and contains no
  credential, sensitive evidence, screenshot, HAR, trace, or raw test data.
- Commit the finished leaf, report its commit and checks in the outbox, then
  run `fractal node finish` with an infrastructure-only reason. Independent
  Grok review is owned by the parent after merge and is not a child completion
  gate.
