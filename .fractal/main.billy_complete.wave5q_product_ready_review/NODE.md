...
Perform an independent Grok review of the committed research package
`wiki/wave_fiveq_users_product_ready_research.md`. You are a research-review
leaf: do not implement product code, register tools, change tests, coverage
manifests, status reports, or the users freeze contract.

Your exclusive project deliverable is
`wiki/wave_fiveq_users_product_ready_research_independent_review.md`; you may
also maintain only your own node memory, plans, and seed. The parent will merge
the review page.

After PREPARE merges the parent, independently re-fetch or inspect the current
official Billy documentation at https://www.billy.dk/api/ and validate the
research package against the locked base https://api.billysbilling.com/v2,
...
`coverage/status.json`, the approved design, and
`.fractal/main.billy_complete/tmp/grok-research.md` when present.

Check that research supports only the prospective users update pair:
`api_users_update_preview` / `api_users_update_execute`, PUT `/users/:id`,
strict outer id plus opaque user map, optional matching inner id, `users`
response root, shared ticket protocol, and one no-retry write. Verify the
401/405 gates, PII and privilege sensitivity boundary, fail-closed cleanup
language, and 177-to-178 offline arithmetic. Confirm all non-claims: no
create/delete/bulk/webhook tools, no live/UI/vision qualification, no complete
status, and no product source before the separate freeze independent ACCEPT.

Deliver a cited non-sensitive **ACCEPT as research** or **REJECT** page. This
verdict cannot accept the freeze contract or authorise implementation by itself;
it only corroborates (or rejects) the product-ready research. Keep all browser
activity headless, do not use credentials or create records, and retain no raw
evidence in git.

Report progress and final verdict via radio; do not wait for a parent reply.

## Completion Requirements

1. `wiki/wave_fiveq_users_product_ready_research_independent_review.md` exists
   with valid frontmatter, cited current evidence, and an explicit ACCEPT as
   research or REJECT verdict.
2. It verifies the exact prospective two-tool users-update scope and all
   fail-closed exclusions, including the still-separate freeze-review gate.
3. It makes no implementation, coverage-green, live, UI, vision, bulk, delete,
   or completeness claim, and changes no project file outside its review page
   and node seed.
4. `wiki update` and `wiki lint` pass for the project wiki, and the node test
   and lint scripts pass without tracked sensitive evidence.
5. The final radio update names the committed review page, commit, verdict, and
   remaining gate. Then run `fractal node finish` in the same iteration.
