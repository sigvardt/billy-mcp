...
Perform the mandatory independent Grok review of the integrated Wave-5a
registration product. This is a repository review and current-official-doc
review only. Do not implement or fix product code. Your exclusive project
deliverable is
`wiki/wave_fivea_root_registration_independent_review.md`; do not edit source,
tests, coverage artifacts, inventories, status, scripts, or existing wiki
pages.

Review the parent baseline at `36e5b62` (the labelled registration merge is
`45f099f`; child product tip is `7cb74e2`). Start from current official Billy
documentation and produce direct citations. Read first:
...
- `wiki/wave_fivea_root_registration_baseline_review.md`, and
- `docs/superpowers/specs/2026-07-28-billy-mcp-complete-design.md` sections 6
  and 8.

Independently inspect the integrated server, the four accepted write modules,
the shared confirmation and write-protocol implementations, the coverage
generator, generated API manifest/status, and the registry test. Use a clean
`git archive 36e5b62` export for executable checks. Re-fetch the official Billy
docs yourself and compare its fingerprint and singular write contract against
the cited research brief; clearly label any inference.

Verify all of the following:

1. One `ConfirmationStore` and one `WriteProtocolService` are constructed per
   `create_server` call and shared by contact, contact-person, catalog, and
   daybook write registrars.
2. The runtime has exactly 124 `api_*` tools plus `coverage_status` and
   `coverage_report`, with all 30 expected preview/execute tools registered.
3. Preview is non-mutating; execute accepts only `confirmation_ticket`; ticket
   binding covers tool, organisation, target, request and expected effect; and
   replay, expiry, tampering, wrong target/tool/request/effect, and no-retry
   boundaries have executable evidence.
4. The 15 clear CUD inventory rows alone are offline green with real test
   references: implemented 109, contract-tested 109, live-tested 0, complete
   false. The 92 ambiguous bulk rows retain empty tool names; all UI/vision,
   specials, and unauthenticated-delete cleanup claims stay red/fail-closed.
5. Tests cover the exact singular request roots, methods, relative paths
   without duplicate `/v2`, typed error mapping, response/deleted-record
   mapping, flat Pydantic surfaces, and sensitive-value redaction.

Run the focused write/coverage suites plus formatter, Ruff, Pyright, coverage
policy, and repository-policy checks from clean committed bytes where useful.
Do not use credentials, perform live writes, open a visible browser, retain
screenshots/frames/HAR/traces, or claim full product completion.

The report must contain the exact reviewed commits, files and commands, direct
official-doc citations, a concise per-gate verdict table, tested evidence, and
an explicit scoped ACCEPT or FAIL verdict. A scoped ACCEPT can approve only
the integrated offline registration slice; it must explicitly say that live,
UI, vision, bulk, specials, remaining writes, and overall completion remain
red or incomplete. A FAIL must name reproducible defects and preserve the same
fail-closed boundary.

## Completion Requirements

- `wiki/wave_fivea_root_registration_independent_review.md` is the sole
  project deliverable and has valid `desc:` frontmatter, direct official-doc
  citations, exact commits, verification commands/results, and a scoped
  ACCEPT/FAIL verdict.
- The report independently verifies or rejects the shared ticket-store wiring,
  124-tool runtime surface, 15-row offline evidence, and fail-closed coverage
  boundaries described above.
- `wiki update --path=$WIKI_DIR` and `wiki lint --path=$WIKI_DIR` pass after
  the report is written.
- No source, tests, coverage files, status files, configuration, or raw review
  evidence are changed or committed.
- Post a concise outbox handoff with the report path and verdict, then run
  `fractal node finish --reason="independent Wave-5a registration review delivered"`.
