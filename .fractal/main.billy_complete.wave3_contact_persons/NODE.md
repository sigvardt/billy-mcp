...
Implement only the Wave-3 contact-person read cluster under the frozen contract
at `wiki/wave_three_unfiltered_reads_contract.md` and its cited Grok brief.

Own exactly `src/billy_mcp/api/contact_person_reads.py` and
`tests/api/test_contact_person_reads.py`. Add real, typed
`api_contact_persons_get` and `api_contact_persons_list` using the established
typed read-tool pattern and a module-local registration function. Root owns
server registration, redaction, coverage evidence/manifests, and wiki files.

Use `/contactPersons` and `/contactPersons/{id}`, roots `contactPerson` and
`contactPersons`, URL-encode IDs, keep payloads opaque, and map typed 401
errors. Lists admit only page, pageSize (1-1000), include, free sortProperty,
and ASC/DESC sortDirection. Reject offset, contactId, other invented filters,
and every undeclared input. Treat email payload content as sensitive: do not
log fixtures or live data.

Do not implement writes, bulk, UI, live calls, generic tools, or any scope
outside the two owned files. Commit and report the delivery to parent radio.

## Completion Requirements

- Owned module exposes the two real typed tools; focused tests cover paths,
  query surface, URL encoding, root/paging mapping, unknown input rejection,
  and typed 401 mapping.
- Focused pytest, Ruff format/check, and Pyright pass from committed bytes.
- Scoped files are committed and reported to parent. Do not claim root
  registration, coverage green, live qualification, or completeness.
