...
Author the offline Wave-5k freeze contract and nothing else. You are the
Codex Power repository-authoring leaf for exactly one deliverable:
`wiki/wave_fivek_ticketed_writes_contract.md`.

Ownership is file-level: modify that one project-wiki page only. Do not modify
source, tests, coverage manifests or status, existing wiki pages or indexes,
or any product behavior. Your node seed, plan, and private memory are normal
node-owned operational files. Do not spawn children.

Use the already accepted, cited evidence in:
...
- `coverage/api_v2_manifest.yaml`, `coverage/status.json`, and
  `wiki/wave_fivej_ticketed_writes_contract.md` as the structural template.

Do not perform new web research: the parent supplied the current Grok-backed
research chain. The contract must state its evidence fingerprint exactly as
ETag `hsisik4g9p3603`, MD5 `c2efda0ee4cf9cf200e14910c5fc6996`, body 147934
bytes, and remain an offline contract only.

Freeze exactly these two inventory rows and their preview/execute twins:

| Inventory id | Preview | Execute | HTTP | root | required success root |
| --- | --- | --- | --- | --- | --- |
...
Require client-relative paths (no duplicate `/v2`), strict outer typed inputs,
opaque inner `bankPayment` maps, partial PUT with inner/route id equality when
the inner id is present, root shared `ConfirmationStore` and
`WriteProtocolService`, exact server-owned executor binding, short-lived
single-use tickets, execute input containing only `confirmation_ticket`, and
one write request with no retry. Additional plural response roots may be
returned only when actually present; never promise a fixed related-root set.

Document the official boundaries without inventing wire schemas: immutable
organization/contact/entryDate/cashAmount/cashSide/cashAccount/cashExchangeRate
and subjectCurrency; `cashSide` documents `debit`/`credit` but remains opaque;
feeAccount is conditional on feeAmount; createdTime and contactBalancePostings
are readonly; associations are immutable; and `isVoided=true` is the primary
documented mutable void path and cannot be reversed. Do not authorize a live
cleanup claim.

Singular bank-payment delete is explicitly excluded: unauthenticated DELETE
returned 405 `METHOD_NOT_ALLOWED` with `Resource at bankPayments does not
support deleting a single record.` This overrides the Supports table for this
offline contract. Keep delete, bulk save/delete, UI, vision, live qualification,
and coverage rows red. State the arithmetic as current 238 tools/166 offline
rows, later product target 242/168 only after real implementation and tests;
this page itself changes neither.

Use accurate frontmatter with a descriptive `title` and `desc`, cite the source
pages above, run wiki lint for the project wiki, and report the resulting
contract path and remaining product gate via your outbox. Do not claim that a
contract page is accepted; an independent Grok freeze review remains required.

## Completion Requirements

- `wiki/wave_fivek_ticketed_writes_contract.md` exists with valid frontmatter
  and a cited offline-only contract matching the exact two-row/four-tool table.
- The page contains the protocol, payload, response, field-boundary, 405-delete,
  bulk, coverage, and exclusion limits stated in the Instructions; it introduces
  no unsupported route, webhook, generic control, coverage green, or product
  claim.
- The project diff outside this node seed contains only the owned contract page.
- `wiki lint --path="$WIKI_DIR"` and `bash "$NODE_DIR/scripts/test.sh"` pass;
  report the result via outbox before finishing.
