...
Do not edit generate_coverage_report.py, scripts/check_coverage.py, src/billy_mcp/server.py, or src/billy_mcp/browser.py. Root already wired register_ui_invoice_write_tools(server, ui_write_protocol). Fill that function.
...
Implement draft create (Gem som kladde) first. Never Godkend, Send, or email.
Fail closed on send/email. Drafts only. Delete drafts in cleanup.

Tools to register: ui_invoices_create_preview/execute, ui_invoices_update_preview/execute, ui_invoices_delete_preview/execute
Coverage rows: ui.parity.invoices.create, ui.parity.invoices.update, ui.parity.invoices.delete
Read wiki/ui_write_ticket_protocol.md and the parent plan 190.1.


## Completion Requirements

1. register_ui_invoice_write_tools registers the family preview and execute tools from src/billy_mcp/ui_writes/invoices.py.
2. Offline unit tests in tests/unit/test_ui_invoices_writes.py pass (preview, execute-once, replay, expiry, wrong tool).
3. Live MCP test in tests/live/test_ui_invoices_writes.py passes, or the node recorded a concrete live blocker that is not an unsafe skipped submit.
4. wiki/ui_invoices_writes.md describes the tools, fail-closed rules, and cleanup. No secrets.
