...
Implement the reusable, process-volatile ticketed write protocol for the first
Billy API write cohort. This is Codex Power implementation work, governed by:

- `wiki/wave_five_ticketed_writes_contract.md` (frozen Wave-5 contract)
- `.fractal/main.billy_complete/tmp/grok-research.md` (cited official research)
- `docs/superpowers/specs/2026-07-28-billy-mcp-complete-design.md`

You exclusively own these product files:

- `src/billy_mcp/api/write_protocol.py` (new)
- `tests/unit/test_write_protocol.py` (new)

Do not edit any other product, test, coverage, generated, wiki, or root-server
file. In particular, do not edit `src/billy_mcp/server.py`,
`src/billy_mcp/confirmations.py`, `src/billy_mcp/client.py`,
`scripts/generate_coverage_report.py`, `coverage/`, or existing read modules.
Escalate any needed shared-contract change by radio instead of widening scope.

Deliver a small, production-ready shared module for the later resource leaves:

1. Typed Pydantic `extra="forbid"` models for a write preview result, an
   execute-only confirmation-ticket input, and mapped write execution result.
   The preview result includes summary, exact canonical request, expected
   effect state, opaque ticket, and UTC expiry. The execute model has only
   `confirmation_ticket`.
2. A typed operation specification and service/helpers that let resource
   modules preview singular-root POST / PUT / DELETE operations and execute
   the exact stored request through `BillyHttpClient.request`. The service must
   retain the authoritative binding server-side so execute never accepts a
   caller-supplied business payload.
3. Exact `ConfirmationStore` binding and atomic consume semantics using the
   existing store: execute tool name, organisation, target, canonical request,
   expected effect, plus null file/destination bindings for this cohort. Do not
   alter the store's ticket entropy or TTL contract.
4. No Billy request during preview. Execute has no automatic write retry. Map
   only documented changed-record roots and optional `meta.deletedRecords`; do
   not fabricate a success if the expected root shape is absent. Pass existing
   typed upstream errors through unchanged.

Keep resource payloads opaque at this shared layer; resource leaves own their
root names, paths, Pydantic tool inputs, registrations, and property-specific
validation. Do not add a generic HTTP or browser interface. Do not make a
network call outside MockTransport-based tests. Do not claim live, UI, vision,
bulk, special-route, or coverage completion.

Use focused tests to prove canonical request construction, preview non-mutation,
exact method/path/body on execute, no write retry, changed-record/deleted-record
mapping, malformed success handling, 401/404 passthrough, and ticket tampering,
tool/org/target/request/effect mismatch, replay, expiry, and concurrency. Test
actual FastMCP-compatible typed model serialization where useful. Redact ticket
and sensitive values from any error/log-facing details.

Before committing, run your focused tests plus formatting, Ruff, Pyright,
coverage checks, repository policy checks, and `bash $NODE_DIR/scripts/test.sh`.
Post an outbox report naming the exported protocol API, exact tests run, and any
contract issue. Commit only the two owned product files and your node state.

## Completion Requirements

1. `src/billy_mcp/api/write_protocol.py` exposes a coherent typed protocol that
   resource leaves can use for all three singular write shapes without generic
   caller-controlled HTTP.
2. Preview never sends an HTTP mutation; execute accepts only a ticket and
   sends the exact stored canonical request once through the locked client.
3. The exact confirmation binding, TTL, single-use, and failure-code behavior
   are exercised for tampering, mismatch, replay, expiry, and concurrent use.
4. Success mapping and typed upstream errors are exercised with MockTransport;
   no live Billy traffic or coverage state change occurs.
5. `tests/unit/test_write_protocol.py`, Ruff, Pyright, coverage/repository
   policy checks, and `bash $NODE_DIR/scripts/test.sh` pass from committed
   bytes; the child worktree is clean and within its two-file scope.
6. The child posts its result to outbox and runs `fractal node finish` with a
   concise reason after its committed deliverable is verifiably complete.
