...
Author the Wave-5m offline ticketed-write **freeze contract only** for
singular Billy `contactBalancePayments` create + update. You are a Codex Power
repository author working from the cited Grok evidence supplied below. Do not
perform new external research, product implementation, coverage greening,
independent review, live testing, UI/browser work, or any unrelated edit.

You exclusively own `wiki/wave_fivem_ticketed_writes_contract.md` and its
derived `wiki/_index.md` entry. Do not edit source modules, tests, generated
coverage, status, probe rules, any Wave-5l review page, or any other wiki page.
The parent will request a separate Grok freeze review after this committed page
is integrated.

Frozen Grok research evidence:

- Official source: https://www.billy.dk/api/; current cited fingerprint ETag
  `hsisik4g9p3603`, MD5 `c2efda0ee4cf9cf200e14910c5fc6996`, 147934-byte body.
  API traffic is locked to `https://api.billysbilling.com/v2` and all page paths
  below are client-relative.
- Unauthenticated probes: POST `/contactBalancePayments` and PUT
  `/contactBalancePayments/nonexistent-probe-id` returned 401
  `AUTHENTICATION_REQUIRED`; singular DELETE returned 405
  `METHOD_NOT_ALLOWED` with the documented single-record-delete refusal.
- Supports are get, list, create, update, bulk save, bulk delete; no singular
  delete. Reads already exist. Create/update inventory rows remain red until a
  future product leaf; bulk rows remain `ambiguous_bulk` with empty tools.
- Exact future tool pairs: `api_contact_balance_payments_create_preview` /
  `_execute` for `POST /contactBalancePayments`, and
  `api_contact_balance_payments_update_preview` / `_execute` for
  `PUT /contactBalancePayments/:id`. Both use singular root
  `contactBalancePayment` and require response root `contactBalancePayments`.
- Inputs are strict outer objects only: create exactly
  `{contactBalancePayment: map}`, update exactly `{id: non-empty string,
  contactBalancePayment: map}`; inner map is opaque. If an inner id is present,
  it equals the route id. Preview is non-mutating; execute is exactly
  `{confirmation_ticket: non-empty string}` and uses the shared short-lived,
  single-use exact-operation confirmation protocol with one request/no retry.
- Official property boundaries: organization, entryDate, amount, and
  associations are immutable/required; contact and currency are auto-set;
  side is auto-set debit/credit; createdTime is readonly; isVoided is only a
  documented mutable update candidate. Do not invent a map schema, `*Id`
  aliases, relation wire form, date/amount encoding, unvoid semantics, or
  cleanup strategy. The official bankPayment sample is for a different resource
  and must not be copied.
- No webhooks are documented. Delete, bulk, generic HTTP, browser actions,
  live/UI/vision qualification, credentials, persistent data, invoice-late-fee,
  invoice-reminder, contactBalancePostings, and invoiceReminderAssociations
  product work are excluded. The latter two create/update surfaces are 405
  blocked in the cited probes.

Model the page after `wiki/wave_fivel_ticketed_writes_contract.md`, but make it
self-contained and truthful for contact-balance payments. State the root's
current post-Wave-5l offline counts (246 `api_*` tools, 170 implemented and
contract-tested API rows) as context only; Wave-5m stays red and
`complete: false`. Record that a product leaf may not start until a separate
Grok freeze ACCEPT. After final formatting, calculate the page MD5 and report
it via radio so the separate reviewer can bind its findings to exact bytes.

Run `wiki update --path="$WIKI_DIR"`, `wiki lint --path="$WIKI_DIR"`, and the
non-live root test script. Commit only the contract page/index product of this
task and report its commit, page MD5, exact scope, and fail-closed boundaries.

## Completion Requirements

Finish only when all of the following are true:

1. `wiki/wave_fivem_ticketed_writes_contract.md` is a cited, self-contained,
   correctly indexed contract covering exactly the two create/update inventory
   rows and four future ticketed tools above.
2. The page preserves opaque inner-map and shared ticket protocol constraints,
   records the 401/405 gate evidence and field boundaries, and explicitly
   excludes delete, bulk, webhooks, live, UI, vision, generic controls, and
   completeness claims.
3. No production, test, coverage, status, review, or other wiki file changed;
   no live credentials, raw evidence, browser artifacts, or persistent test
   data exist.
4. Wiki update/lint and `BILLY_TEST_MODE=commit bash "$NODE_DIR/scripts/test.sh"`
   pass; the branch commit contains the page and derived index only; the outbox
   reports page MD5 and the contract is ready for a separate Grok review.
5. The node has run `fractal node finish --reason="Wave-5m freeze contract authored"`
   after committing its own page. It must not await a parent merge or a review
   that occurs on a separate node.
