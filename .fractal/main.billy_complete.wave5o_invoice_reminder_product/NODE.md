...
Implement the accepted offline Wave-5o invoice-reminder product slice. You are
the Codex Power implementation owner. Do not research a wider API contract,
make live requests, use a browser, or delegate: this leaf has no child capacity.

### Binding authority

- The accepted freeze is `wiki/wave_fiveo_ticketed_writes_contract.md`; the
  independent acceptance is `wiki/wave_fiveo_freeze_independent_review.md`.
- The cited Grok research handoff reconfirmed the official documentation at
  https://www.billy.dk/api/ (ETag `wcw4x9hqvu3603`, MD5
  `8b94b0135c91fd15fe54ea33e088a4be`) and object-body gates on
  `https://api.billysbilling.com/v2`: POST `/invoiceReminders` is 401, while
  PUT, singular DELETE, and bulk DELETE are 405.
- Therefore implement exactly one inventory operation,
  `api.invoiceReminders.create`, as exactly two tools:
  `api_invoice_reminders_create_preview` and
  `api_invoice_reminders_create_execute`.

### Owned project files

You may change only these project files plus your own `.fractal/` seed:

- `src/billy_mcp/api/invoice_reminder_writes.py` (new)
...
Escalate through radio rather than widening this list. Do not change accepted
freeze/review wiki pages, other write modules, UI manifests, or any shared
write-protocol implementation.

### Required implementation

Mirror the create half of `invoice_late_fee_writes.py` while preserving these
exact reminder-specific settings:

- Preview input forbids undeclared outer fields and exposes only
  `invoiceReminder: dict[str, JsonValue]`. The inner map remains opaque.
- Execute exposes only nonempty `confirmation_ticket` through
  `WriteExecuteInput`.
- `WriteOperationSpec` uses POST, collection path `/invoiceReminders`, singular
  root `invoiceReminder`, plural root `invoiceReminders`,
  `additional_plural_roots=()`, no resource id or organisation id, and expected
  effect `{action: create, resource: invoiceReminder}`.
- Register only the two exact tool names on the shared server immediately after
  invoice-late-fee write registration. Reuse the existing one confirmation
  store and `WriteProtocolService`; execution must make exactly one write and
  must not retry a failed mutation.
- Do not add update, delete, bulk, association, webhook, UI, browser, live,
  email-side-effect, download, or cleanup-success behaviour. Do not infer any
  relation wire form or nested association contract.

### Required tests and coverage

Add a focused create-only contract suite, based on the invoice-late-fee create
cases, proving flat typed registration/schema, extra and missing outer-field
rejection, opaque inner-map preservation, mutation-free preview, exact one POST
to the client-relative path, only the declared `invoiceReminders` success root,
ticket tamper/wrong-tool/expiry/replay rejection, typed auth failure, no retry,
and absence of update/delete/bulk registrations.

After tests pass, green only `api.invoiceReminders.create`:

- `implemented: true`, `contract_tested: true`, `live_tested: false`
- response inventory reflects only `invoiceReminders[]`
- test references include the new API suite and coverage-server test
- cleanup is exactly `live non-production cleanup strategy unqualified; singular DELETE is unsupported`
- regenerate status/report with 175 implemented and contract-tested rows, 256
  `api_*` tools, zero live/vision rows, 92 ambiguous bulk rows, and
  `complete: false`

Run formatting, lint, strict typing, the focused suite, coverage tests and
checks, policy checks, and the non-live pytest suite. Keep all evidence and
credentials outside git. Send the parent a concise radio report with the commit
and verification results; do not wait for its reply.

## Completion Requirements

- The new module registers exactly the two approved invoice-reminder create
  tools with Pydantic typed input/output and the shared ticketed-write protocol.
- The focused contract tests cover the required validation, request, response,
  ticket binding, typed-error, no-retry, and no-extra-tool cases and pass.
- Only the approved create inventory row is greened with the exact cleanup
  wording and regenerated honest coverage arithmetic; bulk, live, UI, vision,
  and completeness remain red or false as applicable.
- `uv run ruff format --check .`, `uv run ruff check .`, `uv run pyright`,
  ...
  `uv run python scripts/check_repository_policy.py` pass on your committed
  branch.
- Your project work and seed are committed and pushed by `fractal commit`; you
  have reported the result through radio. Do not require or await an independent
  review to finish this leaf.
