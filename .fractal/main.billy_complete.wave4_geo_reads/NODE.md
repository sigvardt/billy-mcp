...
Implement only the eight Wave-4 clear read tools for country groups, cities,
states, and zipcodes. Own exactly `src/billy_mcp/api/geo_reads.py` and
`tests/api/test_geo_reads.py`; do not edit registration, coverage, redaction,
wiki, generated files, or any other product files.

Read `wiki/wave_four_remaining_clear_reads_contract.md` and the cited
`.fractal/main.billy_complete/tmp/grok-research.md` before coding. The frozen
contract is `GET /countryGroups`, `/cities`, `/states`, and `/zipcodes`, with
the documented singular/list roots. Use typed, flat Pydantic inputs,
`extra="forbid"`, encoded IDs, opaque payloads, optional `meta.paging`, and
typed Billy error mapping. Do not invent fields or filters.

Cities, states, and zipcodes lists require non-empty `countryId`, based on the
cited live non-production-safe evidence. Country groups uses only the default
page/pageSize/include/sortProperty/sortDirection list surface. All lists reject
`offset`, empty countryId, parent IDs, and unknown keys. Register direct flat
FastMCP parameters in the module's register function; do not use nested
`request` wrappers.

Use only the existing locked Billy client. Do not make live claims, perform
writes, add generic transport controls, or change coverage flags. Run focused
tests plus formatting, Ruff, and Pyright for the owned work. Commit only a
real implementation and tests, then report the commit and results to the
parent radio outbox.

## Completion Requirements

- `geo_reads.py` provides all eight real typed `api_*` get/list tools and its
  registration hook, following the established Wave-3 read pattern.
- Focused tests prove each route/root, query serialization, paging envelope,
  encoded IDs, local validation/unknown-key rejection, and typed 401 mapping;
  they specifically prove `countryId` is mandatory only for geo dependent
  lists.
- `uv run ruff format --check`, `uv run ruff check`, `uv run pyright`, and
  `uv run pytest tests/api/test_geo_reads.py` pass.
- No coverage, live, UI, bulk, write, or completeness claim changes. The
  branch contains no credentials or browser evidence.
