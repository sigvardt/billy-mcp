...
Implement the reviewed offline ticketed CUD tools for Billy products and product
prices. You own exactly these two files:

- `src/billy_mcp/api/catalog_writes.py`
- `tests/api/test_catalog_writes.py`

Do not edit read modules, `server.py`, shared write protocol, coverage
manifests/status, browser/auth, generated reports, or another leaf's files.
Root owns registration, inventory greening, and the independent Grok review.

This is a research-backed implementation. The root's cited Grok brief established
the current official-doc fingerprint ETag `hsisik4g9p3603` / MD5
`c2efda0ee4cf9cf200e14910c5fc6996` for https://www.billy.dk/api/. Follow the
frozen contract in `wiki/wave_five_ticketed_writes_contract.md` and the approved
design §§6 and 8. The root base already contains the reviewed multi-root
protocol. Do not conduct new non-Billy research or invent missing contracts.

Implement 12 typed FastMCP tools, all with flat `extra="forbid"` Pydantic outer
inputs and typed success/error output:

- `api_products_create_preview` / `_execute`
- `api_products_update_preview` / `_execute`
- `api_products_delete_preview` / `_execute`
- `api_product_prices_create_preview` / `_execute`
- `api_product_prices_update_preview` / `_execute`
- `api_product_prices_delete_preview` / `_execute`

Use one `register_catalog_write_tools(server, client, write_protocol)` helper.
The six previews accept an opaque object at the singular `product` or
`product_price` parameter plus a non-empty `id` for update/delete. Executes
accept only `confirmation_ticket`. Construct `WriteOperationSpec` with:

- products: root `product` / plural `products`, paths `/products` and
  `/products/{escaped id}`, and `additional_plural_roots=("productPrices",)`
  for all three product CUD operations;
- product prices: root `productPrice` / plural `productPrices`, paths
  `/productPrices` and `/productPrices/{escaped id}`, no additional roots;
- POST create with `{singular_root: payload}`, PUT update with that same
  singular root, and DELETE with no JSON body.

Route previews through the shared `WriteProtocolService.preview` and executes
only through `WriteProtocolService.execute`. Never make preview HTTP writes,
retry a write, reimplement confirmation storage, pass a `/v2` client path,
invent bulk save/delete, add webhooks, or mutate coverage. Keep payload values
opaque offline; do not invent stricter validation for docs ambiguities. The
official product sample permits embedded `prices[]` with `unitPrice` and
`currencyId`; the response fixture must retain returned `productPrices` through
the reviewed multi-root mapper.

Tests must be real mocked-client contract tests covering flat schemas,
preview non-mutation, exact POST/PUT/DELETE request/path shapes, ticket-only
execution, invalid/replay/expired confirmation paths, empty-token auth without
network, typed 401/404 propagation, no fabricated deleted records, and product
multi-root response survival. Never claim live, UI, vision, bulk, inventory
green, or root server registration.

Do not attempt a Grok review in this leaf. Run focused tests, the inherited
non-live suite, and lint. Report the committed tip SHA and exact evidence to the
parent; preserve all work if a root-only gate fails.

## Completion Requirements

- Only the two owned product/test files contain product changes.
- The twelve named tools have typed flat schemas and call the shared protocol;
  they never expose generic HTTP/browser control.
- Focused tests and the inherited non-live suite/lint pass from committed bytes.
- Coverage/status remains fail-closed and unchanged; no live/UI/bulk claim is
  made.
- The parent receives the tip SHA plus reproducible test evidence before the
  node finishes.
