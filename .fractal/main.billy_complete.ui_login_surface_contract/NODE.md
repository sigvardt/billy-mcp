...
You are a research-only Grok leaf. Produce the current, cited contract brief
that decides the next smallest honest typed UI/auth workflow. Your only
project deliverable is `wiki/ui_login_surface_contract.md`; do not modify
application source, tests, coverage inventories/status, package metadata,
or non-wiki documentation. Child seed files are allowed only to configure and
run this node.

Read the approved design, the current source around `auth_status`,
`auth_login_start`, and `auth_login_wait`, existing UI/auth wiki pages, and
`$NODE_DIR/tmp/grok-research.md` if present. Treat its uncited claims as leads.
Research current official Billy documentation first and cite every contract
claim with a canonical official URL, retrieval timestamp, and a short note of
what it proves. You may inspect the login surface only through a headless,
bounded Playwright workflow on Billy's allowlisted host. Do not use a visible
browser, submit credentials, or make application writes.

The brief must choose exactly one of these outcomes from evidence, without
inventing a generic browser wrapper:

1. An interface-only, stable `ui_*` read workflow that is genuinely distinct
   from existing shared `auth_*` tools; or
...
Seed evidence to verify rather than assume: the unauthenticated login page is
`https://mit.billy.dk/login`; observed controls are
...
1. `wiki/ui_login_surface_contract.md` exists with `desc:` frontmatter and a
   cited, current official-document/observed-surface brief containing the
   selected outcome and all contract details above.
