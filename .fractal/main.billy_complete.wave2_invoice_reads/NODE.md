...
Implement the Wave 2 invoice read-only API slice only.

Own exactly these product files:

- `src/billy_mcp/api/invoice_reads.py`
- `tests/api/test_invoice_reads.py`

Implement real typed FastMCP registration for `api_invoices_get` and
`api_invoices_list` only. Read `wiki/wave_two_filtered_reads_contract.md` and
the `api.invoices.get` / `api.invoices.list` rows in
`coverage/api_v2_manifest.yaml` as the frozen cited contract. Use only the
locked `BillyHttpClient`, relative `/invoices` paths, Pydantic input and success
models, and typed `ToolError` failures.

The list input may expose only documented global `page`, `pageSize`, `include`,
plus the invoice filter keys and sort-value enum in the manifest. Reject unknown
keys and invalid enum values locally. Preserve opaque documented period strings;
do not parse or rewrite them. Map `invoice` / `invoices` roots and optional
`meta.paging`, and cover both documented 401 envelopes through the existing
client mapping.
...
bulk operations, invoice emails/deliveries/logs, browser/UI code, generic HTTP
controls, offset paging, credentials, or a live coverage claim. Root owns
server wiring and all manifest status changes.

## Completion Requirements

- The two owned files implement only the two real invoice read tools and their
  focused mock-transport contract tests.
- Tests prove exact request paths and allowed query construction, local
  rejection of unknown/invalid filter input, singular/list response mapping,
  optional paging, typed authentication errors, and exact tool registration.
