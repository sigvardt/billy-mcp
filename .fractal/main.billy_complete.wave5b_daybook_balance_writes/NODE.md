...
Implement the cited Wave-5b offline ticketed-write module for daybook balance
accounts. The binding contract is
...
Implement exactly six real FastMCP tools: preview and execute for create,
update, and delete of `daybookBalanceAccounts`. Use the client-relative
collection `/daybookBalanceAccounts`, singular request root
`daybookBalanceAccount`, POST/PUT/DELETE respectively, and no delete body.
...
- The two owned files contain all six tools and their contract tests; no
  placeholder or generic-control surface exists.
- Every preview has no observable HTTP mutation, every execute is ticket-only,
  and request construction exactly matches the frozen paths, methods, root,
