...
Perform the full Wave-5k freeze review retry required after the Grok route
failed before any reviewer edit because it was unauthenticated. You are Codex
Power, so your deliverable is explicitly a **non-authoritative fallback review**:
it cannot satisfy or replace the mandatory independent Grok freeze gate and it
cannot authorise product implementation.

Review the committed root `wiki/wave_fivek_ticketed_writes_contract.md` against
the current official Billy API documentation at `https://www.billy.dk/api/` and
the approved design at `docs/superpowers/specs/2026-07-28-billy-mcp-complete-design.md`.
Independently verify the locked base `https://api.billysbilling.com/v2` and make
only unauthenticated, non-mutating probes if needed. Do not use credentials,
production data, browser UI, visible windows, raw screenshots, HAR/traces,
arbitrary URLs, or arbitrary local files.

Your only shared-project deliverable is
`wiki/wave_fivek_freeze_codex_fallback_review.md`. Start it with a prominent
statement that it is a Codex Power fallback caused by a fail-before-edits Grok
authentication failure; it is not an independent Grok review, not a freeze
acceptance, and not a product approval. It must have current official-source
citations, identify the reviewed root commit, and issue a bounded ACCEPT or
REJECT of the page's offline contract fidelity.

Verify and record all of the following:

- the exact create and update preview/execute pairs: `POST /bankPayments` and
  partial `PUT /bankPayments/:id`, singular `bankPayment` request root, required
  `bankPayments` success root, strict outer fields, opaque inner map, and inner
  `id` equality when supplied;
- the shared short-lived, single-use, exact-operation ticket binding and the
  one-write/no-retry execution boundary;
- POST/PUT authentication gating and singular DELETE 405 exclusion; no delete,
  bulk, generic HTTP/browser, webhook, invented cleanup, or live/UI claim;
- the unchanged fail-closed baseline: 238 `api_*` tools, 166 offline API rows,
  zero live/vision rows, 92 ambiguous bulk rows, and `complete: false`.

Do not modify source, tests, manifests, coverage, credentials, browser evidence,
or any existing review/contract page. Change only the fallback review and the
generated `wiki/_index.md`, plus private node records. A bounded ACCEPT still
leaves the Grok gate and product work blocked. Run wiki update/lint and the
configured non-live checks before commit.

## Completion Requirements

- `wiki/wave_fivek_freeze_codex_fallback_review.md` is a cited,
  non-authoritative Codex Power fallback review with a bounded ACCEPT or REJECT,
  the reviewed root commit, and an explicit statement that it does not replace
  an independent Grok acceptance.
- The review correctly preserves the create/update-only four-tool boundary,
...
- No production source, test, manifest, coverage, credential, browser-evidence,
  or unrelated wiki files are changed.
- `wiki update --path="$WIKI_DIR"`, `wiki lint --path="$WIKI_DIR"`, and the
  configured lint and non-live test scripts pass. The branch is committed and
  pushed.
- Report the bounded verdict through the outbox and run `fractal node finish`
  in the same iteration. Do not wait for parent merge or for Grok access after
  delivering this fallback record.
