...
Perform the policy-required full fallback review of the committed Wave-5f tax
ticketed-write freeze at `d412e8c`. The Grok review route failed before any
project edit because its environment was unauthenticated; this Codex Power
review retries the full static/contract audit under the permitted fallback
route. It is not an independent Grok review and cannot satisfy or replace the
mandatory Grok gate.

Your sole project deliverable is
`wiki/wave_fivef_freeze_codex_fallback_review.md`; generated `wiki/_index.md`
...
Review the exact root commit and the cited research retained in the committed
freeze record. Validate all six singular routes, methods, preview/execute names,
singular and response roots, API base lock, shared ticket/executor binding,
strict outer schemas, opaque inner payloads, bodyless deletes, partial update
id matching, no-write-retry boundary, and fail-closed response mapping. Verify
the optional parent `taxRateDeductionComponents` response root is not required,
and child writes do not invent `taxRates`. Check the documented immutable and
readonly field notes, unknown `source` enum values, unauthenticated-probe
limits, all four empty-body-contract bulk rows, and excluded live/UI/vision/
special routes.

Record a durable non-sensitive PASS or REJECT for this fallback inspection. If
PASS, state prominently that it is supplemental only: no implementation,
coverage status, live, UI, vision, bulk, special, or completeness claim is
authorised until a later authenticated independent Grok review. If REJECT, name
the exact conflicting statement and correction. Do not use credentials, create
Billy records, start browser workflows, preserve browser evidence, or claim
current external research beyond the cited committed record.

Run relevant non-live verification and project wiki lint before committing. Run
`fractal node finish` after your review commit: your child-complete fallback
record is useful to the parent regardless of PASS/REJECT, and the parent owns
the remaining Grok-authentication blocker.

## Completion Requirements

1. `wiki/wave_fivef_freeze_codex_fallback_review.md` exists, is indexed and
   lint clean, records a reproducible PASS or REJECT for the exact `d412e8c`
   contract baseline, and identifies the cited source surfaces and checks.
2. The review covers all six operations, routes, roots, optional parent mapping,
   child non-inference, ticket/executor binding, base URL lock, field boundaries,
   probe limits, and unresolved bulk/live/UI/vision/special exclusions.
3. A PASS explicitly states it is a non-authoritative Codex fallback and leaves
   the mandatory independent Grok gate unmet. No code, coverage, frozen
   contract, or status artifact change is committed.
4. `bash $NODE_DIR/scripts/test.sh`, `bash $NODE_DIR/scripts/lint.sh`, and
   `wiki lint --path=wiki` pass from the child worktree, and the child branch is
   clean after its review commit.
