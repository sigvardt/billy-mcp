...
Implement the reference-data API read module only, using the frozen official
contract in `wiki/wave_one_reads_contract.md` and
`wiki/billy_api_v2_research_seed.md`. Codex Power owns implementation and
tests; do not perform fresh external contract research.

Own exactly these product files: `src/billy_mcp/api/reference_reads.py` and
`tests/api/test_reference_reads.py`. You may create the test directory. Do not
edit `server.py`, shared models, client/config/errors, coverage manifests,
scripts, the wiki, or any other API module. Escalate any needed shared-interface
change to the parent via radio.

Implement real typed Pydantic request/success models and tool-registration
support for currencies, countries, and locales get/list tools:
`api_currencies_get/list`, `api_countries_get/list`, and
`api_locales_get/list`. Use only `BillyHttpClient` and documented relative
paths. Lists may expose only page/pageSize/include/sortProperty/sortDirection;
do not infer filters or writes. Map singular/list roots and optional paging
without fabricating unknown fields.

Use mock-transport fixtures to cover requests, root mapping, paging boundaries,
and typed authentication errors. This is read-only: no writes, tickets,
browser/UI controls, bulk paths, live requests, generic HTTP, or coverage flag
changes. Report exact coverage IDs and test references to the parent; the parent
owns manifest updates and server wiring after review.

## Completion Requirements

- Only the two owned files contain the complete implementation and tests.
- All six named tools have real typed registration support, Pydantic input and
  success/error outputs, and focused contract tests; no placeholder tool exists.
- `ruff format --check`, `ruff check`, Pyright, focused pytest, and the node
  lint/test scripts pass from committed bytes.
- The report to parent names the commit, exact files, tool names, coverage IDs,
  checks, and the fact that `live_tested` remains false without credentials.
