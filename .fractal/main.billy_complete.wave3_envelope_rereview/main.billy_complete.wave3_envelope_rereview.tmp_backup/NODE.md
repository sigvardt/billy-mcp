...
Perform a fresh, independent Grok re-review of the Wave-3 corrections at
`main.billy_complete` revision `7965072`. You own only
`wiki/wave_three_envelope_rereview.md`.

The first independent review found two tool-surface issues: files/attachments
and catalog reads exposed nested `request` inputs, and file/attachment lists
flattened documented `meta.paging`. Read current official Billy documentation,
the approved design, `wiki/wave_three_unfiltered_reads_contract.md`, the
relevant source/tests, and the generated coverage inventory. Verify, by source
and actual FastMCP schemas/structured tool results, that:

1. `api_files_*`, `api_attachments_*`, `api_products_*`, and
   `api_product_prices_*` have documented flat input fields rather than a
   nested `request` object.
2. `api_files_list` and `api_attachments_list` preserve the official optional
   `meta.paging` response envelope without inventing a top-level `paging`.
3. The corrections neither widen filters nor alter the current truthful
   `live_tested=false`, UI-red, bulk-red, and `complete=false` coverage state.

Use only headless/non-sensitive work. Do not modify production code, tests,
coverage outputs, credentials, or raw evidence. Write a concise cited report
with frontmatter `desc:` that explicitly says whether the two findings are
closed or names an exact remaining defect. Send the parent any remaining
required issue immediately by radio; otherwise announce a re-review pass.

The earlier `wave3_independent_review` branch is intentionally closed and must
not be merged because its report predates this correction and its branch has
runtime scaffolding.

## Completion Requirements

1. `wiki/wave_three_envelope_rereview.md` contains a non-sensitive, cited
   independent verdict on the eight flat input schemas and two `meta.paging`
   outputs at revision `7965072`.
2. It records no live/UI/vision/completeness claim and names the current
   official documentation source/fingerprint used for the re-review.
3. Any remaining required issue is in the parent inbox with exact evidence;
   otherwise the outbox reports a re-review pass.
4. The node changes and commits only its wiki report, passes its non-live test
   and lint scripts, then finishes with a reason naming the report.
