...
Implement the bounded offline Wave-5r product for the one authorised inventory
operation `api.salesTaxReturns.update`. You are the Codex Power implementation
leaf. Do not research the contract anew: use the cited parent baseline at
`wiki/wave_fiver_product_implementation_research.md`, the accepted freeze
review `wiki/wave_fiver_freeze_independent_review.md`, and the frozen contract
`wiki/wave_fiver_ticketed_writes_contract.md`. The approved contract is
`PUT /salesTaxReturns/:id`, request body root `salesTaxReturn`, and required
success root `salesTaxReturns`.

You exclusively own only these deliverables:

- `src/billy_mcp/api/sales_tax_return_writes.py` (new)
- `tests/api/test_sales_tax_return_writes.py` (new)
- `src/billy_mcp/server.py`
- `tests/unit/test_coverage_server.py`
- `scripts/generate_coverage_report.py`
- generated `coverage/` artifacts required by the generator

Escalate any need to edit other project files or any contract ambiguity to the
parent through radio; do not broaden the slice. Do not alter the accepted
freeze, Research76 wiki page, manifests by hand, UI/browser/auth code, or any
other inventory row.

Clone the update-only architecture in `src/billy_mcp/api/user_writes.py` and
its contract tests. Expose exactly these two flat typed tools:

- `api_sales_tax_returns_update_preview`
- `api_sales_tax_returns_update_execute`

Outer Pydantic input must forbid undeclared fields. Keep
`salesTaxReturn: dict[str, JsonValue]` opaque, but reject an optional inner
`salesTaxReturn.id` that differs from route `id`. Preview is mutation-free and
issues the shared short-lived single-use ticket bound to tool, organisation,
target, canonical request, and expected effect. Execute accepts only
`confirmation_ticket`, makes one non-retried PUT, and maps only a list-valued
`salesTaxReturns` root. Preserve the shared error and redaction behavior.

Register the tools alongside the current Wave-5 write registrations. Add a
two-name `WAVE_FIVER_WRITE_API_TOOL_NAMES` set and update the registry total
from 262 to 264. Add exactly `api.salesTaxReturns.update` to
`OFFLINE_API_IMPLEMENTATION_EVIDENCE`, then regenerate coverage honestly to
179 implemented/contract-tested and 0 live/vision with `complete: false`.

Hard exclusions: no create or singular delete (documented 405), no bulk
operations, webhooks, UI, browser interaction, live qualification, field-level
validation for opaque values, settlement claims, or completeness claims. Never
use credentials, create live data, open a visible browser, or store frames,
HARs, traces, or sensitive data.

## Completion Requirements

1. The two named FastMCP tools are the only salesTaxReturns write tools and
   have flat typed schemas with `additionalProperties: false`.
2. Contract tests cover outer-field rejection, empty identifiers/tickets,
   mismatched inner IDs, opaque-map preservation, mutation-free preview, exact
   escaped PUT request construction, required-root validation, tampered/expired/
   replayed tickets, one-write typed HTTP errors, missing-token no-network
   behavior, and log redaction.
3. The server registers both tools; `tests/unit/test_coverage_server.py`
   asserts the Wave-5r set length is 2 and total API tool count is 264.
4. The coverage generator is the sole source of inventory-state changes. Its
   regenerated artifacts mark only `api.salesTaxReturns.update` implemented and
   contract-tested, produce offline counts 179/179/0/0, and keep
   `complete: false`.
5. `uv run ruff format --check .`, `uv run ruff check .`, `uv run pyright`, the
   new contract test, `uv run python scripts/generate_coverage_report.py`,
   `uv run python scripts/check_coverage.py --reject-false-completeness`, and
   `BILLY_TEST_MODE=commit bash "$NODE_DIR/scripts/test.sh"` all pass.
6. The worktree contains no credentials or sensitive evidence; no excluded
   capability or additional coverage row is claimed. Commit the completed
   product with `fractal commit`, post a concise outbox handoff, and run
   `fractal node finish --reason="Wave-5r offline product delivered"`.
