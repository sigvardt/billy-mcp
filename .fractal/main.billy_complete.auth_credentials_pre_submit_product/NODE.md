...
Implement the offline-only auth credential-reference and pre-submit-login slice
frozen in `wiki/auth_credentials_pre_submit_research.md`. Treat that page,
`.fractal/main.billy_complete/tmp/grok-research.md` (research92), and
`docs/superpowers/specs/2026-07-28-billy-mcp-complete-design.md` as the
contract. You are a `codex-power` implementation leaf: do not perform external
research, do not spawn children, and do not declare a Grok, live, vision, or
completeness gate satisfied.

Own only these project paths:

- `src/billy_mcp/config.py`
- `src/billy_mcp/credentials.py` if a separate module is needed
- `src/billy_mcp/browser.py`
- `src/billy_mcp/models.py`
- `src/billy_mcp/server.py`
- focused auth/config/browser/server tests under `tests/`

Do not edit coverage manifests, generated coverage status, any `api_*` tool,
the project wiki, the approved design, CI, dependencies, or unrelated source
files. Keep the actual deliverable in the paths above; node plans, memory, and
seed files are not a parent integration deliverable.

Required product boundary:

1. Add serialisable opaque browser credential references and an injectable
   resolver protocol/implementation. Models, config, results, exceptions, logs,
   ticket state, tests, and source must never retain or emit email, password,
   TOTP, cookie, token, or a credential account name. Required missing/empty or
   unresolvable references return typed `AUTH_REQUIRED` without leakage.
2. Register only purpose-built typed `auth_*` tools with Pydantic input and
   output models. The login path must never accept caller-provided credentials,
   selectors, URLs, click instructions, or browser controls. Keep the design
   naming direction (`auth_login_start` / `auth_login_wait`) unless an existing
   validated server pattern requires a stricter equivalent.
3. Extend the bounded `BrowserRuntime` through fixed internal workflow seams
   only. It must force headless Chromium, downloads disabled, reviewed egress,
   fixed `https://mit.billy.dk/` navigation, the exact `/login` URL, and the
   research92 signature: one visible email/password/remember control and one
   `button[data-cy=login-button]` labelled `Log in` or `Log ind`. Never regress
   to `button[type=submit]` or `form button`.
4. Validate URL and controls before credential resolution, then validate again
   immediately before any fill/click. Tests may use fakes to prove the ordered
   internal transition; no test may send a real credential, make live API or
   browser requests, save frames/traces/HAR/DOM dumps, or create Billy data.
   TOTP must not be resolved or located; post-login state, organisation picker,
   token bootstrap, expiry, and reauthentication remain unimplemented until
   later observation.
5. Fail closed: pre-resolution signature drift is `UI_CHANGED` with zero
   resolver/fill/click calls; egress-policy failure is `EGRESS_DENIED`; redacted
   unexpected runtime failure is `BILLY_ERROR`. Do not manufacture
   `AUTH_INTERACTION_REQUIRED` for an unobserved challenge. Preserve page
   closure in every path.
6. Do not change any coverage state. All UI rows and API live/vision statuses
   must remain red and incomplete.

Write focused tests that prove missing credentials, resolver exceptions,
signature drift before resolution, duplicate/hidden/missing controls, both
observed labels, resolver/fill/click ordering, final page closure, redaction,
headless/download/egress invariants, and absence of secret-bearing schemas.
Run formatting, Ruff, Pyright, focused tests, repository safety checks, and the
node test script. Use `apply_patch` for edits. Commit the completed source/test
diff with `fractal commit` and report the commit, tests, files changed, and
remaining live/vision gates via radio outbox.

## Completion Requirements

- The committed scoped source/test diff implements only the frozen offline auth
  slice and does not touch coverage, API tools, wiki, dependencies, or CI.
- Every exposed auth tool has Pydantic input/output models and returns only the
  approved stable success/error shapes; no generic browser or secret-bearing
  interface exists.
- Tests prove all required fail-closed, ordering, egress/headless, closure, and
  redaction properties using fakes only. No live credential or browser/API
  request is made.
- Formatting, Ruff, Pyright, focused auth tests, repository safety checks, and
  `bash $NODE_DIR/scripts/test.sh` pass from the child worktree.
- The branch is committed, clean, and pushed. Its completion radio message
  names the commit and explicitly says that live login, organisation UI,
  bootstrap, coverage, and Grok review remain open.
