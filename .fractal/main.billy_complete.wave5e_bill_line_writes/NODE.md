...
Implement the accepted Wave-5e **bill-line** ticketed-write slice. You are a
Codex Power leaf. Do not spawn children and do not perform new documentation,
UI, browser, live, or Grok research.

Your exclusive project-file ownership is:

- `src/billy_mcp/api/bill_line_writes.py`
- `tests/api/test_bill_line_writes.py`

Do not edit `server.py`, coverage files/generators, cross-resource tests,
shared protocol/confirmation code, wiki pages, or any other project file. If a
shared change is necessary, report the exact requirement to the parent by radio
instead of editing it.

Contract authority is the accepted root freeze
`wiki/wave_fivee_ticketed_writes_contract.md` and the cited handoff at
`.fractal/main.billy_complete/tmp/grok-research.md`. Clone only the structural
pattern in `invoice_line_writes.py` and `test_invoice_line_writes.py`; do not
copy invoice-line field assumptions.

Implement six typed FastMCP tools:

- `api_bill_lines_{create,update,delete}_preview`
- `api_bill_lines_{create,update,delete}_execute`

The operation specifications are exactly `POST /billLines`, `PUT /billLines/:id`,
and bodyless `DELETE /billLines/:id`, with singular `billLine`, plural
`billLines`, and **`additional_plural_roots=("bills",)`**. Outer Pydantic
inputs forbid extra fields while the inner `billLine` object remains opaque
`dict[str, JsonValue]`. An optional body id on update must equal the route id.
Use the parent-provided shared `WriteProtocolService` and exact server-owned
executor names; never construct a local store or add a write retry.

Your local contract suite must prove flat schemas, strict outer validation,
preview non-mutation and canonical request binding, exact encoded paths and
HTTP bodies, bodyless delete, typed auth/not-found outcomes, tamper/expiry/
replay and mutated ticket bindings, same-module executor mismatch before HTTP,
and no extra mutation. It must prove both present and absent optional `bills`
response roots; absent must not be fabricated. Fixtures and claims must use
bill-line fields `billId`, `accountId`, `taxRateId`, `description`, and `amount`
and must not claim `productId` or `unitPrice`. Do not mark coverage green or
claim product acceptance.

Post concise status/blocker reports through radio. Keep raw evidence and any
scratch material outside the repository.

## Completion Requirements

- Only the two owned project files are changed.
- `bill_line_writes.py` registers exactly the six required bill-line
  preview/execute tools with the frozen paths, roots, optional parent mapping,
  ticket binding, and no retry.
- `test_bill_line_writes.py` covers every local contract boundary above,
  including optional present/absent `bills` response mapping, and passes with
  `uv run pytest tests/api/test_bill_line_writes.py`.
- `uv run ruff format --check src/billy_mcp/api/bill_line_writes.py
  tests/api/test_bill_line_writes.py`, `uv run ruff check` on those files, and
  `uv run pyright` pass.
- The worktree is clean apart from the node seed, your committed project files
  are pushed by `fractal commit`, and an outbox handoff identifies the commit
  and focused test result. Then run `fractal node finish` with a reason that
  states this is an offline implementation slice, not product acceptance.
