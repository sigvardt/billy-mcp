...
Implement the Wave 2 daybook-transaction read-only API slice only.

Own exactly these product files:

- `src/billy_mcp/api/daybook_transaction_reads.py`
- `tests/api/test_daybook_transaction_reads.py`

Implement real typed FastMCP registration for `api_daybook_transactions_get`
and `api_daybook_transactions_list` only. Read
`wiki/wave_two_filtered_reads_contract.md` and the
`api.daybookTransactions.get` / `api.daybookTransactions.list` rows in
`coverage/api_v2_manifest.yaml` as the frozen cited contract. Use only the
locked `BillyHttpClient`, relative `/daybookTransactions` paths, Pydantic input
and success models, and typed `ToolError` failures.

The list input may expose only documented global `page`, `pageSize`, `include`,
plus `organizationId`, `daybookId`, `apiType`, `state`, `minEntryDate`,
`maxEntryDate`, `q`, and the documented sort-value enum. Reject unknown keys
and invalid enum values locally. Map `daybookTransaction` /
`daybookTransactions` roots and optional `meta.paging`, and cover both
documented 401 envelopes through the existing client mapping.

Do not edit `server.py`, coverage artifacts or generators, shared models,
configuration, scripts, wiki, or any other API module/test. Do not add writes,
bulk operations, parent-daybook operations, invoice specials, browser/UI code,
generic HTTP controls, offset paging, credentials, or a live coverage claim.
Root owns server wiring and all manifest status changes.

## Completion Requirements

- The two owned files implement only the two real daybook-transaction read
  tools and their focused mock-transport contract tests.
- Tests prove exact request paths and allowed query construction, rejection of
  unknown/invalid filter input, singular/list response mapping, optional
  paging, typed authentication errors, and exact tool registration.
- No out-of-scope product, coverage, server, wiki, or seed change is present.
- Ruff format/check, Pyright, the focused pytest file, node lint, and the node
  non-live test script pass from committed bytes.
- No live request, credential, browser artifact, or non-production record is
  created. Report the commit hash and exact test result through radio, then run
  `fractal node finish` once these requirements are met.
