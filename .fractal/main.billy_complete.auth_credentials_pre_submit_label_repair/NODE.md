...
Repair exactly one blocking defect in the unmerged offline auth pre-submit
candidate that this node is based on. You are a `codex-power` implementation
leaf. Do not research externally, do not change coverage, and do not make live
Billy requests or submit browser credentials.

Authoritative evidence is the parent node's
`.fractal/main.billy_complete/tmp/grok-review.md` (review93), research93 in
`.fractal/main.billy_complete/tmp/grok-research.md`, and
`wiki/auth_credentials_pre_submit_research.md`. The live observed login button
text is exactly `Log in`; the allowed labels are exactly `Log in` and `Log ind`.
`Login` is a page title, not an allowed submit label.

Own only these project files:

- `src/billy_mcp/browser.py`
- `tests/unit/test_browser.py`

Repair the label regression in production and fake fixtures/tests. The default
valid fake submit text and happy-path parametrization must use `Log in` and
`Log ind`; add or retain direct negative proof that `Login` is rejected with
`UI_CHANGED` before credential resolution, fill, or click. Preserve the fixed
headless target, exact `data-cy=login-button` selector, no-title requirement,
CAPTCHA-config-as-noise rule, opaque-reference behavior, redaction, and all
other candidate behavior. Do not edit `coverage/`, config, models, server,
wiki, node seed files, or unrelated tests.

Run the focused browser tests plus the project node test and lint scripts.
Commit only the scoped source/test repair on this branch, report the hash and
test results through outbox, and finish. The parent will clean-archive review
and integrate it; do not claim product, live, UI, vision, or completeness
acceptance.

## Completion Requirements

- `browser.py` permits only exact trimmed `Log in` and `Log ind` submit labels.
- Browser tests prove `Login` is rejected before resolution, fill, and click.
- No credential, token, selector input, generic browser tool, coverage change,
  live submission, screenshot, HAR, trace, or unrelated source edit is present.
- Focused browser tests, `bash $NODE_DIR/scripts/lint.sh`, and
  `bash $NODE_DIR/scripts/test.sh` pass.
- A committed repair hash and concise non-sensitive outbox report are present.
