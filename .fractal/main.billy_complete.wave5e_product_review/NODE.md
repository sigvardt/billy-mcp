...
Review the exact root baseline that this branch starts from, including the two
labelled selective merges immediately preceding it and the integration commit.
Treat `wiki/wave_fivee_ticketed_writes_contract.md` and the cited official
Billy API v2 documentation as the contract. Re-fetch or inspect current
official documentation as needed, recording direct citations in the review.
Verify that the documentation identity still supports the six exact operations:

- `POST`, `PUT`, and bodyless `DELETE` for `/bills`, rooted at `bill`, with
  `bills` as the only required response root.
- `POST`, `PUT`, and bodyless `DELETE` for `/billLines`, rooted at `billLine`,
  with `billLines` and an optional, never fabricated `bills` response root.

Independently inspect the functional modules, server registration, local and
cross-module tests, generator evidence, generated coverage artifacts, and
root non-live verification. Confirm typed outer inputs, opaque inner payloads,
exact executor-name ticket binding, preview non-mutation, single-use/replay
behaviour, update-id equality, bodyless deletes, no retry, optional line-parent
mapping, six-row coverage arithmetic, and fail-closed live/UI/vision/bulk/
special/completeness state.

Your sole project deliverable is
`wiki/wave_fivee_product_independent_review.md`. It must have a clear
ACCEPT or REJECT verdict, name the reviewed commit, cite the official source,
enumerate reviewed files and verification commands/results, record the 178
`api_*` / 136 implemented-and-contract-tested / zero live / zero vision /
`complete: false` evidence, and explicitly limit any ACCEPT to this offline
product slice. Do not add code, generated coverage, tests, plan files, source
changes, screenshots, credentials, or raw browser evidence. Do not use a
fallback reviewer or a headed browser.

If a material defect is found, publish a REJECT review page with exact file and
line evidence, radio the parent immediately, and leave correction to the root.
Do not weaken the gate or make a completeness claim. If it passes, publish the
offline-only ACCEPT page and radio the parent with the reviewed commit and page
path.
