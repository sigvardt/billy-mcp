...
Implement the cited, freeze-approved Wave-5m offline product slice for singular
Billy `contactBalancePayments` create and update. You are the Codex Power
implementation owner; current-document research was completed by Grok before
this launch. Read these committed contract sources before editing:

- parent plan `.fractal/main.billy_complete/plans/2026-07-30T07:59:19.230Z-110.2-wave5m_product_implementation.md`
- `wiki/wave_fivem_ticketed_writes_contract.md`
- `wiki/wave_fivem_freeze_independent_review.md`
- `coverage/api_v2_manifest.yaml`
- `src/billy_mcp/api/sales_tax_payment_writes.py` and its focused test module

The exact authorised surface is four tools and two inventory rows only:

| Operation | Preview | Execute | HTTP | Request / success root |
| --- | --- | --- | --- | --- |
...
- the create/update rows only in `coverage/api_v2_manifest.yaml` and their
  generated coverage artefacts

Mirror the sales-tax-payment module's established shape. Use strict flat
Pydantic outer inputs, an opaque `dict[str, JsonValue]` inner map, non-empty
route ID, and update inner-ID equality when supplied. Use the root's one shared
`WriteProtocolService` and its confirmation store; preview is non-mutating and
execute accepts only the confirmation ticket. Bind the exact executor, request,
target, effect, and organisation, issue one write with no retry, and map only
the required `contactBalancePayments` success root.

Real contract tests must cover schema flatness and extra rejection,
mutation-free preview, opaque-map preservation, update-ID mismatch, exact JSON
envelopes, URL encoding, one write only, response-root validation, typed
auth/HTTP failures, empty ticket rejection, tamper/wrong-executor/expiry/replay
fail-closed behaviour, and absence of delete or bulk tools. Reuse test helpers
where the existing product pattern allows it without weakening coverage.

After genuine tests pass, set only the two authorised rows' `implemented` and
`contract_tested` fields true; add the focused test reference; and change create
cleanup wording to `live non-production cleanup strategy unqualified; singular
DELETE is unsupported`. Regenerate coverage. Target 250 `api_*` tools and 172
implemented plus contract-tested API rows, while live and vision remain zero
and `complete` remains false.

Hard boundaries: no singular delete, bulk, PATCH, UI/browser work, live calls,
credentials, webhook, generic controls, second confirmation store, invented
inner payload schema, bank-payment field reuse, association wire schema, or
invented related response roots. Do not touch contactBalancePostings,
invoiceLateFees, invoiceReminders, or the accepted freeze pages. Do not claim
product acceptance; root will obtain a separate independent review after merge.

Keep raw evidence and credentials outside git. Post one concise outbox handoff
with the committed tip, exact checks, coverage counts, and remaining red gates.

## Completion Requirements

1. The four authorised tools are implemented and registered with typed strict
   inputs and typed success/error output, backed by the shared confirmation
   protocol with exact executor binding.
2. The owned focused test module proves the complete offline contract matrix;
   no delete or bulk tool is registered.
3. Only `api.contactBalancePayments.create` and `.update` become implemented
   and contract-tested; their inventory/test references and generated coverage
   artefacts are consistent. The create cleanup wording makes no delete claim.
4. Generated counts are exactly 250 `api_*` tools and 172 API rows implemented
   plus contract-tested; live/vision remain zero and `complete: false`.
5. Run and pass: Ruff formatting/check, Pyright, the focused product plus
   coverage/server tests, coverage generator/checker, policy checks, and
   `BILLY_TEST_MODE=commit bash "$NODE_DIR/scripts/test.sh"`. Do not run or
   claim the false-completeness full mode as a product success.
6. Commit all owned work, leave the child tree clean, post the non-sensitive
   handoff, fold its memory, and run `fractal node finish` in the same run.
