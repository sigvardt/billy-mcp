...
Perform an independent Grok review of the frozen Wave-5e offline contract for
Billy bill and bill-line ticketed writes. You own exactly one project
deliverable: `wiki/wave_fivee_freeze_independent_review.md`. Do not modify
production code, tests, coverage artifacts, the existing contract page, or any
node seed outside your own node directory.

Review `wiki/wave_fivee_ticketed_writes_contract.md` against the current
official Billy API documentation at https://www.billy.dk/api/ and the approved
design at `docs/superpowers/specs/2026-07-28-billy-mcp-complete-design.md`.
Produce your own cited official-document findings; the root's scratch brief is
context, not a substitute for independent verification. Check the official
endpoint base, paths, methods, singular request roots, supported operations,
response roots, list filters/pagination where relevant, documented errors, and
the exact bill and bill-line field boundaries.

Verify the six-row map only:

- `api.bills.{create,update,delete}` with `api_bills_*_{preview,execute}` and
  `/bills` / singular `bill`.
- `api.billLines.{create,update,delete}` with
  `api_bill_lines_*_{preview,execute}` and `/billLines` / singular `billLine`.

Audit the frozen offline safety constraints: locked API base, no duplicated
`/v2` path segment, `PUT` partial updates, bodyless delete, opaque inner
payloads with strict outer typed models, one shared ticket service/store, exact
server-owned `execute_tool_name` binding, no write retry, and optional-only
`bills` parent mapping for bill-line response roots. Confirm that the contract
does not infer invoice embedded-line/state rules, bulk bodies, webhook APIs,
live cleanup, browser/UI parity, or coverage green status.

Inspect `coverage/api_v2_manifest.yaml`, `coverage/status.json`, and
`src/billy_mcp/server.py` only to verify the baseline arithmetic and fail-closed
state: 166 `api_*` tools, 130 offline API rows, zero live and vision rows,
92 bulk rows red, and `complete: false`. Do not run live mutations or use a
browser with a visible window. Do not add credentials, raw frames, HAR files,
traces, or sensitive data to the repository.

Write a concise, non-sensitive durable review page with frontmatter including
`name`, `title`, `desc`, `sources`, and an explicit ACCEPT or REJECT verdict.
It must state that ACCEPT is for the offline contract only and is not product,
live, UI, vision, bulk, special-route, or completeness acceptance. If REJECT,
name exact corrections required and do not soften the gate. Run `wiki update`
and `wiki lint` for the project wiki, use only the provided scripts/checks that
are valid for a documentation review, post the verdict and exact reviewed root
tip to your outbox, then self-finish only after the page and checks are
committed. If Grok authentication, quota, or rate limiting prevents review
before any deliverable edit, report that exact blocker to the parent by radio
without making a substitute claim; the parent owns the required routing retry.

## Completion Requirements

- `wiki/wave_fivee_freeze_independent_review.md` exists, is cited, has valid
  wiki frontmatter, and records an unambiguous ACCEPT or REJECT for the exact
  frozen Wave-5e contract.
- The page independently verifies or rejects every one of the six CUD rows,
  their preview/execute names, paths, roots, and the bill-line optional parent
  response-root rule.
- The page records the current official-doc evidence and root baseline without
  changing product code or greening coverage.
- `wiki update --path=wiki`, `wiki lint --path=wiki`, and the node's configured
  documentation-safe checks pass; the worktree is committed and clean.
- A concise non-sensitive verdict is posted to the parent through the outbox.
