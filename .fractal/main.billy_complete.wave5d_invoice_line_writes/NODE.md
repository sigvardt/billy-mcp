...
Implement the Wave-5d offline ticketed-write leaf for **invoice lines** only.

This child has exclusive file ownership of exactly:

- `src/billy_mcp/api/invoice_line_writes.py`
- `tests/api/test_invoice_line_writes.py`

Do not edit any other production, test, coverage, manifest, wiki, server,
protocol, configuration, or parent-owned file. Escalate any necessary shared
change to the parent by radio. Do not spawn children.

The frozen contract is `wiki/wave_fived_ticketed_writes_contract.md`, based on
current official Billy documentation at ETag `hsisik4g9p3603` / MD5
`c2efda0ee4cf9cf200e14910c5fc6996`. Parent-provided Grok research already
opens this product gate. Use `daybook_transaction_line_writes.py` and its tests
as the local pattern; do not reimplement `ConfirmationStore` or
`WriteProtocolService`.

Implement exactly these six flat FastMCP tools with typed Pydantic success and
error paths:

- `api_invoice_lines_create_preview` / `api_invoice_lines_create_execute`
- `api_invoice_lines_update_preview` / `api_invoice_lines_update_execute`
- `api_invoice_lines_delete_preview` / `api_invoice_lines_delete_execute`

Use client-relative `POST /invoiceLines`, `PUT /invoiceLines/:id`, and bodyless
`DELETE /invoiceLines/:id`; `invoiceLine` is the singular request root and
`invoiceLines` the primary response root. Create and update take an opaque inner
`invoiceLine` dictionary inside strict outer models. Delete binds only a
nonempty `id`. Every line `WriteOperationSpec` must declare
`additional_plural_roots=("invoices",)`: map the parent root only when present
and never fabricate it in primary-only responses. Executors accept only
`confirmation_ticket` and every preview/execute binds its exact server-owned
execute name through the shared protocol. No write retry is allowed. Do not make
bulk, special, live, UI, or coverage claims.

The Grok research and review steps may verify only this frozen contract against
official evidence. If documentation drift or a contract discrepancy appears,
report it to the parent before product changes; do not broaden this leaf.

## Completion Requirements

1. Only the two owned files contain substantive product changes.
2. All six invoice-line tools are registered by the module using the exact names
   and typed Pydantic input/success/error models.
3. Focused tests prove preview non-mutation; exact method, route, singular-root
   body, and bodyless delete; ticket tamper, expiry, replay, wrong binding, and
   same-module executor mismatch before HTTP; strict outer inputs; typed auth
   and not-found propagation; and both primary-only and primary-plus-parent
   multi-root mapping without fabricated parent records.
4. `uv run pytest -q tests/api/test_invoice_line_writes.py`, the node lint
   script, and the node test script pass from the child worktree.
5. The child records a concise non-sensitive review outcome, commits only its
   scoped files, posts its result to the parent, and runs `fractal node finish`
   in the same iteration once these requirements are met.
