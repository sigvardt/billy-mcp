...
Perform a read-only **Codex Power fallback** review of the committed Wave-5b
P1/P2 repair at `a2a0996`. Grok could not authenticate before any review edit;
this retry is required by routing policy but is **not** the mandatory Grok
independent audit. It must never authorize Wave-5c, regardless of its verdict.

Own only these project deliverables:

- `wiki/wave_fiveb_repair_codex_fallback_review.md`
- the generated row for that page in `wiki/_index.md`

Do not alter product code, tests, coverage artifacts, inventories, status, or
any other wiki page. Do not green coverage or make live/UI/vision/bulk/
completeness claims.

Review committed product bytes only, using a clean `git archive a2a0996` export
and comparing to `920ceab`. Treat the approved design,
`wiki/wave_fiveb_ticketed_writes_contract.md`,
`wiki/wave_fiveb_product_independent_review.md`, and current official Billy
documentation at `https://www.billy.dk/api/` as the contract. Re-check and cite
the official-doc fingerprint; use no credentials, production data, visible
browser, or non-Billy hosts.

Independently verify:

1. all 24 registered write preview/execute handlers bind and pass their exact
   server-owned `execute_tool_name` literals;
2. same-module and cross-module ticket misuse return `CONFIRMATION_MISMATCH`
   before HTTP and do not consume the valid matching ticket;
3. terminal prepared request removal, expired pending/replay-marker pruning,
   and preserved typed expiry/replay behavior;
4. the clean-export focused P1/P2 tests, root registry/coverage checks, Ruff,
   Pyright, and full non-live suite; and
5. that no sensitive or raw review material enters the tracked report.

Write a self-contained report with frontmatter, reviewed commits, official-doc
fingerprint, commands/counts, explicit fallback ACCEPT or REJECT, and the
non-authoritative boundary. An ACCEPT may say only that the fallback found P1/P2
closed offline; it must explicitly state that the missing Grok audit still
blocks Wave-5c. A REJECT must contain actionable source/test evidence. Run wiki
update/lint, purge raw scratch evidence, post the result to the parent, and
commit the report.

## Completion Requirements

1. The report and generated index row are committed, lint-clean, and
   non-sensitive.
2. The report names `a2a0996` and `920ceab`, cites current official Billy
   documentation, records all required P1/P2 evidence, and gives a fallback
   ACCEPT or REJECT.
3. The report explicitly says that this Codex Power result cannot satisfy the
   mandatory Grok audit or authorize Wave-5c.
4. The verdict is posted to the parent, scratch evidence is purged, and the
