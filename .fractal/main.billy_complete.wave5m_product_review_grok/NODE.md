...
Perform the mandatory independent Grok product review for the merged Wave-5m
contact-balance-payment write slice at root commit `8297713`. You own the
review only, not implementation.

Use current official Billy API documentation as an external source. Produce a
cited, non-sensitive review of exactly these four tools:
...
Review the implementation against `wiki/wave_fivem_ticketed_writes_contract.md`
and its accepted freeze review. Independently re-check the official docs
fingerprint and the documented method boundaries: POST/PUT are auth-gated,
singular DELETE is 405 and excluded, and bulk rows remain ambiguous/red.

Verify the merged source, registration, tests, inventory, and coverage result:

1. Strict Pydantic outer inputs; opaque inner map; update body `id` equals
   route id.
2. Preview is non-mutating; execute accepts a ticket only; confirmation is
   short-lived, single-use, exact-operation/executor/payload/target bound, and
   prepared writes are never retried.
3. The exact POST/PUT client-relative paths and singular/plural roots are used;
   response handling requires `contactBalancePayments` and fabricates no
   additional roots.
4. No contact-balance-payment delete or bulk tool is registered. The create
   cleanup wording states that singular DELETE is unsupported.
5. Focused product, coverage, and server tests pass from committed root bytes;
   250 `api_*` tools and 172 implemented/contract-tested API rows are
   evidenced. `complete` remains false and live/UI/vision/bulk remain red.

Run bounded, headless, non-live checks only. Do not use credentials, create
Billy records, open a browser, record screenshots/HAR/traces, or make product
code, inventory, status, or test changes. Keep raw research material only in
scratch. Your only project deliverable is
`wiki/wave_fivem_product_independent_review.md`, containing frontmatter,
cited evidence, the exact test commands/results, an explicit ACCEPT or REJECT
verdict, limitations, and a concise remediation list if rejected. Run wiki
lint. An honest REJECT is a successful completion of this review task; do not
wait for the parent to act.

If a Grok authentication, quota, or rate-limit failure happens before any
review edit, report the full blocked step and evidence through radio so the
parent can retry that whole step using the permitted alternate route.

## Completion Requirements

1. `wiki/wave_fivem_product_independent_review.md` records a cited independent
   ACCEPT or REJECT of the precise Wave-5m product slice at `8297713`.
2. The record proves the documented write/delete/bulk boundaries, ticket safety,
   registration and coverage arithmetic, and clearly preserves all unqualified
   lanes as red.
3. The focused non-live tests, formatting/lint/type checks, coverage checker,
   repository policy checker, and wiki lint pass, or the review records the
   exact failure and its impact.
4. No tracked change exists outside the review page and node-owned Fractal
   state. No credentials or raw evidence are tracked.
5. The node posts the verdict to its outbox and runs `fractal node finish` in
   the same iteration.
