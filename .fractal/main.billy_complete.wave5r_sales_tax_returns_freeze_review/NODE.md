...
Perform the independent Grok freeze review for the existing Wave-5r
salesTaxReturns update-only contract. This is a research and review leaf, not a
product implementation task. The reviewed parent baseline is
`main.billy_complete` at commit `322f301`; the fixed freeze-page content MD5 is
`078aca13828b5e0d71b454c1aa2dc00f`.

You own only:

- `wiki/wave_fiver_freeze_independent_review.md`; and
- the generated `wiki/_index.md` entry for that page, if `wiki update` changes
  it; and
- your own `.fractal/main.billy_complete.wave5r_sales_tax_returns_freeze_review/`
  seed artifacts.

Do not modify source, tests, scripts, coverage inventories/status/report,
manifests, configuration, or any other wiki content. Do not create or plan a
product child. Do not claim any coverage, live, UI, vision, cleanup, bulk, or
overall-completeness success.

Independently verify the contract rather than repeating prior assertions. You
may use prior root documents as cited context, especially:

- `wiki/wave_fiver_ticketed_writes_contract.md` (the freeze target);
- `wiki/wave_fiver_freeze_ir_ready_research.md` (research-only readiness);
- `wiki/wave_fiver_sales_tax_returns_freeze_ready_research.md`; and
- `wiki/wave_fiver_sales_tax_returns_product_ready_research.md`.

Fetch and cite the current official docs at `https://www.billy.dk/api/`. For
unauthenticated method-gate checks, use only
`https://api.billysbilling.com/v2`; use no credentials and no persistent
records. Keep raw responses, if needed, solely in your owner-only scratch
directory and never commit them. Never open a visible browser, use a headed
browser, or save frames, HARs, traces, or secrets.

The review must independently establish or explicitly report drift for:

1. the root freeze page MD5 and presence on the reviewed parent baseline;
2. the one inventory id `api.salesTaxReturns.update` and exactly two future
   tool names `api_sales_tax_returns_update_preview` and
   `api_sales_tax_returns_update_execute`;
3. `PUT /salesTaxReturns/:id`, strict outer `{id, salesTaxReturn}`, an opaque
   inner map, optional inner-id/path-id equality, and the required
   `salesTaxReturns` success root;
4. the shared ticket protocol: preview non-mutating, ticket at most five
   minutes and single use, execute ticket-only, exact binding, one write, and
   no write retry;
5. Supports/property boundaries, including readonly fields and opaque
   `periodText`, `reportDeadline`, and `isSettled` with no live/reversibility
   claim;
6. method gates: POST and singular DELETE closed with 405, JSON-object PUT
   reaches the authentication gate with 401, and empty-body PUT returns 400;
7. no create/delete/bulk/webhook or generic-control expansion; and
8. the still-red inventory and status boundary: 178 implemented + contract,
   zero live and vision, 92 ambiguous bulk rows, and `complete: false`.

Write a cited, non-sensitive page with an unambiguous **ACCEPT** or **REJECT**.
An ACCEPT authorises only a later Codex Power leaf to implement the two offline
update tools. It never authorises coverage greening by itself, live/UI/vision
work, bulk, create/delete, cleanup claims, or completion. Any material drift,
missing evidence, ambiguous safety claim, or violated boundary must be a
fail-closed REJECT. Report progress or blockers through radio; do not wait for
parent replies.

## Completion Requirements

1. `wiki/wave_fiver_freeze_independent_review.md` exists, is cited and
   non-sensitive, and records an unambiguous ACCEPT or REJECT based on an
   independent current-doc and method-gate review.
2. The page verifies the frozen MD5 and every contract boundary enumerated in
   the Instructions, or records an explicit fail-closed discrepancy.
3. The page clearly limits any ACCEPT to later offline product authority and
   makes no false coverage, live, UI, vision, cleanup, bulk, or completeness
   claim.
4. No files outside the listed ownership are changed, aside from the generated
   project-wiki index row. No credentials or raw evidence are tracked.
5. `wiki update --path=wiki`, `wiki lint --path=wiki`, and
   `bash "$NODE_DIR/scripts/test.sh"` pass before committing the review page.
6. The review page and any generated index entry are committed and pushed on
   this node branch, the worktree is clean, and `fractal node finish` is run in
   the same iteration.
