...
Author one cited, wiki-only offline freeze contract for singular Billy users
update. You are the Codex Power implementation-authoring leaf, but this task is
documentation-only: use the committed Grok research package
`wiki/wave_fiveq_users_freeze_ready_research.md` as the contract evidence and
write exactly `wiki/wave_fiveq_ticketed_writes_contract.md`.

Your exclusive project-file ownership is that new wiki page. You may change
only your node seed, plans, and memory besides that page. Do not modify source,
tests, coverage manifests/status/report, existing wiki pages, browser settings,
credentials, or temporary research. Do not call live APIs, open a browser, or
create records. No screenshots, frames, HAR files, traces, tokens, or sensitive
user data may enter the repository.

Freeze exactly one future inventory operation: `api.users.update`, using only
two future tools: `api_users_update_preview` and
`api_users_update_execute`. The client-relative request is `PUT /users/:id`,
with strict outer preview input `{id: non-empty string, user: map}` and required
success root `users`. The inner `user` is an opaque
`dict[str, JsonValue]`; if `user.id` is supplied it must equal the path id.
Do not invent field-level validation, password/invite/MFA behavior, relation
wiring, enum values, or an update payload sample.

The page must faithfully cite Research70's current official-doc fingerprint and
method gates: users supports update, PUT with `{}` gates at 401, and POST plus
singular DELETE are 405. It must record user PII (`email`, `phone`) and high
privilege flags (`isStaff`, `isSupporter`, `isAdmin`) as sensitive boundaries;
do not claim those flags are safe to mutate. State the shared confirmation
protocol precisely: non-mutating preview; a single-use ticket lasting at most
five minutes bound to exact execute tool, organization, target id, canonical
request, and expected effect; execute takes only a ticket; one HTTP write; no
retry; all invalid/expired/consumed/mismatched tickets fail closed.

Explicitly exclude create, delete, every bulk operation, webhooks, generic
HTTP/browser controls, source implementation, server registration, contract
tests, coverage/status changes, live work, UI/vision, cleanup qualification,
and completeness claims. A freeze page is neither independent freeze ACCEPT nor
authority for a users product leaf. Record that an independent Grok freeze
review is required before later implementation.

## Completion Requirements

- `wiki/wave_fiveq_ticketed_writes_contract.md` exists with valid frontmatter,
  direct cited Research70 and official-doc evidence, and an explicit
  research/freeze/product/live/UI/completeness authority boundary.
- The page declares only the two future users update tool names, exact request
  and success-root contract, strict/opaque input boundary, PII/privilege
  sensitivity, and every shared confirmation-ticket invariant above.
- The page explicitly records POST/DELETE 405, PUT 401, no bulk body contract,
  no webhook evidence, unproven restore/cleanup, and the required later
  independent Grok freeze ACCEPT; it makes no green coverage claim.
- `bash $NODE_DIR/scripts/test.sh`, `wiki update --path=$WIKI_DIR`, and
  `wiki lint --path=$WIKI_DIR` pass. The project diff is limited to the
  assigned page; no source, coverage, test, status, or unrelated wiki change
  is made.
- Before finishing, post the page and scope to the outbox, fold durable facts
  into memory, commit the freeze page, and run
  `fractal node finish --reason="Wave-5q users update freeze contract recorded"`.
