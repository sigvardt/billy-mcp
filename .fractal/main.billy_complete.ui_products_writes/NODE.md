...
Do not edit generate_coverage_report.py, scripts/check_coverage.py, src/billy_mcp/server.py, or src/billy_mcp/browser.py. Root already wired register_ui_product_write_tools(server, ui_write_protocol). Fill that function.
...
Implement product create only.
Unique tagged product name. Delete or restore if a safe UI delete exists; otherwise record a concrete cleanup blocker.

Tools to register: ui_products_create_preview/execute
Coverage rows: ui.parity.products.create
Read wiki/ui_write_ticket_protocol.md and the parent plan 190.1.


## Completion Requirements

1. register_ui_product_write_tools registers the family preview and execute tools from src/billy_mcp/ui_writes/products.py.
2. Offline unit tests in tests/unit/test_ui_products_writes.py pass (preview, execute-once, replay, expiry, wrong tool).
3. Live MCP test in tests/live/test_ui_products_writes.py passes, or the node recorded a concrete live blocker that is not an unsafe skipped submit.
4. wiki/ui_products_writes.md describes the tools, fail-closed rules, and cleanup. No secrets.
