...
Perform an independent, defect-first Grok review of the integrated Wave-5i
sales-tax account and meta-field write product. This is a review-only leaf;
current official Billy documentation and the approved design are the contract.

Review root commits `084ad77` and `a0b55c2`, compared with their merge base,
alongside the accepted contract pages:
...
Re-fetch and cite the current official Billy API documentation before deciding.
Inspect all changed production, coverage, registration, and focused-test files.
Verify the exact twelve named preview/execute tools, flat strict outer schemas,
opaque inner payload boundary, client-relative paths and HTTP methods, exact
ticket binding and executor-mismatch-before-consumption behaviour, no write
retry, plural-root/deleted-metadata mapping, and exactly six evidence-backed
coverage rows. Independently run relevant checks from the committed checkout.

Own only `wiki/wave_fivei_product_independent_review.md` (plus generated index
maintenance if required). Do not change implementation, tests, coverage status,
inventories, contracts, credentials, or browser state. Do not open a visible
browser or retain raw browser evidence. Never turn live/UI/vision/bulk rows
green. The result must state a clear ACCEPT or REJECT for the offline product
only; overall completion remains fail-closed.

If Grok cannot authenticate, is rate limited, or lacks quota before edits, do
not edit files: report the exact failure to the parent so it can retry the full
review with the allowed fallback route.

## Completion Requirements

1. A durable, non-sensitive review exists at
   `wiki/wave_fivei_product_independent_review.md` with a cited official-doc
   revalidation, reviewed root commit, executed check references, and an
   unambiguous ACCEPT or REJECT.
2. The review checks the complete Wave-5i product diff and confirms or rejects
   every contract boundary named above; any finding is concrete, actionable,
   and tied to a precise path.
3. The review makes no product code, test, inventory, coverage-value, live,
   UI, vision, bulk, credential, or raw-evidence change.
4. The project wiki is lint-clean, the review commit is pushed, and an outbox
   handoff reports the immutable tip and verdict while this node is still
   running.
