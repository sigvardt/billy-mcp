...
Implement the reviewed offline ticketed CUD tools for Billy daybooks. You own
exactly these two files:

- `src/billy_mcp/api/daybook_writes.py`
- `tests/api/test_daybook_writes.py`
...
design §§6 and 8. Do not conduct new non-Billy research or invent missing
contracts.

Implement six typed FastMCP tools, all with flat `extra="forbid"` Pydantic outer
inputs and typed success/error output:

- `api_daybooks_create_preview` / `_execute`
- `api_daybooks_update_preview` / `_execute`
- `api_daybooks_delete_preview` / `_execute`

Use one `register_daybook_write_tools(server, client, write_protocol)` helper.
Previews accept an opaque `daybook` object and a non-empty `id` for update/delete.
Executes accept only `confirmation_ticket`. Construct `WriteOperationSpec` with
singular root `daybook`, plural `daybooks`, paths `/daybooks` and
`/daybooks/{escaped id}`, and `additional_plural_roots=()`. Use POST create,
PUT update, and DELETE with no JSON body. The docs list required `name` and
`isTransactionSummaryEnabled`, but preserve opaque payload validation offline;
live qualification decides unresolved field semantics. Do not implement
daybookBalanceAccounts or daybookTransactions writes.
...
invent bulk save/delete, add webhooks, or mutate coverage. Keep no sensitive
values in logs.
...
network, typed 401/404 propagation, and no fabricated deleted records. Never
claim live, UI, vision, bulk, inventory green, or root server registration.
...
- The six named tools have typed flat schemas and call the shared protocol;
