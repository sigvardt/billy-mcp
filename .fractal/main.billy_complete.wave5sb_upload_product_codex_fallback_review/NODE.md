...
Retry the full Wave-5s-B static product review at root baseline `98484f3` as
the required Codex Power fallback after Grok failed before edits with a verified
not-signed-in error. You own exactly one deliverable:
`wiki/wave_fivesb_files_upload_product_codex_fallback_review.md`. Do not modify
production source, tests, coverage, inventory, or other wiki pages.

Review against the approved design §§8.3–8.4 and committed contract records
`wiki/wave_fivesb_files_upload_research.md`,
`wiki/wave_fivesb_files_upload_product_ready_research.md`, and
`wiki/review_provenance_rules.md`. Independently inspect upload tools, client,
registration, and focused tests for strict FastMCP inputs, configured-root and
symlink containment, ticket binding/replay/expiry, header injection, base URL
lock, redaction, no-preview HTTP, exact one-upload execution, response failure
closure, and exact bytes-versus-ticket-digest semantics. Explicitly test the
replacement window between identity calculation and `read_bytes()`.

This is a static non-authoritative fallback: record every precise finding and
an explicit fallback verdict, but state that it cannot replace the mandatory
Grok audit. Never claim live, UI, vision, bulk, coverage completeness, or Grok
acceptance. No external research, live API traffic, browser, credential, raw
evidence, or code edit is allowed.

## Completion Requirements

1. The owned wiki page has complete `desc:` frontmatter, evidence citations,
   the verified Grok-auth failure provenance, static review findings, and an
   explicit non-authoritative fallback verdict.
2. It evaluates the exact-byte/digest race and all listed safety boundaries,
   contains no sensitive data, and makes no prohibited completion claim.
3. Focused non-live upload tests and wiki lint pass.
4. No production source, tests, coverage, or inventory files are changed.
5. Commit, report the fallback result to the outbox, and call
