...
Author the one frozen, offline-only Wave-5j contract page:
`wiki/wave_fivej_ticketed_writes_contract.md`. You own that page and the
mechanically generated `wiki/_index.md` link row only. Do not edit production
code, tests, coverage inventory or status, existing wiki pages, scripts, or
any API/UI implementation.

The parent has already committed the cited Grok research handoff and a plan.
Use the handoff facts below as the current official-document contract; do not
perform an uncited implementation inference and do not re-open product scope.
The authoring gate is ACCEPT in
`wiki/wave_fivej_freeze_ready_research_independent_review.md`. This is only a
freeze-page authoring gate, not a product, live, UI, vision, bulk, or
completeness approval.

Required source identity: `https://www.billy.dk/api/`, re-fetched at ETag
`hsisik4g9p3603`, MD5 `c2efda0ee4cf9cf200e14910c5fc6996`, 147934 bytes. The
locked API base is `https://api.billysbilling.com/v2`. The detailed parent
research record is scratch-only at
`/Volumes/ssd_1/Repositories/billy-mcp/.worktrees/main.billy_complete/.fractal/main.billy_complete/tmp/grok-research.md`;
use it read-only if required, never copy raw evidence into the repository.

The page must freeze exactly these nine singular CUD rows and their eighteen
preview/execute tools, all still red until a later product leaf supplies code
and tests:

- `bankLineMatches`: `POST /bankLineMatches` / `bankLineMatch` /
  `bankLineMatches`; `PUT` and `DELETE` twins at `/:id`.
- `bankLines`: `POST /bankLines` / `bankLine` / `bankLines`; `PUT` and
  `DELETE` twins at `/:id`.
- `bankLineSubjectAssociations`: `POST /bankLineSubjectAssociations` /
  `bankLineSubjectAssociation` / `bankLineSubjectAssociations`; `PUT` and
  `DELETE` twins at `/:id`.

Use the preview names in the accepted research and derive the exact snake-case
execute twin names. Create/update request roots and bodyless delete behavior
must be explicit. Match responses may optionally map `bankLines` and
`bankLineSubjectAssociations`; child-resource write responses have no invented
additional roots. Preserve standard shared-protocol requirements: non-mutating
preview; short-lived, exact, single-use confirmation ticket; execute accepts
only the ticket; server-owned execute-tool binding fails before consumption and
HTTP; one shared store/service; no write retry; outer inputs forbid extras and
inner maps stay opaque.

Document field and evidence boundaries: unpublished bank-line `side` and
`differenceType` enums; opaque relation shapes; `lines` and
`subjectAssociations` have replace-on-set text but readonly notes and require
live proof; association subject wire form is unproven. Unauthenticated probes
for all nine routes gave POST/PUT 401 and missing-id DELETE 200; DELETE 200 is
not cleanup proof. Keep bulk rows ambiguous and empty-tool. Exclude
`bankPayments` (its singular DELETE is 405), `balanceModifiers`, files
specials, UI/live/vision work, webhooks, and generic HTTP/browser controls.

Use `wiki/wave_fivei_ticketed_writes_contract.md` for structure, but make the
content precise for this three-resource cohort. State later product arithmetic
as targets only: 238 API tools, 166 offline API rows, 43 clear red CUD rows,
and `complete: false`. Finish by running `wiki update --path=$WIKI_DIR` and
`wiki lint --path=$WIKI_DIR`, committing the document and index, reporting
the exact commit and resulting gate status to the parent.

## Completion Requirements

1. `wiki/wave_fivej_ticketed_writes_contract.md` is committed and contains the
   cited authority, all nine operation/tool mappings, protocol safety rules,
   three resource boundaries, response-root policy, probes, exclusions, and
   later product-test obligations described above.
2. The page explicitly preserves every relevant coverage row as unimplemented,
   untested, and not live-tested; it makes no unsupported product, live, UI,
   vision, bulk, or completeness claim.
3. Only the new contract page and generated shared-wiki index work are merged
   into the child branch; source, tests, coverage files, and existing wiki
   pages are unchanged.
4. `wiki update --path=$WIKI_DIR` and `wiki lint --path=$WIKI_DIR` pass, and
   the child commits its work and posts a concise parent-facing outbox report.
5. The child calls `fractal node finish` after these conditions are true. The
   independent Grok freeze review is intentionally a parent-owned next gate,
   not a leaf completion condition.
