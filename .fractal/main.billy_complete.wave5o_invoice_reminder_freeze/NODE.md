...
Author one authoritative, cited, fail-closed wiki contract page for the next
Billy API write gate. You are a `codex-power` implementation leaf. You do not
own research, independent review, product code, test code, coverage greening,
or root integration.

## Exact deliverable and ownership

Create only `wiki/wave_fiveo_ticketed_writes_contract.md`. You may edit only
that page and your own `.fractal/main.billy_complete.wave5o_invoice_reminder_freeze/**`
seed files (plans/memory as needed). Do not edit `wiki/_index.md` (parent
regenerates it after merge), `coverage/**`, `src/**`, `tests/**`, `.github/**`,
existing wiki records, or configuration outside your seed.

Read these committed sources before authoring:

- `.fractal/main.billy_complete/tmp/grok-research.md` — research64 cited
  freeze-authoring package;
- `wiki/wave_fiveo_freeze_ready_research_independent_review.md` — prior
  research ACCEPT only;
- `wiki/wave_fiven_product_independent_review.md` — prerequisite Wave-5n
  product ACCEPT offline only;
- `wiki/wave_fiven_ticketed_writes_contract.md` — style/structure reference;
- `wiki/offline_write_probe_rules.md`, `coverage/api_v2_manifest.yaml`,
  `coverage/status.json`, and the approved design.

## Frozen authority boundary

The new page freezes exactly one future API operation:

| Inventory id | Preview tool | Execute tool | Client path | Request root | Required success root |
| --- | --- | --- | --- | --- | --- |
...
The page must state all of the following faithfully:

- It is a wiki-only freeze. It authorises no product source, tests, server
  registration, inventory/status change, or coverage greening.
- Its prerequisite is Wave-5n's accepted offline product review. A separate
  independent Grok freeze review must ACCEPT this page before any product child
  starts.
- Current official-doc evidence is ETag `wcw4x9hqvu3603`, MD5
  `8b94b0135c91fd15fe54ea33e088a4be`, 147934-byte body; contrast the older
  inventory lock only as access/CDN metadata drift. The locked API base is
  `https://api.billysbilling.com/v2`, while client paths omit `/v2`.
- Present root honesty is 254 `api_*` tools, 174 implemented plus
  contract-tested rows, zero live/vision rows, 92 ambiguous bulk rows, and
  `complete: false`.
- Preview accepts exactly `{invoiceReminder: map}` with undeclared outer fields
  forbidden. The inner map is opaque: do not define relation wire forms,
  `*Id` aliases, date/number encodings, email side effects, association schema,
  or any more-specific payload schema.
- Execute accepts exactly `{confirmation_ticket: non-empty string}`; no boolean
  approval or replacement business payload. It uses the shared
  `ConfirmationStore` and `WriteProtocolService`, binds the exact
  executor/organisation/target/canonical request/effect, is short-lived and
  single-use, makes one write, and never retries a prepared write.
- Future response mapping takes only the required `invoiceReminders` root;
  do not freeze additional response roots.
- Field boundaries must preserve the official table: `organization`, `contact`,
  `feeCurrency`, `contactPerson`, `emailSubject`, and `emailBody` are required
  opaque values; `createdTime` is readonly; `associations`, `flatFee`,
  `percentageFee`, `sendEmail`, and `copyToUser` are opaque/unproven without
  semantics; `downloadUrl` is unproven and not required on create.
- Method gates: `POST {}` returns 401 and opens this offline create freeze;
  PUT, singular DELETE, and bulk DELETE `?ids[]=` return 405. Supports omits
  update and singular delete. Both bulk rows remain empty-tool red; bulk save
  has no body contract; no webhook is documented.
- The later product, only after review ACCEPT, is the two named create tools and
  must correct the create cleanup wording while greening. That future work is
  not this child.

Explicit exclusions: update, singular delete, both bulk operations, all
invoice-reminder-association writes, relation wire forms, payload schemas,
email-send promises, response extras, webhooks, generic HTTP/browser controls,
credentials, live/UI/vision work, cleanup claims, product implementation,
coverage changes, and completeness claims.

Use only the cited materials above. Do not make network calls, open a browser,
or handle credentials. Run `wiki lint --path="$WIKI_DIR"` and your configured
non-live checks. If documentation or the provided evidence conflicts, do not
guess: leave the page uncommitted and report the precise conflict over radio.

## Completion Requirements

1. `wiki/wave_fiveo_ticketed_writes_contract.md` is committed and is the only
   project deliverable change outside the child seed.
2. The page has valid wiki frontmatter with a precise `desc`, cites research64,
   prior research acceptance, the Wave-5n product acceptance, official docs,
   probe rules, inventory/status, and the approved design, and passes
   `wiki lint --path="$WIKI_DIR"`.
3. The page names exactly the one frozen inventory operation and exactly the two
   authorised future create tool names; it contains no authorised update/delete
   or bulk tool name.
4. It preserves all stated 401/405 gates, opaque-field boundaries, ticket
   protocol, no-retry, required-root-only mapping, and explicit fail-closed
   exclusions.
5. No source, test, server, coverage/status, inventory, or existing wiki page
   is changed; no coverage state is greened; no network/browser/credential/live
   work occurs.
6. The child runs its configured checks, records their results in its plan
   post-mortem, reports the committed deliverable to the parent over radio, and
   calls `fractal node finish` in the same iteration once the conditions hold.
