...
This is the authorised Codex Power fallback for the Grok research review, which
failed before any edit because the Grok CLI was not signed in. Perform a
defect-first static revalidation of the Wave-5j freeze-ready research using the
existing cited project evidence and current official Billy documentation where
available. Do not represent this fallback as a Grok review.
...
update, and delete for each. Verify their exact paths, methods, documented
payload/response semantics, and whether the stated offline 401 and missing-id
DELETE 200 probes justify the proposed contract. Explicitly validate the
`bankPayments` DELETE 405 exclusion. Check that all 92 ambiguous bulk entries
remain unresolved, that undocumented enums/owner/subject relationships stay
opaque, and that optional additional roots apply only to match resources.

Use the root API manifest, status, offline-probe rules, and cited research as
context; do not create a freeze page, tools, tests, or coverage changes. Do not
claim live/UI/vision/bulk/product qualification or persist raw evidence.

Own only `wiki/wave_fivej_freeze_ready_research_codex_fallback_review.md` (and
generated index). It must state whether the research is accepted or rejected
for a later Codex freeze decision, with evidence and exactly what remains red.

## Completion Requirements

- Deliver the specified durable non-sensitive review with an explicit ACCEPT or
  REJECT verdict, a nine-operation evidence table, exact defects, and a clear
  Codex-fallback (not Grok) label.
- Keep the unresolved bulk/live/UI/vision/product work red and make no
  implementation or coverage-state edit.
- Run `wiki lint --path="$WIKI_DIR"`, commit and push with `fractal commit`,
  then announce the outcome via radio before finishing.
