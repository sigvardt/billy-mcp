...
Act as the Codex Power fallback for the Wave-5h attachment freeze-contract
step. Grok failed before making any edit because its CLI was unauthenticated;
this fallback is required by the routing policy. It is not an independent Grok
review and must not issue an acceptance verdict.

Your exclusive project-file ownership is exactly
...
Re-run the scoped, read-only contract research against
`https://www.billy.dk/api/`, recording the source fingerprint and citations.
The current parent Grok brief at
`/Volumes/ssd_1/Repositories/billy-mcp/.worktrees/main.billy_complete/.fractal/main.billy_complete/tmp/grok-research.md`
is a cited lead and its raw evidence may be read, but it must not be copied
into git. Report source drift or a conflict with the API manifest to the parent
immediately. Never use credentials, a browser, or non-Billy hosts; do not
create Billy records.

Freeze exactly three offline-only attachment JSON CUD operations:

- `api.attachments.create`: `POST /v2/attachments`, one `attachment` root,
  changed `attachments` root.
- `api.attachments.update`: `PUT /v2/attachments/:id`, one `attachment` root,
  changed `attachments` root.
- `api.attachments.delete`: `DELETE /v2/attachments/:id`, no body, canonical
  ticket target `{"id": id}`, optional `meta.deletedRecords.attachments`.

The locked runtime base is `https://api.billysbilling.com/v2`; internal paths
must not repeat `/v2`. Freeze strict outer Pydantic input, opaque inner
attachment values, only `priority` as a clear mutable candidate, partial PUT
with a matching body/route ID when an ID is supplied, no-write preview,
ticket-only execute, exact server-owned execute-tool binding, one root
confirmation store/write-protocol service, and no write retry. Do not invent
owner serialization for its belongs-to-reference field, do not add response
roots, and do not infer file-upload behaviour.

Explicitly separate these JSON writes from high-sensitivity binary
`POST /files`. Keep attachment bulk save/delete, `api.files.create`,
`api.special.files_upload`, all UI work, live tests, vision, coverage greening,
and completeness claims red and out of scope. A missing-ID unauthenticated
DELETE 200 is not cleanup proof.

The finished page is a candidate for a later independent Grok review. Its prose
must make the fallback authority limitation unambiguous.

## Completion Requirements

1. `wiki/wave_fiveh_ticketed_writes_contract.md` is the only project
   deliverable, has valid frontmatter and description, and identifies itself as
   a Codex Power fallback contract candidate rather than a Grok acceptance.
2. It cites the current official API evidence and freezes exactly the three
   attachment JSON CUD rows, their six preview/execute names, paths, roots,
   request/response boundaries, errors, sensitivity, cleanup rules, and test
   requirements.
3. It forbids invented owner or file-upload serialization, bulk contracts,
   webhooks, unobserved response roots, and all coverage/live/UI/vision claims.
4. `git diff --check`, `wiki lint --path="$WIKI_DIR"`, the inherited test
   script, and inherited lint script pass. Commit, post a concise outbox report
   to the parent, then run `fractal node finish` in the same run.
