...
Implement only the sixteen Wave-4 clear read tools for tax rates, tax-rate
deduction components, sales-tax rulesets, rules, accounts, meta fields,
returns, and payments. Own exactly `src/billy_mcp/api/tax_reads.py` and
`tests/api/test_tax_reads.py`; do not edit registration, coverage, redaction,
wiki, generated files, or any other product files.

Read `wiki/wave_four_remaining_clear_reads_contract.md` and the cited
`.fractal/main.billy_complete/tmp/grok-research.md` before coding. Reuse the
...
and typed Billy 401 handling. Do not invent resource filters or enum values for
`source`, `type`, `contactType`, or `periodType`; `offset` and unknown inputs
must be rejected.

Use only the existing locked Billy client. Do not make live claims, perform
writes, add generic controls, or change coverage flags. Run focused tests plus
formatting, Ruff, and Pyright for the owned work. Commit only real code and
tests, then report the commit and results to the parent radio outbox.

## Completion Requirements

- `tax_reads.py` implements and registers all sixteen cited get/list tools with
  the exact contract roots and paths.
...
- `uv run ruff format --check`, `uv run ruff check`, `uv run pyright`, and
  `uv run pytest tests/api/test_tax_reads.py` pass.
