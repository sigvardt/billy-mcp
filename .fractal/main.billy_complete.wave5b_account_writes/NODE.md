...
Implement the cited Wave-5b offline ticketed-write module for account groups
and accounts. The binding contract is
`wiki/wave_fiveb_ticketed_writes_contract.md`; the approved design and current
official Billy documentation remain authoritative.
...
Implement exactly twelve real FastMCP tools: preview and execute for create,
update, and delete of `accountGroups` and `accounts`. Use client-relative
collections `/accountGroups` and `/accounts`, singular request roots
`accountGroup` and `account`, POST/PUT/DELETE respectively, and no delete body.

Follow `src/billy_mcp/api/daybook_writes.py` and the frozen shared
`WriteProtocolService` pattern. Reuse the passed service; do not create a store,
protocol, generic HTTP tool, or retry path. Preview models must forbid outer
extras, payloads remain `dict[str, JsonValue]`, and every execute function
accepts only `confirmation_ticket`.

Write focused contract tests for schemas, exact tool registration shape, no
preview HTTP request, execute method/path/body, typed errors, and ticket
tamper/replay/expiry/binding rejection. Tests are offline only: do not access
Billy, credentials, UI, coverage flags, or live qualification.

Do not edit `server.py`, coverage manifests or generators, the shared protocol,
project wiki, or the sibling's files. Escalate any contract conflict to the
parent by radio rather than broadening scope. Commit only your owned module and
test; the parent registers tools and generates coverage after integration.

## Completion Requirements

- The two owned files contain all twelve tools and their contract tests; no
  placeholder or generic-control surface exists.
- Every preview has no observable HTTP mutation, every execute is ticket-only,
  and request construction exactly matches the frozen paths, methods, roots,
  and bodyless delete rule.
- Focused pytest, Ruff, Pyright, and the node lint/test scripts pass from
  committed bytes.
- No live test, credential, browser artifact, coverage greening, or
  out-of-scope project change is present.
- The branch is committed and pushed, a concise outbox report names the commit
  and checks, and `fractal node finish` is called with an evidence-based reason.
