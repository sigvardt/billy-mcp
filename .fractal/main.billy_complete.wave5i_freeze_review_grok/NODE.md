...
Perform an independent Grok review of the Wave-5i freeze-ready research for
singular Billy API v2 `salesTaxAccounts` and `salesTaxMetaFields` JSON CUD.
You own research judgement only: do not draft the freeze contract and do not
implement tools. Use current official Billy documentation as authority and
produce cited findings before any Codex Power implementation may begin. The
parent's non-sensitive lead is readable at
`/Volumes/ssd_1/Repositories/billy-mcp/.worktrees/main.billy_complete/.fractal/main.billy_complete/tmp/grok-research.md`,
but verify it independently against the official docs rather than treating it
as authority.

Check the manifest rows, `coverage/status.json`,
`wiki/offline_write_probe_rules.md`, the approved design, and the existing
Wave-5g sales-tax write contract/product patterns. Re-fetch/read official docs
and verify the current fingerprint, endpoint lock, generic singular
create/update/delete narratives, resource Supports/property tables, documented
authentication errors, and the scoped unauthenticated probe interpretation.

The only candidate scope is these six singular operations and their existing
preview/execute tool pairs:

- sales-tax accounts: create `POST /salesTaxAccounts`, partial update
  `PUT /salesTaxAccounts/:id`, bodyless delete
  `DELETE /salesTaxAccounts/:id`, with one `salesTaxAccount` root and
  `salesTaxAccounts` primary response root;
- sales-tax meta-fields: create `POST /salesTaxMetaFields`, partial update
  `PUT /salesTaxMetaFields/:id`, bodyless delete
  `DELETE /salesTaxMetaFields/:id`, with one `salesTaxMetaField` root and
  `salesTaxMetaFields` primary response root.

Verify the research preserves opaque inner payloads; does not invent enum
values, belongs-to encodings, field mutability, sibling roots, bulk request
bodies, or file/webhook behavior; requires body/route id equality for update;
and binds delete tickets to the id with no request body. Check that
unauthenticated POST/PUT 401 is only a compatible auth gate and missing-id
DELETE 200 is not cleanup or live proof. Bulk, binary files, UI, vision, live
qualification, product implementation, coverage-green transitions, and overall
completion must all remain explicitly excluded.

Your sole project deliverable is
`wiki/wave_fivei_freeze_independent_review.md`. Write a durable, non-sensitive
cited record with the exact review scope, authoritative source fingerprint,
independent checks, official field/route evidence, manifest comparison,
ACCEPT or REJECT verdict, and explicit non-claims. An ACCEPT opens only a later
Codex Power task to draft `wiki/wave_fivei_ticketed_writes_contract.md`; it
does not authorise product code. A REJECT must name precise blockers. Do not
modify any source, tests, coverage files, other wiki pages, or freeze/product
contract. You may alter only your own
`.fractal/main.billy_complete.wave5i_freeze_review_grok/` seed, memory, plans,
and scratch in addition to the one review page.

Keep any raw fetched evidence owner-only under node scratch and purge it before
finish. Post a concise result to your outbox and do not wait for parent
approval.

## Completion Requirements

- `wiki/wave_fivei_freeze_independent_review.md` exists, is indexed, and
  records an unambiguous cited ACCEPT or REJECT for the Wave-5i freeze-ready
  research, with exact scope and explicit non-claims.
- The verdict independently compares current official documentation with the
  manifest, probe policy, approved design, and parent research lead; it does
  not rely on uncited claims alone.
- An ACCEPT covers only later drafting of the six singular ticketed-write
  operations. It does not claim implementation, contract testing, live tests,
  UI/vision verification, bulk behavior, or overall completeness.
- No project files change outside the one owned review page; no secrets,
  credentials, customer data, frames, HARs, traces, or raw fetch artifacts
  enter git.
- `bash $NODE_DIR/scripts/lint.sh` passes, project wiki structure is valid,
  scratch evidence is purged, memory has terminal facts, and the node posts its
  verdict before `fractal node finish`.
