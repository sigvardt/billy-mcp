...
Perform an independent Grok product audit of the merged Wave-5s-B binary upload
slice at root baseline `98484f3`. You own exactly one shared deliverable:
`wiki/wave_fivesb_files_upload_product_independent_review.md`. Do not modify
production source, tests, coverage state, inventories, or other wiki pages.

Use the approved design §§8.3–8.4, current official Billy documentation where
needed, and these committed contract records:
...
Audit `src/billy_mcp/api/file_upload_writes.py`, `src/billy_mcp/client.py`,
registration, and `tests/api/test_file_upload_writes.py`. Explicitly assess
strict FastMCP inputs, configured-root containment and symlinks, ticket tool /
organisation / request / expected-effect / path / digest binding, replay and
expiry, CR/LF headers, base URL lock, redaction, no-preview HTTP, one HTTP
upload only, and response failure closure. Treat the following as a mandatory
adversarial check: bytes may be replaced after preview or between identity
calculation and `read_bytes()`; the bytes passed to `post_file` must still match
the ticket digest or transport must not occur.

The review must be independent and non-sensitive: cite exact code/test evidence
and any official-doc source used, name every finding, distinguish proven facts
from inference, and issue an explicit ACCEPT or FAIL. A fail must say what a
repair needs; it must never green live, UI, vision, bulk, or completeness
coverage. Browser work and live traffic are out of scope; never use a visible
browser or credentials. If Grok authentication or quota prevents the review
before any edit, report that exact blocker on radio immediately and make no
authoritative fallback claim.

## Completion Requirements

1. `wiki/wave_fivesb_files_upload_product_independent_review.md` has complete
   frontmatter including `desc:`, a durable cited review record, and explicit
   ACCEPT or FAIL for the merged upload product.
2. The record assesses the exact-byte/digest race and every safety boundary
   named above; it contains no token, local upload content, raw frame, trace, or
   credential.
3. `wiki lint --path="$WIKI_DIR"` and the focused non-live upload test file
   pass from the child worktree.
4. No production source, test, coverage, or inventory file is changed.
5. Commit the review, post a concise outcome to the outbox, and call
   `fractal node finish --reason="..."` in the same iteration.
