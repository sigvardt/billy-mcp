...
Implement the frozen offline-only Wave-5c parent-resource contract from
`wiki/wave_fivec_ticketed_writes_contract.md`. You own only these product files:

- `src/billy_mcp/api/daybook_transaction_writes.py`
- `tests/api/test_daybook_transaction_writes.py`

Create exactly six typed FastMCP tools: preview/execute pairs for documented
create, update, and delete operations on `/daybookTransactions`, with singular
root `daybookTransaction` and plural response root `daybookTransactions`.
Follow `daybook_writes.py` and the repaired Wave-5b modules exactly: Pydantic
outer inputs forbid extras; payload remains `dict[str, JsonValue]`; update and
delete ids have `min_length=1`; executes accept only `confirmation_ticket`.
...
confirmation store. Use `/daybookTransactions` with no duplicate `/v2`, one
singular-root POST/PUT body, and a bodyless DELETE. Set no additional plural
roots without live evidence. Do not add retries, live calls, bulk, UI, special
routes, or any coverage/manifest/server/wiki changes.
...
1. The two owned files contain exactly the six documented parent-resource tools
