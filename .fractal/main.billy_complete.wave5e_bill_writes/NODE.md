...
Implement the accepted Wave-5e **parent bill** ticketed-write slice. You are a
...
- `src/billy_mcp/api/bill_writes.py`
...
pattern in `invoice_writes.py` and `test_invoice_writes.py`; do not copy
invoice-specific business rules.

Implement six typed FastMCP tools:

- `api_bills_{create,update,delete}_preview`
- `api_bills_{create,update,delete}_execute`

The operation specifications are exactly `POST /bills`, `PUT /bills/:id`, and
bodyless `DELETE /bills/:id`, with singular `bill`, plural `bills`, and
`additional_plural_roots=()`. Outer Pydantic inputs forbid extra fields while
the inner `bill` object remains opaque `dict[str, JsonValue]`. An optional body
id on update must equal the route id. Use the parent-provided shared
`WriteProtocolService` and exact server-owned executor names; never construct a
local store or add a write retry.
...
and no extra mutation. Do not mark coverage green or claim product acceptance.
...
- `bill_writes.py` registers exactly the six required bill preview/execute
  tools with the frozen paths, roots, ticket binding, and no additional roots.
- `test_bill_writes.py` covers every local contract boundary above and passes
  with `uv run pytest tests/api/test_bill_writes.py`.
- `uv run ruff format --check src/billy_mcp/api/bill_writes.py
  tests/api/test_bill_writes.py`, `uv run ruff check` on those files, and
  `uv run pyright` pass.
