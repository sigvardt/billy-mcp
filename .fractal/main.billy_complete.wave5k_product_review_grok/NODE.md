...
Perform an independent, current-evidence Grok product review of the committed
Wave-5k bank-payment ticketed-write integration. You are a reviewer, not an
implementer: do not modify source, tests, coverage manifests/status, server
registration, contracts, or any existing wiki page.

Review the root baseline you received, whose Wave-5k integration commit is
`ee40a40`. Your only tracked project deliverable is
`wiki/wave_fivek_product_independent_review.md`. You may use this node's
memory, plans, and ignored scratch for private work as usual.

First obtain and cite current official Billy documentation evidence from
https://www.billy.dk/api/. Compare it with the accepted freeze at
`wiki/wave_fivek_ticketed_writes_contract.md`, the authoritative freeze review
at `wiki/wave_fivek_freeze_independent_review.md`, the API manifest, and the
...
- `coverage/api_v2_manifest.yaml`, `coverage/ui_workflows_manifest.yaml`, and
  `coverage/status.json`

Independently verify the exact four-tool surface, strict outer inputs and opaque
inner map, client-relative POST/PUT paths and roots, partial-update id binding,
ticket executor binding before consumption/HTTP, single-use/expiry/tamper/replay
behaviour, one-write/no-retry rule, required-root response mapping, registration
count, and the two allowed offline coverage rows. Verify that singular delete,
bulk, live, UI, vision, and completeness remain fail-closed. Check the
bank-payment cleanup wording is evidence-backed and does not invent singular
delete. Run focused and full non-live tests plus lint/type/coverage/policy checks
as proportionate evidence.

The review page must give a decisive **ACCEPT** or **REJECT** for the merged
offline product, cite the official source and committed root evidence, distinguish
offline contract testing from live/UI/vision qualification, and list every
remaining red surface. An ACCEPT authorises only the next separately frozen
slice; it does not change coverage flags or claim overall completion. A REJECT
must name reproducible defects and retain the fail-closed state.

Do not use credentials, live organisations, browser UI, screenshots, traces,
or raw evidence in the repository. Do not create fallback review records: if
current evidence cannot support a verdict, record a REJECT with the precise
evidence gap. Report the verdict to the parent over radio and finish only after
your own observable deliverable and checks are complete.

## Completion Requirements

1. `wiki/wave_fivek_product_independent_review.md` exists and contains a cited,
   decisive ACCEPT or REJECT of the committed Wave-5k product on the inherited
   root baseline.
2. The review independently checks all four frozen bank-payment tools, their
   ticketed-write safeguards, response mapping, registration, and the exact
   242-tool / 168-offline-row arithmetic.
3. The review explicitly keeps singular delete, bulk, live, UI, vision, and
   overall completeness fail-closed; it makes no unsupported live or interface
   claims.
4. No source, test, coverage, server, or pre-existing wiki file was modified;
   no sensitive evidence is tracked.
5. Relevant non-live tests, lint/type checks, coverage validation, and policy
   checks pass, and the parent receives a concise radio verdict.
