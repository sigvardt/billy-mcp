...
Implement the frozen, read-only Wave-4 balance and invoice-extension API slice
as a Codex Power leaf. The root has a cited Grok contract brief at
`.fractal/main.billy_complete/tmp/grok-research.md` and the shared freeze at
`wiki/wave_four_remaining_clear_reads_contract.md`; use those sources as the
contract. Do not research a new contract or widen its claims.

You exclusively own these two product files and no others:

- `src/billy_mcp/api/balance_invoice_ext_reads.py`
- `tests/api/test_balance_invoice_ext_reads.py`

Implement exactly ten real typed read tools and their service functions:

- `api_contact_balance_payments_get/list` → `/contactBalancePayments`, roots
  `contactBalancePayment` / `contactBalancePayments`
- `api_contact_balance_postings_get/list` → `/contactBalancePostings`, roots
  `contactBalancePosting` / `contactBalancePostings`
- `api_invoice_late_fees_get/list` → `/invoiceLateFees`, roots
  `invoiceLateFee` / `invoiceLateFees`
- `api_invoice_reminders_get/list` → `/invoiceReminders`, roots
  `invoiceReminder` / `invoiceReminders`
- `api_invoice_reminder_associations_get/list` →
  `/invoiceReminderAssociations`, roots `invoiceReminderAssociation` /
  `invoiceReminderAssociations`

Follow the established Wave-3 read-module pattern. Inputs must be flat
Pydantic models with `extra="forbid"`; get requires a non-empty `id` with an
optional non-empty `include`; list permits only `page`, `pageSize` (1–1000),
optional non-empty `include`, optional free-form `sortProperty`, and
`sortDirection` `ASC|DESC`. Reject `offset`, nested `request`, invented parent
filters, and every unknown key. URL-encode identifiers. Success models map only
the documented singular/plural roots and preserve an optional `meta.paging`
envelope. Payload records remain opaque allow-extra objects. Map both documented
401 authentication codes to typed `AUTH_REQUIRED`; malformed response roots are
typed errors. Register direct, stable FastMCP tool names with flat schemas.

This leaf is reads only. Do not add writes, bulk, webhook, browser, generic
HTTP, preview/execute, coverage, server-registration, redaction, inventory,
generated-report, or wiki changes. Do not set coverage flags, `live_tested`, or
`complete`. Reminder email fields and `downloadUrl` are payload facts only;
redaction integration is root-owned. Keep raw evidence and credentials out of
the repository.

Add focused synthetic contract tests that cover every path/root, encoded IDs,
the exact query allowlist and defaults, paging inside `meta`, absent `meta`,
unknown-key rejection, all bounds, typed 401 responses, flat FastMCP tool
schemas, and structured tool output. No live API call is authorised.

Before committing, run focused Ruff format/check, Pyright on the owned files,
the focused pytest module, and the node lint/test scripts. Report only actual
results. Keep the product commit limited to the two owned product files; do not
commit injected `.system` skills, runtime logs, screenshots, downloaded docs, or
other generated seed noise. Use radio to report progress, questions, review
findings, and the final commit hash to the parent.

## Completion Requirements

- The two owned product files contain the ten real typed read tools and their
  focused contract tests, with no out-of-scope product edit.
- The implementation conforms exactly to the cited frozen paths, roots, input
  rules, pagination envelope, and typed 401 handling; no unproven filter or
  response field is invented.
- Focused Ruff format/check, Pyright, and pytest pass, as do the node lint and
  non-live test scripts. Full mode may fail only at the parent-wide incomplete
  coverage gate and must not be reported as a pass.
- A self-contained, scoped product commit is present on this branch; its diff
  contains no credentials, raw evidence, runtime logs, `.system` skill copies,
  or unrelated generated node-seed changes.
- A concise radio report gives the parent the commit hash, files, test results,
  and an explicit statement that live/UI/vision/coverage claims remain red.
- After recording those facts in private memory, run `fractal node finish` with
  a reason limited to this offline leaf deliverable.
