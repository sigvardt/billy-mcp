...
Perform an independent Grok review of the merged Wave-5o offline
invoice-reminder create product. Review the parent baseline
`afbe5188b029e89e7e865a6b9ee8b9a5881f51f4`, which contains product merge
`78108f2`; it adds exactly `api_invoice_reminders_create_preview` and
`api_invoice_reminders_create_execute` in
`src/billy_mcp/api/invoice_reminder_writes.py`.

Your sole tracked project deliverable is
`wiki/wave_fiveo_product_independent_review.md` and its generated index entry.
You own that page, the index row generated by `wiki update`, and this node's
seed only. Do not modify product source, tests, coverage, status, prior
contracts/reviews, plans outside your own node, or any other wiki page.

Read and verify current official evidence at `https://www.billy.dk/api/` and
cite it in the review. The known public-doc body fingerprint is ETag
`wcw4x9hqvu3603`, 147934 bytes, MD5 `8b94b0135c91fd15fe54ea33e088a4be`.
Treat `wiki/wave_fiveo_ticketed_writes_contract.md` and
`wiki/wave_fiveo_freeze_independent_review.md` as the frozen preconditions;
the approved design, the committed code, focused tests, coverage generator,
manifest, and status are the reviewed product evidence.

Independently check: the exact two-tool surface and no update/delete/bulk,
webhook, generic HTTP, or generic browser tools; strict flat typed preview and
ticket-only execute schemas; mutation-free preview; exact one non-retried
`POST /invoiceReminders`; confirmation ticket binding, expiry, replay, and
wrong-executor rejection; mapping only `invoiceReminders`; server registration;
focused test coverage; generator evidence; and honest counts (256 `api_*`
tools, 175 implemented and contract-tested rows, zero live/vision rows, 92
ambiguous bulk rows, `complete: false`). Run relevant read-only checks. Keep
all actual live, UI, vision, cleanup qualification, bulk, webhooks, and
completeness claims fail-closed.

Write a self-contained cited review that records either `ACCEPT` or concrete
actionable defects. An ACCEPT is only an offline product-slice acceptance and
never a live/UI/vision/completeness acceptance. Do not retain credentials, raw
browser evidence, or sensitive values. Use only the locked Billy API/docs
hosts; never open a visible browser.

## Completion Requirements

1. `wiki/wave_fiveo_product_independent_review.md` exists, has valid metadata,
   cites its official and repository evidence, and records a clear ACCEPT or
   defect verdict for the fixed parent baseline.
2. The review verifies every listed scope/safety assertion without changing
   source, tests, coverage, status, or prior contract evidence.
