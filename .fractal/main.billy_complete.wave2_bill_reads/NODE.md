...
Implement the Wave 2 bill read-only API slice only.

Own exactly these product files:

- `src/billy_mcp/api/bill_reads.py`
- `tests/api/test_bill_reads.py`

Implement real typed FastMCP registration for `api_bills_get` and
`api_bills_list` only. Read `wiki/wave_two_filtered_reads_contract.md` and the
`api.bills.get` / `api.bills.list` rows in `coverage/api_v2_manifest.yaml` as
the frozen cited contract. Use only the locked `BillyHttpClient`, relative
`/bills` paths, Pydantic input and success models, and typed `ToolError`
failures.

The list input may expose only documented global `page`, `pageSize`, `include`,
plus the bill filter keys and sort-value enum in the manifest. Reject unknown
keys and invalid enum values locally. Bills do not accept invoice-only period,
approval-time, quote, recurring-invoice, invoice-number, or external-id
filters. Map `bill` / `bills` roots and optional `meta.paging`, and cover both
...
bulk operations, invoice specials, browser/UI code, generic HTTP controls,
offset paging, credentials, or a live coverage claim. Root owns server wiring
and all manifest status changes.

## Completion Requirements

- The two owned files implement only the two real bill read tools and their
  focused mock-transport contract tests.
- Tests prove exact request paths and allowed query construction, rejection of
  unknown and invoice-only filters, singular/list response mapping, optional
