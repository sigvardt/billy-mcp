...
Implement only the ten Wave-4 clear read tools for bank payments, bank-line
matches, bank lines, bank-line subject associations, and balance modifiers.
Own exactly `src/billy_mcp/api/bank_reads.py` and
`tests/api/test_bank_reads.py`; do not edit registration, coverage, redaction,
wiki, generated files, or any other product files.

Read `wiki/wave_four_remaining_clear_reads_contract.md` and the cited
`.fractal/main.billy_complete/tmp/grok-research.md` before coding. Follow the
Wave-3 typed-read pattern: direct flat Pydantic tool parameters,
`extra="forbid"`, encoded IDs, opaque allow-extra records, optional
`meta.paging`, standard page/pageSize/include/sortProperty/sortDirection lists,
and typed Billy 401 handling. Reject `offset`, parent identifiers, and unknown
inputs. Do not infer bank-write semantics, fields, or enum value sets.

Bank data is sensitive: successful typed output remains opaque, while existing
structured logging/error redaction must continue to protect included bank
identifiers. Do not log fixtures containing sensitive values beyond synthetic
data. Use only the locked client; do not make live claims, perform writes, add
generic controls, or change coverage flags. Run focused tests plus formatting,
Ruff, and Pyright. Commit only real code and tests, then report the commit and
results to the parent radio outbox.

## Completion Requirements

- `bank_reads.py` implements and registers all ten cited get/list tools with
  their exact contract paths and roots.
- Focused tests cover every route/root, query construction, optional
  `meta.paging`, encoded IDs, validation and unknown-key rejection, and typed
  401 mapping using synthetic payloads.
- `uv run ruff format --check`, `uv run ruff check`, `uv run pyright`, and
  `uv run pytest tests/api/test_bank_reads.py` pass.
