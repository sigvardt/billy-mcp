...
Perform an independent Grok freeze review of the exact merged Wave-5j
bank-line contract at `wiki/wave_fivej_ticketed_writes_contract.md`. You own
only a new durable review record at
`wiki/wave_fivej_freeze_independent_review.md` and the mechanically generated
`wiki/_index.md` link row. Do not edit the contract, production source, tests,
coverage manifests or status, other wiki pages, or node scripts.

This is a clean-context review, not a product task. Re-fetch the current
official Billy API documentation at `https://www.billy.dk/api/` yourself and
cite the direct source in the review page. The parent research handoff reported
ETag `hsisik4g9p3603`, MD5 `c2efda0ee4cf9cf200e14910c5fc6996`, and 147934
bytes; any discrepancy is a finding, not an inference. Use only headless or
textual access. Do not use credentials, create business data, open a browser
window, or retain raw frames, HAR files, or response bodies in the repository.

Independently compare the contract against the official documentation,
`coverage/api_v2_manifest.yaml`, `coverage/status.json`,
`wiki/offline_write_probe_rules.md`, and
`docs/superpowers/specs/2026-07-28-billy-mcp-complete-design.md`. Verify all of
the following printed contract claims:

- exactly nine clear singular CUD rows and eighteen preview/execute tool names
  for `bankLineMatches`, `bankLines`, and `bankLineSubjectAssociations`;
- `POST`/`PUT`/bodyless `DELETE` paths, singular request roots, primary and
  optional additional plural-root policy, and all-red inventory state;
- preview/execute ticket binding, one shared store and service, mismatched
  execute-tool rejection before consumption and HTTP, forbidden extra outer
  fields, opaque inner maps, and no write retry;
- the immutable/readonly/opaque field boundaries, match embedded-array
  replace-versus-readonly ambiguity, unpublished enums, unproven association
  subject wire form, and no invented child or parent response roots;
- POST/PUT 401 and missing-id DELETE 200 probe interpretation, the
  `bankPayments` DELETE 405 and `balanceModifiers` POST 405 boundaries, all
  ambiguous bulk rows remaining empty-tool red, and excluded UI/live/vision/
  webhook/generic-control work.

Write a concise, non-sensitive review record with a durable explicit
**ACCEPT** or **REJECT**. An ACCEPT may approve only the offline freeze
contract as a future product-authoring input. It must explicitly preserve
`complete: false`, every nine-row implementation/contract/live test flag as
false, and all UI, vision, bulk, live, and product gates as unadvanced. A
REJECT must name exact corrections, but do not repair them yourself. Neither
verdict authorizes product implementation.

Run `wiki update --path=$WIKI_DIR`, `wiki lint --path=$WIKI_DIR`, and the node
lint/test scripts before committing. Report the verdict, current-doc evidence,
and exact commit to the parent via outbox. Call `fractal node finish` after
your own requirements are met; the parent-owned merge and any later product
work are not your completion conditions.

## Completion Requirements

1. `wiki/wave_fivej_freeze_independent_review.md` is committed and gives an
   independently sourced, explicit ACCEPT or REJECT of the exact frozen
   contract, with direct official-doc citation and enough evidence to audit
   each required comparison.
2. The review does not make an unsupported product, coverage, live, UI, vision,
   bulk, browser, or completeness claim. It records that the relevant nine
   manifest rows remain false and `coverage/status.json` remains incomplete.
3. The only project deliverables are the new review page and generated wiki
   index work; the contract itself, source, tests, coverage artifacts, and
   other wiki records remain unchanged.
4. `wiki update --path=$WIKI_DIR`, `wiki lint --path=$WIKI_DIR`, and the node
   lint/test scripts pass. The node posts its verdict and exact commit to its
   parent, clears any saved radio queue it owns, and calls `fractal node finish`.
