...
- tests/unit/test_ui_contacts_writes.py
- tests/live/test_ui_contacts_writes.py
- wiki/ui_contacts_writes.md

Do not edit generate_coverage_report.py, scripts/check_coverage.py, src/billy_mcp/server.py, or src/billy_mcp/browser.py. Root already wired register_ui_contact_write_tools(server, ui_write_protocol). Fill that function.
...
Disposable uniquely tagged customer names. Cleanup via delete in reverse. Independent second UI session list read-back. No live API.

Tools to register: ui_clients_create_preview/execute, ui_clients_update_preview/execute, ui_clients_delete_preview/execute
Coverage rows: ui.parity.contacts.create, ui.parity.contacts.update, ui.parity.contacts.delete
Read wiki/ui_write_ticket_protocol.md and the parent plan 190.1.


## Completion Requirements

1. register_ui_contact_write_tools registers the family preview and execute tools from src/billy_mcp/ui_writes/contacts.py.
2. Offline unit tests in tests/unit/test_ui_contacts_writes.py pass (preview, execute-once, replay, expiry, wrong tool).
3. Live MCP test in tests/live/test_ui_contacts_writes.py passes, or the node recorded a concrete live blocker that is not an unsafe skipped submit.
4. wiki/ui_contacts_writes.md describes the tools, fail-closed rules, and cleanup. No secrets.
