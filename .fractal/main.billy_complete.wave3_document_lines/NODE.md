...
Implement only the Wave-3 document-line read cluster from the frozen contract
at `wiki/wave_three_unfiltered_reads_contract.md` and the cited brief at
`.fractal/main.billy_complete/tmp/grok-research.md`.

Own exactly `src/billy_mcp/api/line_reads.py` and
`tests/api/test_line_reads.py`. Add real, typed `api_invoice_lines_get/list`,
`api_bill_lines_get/list`, and `api_daybook_transaction_lines_get/list` tools
using the established FastMCP/Pydantic/service pattern. Register them only in
the module-local `register_*` function; root owns `server.py` integration.

Collections accept exactly page, pageSize, include, sortProperty, and
sortDirection (ASC/DESC), and reject offset, parent-id filters, and any
undeclared key. Use the documented camel-case paths and singular/plural roots,
URL-encode IDs, preserve optional paging, and map 401 to the existing typed
AUTH_REQUIRED error. Payload bodies remain opaque allow-extra models.

Do not edit server registration, redaction, coverage evidence/manifests, wiki,
or any other product/test file. Do not implement writes, bulk, UI, upload,
live probes, or generic tools. Report a committed delivery to parent radio.

## Completion Requirements

- Both owned files contain six real typed read tools and focused contract tests
  for paths, query construction, URL-encoded IDs, roots, paging, unknown-input
  rejection, and typed 401 behavior.
- The focused pytest file, Ruff format/check, and Pyright pass from committed
  bytes.
- The worktree is clean, the scoped files are committed, and parent radio has a
  concise commit/test report. Do not claim server registration, coverage green,
  live qualification, or product completeness.
